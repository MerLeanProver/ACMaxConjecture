...
/-!
# Structural leaves for the `n = 16`, `e(M) = 3`, `|D| = 8` hub-triangle corner

This file collects the *concrete, decidable-free* structural facts about the residual
configuration consumed by `exists_hub_triangle_config_residual_sixteen` in `TwinCert16HubTriangle`:
the eight degree-`4` hubs (`Dᶜ`), the four `M`-isolated degree-`3` twins (`Iso`), and the
`M = P₄` path `L₁–c₁–c₂–L₂`.  Each lemma takes exactly the residual hypotheses it needs and
proves a single counting fact used by the triangle-forcing case analysis.  The `n = 16` deltas
from `n = 15` are `|Hub| = 8` (vs `7`), so each cherry has `≥ 4` avoiders (vs `≥ 3`) and the
hub-internal-degree total is `∑ int = 14` (vs `10`).
-/
...
theorem nbr_split_DC (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16)) (v : Fin 16) :
...
`L₂ ≁ c₁`. -/
theorem leaf_nbr_D (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16)) (L₁ c₁ c₂ L₂ : Fin 16)
...
theorem path_hub_incidence (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16)) (L₁ c₁ c₂ L₂ : Fin 16)
    (hcov : ∀ p q : Fin 16, p ∈ D → q ∈ D → G.Adj p q →
      p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
...
/-- **Hub and twin counts.**  In the `|D| = 8` corner there are eight hubs (`Dᶜ.card = 8`) and four
...
theorem hub_struct (G : SimpleGraph (Fin 16)) (D Iso : Finset (Fin 16)) (L₁ c₁ c₂ L₂ : Fin 16)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    ...
    (hD8 : D.card = 8) :
    Dᶜ.card = 8 ∧ Iso.card = 4 := by
  classical
  have hDc : Dᶜ.card = 8 := by
  ...
  set P : Finset (Fin 16) := {L₁, c₁, c₂, L₂} with hP
...
hub-neighbours and there are `8` hubs, then at least `4` hubs avoid all of `a, b, c`. -/
theorem avoiders_ge_four_aux (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16)) (a b c : Fin 16)
    (hDc8 : Dᶜ.card = 8)
    (hbound : (G.neighborFinset a ∩ Dᶜ).card + (G.neighborFinset b ∩ Dᶜ).card
      + (G.neighborFinset c ∩ Dᶜ).card ≤ 4) :
    4 ≤ (Dᶜ.filter (fun g => ¬G.Adj g a ∧ ¬G.Adj g b ∧ ¬G.Adj g c)).card := by
  classical
...
/-- **Each cherry has at least four avoiding hubs.**  Cherry `{L₁, c₁, c₂}` collects only
`2 + 1 + 1 = 4` hub-incidences (`L₁` two, `c₁`/`c₂` one each), so `≥ 4` of the eight hubs avoid it;
symmetrically for `{c₁, c₂, L₂}`. -/
theorem cherry_avoiders_ge_four (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16))
    (L₁ c₁ c₂ L₂ : Fin 16) (hDc8 : Dᶜ.card = 8)
    ...
    4 ≤ (Dᶜ.filter (fun g => ¬G.Adj g L₁ ∧ ¬G.Adj g c₁ ∧ ¬G.Adj g c₂)).card ∧
      4 ≤ (Dᶜ.filter (fun g => ¬G.Adj g c₁ ∧ ¬G.Adj g c₂ ∧ ¬G.Adj g L₂)).card := by
  refine ⟨avoiders_ge_four_aux G D L₁ c₁ c₂ hDc8 (by omega),
    avoiders_ge_four_aux G D c₁ c₂ L₂ hDc8 (by omega)⟩
...
theorem avoider_internal_ge_two (G : SimpleGraph (Fin 16)) (D Iso : Finset (Fin 16))
    (g allowed : Fin 16) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
    (hclass : ∀ x : Fin 16, G.Adj g x → x ∈ D → x = allowed ∨ x ∈ Iso) :
...
theorem fully_free_internal_ge_three (G : SimpleGraph (Fin 16)) (D Iso : Finset (Fin 16))
    (g : Fin 16) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
    (hclass : ∀ x : Fin 16, G.Adj g x → x ∈ D → x ∈ Iso) :
...
theorem path_iso_partition (G : SimpleGraph (Fin 16)) (D Iso : Finset (Fin 16))
    (L₁ c₁ c₂ L₂ : Fin 16)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hisochar : ∀ w : Fin 16, w ∈ D → w ≠ L₁ → w ≠ c₁ → w ≠ c₂ → w ≠ L₂ → w ∈ Iso)
    (hL1D : L₁ ∈ D) (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hL2D : L₂ ∈ D)
    (hac1L1 : G.Adj c₁ L₁) (hc12 : G.Adj c₁ c₂) (hac2L2 : G.Adj c₂ L₂) :
    ({L₁, c₁, c₂, L₂} : Finset (Fin 16)) ∪ Iso = D ∧
      Disjoint ({L₁, c₁, c₂, L₂} : Finset (Fin 16)) Iso := by
  classical
  set P : Finset (Fin 16) := {L₁, c₁, c₂, L₂} with hP
  have hIsoD : Iso ⊆ D := by rw [hIsodef]; exact Finset.filter_subset _ _
  have hPD : P ⊆ D := by
...
/-- **Residual incidence sums.**  Over the eight hubs the path-, iso- and hub-internal incidences
total `6`, `12`, `14` respectively, and per hub the three split the degree `4`:
`path(g) + iso(g) + int(g) = 4`. -/
theorem residual_incidence_sums (G : SimpleGraph (Fin 16)) (D Iso : Finset (Fin 16))
    (L₁ c₁ c₂ L₂ : Fin 16)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 16, G.Adj v w → G.degree w ≠ 3))
    (hisochar : ∀ w : Fin 16, w ∈ D → w ≠ L₁ → w ≠ c₁ → w ≠ c₂ → w ≠ L₂ → w ∈ Iso)
    ...
    (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 16))).card = 6) ∧
      (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Iso).card = 12) ∧
      (∀ g ∈ Dᶜ, (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 16))).card
  ...
  -- ∑ path = 6 via cross-count over P and the four leaf hub-incidences.
  have hsum_path : ∑ g ∈ Dᶜ, (G.neighborFinset g ∩ P).card = 6 := by
    ...
    omega
  -- ∑ iso = 12 via cross-count over Iso and each twin's three hub-neighbours.
  ...
  -- per-hub identity: path + iso + int = deg = 4
  obtain ⟨hDeq, hdisj⟩ :=
