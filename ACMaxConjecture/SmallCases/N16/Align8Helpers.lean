...
/-!
# Assembly cluster for the `n = 16` `e(M) = 3` / `e(M) ≥ 4` alignment corners

Ported `Fin 14 → Fin 16` analogues of the single-vertex / two-hub assembly lemmas used to close the
residual alignment corners (`exists_align_six_config_sixteen` `|D| = 8`, `halign8_sixteen`).  The
single-vertex selectors route the proved degree-`4` double-star / `C₅` hub-avoidance lemmas
(`hub_meets_path_le_one_sixteen`, `hub_cycle_cases_sixteen`) through `assemble_single_vertex_config`;
...
theorem assemble_single_vertex_config (G : SimpleGraph (Fin 16))
    (t h₁ h₂ x y z : Fin 16)
    (ht3 : G.degree t = 3) (htiso : ∀ w : Fin 16, G.Adj t w → G.degree w ≠ 3)
  ...
  have hsum0 : ∑ p ∈ ({t, h₁, h₂} : Finset (Fin 16)),
      (G.neighborFinset p ∩ ({x, y, z} : Finset (Fin 16))).card = 0 := by
    ...
    · exact hh₂y hadj
    · exact hh₂z hadj
  have hside : 2 * (∑ p ∈ ({t, h₁, h₂} : Finset (Fin 16)),
      (G.neighborFinset p ∩ ({x, y, z} : Finset (Fin 16))).card)
...
theorem dom_doublestar_leaves (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16))
    ...
    (hdom : ∀ p q : Fin 16, p ∈ D → q ∈ D → G.Adj p q →
      p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂) :
    ∃ ℓ₁ ℓ₂ ℓ₃ ℓ₄ : Fin 16,
      G.degree ℓ₁ = 3 ∧ G.degree ℓ₂ = 3 ∧ G.degree ℓ₃ = 3 ∧ G.degree ℓ₄ = 3 ∧
      ({c₁, c₂, ℓ₁, ℓ₂, ℓ₃, ℓ₄} : Finset (Fin 16)).card = 6 ∧
  ...
  have hcross : ∀ u v : Fin 16, G.neighborFinset u ∩ D = {c₁} →
      G.Adj c₂ v → u ≠ v := by
    intro u v hu hcv e
    subst e
  ...
  have hcard6 : ({c₁, c₂, ℓ₁, ℓ₂, ℓ₃, ℓ₄} : Finset (Fin 16)).card = 6 := by
...
theorem dom_centres_indeg3 (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16))
    (c₁ c₂ : Fin 16) (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hc12 : G.Adj c₁ c₂)
  ...
  have hc1mem : c₁ ∈ G.neighborFinset c₂ ∩ D :=
    Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hc12.symm, hc1D⟩
  have hb1eq : (G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 16)))).card
      = (G.neighborFinset c₁ ∩ D).card - 1 := by
    have heq : G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 16)))
  ...
  have hb2eq : (G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 16)))).card
      = (G.neighborFinset c₂ ∩ D).card - 1 := by
    have heq : G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 16)))
        = (G.neighborFinset c₂ ∩ D).erase c₁ := by
      ext w
...
theorem single_vertex_doublestar_count (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16))
    (hmemD : ∀ v : Fin 16, v ∈ D ↔ G.degree v = 3)
    (hT : ¬∃ x y z : Fin 16, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    ...
    (t a b c : Fin 16) (ht3 : G.degree t = 3)
    (htiso : ∀ w : Fin 16, G.Adj t w → G.degree w ≠ 3)
  ...
  have leafdeg : ∀ w : Fin 16, (w = ℓ₁ ∨ w = ℓ₂ ∨ w = ℓ₃ ∨ w = ℓ₄) → G.degree w = 3 := by
    rintro w (rfl | rfl | rfl | rfl) <;> assumption
  have leafD : ∀ w : Fin 16, (w = ℓ₁ ∨ w = ℓ₂ ∨ w = ℓ₃ ∨ w = ℓ₄) → w ∈ D :=
    fun w hw => (hmemD w).mpr (leafdeg w hw)
  have hlne : ∀ w cen oth : Fin 16, G.neighborFinset w ∩ D = {cen} →
      (G.neighborFinset oth ∩ D).card = 3 → w ≠ oth := by
    intro w cen oth hw hoth e
    subst e
    rw [hw, Finset.card_singleton] at hoth
    omega
  have hleafnec : ∀ w : Fin 16, (w = ℓ₁ ∨ w = ℓ₂ ∨ w = ℓ₃ ∨ w = ℓ₄) → w ≠ c₁ ∧ w ≠ c₂ := by
  ...
  have hleafnonadj : ∀ u v : Fin 16, (u = ℓ₁ ∨ u = ℓ₂ ∨ u = ℓ₃ ∨ u = ℓ₄) →
  ...
  have hubcen : ∀ hh : Fin 16, (hh = a ∨ hh = b ∨ hh = c) →
  ...
  have ind2 : ∀ p q : Prop, ¬(p ∧ q) →
    ...
    · rw [if_neg hp]
      split_ifs <;> omega
  have hpair : ∀ hh : Fin 16, G.degree hh = 4 →
      ¬(G.Adj hh ℓ₁ ∧ G.Adj hh ℓ₂) ∧ ¬(G.Adj hh ℓ₃ ∧ G.Adj hh ℓ₄) := by
    intro hh hh4
    refine ⟨?_, ?_⟩
    · exact (hub_meets_path_le_one_sixteen G hT hC4 hh4 hd1 hc1deg hd2
        hac1.symm hac2
        (hleafnonadj ℓ₁ ℓ₂ (Or.inl rfl) (Or.inr (Or.inl rfl)) hℓ12)
        (G.ne_of_adj hac1).symm (G.ne_of_adj hac2) hℓ12).2.2
    · exact (hub_meets_path_le_one_sixteen G hT hC4 hh4 hd3 hc2deg hd4
        hac3.symm hac4
  ...
  have keyleaf : ∃ w : Fin 16, (w = ℓ₁ ∨ w = ℓ₂ ∨ w = ℓ₃ ∨ w = ℓ₄) ∧
  ...
  have key : ∃ w' h₁ h₂ : Fin 16, (w' = ℓ₁ ∨ w' = ℓ₂ ∨ w' = ℓ₃ ∨ w' = ℓ₄) ∧
...
theorem single_vertex_config_from_C5_three_hubs (G : SimpleGraph (Fin 16))
    (hT : ¬∃ x y z : Fin 16, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 16, ({a, b, c, d} : Finset (Fin 16)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (t h₁ h₂ h₃ : Fin 16) (ht3 : G.degree t = 3)
    (htiso : ∀ w : Fin 16, G.Adj t w → G.degree w ≠ 3)
  ...
  have mg1 : ∀ ha hb : Fin 16, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
  ...
  have mg2 : ∀ ha hb : Fin 16, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
      G.Adj t ha → G.Adj t hb →
      ¬G.Adj ha v₂ → ¬G.Adj ha v₃ → ¬G.Adj ha v₄ →
      ¬G.Adj hb v₂ → ¬G.Adj hb v₃ → ¬G.Adj hb v₄ → SingleVertexConfig G :=
    fun ha hb hca hcb hcne hta htb a1 a2 a3 b1 b2 b3 =>
  ...
  have mg3 : ∀ ha hb : Fin 16, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
      G.Adj t ha → G.Adj t hb →
      ¬G.Adj ha v₃ → ¬G.Adj ha v₄ → ¬G.Adj ha v₅ →
      ¬G.Adj hb v₃ → ¬G.Adj hb v₄ → ¬G.Adj hb v₅ → SingleVertexConfig G :=
    fun ha hb hca hcb hcne hta htb a1 a2 a3 b1 b2 b3 =>
  ...
  have mg4 : ∀ ha hb : Fin 16, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
      G.Adj t ha → G.Adj t hb →
      ¬G.Adj ha v₄ → ¬G.Adj ha v₅ → ¬G.Adj ha v₁ →
      ¬G.Adj hb v₄ → ¬G.Adj hb v₅ → ¬G.Adj hb v₁ → SingleVertexConfig G :=
    fun ha hb hca hcb hcne hta htb a1 a2 a3 b1 b2 b3 =>
  ...
  have mg5 : ∀ ha hb : Fin 16, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
      G.Adj t ha → G.Adj t hb →
      ¬G.Adj ha v₅ → ¬G.Adj ha v₁ → ¬G.Adj ha v₂ →
      ¬G.Adj hb v₅ → ¬G.Adj hb v₁ → ¬G.Adj hb v₂ → SingleVertexConfig G :=
    fun ha hb hca hcb hcne hta htb a1 a2 a3 b1 b2 b3 =>
  ...
  have hB := hub_cycle_cases_sixteen G hT hC4 h₁ v₁ v₂ v₃ v₄ v₅ hh₁4 hd1 hd2 hd3 hd4 hd5
    e12 e23 e34 e45 e51 n13 n14 n24 n25 n35 d12 d13 d14 d15 d23 d24 d25 d34 d35 d45
  have hC := hub_cycle_cases_sixteen G hT hC4 h₂ v₁ v₂ v₃ v₄ v₅ hh₂4 hd1 hd2 hd3 hd4 hd5
    e12 e23 e34 e45 e51 n13 n14 n24 n25 n35 d12 d13 d14 d15 d23 d24 d25 d34 d35 d45
  have hD := hub_cycle_cases_sixteen G hT hC4 h₃ v₁ v₂ v₃ v₄ v₅ hh₃4 hd1 hd2 hd3 hd4 hd5
    e12 e23 e34 e45 e51 n13 n14 n24 n25 n35 d12 d13 d14 d15 d23 d24 d25 d34 d35 d45
...
/-- **Shared degree-`4` hub when there is no degree-`5` vertex (`n = 16`).**  Every hub then has
degree exactly `4`, so `|D| = 8`, `|Hub₄| = 7`, and each of the `≥ 3` `M`-isolated twins meets all
three of its hub-neighbours inside `Hub₄`; the pigeonhole (`3·3 = 9 > 8`) yields a shared
degree-`4` hub. -/
theorem shared_deg4_hub_nodeg5_sixteen (G : SimpleGraph (Fin 16))
  ...
  have hsum48 : ∑ v : Fin 16, G.degree v = 56 := by
  ...
  have hHub4card : Hub4.card = 8 := by rw [hHub4eqDc]; omega
...
/-- **Shared degree-`4` hub in the degree-`5` regime (`n = 16`).**  When a degree-`≥ 5` vertex `g`
exists, the residual is `|D| ∈ {9, 10, 11}` with at most `|D| − 8` degree-`≥ 5` hubs (total
degree-excess `8`).  Unlike `n = 15` (excess `7`), the naive pigeonhole `(3 − |H₅|)·|Iso|` is **not**
strict in the maximal-`e(M)` corner; under the `e(M) ≤ 4` hypothesis (`hs8`, the regime in which the
caller invokes it — a dominating `(3,2)` double-star) we route around the degree-`5` hubs with the
**refined cross count** `∑_{v∈Iso}|N v ∩ Hub₄| = 3·|Iso| − ∑_{h∈H₅}|N h ∩ Iso| ≥ 3·|Iso| − ∑_{H₅}deg`
and the degree identity `∑_{H₅}deg = (56 − 3|D|) − 4·|Hub₄|`.  With `|Iso| ≥ |D| − 5` (from `e(M) ≤
4`) and `|H₅| ≤ |D| − 8` this forces `|Hub₄| < ∑ |N ∩ Hub₄|`, hence two twins share a degree-`4`
hub.  (`hs8` is the n=16-specific delta over the n=15 port; see `shared_deg4_hub_nodeg5_sixteen`,
which already takes the same bound.) -/
theorem shared_deg4_hub_deg5_sixteen (G : SimpleGraph (Fin 16))
    (hm : G.edgeFinset.card = 28) (h3 : ∀ v : Fin 16, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 16, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 16, ({a, b, c, d} : Finset (Fin 16)).card = 4 ∧
  ...
  have hsumD : ∑ v ∈ D, G.degree v = 3 * D.card := by
    rw [Finset.sum_congr rfl (fun v hv => (hmemD v).mp hv), Finset.sum_const, smul_eq_mul,
      mul_comm]
  have hsum52 : ∑ v : Fin 16, G.degree v = 56 := by
    rw [SimpleGraph.sum_degrees_eq_twice_card_edges, hm]
  have hsplit : ∑ v ∈ D, G.degree v + ∑ v ∈ Dᶜ, G.degree v = 56 := by
