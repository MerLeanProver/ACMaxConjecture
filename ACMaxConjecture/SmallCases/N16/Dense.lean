...
/-!
# Dense `TwoTwin`-existence core for the `n = 16` twin certificate

Ported from the proved `n = 14` development (`TwinCert14Align6Dense`, `TwinCert14Align8`,
`TwinCert14C5`).  These are the reusable core lemmas that the `n = 16` alignment dichotomies
(`exists_align_four/six_config_sixteen`, `halign8_sixteen`) all call: the verified `n = 16`
...
* `thin_eM_formula_sixteen` — the dominating-edge `e(M)` formula `∑ + 2 = 2·inM(c₁) + 2·inM(c₂)`
  (structural, `n`-independent up to the ambient `cross_count`);
* `distinct_five_sixteen` / `hub_cycle_cases_sixteen` — the induced-`C₅` geometry helpers;
* `shared_deg4_hub_from_count_sixteen` — pigeonhole producing a shared degree-`4` hub;
* `claw_shared_two_twin_sixteen` — fat claw centre + shared hub ⇒ `TwoTwinConfig`;
* `dom_fat_centre_two_twin_sixteen` — dominating-edge double-star + fat centre ⇒ `TwoTwinConfig`;
* `c5_shared_two_twin_sixteen` — induced-`C₅` + shared hub ⇒ `TwoTwinConfig`.

The geometric/cherry-avoidance arguments port verbatim; only the cardinality bookkeeping changes
(`Fin 16`, the `cross_count` / `card_four_sixteen` / `hub_meets_path_le_one_sixteen` /
...
/-- **`e(M)` formula in a dominating-edge case (`n = 16`).**  When every `M`-edge meets the edge
...
`thin_eM_formula_fourteen`; the underlying handshake `e(M) = 20 − 3·|Hub| + e_H` is `n`-sensitive
but enters only through the calling dichotomy, not this identity. -/
theorem thin_eM_formula_sixteen (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16))
  ...
  have hpair2 : ∑ w ∈ ({c₁, c₂} : Finset (Fin 16)),
    ...
    rw [Finset.sum_insert (by simp [hc12.ne]), Finset.sum_singleton]
  have he1 : (G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 16)))).card
      = (G.neighborFinset c₁ ∩ D).card - 1 := by
    have hset : G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 16)))
  ...
  have he2 : (G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 16)))).card
      = (G.neighborFinset c₂ ∩ D).card - 1 := by
    have hset : G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 16)))
        = (G.neighborFinset c₂ ∩ D).erase c₁ := by
      apply Finset.Subset.antisymm
...
/-- **Distinctness from a five-element literal (`n = 16`).**  A `Finset` literal `{a, b, c, d, e}`
of cardinality `5` over `Fin 16` has its five entries pairwise distinct.  Port of
`distinct_five_fourteen`. -/
theorem distinct_five_sixteen (a b c d e : Fin 16)
    (h : ({a, b, c, d, e} : Finset (Fin 16)).card = 5) :
    a ≠ b ∧ a ≠ c ∧ a ≠ d ∧ a ≠ e ∧ b ≠ c ∧ b ≠ d ∧ b ≠ e ∧
      c ≠ d ∧ c ≠ e ∧ d ≠ e := by
  classical
  have b3 : ∀ x y z : Fin 16, ({x, y, z} : Finset (Fin 16)).card ≤ 3 := by
    intro x y z
    have h1 := Finset.card_insert_le x ({y, z} : Finset (Fin 16))
    have h2 := Finset.card_insert_le y ({z} : Finset (Fin 16))
    simp only [Finset.card_singleton] at *
    omega
  have b4 : ∀ w x y z : Fin 16, ({w, x, y, z} : Finset (Fin 16)).card ≤ 4 := by
    intro w x y z
    have h1 := Finset.card_insert_le w ({x, y, z} : Finset (Fin 16))
    have h2 := b3 x y z
    omega
  have ha : a ∉ ({b, c, d, e} : Finset (Fin 16)) := by
    intro hmem
    have : ({a, b, c, d, e} : Finset (Fin 16)).card ≤ 4 := by
      rw [Finset.insert_eq_self.mpr hmem]; exact b4 b c d e
    omega
  have hca4 : ({b, c, d, e} : Finset (Fin 16)).card = 4 := by
    rw [Finset.card_insert_of_notMem ha] at h; omega
  have hb : b ∉ ({c, d, e} : Finset (Fin 16)) := by
    intro hmem
    have : ({b, c, d, e} : Finset (Fin 16)).card ≤ 3 := by
      rw [Finset.insert_eq_self.mpr hmem]; exact b3 c d e
    omega
  have hcb3 : ({c, d, e} : Finset (Fin 16)).card = 3 := by
    rw [Finset.card_insert_of_notMem hb] at hca4; omega
  have hc : c ∉ ({d, e} : Finset (Fin 16)) := by
    intro hmem
    have h1 := Finset.card_insert_le d ({e} : Finset (Fin 16))
    have : ({c, d, e} : Finset (Fin 16)).card ≤ 2 := by
      rw [Finset.insert_eq_self.mpr hmem]
      simp only [Finset.card_singleton] at *; omega
    omega
  have hcc2 : ({d, e} : Finset (Fin 16)).card = 2 := by
...
/-- **A degree-`4` hub meets at most one cycle vertex (`n = 16`).**  Around an induced `C₅`
`v₁–⋯–v₅` of degree-`3` vertices, a degree-`4` hub `h` is adjacent to at most one `vᵢ`: two hits on
a cycle `P₃` give a triangle or good `C₄` (`hub_meets_path_le_one_sixteen`).  Packaged as a `6`-way
split.  Port of `hub_cycle_cases`. -/
theorem hub_cycle_cases_sixteen (G : SimpleGraph (Fin 16))
    (hT : ¬∃ x y z : Fin 16, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 16, ({a, b, c, d} : Finset (Fin 16)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (h v₁ v₂ v₃ v₄ v₅ : Fin 16) (hh : G.degree h = 4)
  ...
  have P123 := hub_meets_path_le_one_sixteen G hT hC4 hh hd1 hd2 hd3 e12 e23 n13 d12 d23 d13
  have P234 := hub_meets_path_le_one_sixteen G hT hC4 hh hd2 hd3 hd4 e23 e34 n24 d23 d34 d24
  have P345 := hub_meets_path_le_one_sixteen G hT hC4 hh hd3 hd4 hd5 e34 e45 n35 d34 d45 d35
  have P451 := hub_meets_path_le_one_sixteen G hT hC4 hh hd4 hd5 hd1 e45 e51
    (fun hh => n14 hh.symm) d45 d15.symm d14.symm
  have P512 := hub_meets_path_le_one_sixteen G hT hC4 hh hd5 hd1 hd2 e51 e12
...
/-- **Shared degree-`4` hub by pigeonhole (`n = 16`).**  If the degree-`4` hub set `Hub4` carries
strictly more twin-incidences from the `M`-isolated set `Iso` than it has vertices, some hub is
adjacent to two distinct `M`-isolated degree-`3` twins.  Pure pigeonhole; for `n = 16` the calling
dichotomy adapts the count to `|Hub| ∈ {5, 6, 7}` and its degree-`4` subset.  Port of
`shared_deg4_hub_from_count`. -/
theorem shared_deg4_hub_from_count_sixteen (G : SimpleGraph (Fin 16)) (Iso Hub4 : Finset (Fin 16))
  ...
  have hincid : ∃ h ∈ Hub4, 2 ≤ (Iso.filter (fun v => G.Adj v h)).card := by
    ...
    have hswap : ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub4).card
      ...
      have hL : ∀ v : Fin 16, (G.neighborFinset v ∩ Hub4).card
...
/-- **Fat-dominating claw assembly (`n = 16`).**  A degree-`3` centre `c` whose three neighbours
...
theorem claw_shared_two_twin_sixteen (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16))
...
/-- **Dominating-edge fat-centre dispatch (`n = 16`).**  Given a dominating edge `c₁ ∼ c₂` (every
`M`-edge meets it) with `∑_{v∈D}|N v ∩ D| ≥ 8`, the in-`M`-degree formula `thin_eM_formula_sixteen`
forces one endpoint to have in-`M`-degree `3` (a fat centre); routing the shared degree-`4` hub
through `claw_shared_two_twin_sixteen` yields a `TwoTwinConfig`.  Port of `dom_fat_centre_two_twin`;
the dense threshold `8` is supplied by the caller via the `n = 16` handshake.  -/
theorem dom_fat_centre_two_twin_sixteen (G : SimpleGraph (Fin 16)) (D : Finset (Fin 16))
    (hmemD : ∀ v : Fin 16, v ∈ D ↔ G.degree v = 3)
    (hT : ¬∃ x y z : Fin 16, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 16, ({a, b, c, d} : Finset (Fin 16)).card = 4 ∧
    ...
    (k t₁ t₂ : Fin 16) (hhdeg4 : G.degree k = 4) (ht12 : t₁ ≠ t₂)
  ...
  · exact claw_shared_two_twin_sixteen G D hmemD hT hC4 c₁ t₁ t₂ k hc1D hc1 ht1deg ht2deg hhdeg4
      ht12 hAt1k hAt2k ht1iso ht2iso
  · have hc2 : 3 ≤ (G.neighborFinset c₂ ∩ D).card := by omega
    exact claw_shared_two_twin_sixteen G D hmemD hT hC4 c₂ t₁ t₂ k hc2D hc2 ht1deg ht2deg hhdeg4
      ht12 hAt1k hAt2k ht1iso ht2iso

/-- **Induced-`C₅` two-twin assembly (`n = 16`).**  A degree-`4` hub `k` adjacent to two distinct
`M`-isolated degree-`3` twins, together with an induced `C₅` of degree-`3` vertices, yields a
`TwoTwinConfig`: `hub_cycle_cases_sixteen` gives a consecutive triple of the cycle that `k` avoids,
forming the cherry.  Port of `c5_shared_two_twin`. -/
theorem c5_shared_two_twin_sixteen (G : SimpleGraph (Fin 16))
    (hT : ¬∃ x y z : Fin 16, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 16, ({a, b, c, d} : Finset (Fin 16)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
  ...
  have hB := hub_cycle_cases_sixteen G hT hC4 k v₁ v₂ v₃ v₄ v₅ hhdeg4 hd1 hd2 hd3 hd4 hd5
