...
import ACMaxConjecture.SmallCases.N18.DenseLe5
import ACMaxConjecture.SmallCases.N18.Align8Helpers

/-!
# `n = 18`, `|D| = 12`, `|Hub| = 6`, `e(M) = 5` (`s = 10`) shared-hub helper

The genuinely degenerate `e(M) = 5` corner of the `|Hub| = 6` alignment selector
(`two_hub_or_single_vertex_hub6_eighteen`), ported from `TwinCert16TwoHubHub6Deg5.lean`.  At
`e(M) = 5` the residual is a double-star (dominating edge) or an induced `C₅`.  The deg-`4`-shared-hub
pigeonhole used at `e(M) ≤ 4` degenerates; for `n = 18` (degree sum `64`, divisible by `3`) it no
longer even *ties* strictly in the `C₅` branch, so we route through a deg-`≤ 5` shared hub:

* **double-star branch (fully proved):** the `M`-isolated twins each meet `3` hubs; with `|Hub| = 6`
  and at most one degree-`≥ 6` hub the deg-`≤ 5` count is strict, so a deg-`≤ 5` hub is shared by two
  `M`-isolated twins, and the deg-`≤ 5` leaf-cherry (`claw`) assembly closes the cut.
...
* **induced-`C₅` branch (fully proved):** the deg-`4` pigeonhole ties (`X ≥ 3`, `|Hub₄| = 3`), so
  ...
  `exists_nonk_two_twin_hub_eighteen`: those three hit vertices have the deg-`5` hub `k` as their
  unique hub neighbour, so a second deg-`≤ 5` hub `h ≠ k` (with two `M`-isolated twins, extracted by
  the refined `hub5_iso_count_eighteen` pigeonhole on `Hub₅ \ {k}`) avoids that cherry instead.

The helper lemmas (`shared_hub_le5_from_count_eighteen`, `dense_two_twin_assemble_le5_eighteen`,
`claw_shared_two_twin_le5_eighteen`, `dom_fat_centre_two_twin_le5_eighteen`,
`shared_hub_le5_eM5_eighteen`) are all axiom-clean ports.
-/
...
/-- **Shared deg-`≤ 5` hub by pigeonhole (`n = 18`).**  Port of `shared_deg4_hub_from_count_eighteen`
...
theorem shared_hub_le5_from_count_eighteen (G : SimpleGraph (Fin 18)) (Iso Hub : Finset (Fin 18))
    (hIsoiso : ∀ v ∈ Iso, G.degree v = 3 ∧ ∀ w : Fin 18, G.Adj v w → G.degree w ≠ 3)
    (hHubdeg : ∀ h ∈ Hub, G.degree h ≤ 5)
    (hcount : Hub.card < ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub).card) :
    ∃ h t₁ t₂ : Fin 18, h ∈ Hub ∧ G.degree h ≤ 5 ∧ t₁ ≠ t₂ ∧
  ...
  have hincid : ∃ h ∈ Hub, 2 ≤ (Iso.filter (fun v => G.Adj v h)).card := by
    ...
    have hswap : ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub).card
      ...
      have hL : ∀ v : Fin 18, (G.neighborFinset v ∩ Hub).card
...
/-- **Fat-dominating claw assembly with a deg-`≤ 5` hub (`n = 18`).**  Port of
`claw_shared_two_twin_eighteen` permitting `4 ≤ deg h ≤ 5`: the C₄ obstruction `h–i–c–j` has degree
sum `deg h + 9 ≤ 14`, so it is still forbidden by `hC4` at `deg h = 5`, and the hub meets at most
one of `c`'s three degree-`3` neighbours. -/
theorem claw_shared_two_twin_le5_eighteen (G : SimpleGraph (Fin 18)) (D : Finset (Fin 18))
    ...
    (ht1iso : ∀ w : Fin 18, G.Adj t₁ w → G.degree w ≠ 3)
    (ht2iso : ∀ w : Fin 18, G.Adj t₂ w → G.degree w ≠ 3) :
    TwoTwinConfig G := by
  classical
  ...
  have hmem_i : ∀ w : Fin 18, w ∈ ({n₁, n₂, n₃} : Finset (Fin 18)) → G.Adj c w ∧ w ∈ D := by
    intro w hw
  ...
  have hcnbhd : ∀ w : Fin 18, G.Adj c w → w = n₁ ∨ w = n₂ ∨ w = n₃ := by
    intro w hw
  ...
  have hmeet : ∀ i j : Fin 18, G.degree i = 3 → G.degree j = 3 →
    ...
    exact hC4 ⟨h, i, c, j,
      card_four_eighteen h i c j (by rintro rfl; omega) (by rintro rfl; omega)
  ...
  · exact dense_two_twin_assemble_le5_eighteen G t₁ t₂ h n₂ c n₃ ht1deg ht2deg hhge4 hhle5
...
/-- **Dominating-edge fat-centre dispatch with a deg-`≤ 5` hub (`n = 18`).**  Port of
`dom_fat_centre_two_twin_eighteen` with the hub permitted degree `4` or `5`. -/
theorem dom_fat_centre_two_twin_le5_eighteen (G : SimpleGraph (Fin 18)) (D : Finset (Fin 18))
    (hmemD : ∀ v : Fin 18, v ∈ D ↔ G.degree v = 3)
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (c₁ c₂ : Fin 18) (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hc12 : G.Adj c₁ c₂)
    (hcov : ∀ p q : Fin 18, p ∈ D → q ∈ D → G.Adj p q →
      p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
    (hge : 8 ≤ ∑ v ∈ D, (G.neighborFinset v ∩ D).card)
    (hindle : ∀ x : Fin 18, x ∈ D → (G.neighborFinset x ∩ D).card ≤ 3)
    (k t₁ t₂ : Fin 18) (hkge4 : 4 ≤ G.degree k) (hkle5 : G.degree k ≤ 5) (ht12 : t₁ ≠ t₂)
    (ht1deg : G.degree t₁ = 3) (ht2deg : G.degree t₂ = 3)
  ...
  · exact claw_shared_two_twin_le5_eighteen G D hmemD hT hC4 c₁ t₁ t₂ k hc1D hc1 ht1deg ht2deg
      hkge4 hkle5 ht12 hAt1k hAt2k ht1iso ht2iso
  · have hc2 : 3 ≤ (G.neighborFinset c₂ ∩ D).card := by omega
    exact claw_shared_two_twin_le5_eighteen G D hmemD hT hC4 c₂ t₁ t₂ k hc2D hc2 ht1deg ht2deg
      hkge4 hkle5 ht12 hAt1k hAt2k ht1iso ht2iso

/-- **Shared deg-`≤ 5` hub in the `e(M) = 5`, `|D| = 12`, `|Hub| = 6` regime (`n = 18`).**  The
`M`-isolated twins each meet `3` hubs; with `|Hub| = 6` and degree sum `27` (excess `3`) at most one
hub has degree `≥ 6`, so excluding it leaves enough twin-incidences on the deg-`≤ 5` hubs
(`|Iso| ≥ 5`) to force, by a strict pigeonhole, a deg-`≤ 5` hub shared by two `M`-isolated twins. -/
theorem shared_hub_le5_eM5_eighteen (G : SimpleGraph (Fin 18))
  ...
  set Hub5 : Finset (Fin 18) :=
    Finset.univ.filter (fun v => 4 ≤ G.degree v ∧ G.degree v ≤ 5) with hHub5def
  have hmemHub5 : ∀ v : Fin 18, v ∈ Hub5 ↔ 4 ≤ G.degree v ∧ G.degree v ≤ 5 := by
    intro v; rw [hHub5def]; simp
  set H6 : Finset (Fin 18) := Finset.univ.filter (fun v => 6 ≤ G.degree v) with hH6def
  have hmemH6 : ∀ v : Fin 18, v ∈ H6 ↔ 6 ≤ G.degree v := by intro v; rw [hH6def]; simp
  have hHubeqDc : Hub = Dᶜ := by
    ext w; rw [hmemHub, Finset.mem_compl, hmemD]; have := h3 w; omega
  -- `∑_{Hub} deg = 26` from the handshake.
  have hsum56 : ∑ v : Fin 18, G.degree v = 64 := by
  ...
  have hsumD : ∑ v ∈ D, G.degree v = 3 * D.card := by
    rw [Finset.sum_congr rfl (fun v hv => (hmemD v).mp hv), Finset.sum_const, smul_eq_mul,
      mul_comm]
  have hsplit : ∑ v ∈ D, G.degree v + ∑ v ∈ Dᶜ, G.degree v = 64 := by
    rw [Finset.sum_add_sum_compl]; exact hsum56
  have hHubsum : ∑ v ∈ Hub, G.degree v = 64 - 3 * D.card := by
    rw [hHubeqDc]; rw [hsumD] at hsplit; omega
  have hHubcard : Hub.card = 18 - D.card := by
  ...
  -- `Iso`.
  set Iso : Finset (Fin 18) := D.filter (fun v => (G.neighborFinset v ∩ D).card = 0) with hIsodef
  have hIsoiso : ∀ v ∈ Iso, G.degree v = 3 ∧ ∀ w : Fin 18, G.Adj v w → G.degree w ≠ 3 := by
    intro v hv
  ...
  obtain ⟨h, t₁, t₂, _, hd5, hne, hd1, hd2, ha1, ha2, hi1, hi2⟩ :=
    shared_hub_le5_from_count_eighteen G Iso Hub5 hIsoiso hHub5deg hcount
  exact ⟨h, t₁, t₂, hd5, hne, hd1, hd2, ha1, ha2, hi1, hi2⟩

/-- **An induced `C₅` accounts for all of `M` at `e(M) = 5` (`n = 18`).**  If `v₁–⋯–v₅` is an
...
theorem nonIso_le_five_of_C5_eighteen (G : SimpleGraph (Fin 18)) (D : Finset (Fin 18))
    (v₁ v₂ v₃ v₄ v₅ : Fin 18)
    (hv1D : v₁ ∈ D) (hv2D : v₂ ∈ D) (hv3D : v₃ ∈ D) (hv4D : v₄ ∈ D) (hv5D : v₅ ∈ D)
    (hcard5 : ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 18)).card = 5)
  ...
  set C : Finset (Fin 18) := {v₁, v₂, v₃, v₄, v₅} with hCdef
  have two_nbrs : ∀ a x y : Fin 18, G.Adj a x → G.Adj a y → x ∈ D → y ∈ D → x ≠ y →
      2 ≤ (G.neighborFinset a ∩ D).card := by
    intro a x y hax hay hxD hyD hxy
    have hsub : ({x, y} : Finset (Fin 18)) ⊆ G.neighborFinset a ∩ D := by
    ...
    have hcard2 : ({x, y} : Finset (Fin 18)).card = 2 := by
      rw [Finset.card_insert_of_notMem (by simp [hxy]), Finset.card_singleton]
    calc 2 = ({x, y} : Finset (Fin 18)).card := hcard2.symm
...
/-- **Refined `e(M) = 5` handshake count (`|Hub| = 6`, `n = 18`).**  With at most one degree-`6` hub
(`∑_{Hub} deg = 27`), the degree-`≤ 5` hubs `Hub₅` carry, summed over the `M`-isolated set `Iso`,
strictly more than `|Hub₅| + 1` incidences: `|Hub₅| + 2 ≤ ∑_{Iso}|N ∩ Hub₅|`.  This sharpens the
`< ` of `shared_hub_le5_eM5_eighteen` to leave room for deleting one hub. -/
theorem hub5_iso_count_eighteen (G : SimpleGraph (Fin 18))
    (D Hub Hub5 H6 Iso : Finset (Fin 18))
    (hmemD : ∀ v : Fin 18, v ∈ D ↔ G.degree v = 3)
    (hmemHub : ∀ v : Fin 18, v ∈ Hub ↔ 4 ≤ G.degree v)
    (hmemHub5 : ∀ v : Fin 18, v ∈ Hub5 ↔ 4 ≤ G.degree v ∧ G.degree v ≤ 5)
    (hmemH6 : ∀ v : Fin 18, v ∈ H6 ↔ 6 ≤ G.degree v)
    (hIsoeq : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    (hDge9 : 9 ≤ D.card) (hIsoge : D.card - 5 ≤ Iso.card) :
  ...
  have hHub5sub : ∀ v ∈ Hub5, 4 ≤ G.degree v := fun v hv => ((hmemHub5 v).mp hv).1
  have hH6ge : ∀ v ∈ H6, 6 ≤ G.degree v := fun v hv => (hmemH6 v).mp hv
  have hmemIso : ∀ v : Fin 18, v ∈ Iso ↔ v ∈ D ∧ (G.neighborFinset v ∩ D).card = 0 := by
...
/-- **A second deg-`≤ 5` shared hub avoiding a given hub `k` (`e(M) = 5`, `|Hub| = 6`, `n = 18`).**
Suppose `k` is a degree-`≤ 5` hub whose neighbourhood contains a degree-`3` path `a–b–c` (so `a, b, c`
are *non*-`M`-isolated, with `k` their unique hub neighbour).  Then there is a *different* deg-`≤ 5`
hub `h ≠ k` carrying two distinct `M`-isolated degree-`3` twins.  The refined count
`hub5_iso_count_eighteen` (`|Hub₅| + 2 ≤ ∑_{Iso}|N ∩ Hub₅|`) and the bound that `k` absorbs at most
two `Iso`-incidences (its other three neighbours `a, b, c` are non-isolated) keep the pigeonhole of
`shared_hub_le5_from_count_eighteen` strict on `Hub₅ \ {k}`. -/
theorem exists_nonk_two_twin_hub_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    (hDge9 : 9 ≤ (Finset.univ.filter (fun w : Fin 18 => G.degree w = 3)).card)
    (v₁ v₂ v₃ v₄ v₅ : Fin 18)
    ...
    (e12 : G.Adj v₁ v₂) (e23 : G.Adj v₂ v₃) (e34 : G.Adj v₃ v₄)
    (e45 : G.Adj v₄ v₅) (e51 : G.Adj v₅ v₁)
    (hsum10 : ∑ v ∈ Finset.univ.filter (fun w : Fin 18 => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w : Fin 18 => G.degree w = 3)).card = 10)
    (k a b c : Fin 18) (hk4 : 4 ≤ G.degree k) (hk5 : G.degree k ≤ 5)
    ...
    ∃ h tt₁ tt₂ : Fin 18, h ≠ k ∧ 4 ≤ G.degree h ∧ G.degree h ≤ 5 ∧ tt₁ ≠ tt₂ ∧
      G.degree tt₁ = 3 ∧ G.degree tt₂ = 3 ∧ G.Adj tt₁ h ∧ G.Adj tt₂ h ∧
      (∀ w : Fin 18, G.Adj tt₁ w → G.degree w ≠ 3) ∧
      (∀ w : Fin 18, G.Adj tt₂ w → G.degree w ≠ 3) := by
  classical
  set D : Finset (Fin 18) := Finset.univ.filter (fun w => G.degree w = 3) with hDdef
  have hmemD : ∀ v : Fin 18, v ∈ D ↔ G.degree v = 3 := by intro v; rw [hDdef]; simp
  set Hub : Finset (Fin 18) := Finset.univ.filter (fun v => 4 ≤ G.degree v) with hHubdef
  ...
  have hIsoge : D.card - 5 ≤ Iso.card := by
    have hnon5 := nonIso_le_five_of_C5_eighteen G D v₁ v₂ v₃ v₄ v₅
  ...
  have hcount2 : Hub5.card + 2 ≤ ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub5).card :=
    hub5_iso_count_eighteen G D Hub Hub5 H6 Iso hmemD hmemHub hmemHub5 hmemH6 hIsodef hm h3
      hDge9 hIsoge
  ...
  set S : Finset (Fin 18) := Iso.filter (fun v => G.Adj v k) with hSdef
  ...
  -- Pigeonhole on `Hub₅ \ {k}` via `shared_hub_le5_from_count_eighteen`.
  obtain ⟨h, tt₁, tt₂, hhErase, hh5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1iso, ht2iso⟩ :=
    shared_hub_le5_from_count_eighteen G Iso (Hub5.erase k) hIsoiso
...
/-- **`e(M) = 5` (`s = 10`) two-twin cut for the `|D| = 12`, `|Hub| = 6` corner (`n = 18`).**  Unlike
`n = 16`, the degree sum `64` is divisible by `3`, so the *degree-`4`* shared-hub pigeonhole only
*ties* (`X ≥ 3`, `|Hub₄| = 3`) in the induced-`C₅` branch and is no longer strict.  We therefore
route the **dominating-edge** branch through the deg-`≤ 5` shared hub (`shared_hub_le5_eM5`,
strict because `|Hub| = 6`) and the deg-`≤ 5` leaf-cherry (`claw`) assembly, and the **induced-`C₅`**
branch through `c5_shared_two_twin_le5_eighteen` (`TwinCert18DenseLe5`).  The former residual — a
...
extraction `exists_nonk_two_twin_hub_eighteen` threaded as `hnonk`. -/
theorem two_twin_eM5_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
    ...
    (hC4 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (hHub6 : (Finset.univ.filter (fun v : Fin 18 => 4 ≤ G.degree v)).card = 6)
  ...
  -- `|D| = 12` from `|Hub| = |Dᶜ| = 6`.
  have hDc : Dᶜ = Finset.univ.filter (fun v : Fin 18 => 4 ≤ G.degree v) := by
    ext w
  ...
  have hD12 : D.card = 12 := by rw [hDc] at hcc; omega
  ...
  · obtain ⟨c₁, c₂, hc1D, hc2D, hc12, hcov⟩ := hdom
    obtain ⟨k, tw1, tw2, hkle5, htw12, htw1deg, htw2deg, hAtw1k, hAtw2k, htw1iso, htw2iso⟩ :=
      shared_hub_le5_eM5_eighteen G hm h3 h2k2 (by rw [← hDdef]; exact hD12)
        (by rw [← hDdef]; exact hsum10.le)
    have hkge4 : 4 ≤ G.degree k := by
      have := htw1iso k hAtw1k; have := h3 k; omega
    exact dom_fat_centre_two_twin_le5_eighteen G D hmemD hT hC4 c₁ c₂ hc1D hc2D hc12 hcov hge
  ...
  · -- **Induced-`C₅` branch with a deg-`≤ 5` shared hub.**  Extract a shared deg-`≤ 5` hub and
    -- route through `c5_shared_two_twin_le5_eighteen`; the deg-`5` 3-consecutive-hit residual is
    -- closed by `exists_nonk_two_twin_hub_eighteen` supplying a second hub `h ≠ k`.
    ...
    have hkge4 : 4 ≤ G.degree k := by
      have := htw1iso k hAtw1k; have := h3 k; omega
    exact c5_shared_two_twin_le5_eighteen G hT hC4 k tw1 tw2 hkge4 hkle5 htw12 htw1deg htw2deg
      hAtw1k hAtw2k htw1iso htw2iso
      (fun a b c hda hdb hdc hka hkb hkc hab hbc hab_ne hbc_ne hac_ne =>
        exists_nonk_two_twin_hub_eighteen G hm h3 (by rw [← hDdef]; omega)
