...
/-!
# Basic `Z`-vertex structure lemmas for the `n = 18` no-two-hub partition (bottom layer)
...
saturation cluster).  They depend only on the base counting layer (`Core`, `TwoHubSelect`,
`StarTriangleStruct`, `RichCount`), so they sit below the dispatch chain and break the import cycle
between `RichZdeg` and the `Z`-poor regime provers.
-/
...
theorem nbr_split_three_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    (hdisj : Disjoint Hub Iso) (v : Fin 18) :
...
partition of the `18` vertices, every hub degree splits three ways; summing and substituting the
`N3` hub-hub count (`18`) and the iso-incidence count (`18`) against `∑deg = 40` leaves
`∑_{h∈Hub}|N(h) ∩ Z| = 4`. -/
theorem zdeg_sum_four_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
  ...
  have hhubsum : ∑ h ∈ Hub, (G.neighborFinset h ∩ Hub).card = 18 :=
...
`Hub`, `Iso` disjoint of cardinalities `10`, `6`, so `|Z| = 18 − 16 = 2`. -/
theorem z_card_two_eighteen (Hub Iso : Finset (Fin 18))
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 10) (hIso : Iso.card = 6) :
    (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 18)).card = 2 := by
  have hHIcard : (Hub ∪ Iso).card = 16 := by
    rw [Finset.card_union_of_disjoint hdisj, hHub, hIso]
  have hZeq : (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 18)) = (Hub ∪ Iso)ᶜ := by
...
`(10, 6, 40)` profile, the two `M`-edge endpoints `Z = univ \ (Hub ∪ Iso)` carry exactly four
...
theorem z_two_hub_nbrs_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 10) (hIso : Iso.card = 6)
    (hdsum : ∑ w ∈ Hub, G.degree w = 40)
    (hdeg3 : ∀ v : Fin 18, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
...
the triangle `{u, v, h}` would have degree sum `3 + 3 + 4 = 10 ≤ 10`, a good triangle excluded by
`hT`.  Consequently the four `Z`–hub incidences land on four *distinct* hubs. -/
theorem no_hub_adj_both_mends_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    ...
    (h : Fin 18) (hh : h ∈ Hub)
    (u : Fin 18) (hu : u ∈ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 18)))
    (v : Fin 18) (hv : v ∈ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 18)))
    (huv : u ≠ v) :
  ...
  obtain ⟨huiso, huhub, hudeg⟩ :=
    z_two_hub_nbrs_eighteen G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak u hu
  -- `u` meets exactly one `Z`-vertex.
  have huZ : (G.neighborFinset u ∩ Z).card = 1 := by
    have hsp := nbr_split_three_eighteen G Hub Iso hdisj u
  ...
  obtain ⟨_, _, hvdeg⟩ :=
    z_two_hub_nbrs_eighteen G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak v hv
  -- The triangle `{u, v, h}` has degree sum `3 + 3 + 4 = 10`, excluded by `hT`.
  rintro ⟨hhu_adj, hhv_adj⟩
  have hdsum10 : G.degree u + G.degree v + G.degree h ≤ 10 := by
    rw [hudeg, hvdeg, hdeg4 h hh]
