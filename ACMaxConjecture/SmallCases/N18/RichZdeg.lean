...
import ACMaxConjecture.SmallCases.N18.ZPoorDispatch

/-!
# The rigid `Z`-distribution for the `n = 18` no-two-hub partition

For the tight `e(M) = 1`, all-degree-`4`, `(|Hub|, |Iso|) = (10, 6)` profile, the two `M`-edge
endpoints form the set `Z = univ \ (Hub ∪ Iso)` (`|Z| = 2`).  Each hub's degree splits as
`|N ∩ Hub| + |N ∩ Iso| + |N ∩ Z| = 4`; summing over the ten hubs and using `N3`
(`∑|N ∩ Hub| = 18`) and `∑|N ∩ Iso| = 18` gives `∑_{Hub}|N ∩ Z| = 4`.

The headline `zdeg_split_eighteen` pins how those four `Z`-incidences distribute across the
...
`hC4`; the distinct-twin / adjacent-poor residual is delegated to
`z_meets_two_poor_forces_two_hub_eighteen` (the global two-hub extraction among the rich hubs). -/
theorem z_two_poor_hubs_false_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    ...
    False := by
  classical
  obtain ⟨hziso0, _, hzdeg3⟩ := z_two_hub_nbrs_eighteen G Hub Iso hiso3 hdisj hHub hIso hdsum
  ...
  · -- Adjacent poor hubs: residual, delegated to the global two-hub extraction.
    exact z_meets_two_poor_forces_two_hub_eighteen G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3
...
hub. -/
theorem each_z_meets_rich_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 10) (hIso : Iso.card = 6)
    (hdeg3 : ∀ v : Fin 18, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
    ...
    ∀ z ∈ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 18)),
      1 ≤ (G.neighborFinset z ∩ Hub.filter (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card)).card := by
  classical
  set R : Finset (Fin 18) := Hub.filter (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card) with hRdef
  intro z hz
  obtain ⟨_, hhub2, _⟩ := z_two_hub_nbrs_eighteen G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3
    hisodeg3 hleak z hz
  by_contra hlt
  push Not at hlt
  ...
  have hpoor : ∀ g ∈ G.neighborFinset z ∩ Hub, (G.neighborFinset g ∩ Iso).card ≤ 1 := by
    ...
    exact hR0 g hgR
  exact z_two_poor_hubs_false_eighteen G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3 hisodeg3
...
theorem zdeg_split_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 10) (hIso : Iso.card = 6)
    (hdeg3 : ∀ v : Fin 18, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
    ...
    ∑ g ∈ Hub.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card),
          (G.neighborFinset g ∩ (Finset.univ \ (Hub ∪ Iso))).card ≤ 2 := by
  classical
  ...
  have hrichge : 2 ≤ ∑ r ∈ R, (G.neighborFinset r ∩ Z).card := by
    have hcross : ∑ r ∈ R, (G.neighborFinset r ∩ Z).card
        = ∑ z ∈ Z, (G.neighborFinset z ∩ R).card := cross_count G R Z
    rw [hcross]
    have hez : ∀ z ∈ Z, 1 ≤ (G.neighborFinset z ∩ R).card :=
      each_z_meets_rich_eighteen G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3 hisodeg3 hdsum
...
`3 + 4 + 4 = 11 ≤ 11`, a good triangle excluded by `hT` (threshold `11`, the `n = 18` value).  If
...
theorem no_z_two_rich_hubs_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 10) (hIso : Iso.card = 6)
    (hdeg3 : ∀ v : Fin 18, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
  ...
  · -- Non-adjacent rich hubs share exactly one twin `t`; `t–g₁–z–g₂` is a good `C₄`.
    have hsh : (G.neighborFinset g₁ ∩ G.neighborFinset g₂ ∩ Iso).card = 1 :=
      rich_nonadj_share_eq_one G Hub Iso hshare hno2hub g₁ g₂ hg1Hub hg2Hub
...
theorem zdeg_split_sharp_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 10) (hIso : Iso.card = 6)
    (hdeg3 : ∀ v : Fin 18, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
  ...
  have hzRle : ∑ r ∈ R, (G.neighborFinset r ∩ Z).card ≤ 2 := by
    ...
    have hez : ∀ z ∈ Z, (G.neighborFinset z ∩ R).card ≤ 1 :=
      no_z_two_rich_hubs_eighteen G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3 hisodeg3 hdsum
