...
/-!
# Structural foundation for the `n = 18` star-triangle extraction

This file collects the genuinely-clean foundational counting facts used by the star-triangle
dichotomy for the tight `e(M) = 1` profiles `(9, 7, 37)` and `(10, 6, 40)`.

The headline consumer is `no_two_hub_star_triangle_eighteen` (in `TwinCert18TwoHubTight`): from the
no-good-two-hub hypothesis it must produce a `StarTriangleConfig` (a degree-`4` hub with two
`M`-isolated twins, plus a disjoint, totally non-adjacent hub-triangle).  The pigeonhole step
isolated here — that an iso-rich (`a ≥ 2`) degree-`4` hub *exists* — is the easy half; the rigid
hub-triangle extraction is the hard half (see the report attached to this node).
-/
...
theorem rich_nonadj_share_eq_one (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    ...
    (hno2hub : ¬∃ h₁ h₂ : Fin 18, h₁ ∈ Hub ∧ h₂ ∈ Hub ∧
      ...
      2 ≤ ((G.neighborFinset h₂ ∩ Iso) \ G.neighborFinset h₁).card)
    (a b : Fin 18) (haHub : a ∈ Hub) (hbHub : b ∈ Hub)
...
/-- **N3 — the hubs span exactly `18` hub-hub incidences (`9` edges).**  Each hub's degree splits
...
(the leak bound caps it at `4`).  Hence `∑|N∩Hub| = 40 − 18 − 4 = 18`. -/
theorem hub_hub_edge_count_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
    ...
    ∑ h ∈ Hub, (G.neighborFinset h ∩ Hub).card = 18 := by
  classical
...
`strong_deg4_count_le_two_deg5_eighteen` bound specialised through `hno2hub`. -/
theorem rich_a3_count_le_two_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
