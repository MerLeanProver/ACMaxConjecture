...
/-!
# Degree-`6`-aware two-hub corner selection for `n = 18` (`|D| = 11` regime)

This file supplies `two_hub_deg6_select_eighteen`, the hub-pair *selection* lemma for the one
two-hub `e(M) ≤ 1` corner that the degree-`5`-restricted selector
(`two_hub_corner_select_deg5_eighteen`) misses: `|D| = 11`, `|Hub| = 7`, `∑_{Hub} deg = 31`, with a
single **degree-`6`** hub (excess `3`, so `hdeg5` — degree `≤ 5` — fails).
...
`residual_arith`.  Since every `R`-hub has degree `≥ 5` and `≤ 6`, the constraint
`5·|R| ≤ ∑_R deg = 31 − 4·|T|` forces `|R| ≤ 3`, whence
...
/-- **Hub-pair selection for the `n = 18` degree-`6` two-hub corner.**  In the `|Hub| = 7`,
`∑deg = 31` corner with `|Iso| ∈ {9, 11}` (the regime where one hub may have degree `6`, so `hdeg6`
...
theorem two_hub_deg6_select_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso : Finset (Fin 18))
