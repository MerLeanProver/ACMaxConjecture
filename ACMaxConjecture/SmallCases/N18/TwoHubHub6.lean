...
/-!
# `n = 18`, `|D| = 12`, `|Hub| = 6` corner selector

The `|D| = 12`, `|Hub| = 6` regime that the dense alignment dispatcher (`halign8_eighteen`) needs.
...
Unlike `n = 16`, the deg-`4` shared-hub pigeonhole diverges for `n = 18` (degree sum `60` is divisible
by `3`), so the assembly routes through the deg-`≤ 5` shared hub of `TwinCert18TwoHubHub6Deg5`:

* **dominating-edge branch (fully proved):** `shared_hub_le5_eM5_eighteen` (strict because
  `|Hub| = 6`) supplies a deg-`≤ 5` hub shared by two `M`-isolated twins, and
  `dom_fat_centre_two_twin_le5_eighteen` closes the `TwoTwin` cut for any `s ∈ {8, 10}`;
* **induced-`C₅` branch:** an induced `C₅` in `M` forces `e(M) = 5` (`s = 10`).  If the cherry's
  ...
  `SingleVertex` cut; otherwise `two_twin_eM5_eighteen` gives the `TwoTwin` cut.

This selector is therefore proved unconditionally; the only residual `sorry` of the development sits
inside `two_twin_eM5_eighteen` (the deg-`≤ 5` induced-`C₅` sub-case, which needs a deg-`≤ 5`
analogue of `hub_cycle_cases_eighteen`).
-/
...
/-- **`|D| = 12`, `|Hub| = 6` four-way alignment selector for `n = 18` (`e(M) ≥ 4`).**
...
yields a `SingleVertex` cut (all-deg-`4` cherry) or a `TwoTwin` cut (`two_twin_eM5_eighteen`). -/
theorem two_hub_or_single_vertex_hub6_eighteen (G : SimpleGraph (Fin 18))
  ...
  · -- **Dominating-edge branch (TwoTwin, fully proved).**  The deg-`≤ 5` shared hub from
    ...
    exact Or.inr (Or.inr (Or.inl (dom_fat_centre_two_twin_le5_eighteen G D hmemD hT hC4 c₁ c₂
  ...
  · obtain ⟨v₁, v₂, v₃, v₄, v₅, hv1D, hv2D, hv3D, hv4D, hv5D, hcard5,
    ...
    · -- **Degree-`≥ 5` cherry-hub with an induced `C₅` (TwoTwin).**  The `C₅` forces `e(M) = 5`
      -- (`s = 10`); routed through `two_twin_eM5_eighteen` whose deg-`≤ 5` `C₅` sub-case is the
      ...
      have hsum10 : ∑ v ∈ D, (G.neighborFinset v ∩ D).card = 10 := by
        ...
        have two_nbrs : ∀ a b c : Fin 18, G.Adj a b → G.Adj a c → b ∈ D → c ∈ D → b ≠ c →
            2 ≤ (G.neighborFinset a ∩ D).card := by
          intro a b c hab hac hbD hcD hbc
          have hsub : ({b, c} : Finset (Fin 18)) ⊆ G.neighborFinset a ∩ D := by
          ...
          have hbc2 : ({b, c} : Finset (Fin 18)).card = 2 := by
            rw [Finset.card_insert_of_notMem (by simp [hbc]), Finset.card_singleton]
          calc 2 = ({b, c} : Finset (Fin 18)).card := hbc2.symm
      ...
      exact Or.inr (Or.inr (Or.inl
        (two_twin_eM5_eighteen G hm h3 hT h2k2 hC4 hHub6 (by rw [← hDdef]; exact hsum10))))
