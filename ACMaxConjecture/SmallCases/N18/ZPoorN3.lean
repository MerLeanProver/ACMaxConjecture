...
* `rich_isodeg_seq_eighteen` (LEAF): the rich iso-degrees are `(3, 2, 2, 2, 2, 2, 2)` — a unique
...
theorem rich_isodeg_seq_eighteen (G : SimpleGraph (Fin 18)) (Iso : Finset (Fin 18))
    (R : Finset (Fin 18)) (hRge : ∀ a ∈ R, 2 ≤ (G.neighborFinset a ∩ Iso).card)
...
`rich_count_le_seven_eighteen`. -/
theorem offdiag_eq_eighteen (G : SimpleGraph (Fin 18)) (R : Finset (Fin 18)) :
...
theorem eRR_le_seven_eighteen (G : SimpleGraph (Fin 18)) (Hub Iso R : Finset (Fin 18))
...
`(n₀,n₁,n₂,n₃) = (1,0,0,5)`. -/
theorem poor_incidence_concentrates_eighteen (G : SimpleGraph (Fin 18))
  ...
  have hcsum : ∑ t ∈ Iso, (G.neighborFinset t ∩ R).card = 15 := by
    rw [cross_count G Iso R]; exact hS15
  -- Off-diagonal split and the `D ≥ 28` bound.
  have hsplit := offdiag_eq_eighteen G R
  rw [hr7] at hsplit
  have hDadj := eRR_le_seven_eighteen G Hub Iso R hdeg4 hdisj hRHub hRrich hr7
