...
import ACMaxConjecture.SmallCases.N18.TwoHubHub6Deg5

/-!
# Shared `n = 18` cherry-corner extraction helpers
...
(`TwinCert18CherryP3.lean`).  Ported from the axiom-clean `n = 17` development
(`TwinCert17CherryCore.lean`); the `n = 18` deltas are `30 → 32` edges, `60 → 64` total degree, the
internal sum `64 → 68 − 6|D|`, and `cross_count → cross_count`.

The cherry-avoiding pigeonhole here additionally returns a *dense-avoider triangle* in the tie
branch (the `n = 18` `|D| = 9` corner needs it: there the arithmetic does not exclude the
...
/-- **Cherry-avoiding hub assembly into `TwoTwinConfig` (`n = 18`).**  A degree-`≤ 5` hub `h`
avoiding the cherry `x–y–z` (centre `y`) and carrying two distinct `M`-isolated degree-`3` twins
`t₁, t₂` yields a `TwoTwinConfig`. -/
theorem twotwin_assemble_cherry_eighteen (G : SimpleGraph (Fin 18))
    (Iso : Finset (Fin 18)) (x y z h t₁ t₂ : Fin 18)
...
/-- **Raw cherry-avoiding two-twin assembly (`n = 18`).**  Identical conclusion to
`twotwin_assemble_cherry_eighteen`, but the two twins are supplied through their raw degree-`3`
plus no-degree-`3`-neighbour facts rather than membership in an `Iso` set. -/
theorem twotwin_assemble_cherry_raw_eighteen (G : SimpleGraph (Fin 18))
    (x y z h t₁ t₂ : Fin 18)
    (hdegx : G.degree x = 3) (hdegy : G.degree y = 3) (hdegz : G.degree z = 3)
    (hxyA : G.Adj x y) (hyzA : G.Adj y z)
    (hxy_ne : x ≠ y) (hyz_ne : y ≠ z) (hxz_ne : x ≠ z)
    ...
    (ht1no : ∀ w : Fin 18, G.Adj t₁ w → G.degree w ≠ 3)
    (ht2no : ∀ w : Fin 18, G.Adj t₂ w → G.degree w ≠ 3)
    (hAt1h : G.Adj t₁ h) (hAt2h : G.Adj t₂ h) :
    TwoTwinConfig G := by
  classical
  have ht1x : ¬G.Adj t₁ x := fun ha => ht1no x ha (by rw [hdegx])
...
/-- **Triangle from a dense `4`-set (`n = 18`).**  A `4`-element vertex set `S` whose induced
directed-edge count `∑_{h∈S}|N h ∩ S|` is `≥ 10` (i.e. `≥ 5` undirected edges) contains a triangle:
three pairwise-distinct, pairwise-adjacent vertices.  (Mantel: a triangle-free graph on `4`
vertices has `≤ 4` edges.) -/
theorem four_set_triangle_eighteen (G : SimpleGraph (Fin 18)) (S : Finset (Fin 18))
    (hScard : S.card = 4) (hdense : 10 ≤ ∑ h ∈ S, (G.neighborFinset h ∩ S).card) :
    ∃ a b c : Fin 18, a ∈ S ∧ b ∈ S ∧ c ∈ S ∧ a ≠ b ∧ a ≠ c ∧ b ≠ c ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c := by
  classical
  -- Some vertex `h₀` of `S` is adjacent to all other three (`|N h₀ ∩ S| = 3`).
  have hex : ∃ h₀ ∈ S, 3 ≤ (G.neighborFinset h₀ ∩ S).card := by
    by_contra hcon
    push Not at hcon
    have hle : ∑ h ∈ S, (G.neighborFinset h ∩ S).card ≤ ∑ _h ∈ S, 2 :=
      Finset.sum_le_sum (fun h hh => by have := hcon h hh; omega)
    rw [Finset.sum_const, hScard, smul_eq_mul] at hle
    omega
  obtain ⟨h₀, hh0S, hh0deg⟩ := hex
  ...
  have herasecard : (S.erase h₀).card = 3 := by rw [Finset.card_erase_of_mem hh0S, hScard]
  have heq : G.neighborFinset h₀ ∩ S = S.erase h₀ :=
    Finset.eq_of_subset_of_card_le hsub (by rw [herasecard]; exact hh0deg)
  have hadj0 : ∀ w : Fin 18, w ∈ S.erase h₀ → G.Adj h₀ w := by
    intro w hw
    rw [← heq, Finset.mem_inter, G.mem_neighborFinset] at hw
    exact hw.1
  -- Among the remaining three there is an edge, else the directed-edge count is `≤ 6`.
  have hedge : ∃ u v : Fin 18, u ∈ S.erase h₀ ∧ v ∈ S.erase h₀ ∧ u ≠ v ∧ G.Adj u v := by
    by_contra hcon
    push Not at hcon
    have hbound : ∀ h : Fin 18, h ∈ S.erase h₀ → (G.neighborFinset h ∩ S).card ≤ 1 := by
      intro h hh
      have hsub1 : G.neighborFinset h ∩ S ⊆ {h₀} := by
        intro w hw
        rw [Finset.mem_inter, G.mem_neighborFinset] at hw
        rw [Finset.mem_singleton]
        by_contra hwh0
        have hwerase : w ∈ S.erase h₀ := Finset.mem_erase.mpr ⟨hwh0, hw.2⟩
        have hhh0 : h ≠ w := fun e => G.irrefl (e ▸ hw.1)
        exact hcon h w hh hwerase hhh0 hw.1
      calc (G.neighborFinset h ∩ S).card ≤ ({h₀} : Finset (Fin 18)).card :=
            Finset.card_le_card hsub1
        _ = 1 := Finset.card_singleton _
    have hsplit : ∑ h ∈ S, (G.neighborFinset h ∩ S).card
        = (G.neighborFinset h₀ ∩ S).card + ∑ h ∈ S.erase h₀, (G.neighborFinset h ∩ S).card :=
      (Finset.add_sum_erase S _ hh0S).symm
    have hle3 : (G.neighborFinset h₀ ∩ S).card ≤ 3 := by
      rw [heq, herasecard]
    have hrest : ∑ h ∈ S.erase h₀, (G.neighborFinset h ∩ S).card ≤ 3 := by
      calc ∑ h ∈ S.erase h₀, (G.neighborFinset h ∩ S).card ≤ ∑ _h ∈ S.erase h₀, 1 :=
            Finset.sum_le_sum hbound
        _ = 3 := by rw [Finset.sum_const, herasecard, smul_eq_mul, mul_one]
    omega
  obtain ⟨u, v, hu, hv, huv, huvadj⟩ := hedge
  have hu0 : u ≠ h₀ := (Finset.mem_erase.mp hu).1
  have hv0 : v ≠ h₀ := (Finset.mem_erase.mp hv).1
  exact ⟨h₀, u, v, hh0S, (Finset.mem_erase.mp hu).2, (Finset.mem_erase.mp hv).2,
    fun e => hu0 e.symm, fun e => hv0 e.symm, huv, hadj0 u hu, hadj0 v hv, huvadj⟩

/-- **Cherry-avoiding dichotomy (`n = 18`).**  Same counting hypotheses as
`cherry_avoider_pigeonhole_eighteen`, but with no arithmetic side condition: either a cherry-avoiding
...
theorem cherry_avoider_or_dense_eighteen (G : SimpleGraph (Fin 18))
    (Iso K FullHub Hub5 : Finset (Fin 18)) (C Int : ℕ) (hHub5sub : Hub5 ⊆ FullHub)
    (hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 18, G.Adj v w → G.degree w ≠ 3))
    (hHubdeg4 : ∀ h : Fin 18, h ∈ Hub5 → 4 ≤ G.degree h)
    (hHubdeg5 : ∀ h : Fin 18, h ∈ Hub5 → G.degree h ≤ 5)
    (hdecomp : ∀ h : Fin 18, h ∈ Hub5 → (G.neighborFinset h ∩ K).card
    ...
    (∃ h t₁ t₂ : Fin 18, h ∈ Hub5 ∧ (∀ k : Fin 18, k ∈ K → ¬G.Adj h k) ∧
        G.degree h ≤ 5 ∧ t₁ ≠ t₂ ∧ G.degree t₁ = 3 ∧ G.degree t₂ = 3 ∧
        G.Adj t₁ h ∧ G.Adj t₂ h ∧ (∀ w : Fin 18, G.Adj t₁ w → G.degree w ≠ 3) ∧
        (∀ w : Fin 18, G.Adj t₂ w → G.degree w ≠ 3)) ∨
      (∃ S : Finset (Fin 18), S ⊆ Hub5 ∧ (∀ h : Fin 18, h ∈ S → ∀ k : Fin 18, k ∈ K → ¬G.Adj h k) ∧
        Hub5.card ≤ S.card + C ∧ 3 * S.card ≤ Int ∧
        6 * S.card ≤ (∑ h ∈ S, (G.neighborFinset h ∩ S).card) + Int) := by
  classical
  set HubA : Finset (Fin 18) := Hub5.filter (fun h => (G.neighborFinset h ∩ K).card = 0)
    with hHubAdef
  ...
  have hAvoidK : ∀ h : Fin 18, h ∈ HubA → (G.neighborFinset h ∩ K).card = 0 := by
    intro h hh; rw [hHubAdef, Finset.mem_filter] at hh; exact hh.2
  have hAvoid : ∀ h : Fin 18, h ∈ HubA → ∀ k : Fin 18, k ∈ K → ¬G.Adj h k := by
  ...
  · obtain ⟨h, t₁, t₂, hhHubA, _hd5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1no, ht2no⟩ :=
      shared_hub_le5_from_count_eighteen G Iso HubA hIsoprop
  ...
  · rw [not_lt] at hstrict
    refine Or.inr ⟨HubA, hHubA5, hAvoid, by omega, ?_, ?_⟩
    ...
    · -- `6·a ≤ ∑_{HubA}|N ∩ HubA| + Int`.
      set a : ℕ := HubA.card with hadef
      set Rest : Finset (Fin 18) := FullHub \ HubA with hRestdef
      ...
      have hsplitFull : ∀ h : Fin 18, h ∈ HubA → (G.neighborFinset h ∩ FullHub).card
...
/-- **`P₃`-cherry alignment dichotomy (`n = 18`).**  Packages the hub-incidence counting for the
`e(M) = 2` (`P₃` cherry `x–y–z`) regime and feeds it to `cherry_avoider_or_dense_eighteen`: either a
cherry-avoiding degree-`≤ 5` hub carries two `M`-isolated twins (`TwoTwinConfig`), or the
cherry-avoiding hub set `S ⊆ Hub5` is dense, with `Hub5.card ≤ |S| + 5`, `3|S| ≤ 68 − 6|D|` and the
within-avoider edge bound `6|S| ≤ ∑_{h∈S}|N h ∩ S| + (68 − 6|D|)`.  The dense branch routes (for
`|D| = 9`) to a hub triangle, and is arithmetically impossible for `|D| ∈ {10, 11}`. -/
theorem p3_cherry_dichotomy_eighteen (G : SimpleGraph (Fin 18)) (D Iso Hub5 : Finset (Fin 18))
    (x y z : Fin 18) (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    ...
    (hxy_ne : x ≠ y) (hyz_ne : y ≠ z) (hxz_ne : x ≠ z)
    (hHub5sub : Hub5 ⊆ Dᶜ) (hHub5deg5 : ∀ h : Fin 18, h ∈ Hub5 → G.degree h ≤ 5) :
    TwoTwinConfig G ∨
      (∃ S : Finset (Fin 18), S ⊆ Hub5 ∧
        (∀ h : Fin 18, h ∈ S → ¬G.Adj h x ∧ ¬G.Adj h y ∧ ¬G.Adj h z) ∧
        Hub5.card ≤ S.card + 5 ∧ 3 * S.card ≤ 68 - 6 * D.card ∧
        6 * S.card ≤ (∑ h ∈ S, (G.neighborFinset h ∩ S).card) + (68 - 6 * D.card)) := by
  classical
  ...
  have hNxD : G.neighborFinset x ∩ D = ({y} : Finset (Fin 18)) := by
  ...
  have hNzD : G.neighborFinset z ∩ D = ({y} : Finset (Fin 18)) := by
    apply Finset.Subset.antisymm
    · intro w hw
      rw [Finset.mem_inter, G.mem_neighborFinset] at hw
  ...
  have hKIso : ({x, y, z} : Finset (Fin 18)) ∪ Iso = D := by
    apply Finset.Subset.antisymm
    · intro w hw
  ...
  have hKIsodisj : Disjoint ({x, y, z} : Finset (Fin 18)) Iso :=
    Finset.disjoint_left.mpr (fun w hw hw' => by
      simp only [Finset.mem_insert, Finset.mem_singleton] at hw
  ...
  have hother0 : ∀ v : Fin 18, v ∈ D → v ∉ ({x, y, z} : Finset (Fin 18)) →
  ...
  have hxyz_sub : ({x, y, z} : Finset (Fin 18)) ⊆ D := by
  ...
  have hsumDcdeg : ∑ w ∈ Dᶜ, G.degree w + 3 * D.card = 64 := by
  ...
  have hDcD : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ D).card + 4 = 3 * D.card := by
    ...
    omega
  have hIntinc : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 68 - 6 * D.card := by
  ...
  have hCfull : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ ({x, y, z} : Finset (Fin 18))).card = 5 := by
    have hcc := cross_count G Dᶜ ({x, y, z} : Finset (Fin 18))
    ...
    rfl
  have hCinc : ∑ h ∈ Hub5, (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 18))).card ≤ 5 := by
    rw [← hCfull]; exact Finset.sum_le_sum_of_subset hHub5sub
  have hdecomp : ∀ h : Fin 18, h ∈ Hub5 → (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 18))).card
      + (G.neighborFinset h ∩ Iso).card + (G.neighborFinset h ∩ Dᶜ).card = G.degree h := by
    intro h _
    have hd1 : (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 18))).card
        + (G.neighborFinset h ∩ Iso).card = (G.neighborFinset h ∩ D).card := by
      have hdisj : Disjoint (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 18)))
    ...
    have hd2 := hsplit h
    omega
  have hHubdeg4 : ∀ h : Fin 18, h ∈ Hub5 → 4 ≤ G.degree h := by
    intro h hh
    have hhDc : h ∈ Dᶜ := hHub5sub hh
    rw [Finset.mem_compl, hmemD] at hhDc; have := h3 h; omega
  rcases cherry_avoider_or_dense_eighteen G Iso ({x, y, z} : Finset (Fin 18)) Dᶜ Hub5 5
      (68 - 6 * D.card) hHub5sub hIsoprop hHubdeg4 hHub5deg5 hdecomp hCinc hIntinc with hhub | hdense
  · obtain ⟨h, t₁, t₂, hhHub5, havoid, hhdeg5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1no, ht2no⟩ :=
    ...
    exact Or.inl (twotwin_assemble_cherry_raw_eighteen G x y z h t₁ t₂ hdegx hdegy hdegz hxyA hyzA
  ...
  · obtain ⟨S, hSsub, hSavoid, hScard, hS3, hS6⟩ := hdense
    exact Or.inr ⟨S, hSsub, fun h hh => ⟨hSavoid h hh x (by simp), hSavoid h hh y (by simp),
      hSavoid h hh z (by simp)⟩, hScard, hS3, hS6⟩
