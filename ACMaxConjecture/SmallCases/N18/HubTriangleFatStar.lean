...
/-!
# Hub-triangle existence for the `n = 18`, `e(M) = 4`, `|D| = 8` fat double-star corner

This file is the `n = 18` port of `TwinCert17HubTriangleFatStar`.  The `M`-structure is a **fat
...
(`N c₂ ∩ D = {c₁, L₂}`), for `4` `M`-edges.  In the `|D| = 8` (`|Hub| = 10`, all-degree-`4`) corner
there are `|Iso| = 3` `M`-isolated twins and the hub-internal-degree total is
`∑ int = 4·10 − 7 − 9 = 24` (vs the `n = 17` value `20`).
...
via `hub_triangle_cut_certificate`.  The richer constants (`∑ int = 24`, `|A1| ≥ 6`, `|A2| ≥ 7`)
collapse the `|FF|`-dispatch to counting / iso-share contradictions and a Mantel-`6` triangle,
leaving one documented residual: the `|FF| = 6`, `∑_FF iso = 3` `K_{3,3}`-of-avoiders corner.
-/
...
`Dᶜ.card = 10` and `Iso.card = 3`. -/
theorem hub_struct_fatstar (G : SimpleGraph (Fin 18)) (D Iso : Finset (Fin 18))
    (c₁ c₂ L₁ L₁' L₂ : Fin 18)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hisochar : ∀ w : Fin 18, w ∈ D → w ≠ c₁ → w ≠ c₂ → w ≠ L₁ → w ≠ L₁' → w ≠ L₂ → w ∈ Iso)
    ...
    (hD8 : D.card = 8) :
    Dᶜ.card = 10 ∧ Iso.card = 3 := by
  classical
  have hDc : Dᶜ.card = 10 := by rw [Finset.card_compl, Fintype.card_fin, hD8]
  refine ⟨hDc, ?_⟩
  have hc1c2 : c₁ ≠ c₂ := G.ne_of_adj hc12
  have hc1L1 : c₁ ≠ L₁ := G.ne_of_adj hac1L1
  have hc1L1' : c₁ ≠ L₁' := G.ne_of_adj hac1L1'
  have hc2L2 : c₂ ≠ L₂ := G.ne_of_adj hac2L2
  set P : Finset (Fin 18) := {c₁, c₂, L₁, L₁', L₂} with hP
...
theorem path_iso_partition_fatstar (G : SimpleGraph (Fin 18)) (D Iso : Finset (Fin 18))
    (c₁ c₂ L₁ L₁' L₂ : Fin 18)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hisochar : ∀ w : Fin 18, w ∈ D → w ≠ c₁ → w ≠ c₂ → w ≠ L₁ → w ≠ L₁' → w ≠ L₂ → w ∈ Iso)
    (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hL1D : L₁ ∈ D) (hL1'D : L₁' ∈ D) (hL2D : L₂ ∈ D)
    (hac1L1 : G.Adj c₁ L₁) (hac1L1' : G.Adj c₁ L₁') (hc12 : G.Adj c₁ c₂) (hac2L2 : G.Adj c₂ L₂) :
    ({c₁, c₂, L₁, L₁', L₂} : Finset (Fin 18)) ∪ Iso = D ∧
      Disjoint ({c₁, c₂, L₁, L₁', L₂} : Finset (Fin 18)) Iso := by
  classical
  set P : Finset (Fin 18) := {c₁, c₂, L₁, L₁', L₂} with hP
  have hIsoD : Iso ⊆ D := by rw [hIsodef]; exact Finset.filter_subset _ _
  have hPD : P ⊆ D := by
...
theorem path_hub_incidence_fatstar (G : SimpleGraph (Fin 18)) (D : Finset (Fin 18))
    (c₁ c₂ L₁ L₁' L₂ : Fin 18)
...
incidences total `7` and `9` respectively (so the hub-internal total is `4·10 − 7 − 9 = 24`), and
per hub the three split the degree `4`. -/
theorem incidence_sums_fatstar (G : SimpleGraph (Fin 18)) (D Iso : Finset (Fin 18))
    (c₁ c₂ L₁ L₁' L₂ : Fin 18)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    ...
    (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ ({c₁, c₂, L₁, L₁', L₂} : Finset (Fin 18))).card = 7) ∧
      (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Iso).card = 9) ∧
      (∀ g ∈ Dᶜ, (G.neighborFinset g ∩ ({c₁, c₂, L₁, L₁', L₂} : Finset (Fin 18))).card
...
free internal degree `≥ 3`, `|A1| ≥ 6`, `|A2| ≥ 7`, `FF = A1 ∩ A2`, and the hub-internal-degree
total `∑_{Dᶜ} f = 24`, the fully-free count satisfies `2 ≤ |FF| ≤ 8` (the sharper `≤ 6` bound is
derived in `iso_rich_force_fatstar` from the `iso + int = 4` per-hub split). -/
theorem active_hub_budget_fatstar (Dc A1 A2 FF : Finset (Fin 18)) (f : Fin 18 → ℕ)
    (hA1sub : A1 ⊆ Dc) (hA2sub : A2 ⊆ Dc) (hFF : FF = A1 ∩ A2)
    (hA1card : 6 ≤ A1.card) (hA2card : 7 ≤ A2.card)
    (hA1int : ∀ g ∈ A1, 2 ≤ f g) (hA2int : ∀ g ∈ A2, 2 ≤ f g)
    (hFFint : ∀ g ∈ FF, 3 ≤ f g) (hSum24 : ∑ w ∈ Dc, f w = 24) :
    2 ≤ FF.card ∧ FF.card ≤ 8 := by
  classical
  ...
  have hle8 : FF.card ≤ 8 := by omega
  refine ⟨?_, hle8⟩
  have hUsub : A1 ∪ A2 ⊆ Dc := Finset.union_subset hA1sub hA2sub
  have hUsum_le : ∑ g ∈ A1 ∪ A2, f g ≤ 24 := by
    rw [← hSum24]
...
theorem avoider_internal_ge_two_pair (G : SimpleGraph (Fin 18)) (D Iso : Finset (Fin 18))
    (g a₁ a₂ : Fin 18) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
    (hnboth : ¬(G.Adj g a₁ ∧ G.Adj g a₂))
    (hclass : ∀ x : Fin 18, G.Adj g x → x ∈ D → x = a₁ ∨ x = a₂ ∨ x ∈ Iso) :
    2 ≤ (G.neighborFinset g ∩ Dᶜ).card := by
  classical
  set A : Finset (Fin 18) := if G.Adj g a₁ then {a₁} else (if G.Adj g a₂ then {a₂} else ∅) with hA
...
theorem iso_incidence_le_three (G : SimpleGraph (Fin 18)) (Iso A : Finset (Fin 18))
    (hIso3 : Iso.card = 3)
...
/-- **Fully-free internal-edge lower bound (fat-star, `∑ = 24`).**  For `FF ⊆ Dᶜ` with total
`Dᶜ`-internal degree `24`, `∑_{FF}(N ∩ FF) ≥ 2·∑_{FF}(N ∩ Dᶜ) − 24`. -/
theorem ff_internal_edge_lb_twentyfour (G : SimpleGraph (Fin 18)) (Dc FF : Finset (Fin 18))
    (hFFsub : FF ⊆ Dc)
    (hSum : ∑ w ∈ Dc, (G.neighborFinset w ∩ Dc).card = 24) :
    2 * (∑ g ∈ FF, (G.neighborFinset g ∩ Dc).card)
      ≤ (∑ g ∈ FF, (G.neighborFinset g ∩ FF).card) + 24 := by
  classical
  ...
  have hsdiff : (∑ w ∈ Dc \ FF, (G.neighborFinset w ∩ Dc).card)
      + ∑ g ∈ FF, (G.neighborFinset g ∩ Dc).card = 24 := by
...
/-- **Generic avoider count `≥ 7`.**  If three vertices `a, b, c` together have at most `3`
hub-neighbours and there are `10` hubs, then at least `7` hubs avoid all of `a, b, c`. -/
theorem avoiders_ge_seven_aux (G : SimpleGraph (Fin 18)) (D : Finset (Fin 18)) (a b c : Fin 18)
    (hDc10 : Dᶜ.card = 10)
    (hbound : (G.neighborFinset a ∩ Dᶜ).card + (G.neighborFinset b ∩ Dᶜ).card
      + (G.neighborFinset c ∩ Dᶜ).card ≤ 3) :
    7 ≤ (Dᶜ.filter (fun g => ¬G.Adj g a ∧ ¬G.Adj g b ∧ ¬G.Adj g c)).card := by
  classical
...
theorem iso3_other_share_false (G : SimpleGraph (Fin 18)) (D Iso : Finset (Fin 18))
    (hC4 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (hIsoD : Iso ⊆ D)
    (hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 18, G.Adj v w → G.degree w ≠ 3))
    (hdeg4 : ∀ w : Fin 18, w ∈ Dᶜ → G.degree w = 4) (hIso3 : Iso.card = 3)
...
/-- **Vacuity of the `|FF| = 6` `K_{3,3}`-of-avoiders fat-star corner.**  When six hubs avoid the
entire fat double-star, the `2 + 2 + 1 = 5` hub-incidences of `c₁`'s three degree-`3` neighbours
`L₁, L₁', c₂` must all land in `Dᶜ \ FF`, which has only `10 − 6 = 4` hubs.  Pigeonhole forces two
...
theorem fatstar_ff6_vacuous (G : SimpleGraph (Fin 18)) (D FF : Finset (Fin 18))
    (c₁ c₂ L₁ L₁' : Fin 18)
    (hC4 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    ...
    (hdeg4 : ∀ w : Fin 18, w ∈ Dᶜ → G.degree w = 4)
    (hFFsub : FF ⊆ Dᶜ)
    (hFFav : ∀ g : Fin 18, g ∈ FF → ¬G.Adj g L₁ ∧ ¬G.Adj g L₁' ∧ ¬G.Adj g c₂)
    (hcard_c1 : (G.neighborFinset c₁ ∩ Dᶜ).card = 0)
    (hcard_L1 : (G.neighborFinset L₁ ∩ Dᶜ).card = 2)
    (hcard_L1' : (G.neighborFinset L₁' ∩ Dᶜ).card = 2)
    (hcard_c2 : (G.neighborFinset c₂ ∩ Dᶜ).card = 1)
    (hDc10 : Dᶜ.card = 10) (hFF6 : FF.card = 6) : False := by
  classical
  have hDne : ∀ u w : Fin 18, u ∈ D → w ∈ Dᶜ → u ≠ w :=
  ...
  have hc1nohub : ∀ h : Fin 18, h ∈ Dᶜ → ¬G.Adj c₁ h := by
    intro h hh hadj
...
on `|FF| ∈ {2, 3, 4, 5, 6}`.  The richer `n = 18` budget (`|Iso| = 3`, `∑ int = 24`, `|A1| ≥ 6`,
`|A2| ≥ 7`) collapses `|FF| ∈ {2, 3}` to a union-overflow contradiction, `|FF| = 4` and the
`|FF| ∈ {5, 6}`, `|B|` small subcases to pure active-hub counting, the iso-heavy `|FF| = 5`,
`|B| = 2` subcase to an `iso3_other_share_false` contradiction, and the `|FF| = 6`, `|B| = 3`,
`∑_FF iso ≤ 2` subcase to a Mantel-`6` triangle in `FF ⊆ A2`.  The single residual is the
`|FF| = 6`, `∑_FF iso = 3` `K_{3,3}`-of-avoiders corner (`∑_FF(N ∩ FF) = 18`, one short of the
Mantel-`6` threshold), carried as one documented `sorry`. -/
theorem iso_rich_force_fatstar (G : SimpleGraph (Fin 18))
    (D Iso : Finset (Fin 18)) (c₁ c₂ L₁ L₁' L₂ : Fin 18)
    ...
    (hDc10 : Dᶜ.card = 10) (hIso3 : Iso.card = 3)
    (hcard_c1 : (G.neighborFinset c₁ ∩ Dᶜ).card = 0)
    (hcard_c2 : (G.neighborFinset c₂ ∩ Dᶜ).card = 1)
    (hcard_L1 : (G.neighborFinset L₁ ∩ Dᶜ).card = 2)
    (hcard_L1' : (G.neighborFinset L₁' ∩ Dᶜ).card = 2)
    (hcard_L2 : (G.neighborFinset L₂ ∩ Dᶜ).card = 2)
    (hSum24 : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 24)
    (htri1 : ¬∃ a b c : Fin 18, a ∈ Dᶜ ∧ b ∈ Dᶜ ∧ c ∈ Dᶜ ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c ∧
      (¬G.Adj a L₁ ∧ ¬G.Adj a c₁ ∧ ¬G.Adj a L₁') ∧
      (¬G.Adj b L₁ ∧ ¬G.Adj b c₁ ∧ ¬G.Adj b L₁') ∧
      (¬G.Adj c L₁ ∧ ¬G.Adj c c₁ ∧ ¬G.Adj c L₁'))
    (htri2 : ¬∃ a b c : Fin 18, a ∈ Dᶜ ∧ b ∈ Dᶜ ∧ c ∈ Dᶜ ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c ∧
  ...
  set P : Finset (Fin 18) := {c₁, c₂, L₁, L₁', L₂} with hPdef
  ...
  have hBpart : (∑ g ∈ B, (G.neighborFinset g ∩ Dᶜ).card)
      + ∑ g ∈ A1 ∪ A2, (G.neighborFinset g ∩ Dᶜ).card = 24 := by
  ...
  -- forces `|FF| ≤ 6`.
  have hFFintle : (∑ g ∈ FF, (G.neighborFinset g ∩ Dᶜ).card) ≤ 24 := by
    rw [← hSum24]
    exact Finset.sum_le_sum_of_subset_of_nonneg hFFsub (fun _ _ _ => Nat.zero_le _)
  have hFF6 : FF.card ≤ 6 := by omega
  -- Dispatch on `|FF| ∈ {2, 3, 4, 5, 6}`.
  ...
  have hUcardle : (A1 ∪ A2).card ≤ 10 := by rw [← hDc10]; exact Finset.card_le_card hUsub
  rcases hFFcases with hFF | hFF | hFF | hFF | hFF
  · -- `|FF| = 2`: `|A1 ∪ A2| = |A1| + |A2| − 2 ≥ 11 > 10`, impossible.
    exfalso; omega
  · -- `|FF| = 3`: `|A1 ∪ A2| ≥ 10` forces `|B| = 0`, but `∑_B iso ≥ 3 + ∑_FF iso > 0`.
    exfalso
    have hB0 : B.card = 0 := by omega
    have hisoB0 : (∑ g ∈ B, (G.neighborFinset g ∩ Iso).card) = 0 := by
      rw [Finset.card_eq_zero.mp hB0, Finset.sum_empty]
    omega
  · -- `|FF| = 4`: `|B| = 0` dies on `hredist`; `|B| = 1` forces `∑_FF iso = 0`, `∑_FF int = 16`, so
    -- the five active non-`FF` hubs need `∑ int ≥ 10 > 24 − 16`, a counting contradiction.
    exfalso; omega
  · -- `|FF| = 5`: `|B| ≤ 2`.  `|B| ∈ {0, 1}` close by counting; `|B| = 2` carries `∑_B iso ≥ 5`,
    -- forcing an internally-isolated `iso = 3` hub non-adjacent to an `iso ≥ 2` hub — an
    -- `iso3_other_share_false` contradiction.
    exfalso
    ...
    · obtain ⟨h₁, h₂, hh12, hBeq⟩ := Finset.card_eq_two.mp hB2
      ...
      · have hc2 : (G.neighborFinset h₂ ∩ Iso).card = 3 := by omega
        ...
        have hnadj : ¬G.Adj h₂ h₁ := by
          intro hadj
          have hmem : h₁ ∈ G.neighborFinset h₂ ∩ Dᶜ :=
            Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hadj, hh1Dc⟩
          rw [Finset.card_eq_zero] at hint2
          rw [hint2] at hmem; exact (Finset.notMem_empty _) hmem
        exact iso3_other_share_false G D Iso hC4 hIsoD hIsoprop hdeg4 hIso3 h₂ h₁ hh2Dc hh1Dc
          (Ne.symm hh12) hnadj hc2 (by omega)
    · omega
  · -- `|FF| = 6`: `|B| ≤ 3`.  `|B| ∈ {0, 1, 2}` close by counting; for `|B| = 3` the leak bound
    -- gives `∑_FF(N ∩ FF) ≥ 20 ≥ 19` when `∑_FF iso ≤ 2`, a Mantel-`6` triangle in `FF ⊆ A2`.  The
    -- `∑_FF iso = 3` `K_{3,3}`-of-avoiders corner (`∑ = 18`, one short of the Mantel-`6` threshold)
    -- is the documented residual.
    by_cases hiso2 : (∑ g ∈ FF, (G.neighborFinset g ∩ Iso).card) ≤ 2
    · have hedge : 19 ≤ ∑ g ∈ FF, (G.neighborFinset g ∩ FF).card := by
        have hlb := ff_internal_edge_lb_twentyfour G Dᶜ FF hFFsub hSum24
        omega
      obtain ⟨a, b, c, haFF, hbFF, hcFF, hab, hac, hbc⟩ := mantel_six_triangle G FF hFF hedge
    ...
    · -- `∑_FF iso = 3`, `|B| = 3`: `FF` is a `K_{3,3}` of cherry-avoiders, but this corner is
      -- **vacuous** — `c₁`'s three degree-`3` neighbours `L₁, L₁', c₂` carry `2 + 2 + 1 = 5`
      -- hub-incidences into `Dᶜ \ FF` (only `4` hubs), so two share a hub, forcing a good-`C₄`.
...
/-- **Hub-triangle existence in the `n = 18`, `|D| = 8`, `e(M) = 4` fat double-star corner.**  Under
the residual hypotheses (ten degree-`4` hubs, the fat dominating-edge double-star
...
theorem exists_hub_triangle_config_fatstar_eighteen (G : SimpleGraph (Fin 18))
    (D Iso : Finset (Fin 18)) (c₁ c₂ L₁ L₁' L₂ : Fin 18)
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
    ...
    (hdeg4 : ∀ w : Fin 18, w ∈ Dᶜ → G.degree w = 4) (hD8 : D.card = 8)
  ...
  have hA : ∀ t : Fin 18, t ∈ Iso → ∀ p q : Fin 18, p ≠ q →
      G.Adj t p → G.Adj t q →
      ¬((¬G.Adj p L₁ ∧ ¬G.Adj p c₁ ∧ ¬G.Adj p L₁' ∧
          ¬G.Adj q L₁ ∧ ¬G.Adj q c₁ ∧ ¬G.Adj q L₁') ∨
        (¬G.Adj p c₁ ∧ ¬G.Adj p c₂ ∧ ¬G.Adj p L₂ ∧
    ...
    have htiso_prop : ∀ w : Fin 18, G.Adj t w → G.degree w ≠ 3 := (hIsoprop t htIso).2
    ...
    · have hsum0 : ∑ w ∈ ({t, p, q} : Finset (Fin 18)),
          (G.neighborFinset w ∩ ({L₁, c₁, L₁'} : Finset (Fin 18))).card = 0 := by
      ...
      exact ⟨t, p, q, L₁, c₁, L₁', htdeg, hL1deg, hc1deg, hL1'deg, htp, htq,
        ...
        hac1L1.ne', hac1L1'.ne, hL1nL1'⟩
    · have hsum0 : ∑ w ∈ ({t, p, q} : Finset (Fin 18)),
          (G.neighborFinset w ∩ ({c₁, c₂, L₂} : Finset (Fin 18))).card = 0 := by
        apply Finset.sum_eq_zero
        intro w hw
        rw [Finset.card_eq_zero, Finset.eq_empty_iff_forall_notMem]
        intro a ha
  ...
  have hubne : ∀ g : Fin 18, g ∈ Dᶜ → g ≠ c₁ ∧ g ≠ c₂ ∧ g ≠ L₁ ∧ g ≠ L₁' ∧ g ≠ L₂ := by
    intro g hg
  ...
  obtain ⟨hsumP, hsumI, hper⟩ :=
    incidence_sums_fatstar G D Iso c₁ c₂ L₁ L₁' L₂ hIsodef hIsoprop hisochar hc1D hc2D hL1D hL1'D
      ...
      hL1nL2 hL1'nL2
  have hSum24 : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 24 := by
    have htot : ∑ g ∈ Dᶜ, ((G.neighborFinset g ∩ ({c₁, c₂, L₁, L₁', L₂} : Finset (Fin 18))).card
  ...
  by_cases htri1 : ∃ a b c : Fin 18, a ∈ Dᶜ ∧ b ∈ Dᶜ ∧ c ∈ Dᶜ ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c ∧
      (¬G.Adj a L₁ ∧ ¬G.Adj a c₁ ∧ ¬G.Adj a L₁') ∧
      (¬G.Adj b L₁ ∧ ¬G.Adj b c₁ ∧ ¬G.Adj b L₁') ∧
      (¬G.Adj c L₁ ∧ ¬G.Adj c c₁ ∧ ¬G.Adj c L₁')
  ...
  by_cases htri2 : ∃ a b c : Fin 18, a ∈ Dᶜ ∧ b ∈ Dᶜ ∧ c ∈ Dᶜ ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c ∧
      (¬G.Adj a c₁ ∧ ¬G.Adj a c₂ ∧ ¬G.Adj a L₂) ∧
      (¬G.Adj b c₁ ∧ ¬G.Adj b c₂ ∧ ¬G.Adj b L₂) ∧
      (¬G.Adj c c₁ ∧ ¬G.Adj c c₂ ∧ ¬G.Adj c L₂)
