...
/-!
# Hub-triangle existence for the `n = 17`, `e(M) = 3`, `|D| = 8` residual corner

This file isolates the hub-triangle existence obligation in the `|D| = 8` (`|Hub| = 9`,
all-degree-`4`) corner of the `e(M) = 3` `P₄` dispatch for `n = 17`, reached after the
`SingleVertex`/`TwoTwin`/`TwoHub` routes have all been excluded.  It is the `n = 17` port of
`TwinCert16HubTriangle` (`n = 16`, `|Hub| = 8`).

## The residual configuration

* `D` = the degree-`3` vertices, `|D| = 8`; `Dᶜ` = the **nine** hubs, all of degree exactly `4`
  (`hdeg4`).
* `e(M) = 3`: the degree-`3` subgraph is the path `L₁–c₁–c₂–L₂`, plus `|Iso| = 4` `M`-isolated
  degree-`3` twins (`Iso`).
* `e(Hub) = 9` hub-hub edges (`∑_{w ∈ Dᶜ} |N(w) ∩ Dᶜ| = 18`, the per-hub split
  `path(g) + iso(g) + int(g) = 4` over the nine hubs: `4·9 − 6 − 12 = 18`).
* `(W)`, `(A)`, `hth : ¬TwoHubConfig` as in `n = 16`.
...
## Status (`n = 17` delta from `n = 16`)

The two **direct** branches (an avoider triangle for either cherry yields `HubTriangleConfig`
immediately) and all the structural counting (`TwinCert18HubTriangleStruct`: nine hubs, `≥ 5`
avoiders per cherry, `∑ int = 18`) are fully proved here.  The single remaining obligation is the
**no-avoider-triangle ⇒ contradiction** core `core_triangle_force_eighteen`, which carries one
documented `sorry`: the `n = 16` case-split on `|FF| ∈ {2, 3, 4}` does **not** port because the
looser budget `∑ int = 18` (vs `14`) admits `|FF| ∈ {2, 3, 4, 5, 6}` AND breaks every existing
branch.  See `core_triangle_force_eighteen` for the analysis.
-/
...
/-- **No avoider triangle ⇒ contradiction (the `n = 18` combinatorial core).**  Under the residual
hypotheses for the `n = 18`, `|D| = 8`, `|Hub| = 10` corner, the assumption that neither cherry
admits a triangle of pairwise-adjacent avoiding hubs (`htri1`, `htri2`) is contradictory.

It is discharged by `iso_rich_force_eighteen` (`TwinCert18HubTriangleFF`), which dispatches on
`|FF| ∈ {2, …, 7}` via the *isolated-twin incidence bound* `∑_{A1} iso ≤ 4`, `∑_{A2} iso ≤ 4` (each
twin has `≤ 1` neighbour in either avoider set, by `hA`), forcing the iso-mass redistribution
`∑_B iso ≥ 4 + ∑_{FF} iso`.  With the denser budget `∑ int = 22` and avoiders `≥ 6`:

* `|FF| ∈ {2, 3, 7}`: pure counting (`|B| ≤ |FF| − 2`, `∑_B iso ≥ 4`; or `∑_{FF} iso ≥ 6 > 4`);
* `|FF| = 4` (`|B| = 2`, `∑_{FF} int = 14`) and `|FF| ∈ {5, 6}` (`|B| = 3`): two iso-`3`
  non-adjacent inactive hubs (`two_iso3_nonadj_false` / `inactive_iso8_false`);
* `|FF| = 5`, `|B| = 3`, `∑_{FF} int ∈ {17, 18}`: a `TwoHubConfig` (`ff3_extract_twohub_eighteen`)
  or a Mantel-`5` triangle inside `FF ⊆ A2`;
* `|FF| = 6`, `|B| = 4`, `∑_{FF} int ∈ {21, 22}`: a Mantel-`6` triangle inside `FF ⊆ A2`, leaving
  **one documented `sorry`** for the `∑_{FF} int = 20` `K_{3,3}` corner. -/
theorem core_triangle_force_eighteen (G : SimpleGraph (Fin 18))
    ...
    False :=
  iso_rich_force_eighteen G D Iso L₁ c₁ c₂ L₂ hT hC4 hK23 hmemD hIsodef hIsoprop hisochar hcov
...
/-- **Hub-triangle existence in the `n = 18`, `|D| = 8`, `e(M) = 3`, `P₄` residual corner.**  Under
the residual hypotheses (ten degree-`4` hubs, `11` hub-hub edges `∑ int = 22`, `M = P₄`
...
no-avoider-triangle case is discharged by `core_triangle_force_eighteen` (carrying one documented
`sorry` for the `|FF| = 6` `K_{3,3}` corner, see its docstring). -/
theorem exists_hub_triangle_config_residual_eighteen (G : SimpleGraph (Fin 18))
    (D Iso : Finset (Fin 18)) (L₁ c₁ c₂ L₂ : Fin 18)
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
  ...
  have hA : ∀ t : Fin 18, t ∈ Iso → ∀ p q : Fin 18, p ≠ q →
    ...
    · have hsum0 : ∑ w ∈ ({t, p, q} : Finset (Fin 18)),
          (G.neighborFinset w ∩ ({L₁, c₁, c₂} : Finset (Fin 18))).card = 0 := by
  ...
  have hubne : ∀ g : Fin 18, g ∈ Dᶜ → g ≠ L₁ ∧ g ≠ c₁ ∧ g ≠ c₂ ∧ g ≠ L₂ := by
    intro g hg
  ...
  -- The nine hubs (`Dᶜ.card = 9`) and the four `M`-isolated twins (`Iso.card = 4`).
  ...
  -- Hub-internal-degree total: `∑ int = 4·9 − 6 − 12 = 18` (the `n = 17` delta from `n = 16`'s
  -- `14`), derived from the per-hub split `path(g) + iso(g) + int(g) = 4`.
  ...
  have hSum18 : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 22 := by
    have htot : ∑ g ∈ Dᶜ, ((G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 18))).card
  ...
  exfalso
  exact core_triangle_force_eighteen G D Iso L₁ c₁ c₂ L₂ hT hC4 hK23 hmemD hIsodef hIsoprop
