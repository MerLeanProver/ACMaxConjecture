...
/-- **Generalised residual structural counts (`n = 18`, `P₄` cherry).**  From the path/`Iso`
...
theorem cherry_p4_hub_struct_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32)
    ...
    Dᶜ.card = 18 - D.card ∧ Iso.card = D.card - 4 ∧
      ...
      (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Dᶜ).card = 70 - 6 * D.card) ∧
      (∀ g : Fin 18, g ∈ Dᶜ →
  ...
  have hisoND : ∀ t : Fin 18, t ∈ Iso → (G.neighborFinset t ∩ D).card = 0 := by
    intro t ht
    rw [Finset.card_eq_zero, Finset.eq_empty_iff_forall_notMem]
    intro a ha
    rw [Finset.mem_inter, G.mem_neighborFinset] at ha
    exact (hIsoprop t ht).2 a ha.1 ((hmemD a).mp ha.2)
  have hisohub : ∀ t : Fin 18, t ∈ Iso → (G.neighborFinset t ∩ Dᶜ).card = 3 := by
    intro t ht
    have := hsplit t
  ...
  have hDccard : Dᶜ.card = 18 - D.card := by
  ...
  have hsumInternal : ∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Dᶜ).card = 70 - 6 * D.card := by
