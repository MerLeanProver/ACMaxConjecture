...
import ACMaxConjecture.SmallCases.N18.CherryP4HubTriangleStruct
...
import ACMaxConjecture.SmallCases.N18.CherryP4HubTriangleForce

/-!
# `n = 18`, `e(M) = 3`, `P₄`-cherry, `|D| ∈ {9, 10, 11}` hub-triangle existence

This file isolates the hub-triangle existence obligation in the `|D| ∈ {9, 10, 11}`
(`|Hub| ∈ {9, 8, 7}`) both-leaf residual of the `e(M) = 3` `P₄` dispatch for `n = 18`, reached
after the `SingleVertex`/`TwoTwin`/`TwoHub` routes have all been excluded.  It is the `n = 18`
analog of `exists_hub_triangle_config_p4_D9D10_seventeen`.

## The `n = 18` obstruction (why this is a documented `sorry`)
...
* `hub_triangle_force_p4_D11_eighteen` — clean counting contradiction at `|D| = 11`
  (`Dᶜ.card = 7`, `∑ int = 4`); deps: the WA per-hub lemmas, `shared_hub_le5`-style routing of the
  single degree-`≥ 6` hub.  File: `TwinCert18CherryP4HubTriangleForce.lean`.
* `hub_triangle_force_p4_D10_eighteen` — `|D| = 10` (`∑ int = 10`): clean contradiction off the
  fully-free degree-`6` corner, which collapses as in `n = 17 ten_avoider_clean_contra`.  Same file.
* `hub_triangle_force_p4_D9_eighteen` — `|D| = 9` (`∑ int = 16`): the `|FF| = 5` Mantel-`5`
  ...
  `nonadj_deg4_hubs_share_le_one_iso_pointwise_eighteen`.  File:
  `TwinCert18CherryP4HubTriangleForce.lean`.
-/
...
/-- **Hub-triangle existence in the `n = 18`, `|D| ∈ {9, 10, 11}`, `e(M) = 3`, `P₄` both-leaf
...
The previously open `|D| = 9`, `|FF| = 4` bipartite escape (the `n = 18` internal-total dilution
that defeats the `n = 17` Mantel/budget force, not excluded by `hC4` alone) is now closed in
`TwinCert18CherryP4HubTriangleForceD9FF4` by extracting a `TwoHubConfig` from two isolated
degree-`4` hubs, so this declaration is fully proved (axiom-clean). -/
theorem exists_hub_triangle_config_p4_eighteen (G : SimpleGraph (Fin 18))
  ...
  obtain ⟨hDccard, hIsocard, hc1hub, hc2hub, hL1hub, hL2hub, hsumPath, hsumIso, hsumInternal,
    hper⟩ := cherry_p4_hub_struct_eighteen G hm L₁ c₁ c₂ L₂ D Iso hT hmemD hIsoprop hisochar
  ...
  exfalso
  exact iso_rich_force_p4_eighteen G D Iso L₁ c₁ c₂ L₂ h3 hmemD hIsoprop hisochar
    hL1deg hc1deg hc2deg hL2deg hac1L1 hc12 hac2L2 hL1nc2 hL2nc1 hNc1D hNc2D hL1D hc1D hc2D hL2D
    hd_lb hd_ub htt hth hT hC4 hDccard hIsocard hc1hub hc2hub hL1hub hL2hub hsumPath hsumIso
    hsumInternal hper htri1 htri2
