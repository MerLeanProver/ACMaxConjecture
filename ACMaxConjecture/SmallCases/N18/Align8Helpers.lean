...
Ported `Fin 17 → Fin 18` analogues of the single-vertex / two-hub assembly lemmas used to close the
residual alignment corners.  The single-vertex selectors route the proved degree-`4` double-star /
`C₅` hub-avoidance lemmas (`hub_meets_path_le_one_eighteen`, `hub_cycle_cases_eighteen`) through
...
theorem assemble_single_vertex_config (G : SimpleGraph (Fin 18))
    (t h₁ h₂ x y z : Fin 18)
    (ht3 : G.degree t = 3) (htiso : ∀ w : Fin 18, G.Adj t w → G.degree w ≠ 3)
  ...
  have hsum0 : ∑ p ∈ ({t, h₁, h₂} : Finset (Fin 18)),
      (G.neighborFinset p ∩ ({x, y, z} : Finset (Fin 18))).card = 0 := by
    ...
    · exact hh₂y hadj
    · exact hh₂z hadj
  have hside : 2 * (∑ p ∈ ({t, h₁, h₂} : Finset (Fin 18)),
      (G.neighborFinset p ∩ ({x, y, z} : Finset (Fin 18))).card)
...
theorem dom_doublestar_leaves (G : SimpleGraph (Fin 18)) (D : Finset (Fin 18))
    ...
    (hdom : ∀ p q : Fin 18, p ∈ D → q ∈ D → G.Adj p q →
      p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂) :
    ∃ ℓ₁ ℓ₂ ℓ₃ ℓ₄ : Fin 18,
      G.degree ℓ₁ = 3 ∧ G.degree ℓ₂ = 3 ∧ G.degree ℓ₃ = 3 ∧ G.degree ℓ₄ = 3 ∧
      ({c₁, c₂, ℓ₁, ℓ₂, ℓ₃, ℓ₄} : Finset (Fin 18)).card = 6 ∧
  ...
  have hcross : ∀ u v : Fin 18, G.neighborFinset u ∩ D = {c₁} →
      G.Adj c₂ v → u ≠ v := by
    intro u v hu hcv e
    subst e
  ...
  have hcard6 : ({c₁, c₂, ℓ₁, ℓ₂, ℓ₃, ℓ₄} : Finset (Fin 18)).card = 6 := by
...
theorem dom_centres_indeg3 (G : SimpleGraph (Fin 18)) (D : Finset (Fin 18))
    (c₁ c₂ : Fin 18) (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hc12 : G.Adj c₁ c₂)
  ...
  have hc1mem : c₁ ∈ G.neighborFinset c₂ ∩ D :=
    Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hc12.symm, hc1D⟩
  have hb1eq : (G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 18)))).card
      = (G.neighborFinset c₁ ∩ D).card - 1 := by
    have heq : G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 18)))
  ...
  have hb2eq : (G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 18)))).card
      = (G.neighborFinset c₂ ∩ D).card - 1 := by
    have heq : G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 18)))
        = (G.neighborFinset c₂ ∩ D).erase c₁ := by
      ext w
...
theorem single_vertex_doublestar_count (G : SimpleGraph (Fin 18)) (D : Finset (Fin 18))
    (hmemD : ∀ v : Fin 18, v ∈ D ↔ G.degree v = 3)
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    ...
    (t a b c : Fin 18) (ht3 : G.degree t = 3)
  ...
  have leafdeg : ∀ w : Fin 18, (w = ℓ₁ ∨ w = ℓ₂ ∨ w = ℓ₃ ∨ w = ℓ₄) → G.degree w = 3 := by
    rintro w (rfl | rfl | rfl | rfl) <;> assumption
  have leafD : ∀ w : Fin 18, (w = ℓ₁ ∨ w = ℓ₂ ∨ w = ℓ₃ ∨ w = ℓ₄) → w ∈ D :=
    fun w hw => (hmemD w).mpr (leafdeg w hw)
  have hlne : ∀ w cen oth : Fin 18, G.neighborFinset w ∩ D = {cen} →
      (G.neighborFinset oth ∩ D).card = 3 → w ≠ oth := by
    intro w cen oth hw hoth e
    subst e
    rw [hw, Finset.card_singleton] at hoth
    omega
  have hleafnec : ∀ w : Fin 18, (w = ℓ₁ ∨ w = ℓ₂ ∨ w = ℓ₃ ∨ w = ℓ₄) → w ≠ c₁ ∧ w ≠ c₂ := by
  ...
  have hleafnonadj : ∀ u v : Fin 18, (u = ℓ₁ ∨ u = ℓ₂ ∨ u = ℓ₃ ∨ u = ℓ₄) →
  ...
  have hubcen : ∀ hh : Fin 18, (hh = a ∨ hh = b ∨ hh = c) →
  ...
  have ind2 : ∀ p q : Prop, ¬(p ∧ q) →
    ...
    · rw [if_neg hp]
      split_ifs <;> omega
  have hpair : ∀ hh : Fin 18, G.degree hh = 4 →
      ¬(G.Adj hh ℓ₁ ∧ G.Adj hh ℓ₂) ∧ ¬(G.Adj hh ℓ₃ ∧ G.Adj hh ℓ₄) := by
    intro hh hh4
    refine ⟨?_, ?_⟩
    · exact (hub_meets_path_le_one_eighteen G hT hC4 hh4 hd1 hc1deg hd2
        hac1.symm hac2
        (hleafnonadj ℓ₁ ℓ₂ (Or.inl rfl) (Or.inr (Or.inl rfl)) hℓ12)
        (G.ne_of_adj hac1).symm (G.ne_of_adj hac2) hℓ12).2.2
    · exact (hub_meets_path_le_one_eighteen G hT hC4 hh4 hd3 hc2deg hd4
        hac3.symm hac4
  ...
  have keyleaf : ∃ w : Fin 18, (w = ℓ₁ ∨ w = ℓ₂ ∨ w = ℓ₃ ∨ w = ℓ₄) ∧
  ...
  have key : ∃ w' h₁ h₂ : Fin 18, (w' = ℓ₁ ∨ w' = ℓ₂ ∨ w' = ℓ₃ ∨ w' = ℓ₄) ∧
...
theorem single_vertex_config_from_C5_three_hubs (G : SimpleGraph (Fin 18))
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (t h₁ h₂ h₃ : Fin 18) (ht3 : G.degree t = 3)
    (htiso : ∀ w : Fin 18, G.Adj t w → G.degree w ≠ 3)
  ...
  have mg1 : ∀ ha hb : Fin 18, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
  ...
  have mg2 : ∀ ha hb : Fin 18, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
      G.Adj t ha → G.Adj t hb →
      ¬G.Adj ha v₂ → ¬G.Adj ha v₃ → ¬G.Adj ha v₄ →
      ¬G.Adj hb v₂ → ¬G.Adj hb v₃ → ¬G.Adj hb v₄ → SingleVertexConfig G :=
    fun ha hb hca hcb hcne hta htb a1 a2 a3 b1 b2 b3 =>
  ...
  have mg3 : ∀ ha hb : Fin 18, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
      G.Adj t ha → G.Adj t hb →
      ¬G.Adj ha v₃ → ¬G.Adj ha v₄ → ¬G.Adj ha v₅ →
      ¬G.Adj hb v₃ → ¬G.Adj hb v₄ → ¬G.Adj hb v₅ → SingleVertexConfig G :=
    fun ha hb hca hcb hcne hta htb a1 a2 a3 b1 b2 b3 =>
  ...
  have mg4 : ∀ ha hb : Fin 18, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
      G.Adj t ha → G.Adj t hb →
      ¬G.Adj ha v₄ → ¬G.Adj ha v₅ → ¬G.Adj ha v₁ →
      ¬G.Adj hb v₄ → ¬G.Adj hb v₅ → ¬G.Adj hb v₁ → SingleVertexConfig G :=
    fun ha hb hca hcb hcne hta htb a1 a2 a3 b1 b2 b3 =>
  ...
  have mg5 : ∀ ha hb : Fin 18, G.degree ha = 4 → G.degree hb = 4 → ha ≠ hb →
      G.Adj t ha → G.Adj t hb →
      ¬G.Adj ha v₅ → ¬G.Adj ha v₁ → ¬G.Adj ha v₂ →
      ¬G.Adj hb v₅ → ¬G.Adj hb v₁ → ¬G.Adj hb v₂ → SingleVertexConfig G :=
    fun ha hb hca hcb hcne hta htb a1 a2 a3 b1 b2 b3 =>
  ...
  have hB := hub_cycle_cases_eighteen G hT hC4 h₁ v₁ v₂ v₃ v₄ v₅ hh₁4 hd1 hd2 hd3 hd4 hd5
    e12 e23 e34 e45 e51 n13 n14 n24 n25 n35 d12 d13 d14 d15 d23 d24 d25 d34 d35 d45
  have hC := hub_cycle_cases_eighteen G hT hC4 h₂ v₁ v₂ v₃ v₄ v₅ hh₂4 hd1 hd2 hd3 hd4 hd5
    e12 e23 e34 e45 e51 n13 n14 n24 n25 n35 d12 d13 d14 d15 d23 d24 d25 d34 d35 d45
  have hD := hub_cycle_cases_eighteen G hT hC4 h₃ v₁ v₂ v₃ v₄ v₅ hh₃4 hd1 hd2 hd3 hd4 hd5
    e12 e23 e34 e45 e51 n13 n14 n24 n25 n35 d12 d13 d14 d15 d23 d24 d25 d34 d35 d45
...
theorem dense_two_hub_assemble (G : SimpleGraph (Fin 18)) (h₁ h₂ a b c d : Fin 18)
    ...
    (haiso : ∀ w : Fin 18, G.Adj a w → G.degree w ≠ 3)
    (hbiso : ∀ w : Fin 18, G.Adj b w → G.degree w ≠ 3)
...
`|Iso| ≥ |D| − 4 = 4`. -/
theorem shared_deg4_hub_nodeg5_eighteen (G : SimpleGraph (Fin 18))
  ...
  have hsum48 : ∑ v : Fin 18, G.degree v = 64 := by
  ...
  have hHub4card : Hub4.card = 10 := by rw [hHub4eqDc]; omega
  ...
  have hcount : Hub4.card < ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub4).card := by
    ...
    have hge9 : 12 ≤ ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub4).card := by
      ...
      omega
    omega
  exact shared_deg4_hub_from_count_eighteen G Iso Hub4 hIsoiso hHub4deg hcount
...
theorem shared_deg4_hub_deg5_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
  ...
  have hsumD : ∑ v ∈ D, G.degree v = 3 * D.card := by
    rw [Finset.sum_congr rfl (fun v hv => (hmemD v).mp hv), Finset.sum_const, smul_eq_mul,
      mul_comm]
  have hsum52 : ∑ v : Fin 18, G.degree v = 64 := by
    rw [SimpleGraph.sum_degrees_eq_twice_card_edges, hm]
  have hsplit : ∑ v ∈ D, G.degree v + ∑ v ∈ Dᶜ, G.degree v = 64 := by
