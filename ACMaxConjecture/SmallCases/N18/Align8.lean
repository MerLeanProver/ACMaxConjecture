...
import ACMaxConjecture.SmallCases.N18.Align8Mid
import ACMaxConjecture.SmallCases.N18.FatStarStruct

/-!
# Dense `e(M) ≥ 4` alignment dichotomy for `n = 18` (`halign8` helpers)

Ports the proved-axiom-clean `n = 17` `TwinCert17Align8` to `Fin 18`, split by the value of `e(M)`.

* `halign8_eM5_eighteen` (`e(M) = 5`, `s = 10`): an all-degree-`4` `M`-isolated cherry routes the
  ...
  two-twin assembly (`two_twin_eM5_mid_eighteen`) for `|D| ≥ 10`.  The `|D| = 9` thin double-star
  (`|Iso| = 3`, where the degree-`≤ 5` shared-hub pigeonhole only *ties*) is the one documented
  residual.

* `halign8_eM4_eighteen` (`e(M) = 4`, `s = 8`): the induced-`C₅` branch is impossible (`s ≥ 10 > 8`),
  ...
  is the one documented residual (the `n = 17` `fatstar` analog, awaiting the `n = 18` port of
...
/-- **`halign8` for `n = 18`, `e(M) = 5` (`s = 10`).**  An all-degree-`4` `M`-isolated cherry routes
the dominating-edge double-star and the induced-`C₅` to `SingleVertexConfig`; the non-all-degree-`4`
case (a degree-`5` hub on the cherry, `|D| ≥ 10`) routes through `two_twin_eM5_mid_eighteen`.  The
`|D| = 9` thin double-star is isolated as one documented `sorry`. -/
theorem halign8_eM5_eighteen (G : SimpleGraph (Fin 18))
  ...
  · -- **Residual.**  A neighbour of the `M`-isolated twin `t` has degree `≥ 5`, so `|D| ≥ 9`.
    ...
    obtain ⟨w, hw5⟩ : ∃ w : Fin 18, 5 ≤ G.degree w := by
      by_contra hc
      push Not at hc
      exact hall ⟨by have := hc p; omega, by have := hc q; omega, by have := hc r; omega⟩
    have hDge9 : 9 ≤ D.card := by
      have := card_D_ge_nine_of_hub_deg5_eighteen G hm h3 w hw5
      rwa [← hDdef] at this
    ...
    · -- **`|D| = 9` thin double-star (`|Iso| = 3`).**  The degree-`≤ 5` shared-hub pigeonhole only
      ...
      have hnall : ¬∀ w : Fin 18, G.Adj t w → G.degree w = 4 := fun hcon =>
        hall ⟨hcon p htp, hcon q htq, hcon r htr⟩
      rcases eM5_D9_shared_or_alldeg4_eighteen G hm h3 h2k2 hD9 hsum10.le t ht3 htiso hnall with
      ...
      · have hkge4 : 4 ≤ G.degree k := by have := htw1iso k hAtw1k; have := h3 k; omega
        exact Or.inr (Or.inl (two_twin_eM5_from_shared_le5_eighteen G hm h3 hT h2k2 hC4 hDge9
          hsum10 k tw1 tw2 hkge4 hkle5 htw12 htw1deg htw2deg hAtw1k hAtw2k htw1iso htw2iso))
      · exact Or.inl (sv_eM5_from_alldeg4_iso_eighteen G hT h2k2 hC4 hsum10 t' ht'3 ht'iso ht'all)
    · have hDge10 : 10 ≤ D.card := by omega
      exact Or.inr (Or.inl (two_twin_eM5_mid_eighteen G hm h3 hT h2k2 hC4 hDge10 hsum10))

/-- **`halign8` for `n = 18`, `e(M) = 4` (`s = 8`).**  The induced-`C₅` branch is impossible
...
theorem halign8_eM4_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
  ...
  · -- **Dominating-edge double-star (the only surviving structure at `e(M) = 4`).**
    obtain ⟨c₁, c₂, hc1D, hc2D, hc12, hcov⟩ := hdom
    have hD8' : 8 ≤ D.card := (residual_hub_card_le_ten G hm h3).2
    ...
    · -- **Residual `|D| = 8` (`|Hub| = 10`, all degree-`4`, `|Iso| = 3`).**  The dominating
      ...
      -- `fatstar_structure_eM4_D8_eighteen`, supplying the `¬SingleVertex/¬TwoTwin/¬TwoHub` residual.
      ...
      · exact Or.inr (Or.inr (Or.inr (fatstar_structure_eM4_D8_eighteen G hm h3 hT hC4 D hmemD hD8
          c₁ c₂ hc1D hc2D hc12 hcov hindle hs8 hsv htt hth)))
    · -- **`|D| ∈ {9, …, 13}`.**  Extract a degree-`≤ 5` hub shared by two `M`-isolated twins and
      -- route the dominating-edge double-star through `dom_fat_centre_two_twin_le5_eighteen`.
      have hDge9 : 9 ≤ D.card := by omega
      obtain ⟨k, tw1, tw2, hkle5, htw12, htw1deg, htw2deg, hAtw1k, hAtw2k, htw1iso, htw2iso⟩ :=
        shared_hub_le5_eM4_eighteen G hm h3 h2k2 hDge9 hs8.le
      have hkge4 : 4 ≤ G.degree k := by
        have := htw1iso k hAtw1k; have := h3 k; omega
      exact Or.inr (Or.inl (dom_fat_centre_two_twin_le5_eighteen G D hmemD hT hC4 c₁ c₂ hc1D hc2D
