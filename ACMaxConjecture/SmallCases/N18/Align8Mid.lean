...
/-!
# Mid-density shared-hub extraction for `n = 18`, dense `e(M)` (`halign8` residual)

Ports the proved-axiom-clean `n = 17` `TwinCert17Align8Mid` to `Fin 18`.  Two density regimes:
...
* `e(M) = 5` (`s = 10`), `|D| ∈ {10, …, 13}`: a degree-`5` hub sits on the `M`-isolated cherry, so
  ...
  deg-`≤ 5` two-twin assemblies.  (Unlike `n = 17`, the `n = 18` `|Hub| = |D|` shift pushes the strict
...
* `e(M) = 4` (`s = 8`), `|D| ≥ 9`: the induced-`C₅` branch is impossible, so only the dominating-edge
  ...
  the dominating double-star through `dom_fat_centre_two_twin_le5_eighteen`.
-/
...
/-- **`|D| ≥ 9` from a degree-`≥ 5` hub (`n = 18`).**  With `∑ deg = 64` and every non-`D` vertex of
degree `≥ 4`, the existence of one degree-`≥ 5` vertex pushes the hub-degree sum above `4 |Hub| + 1`,
forcing `|D| ≥ 9` by the handshake. -/
theorem card_D_ge_nine_of_hub_deg5_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    (w : Fin 18) (hw5 : 5 ≤ G.degree w) :
    9 ≤ (Finset.univ.filter (fun v : Fin 18 => G.degree v = 3)).card := by
  classical
  set D : Finset (Fin 18) := Finset.univ.filter (fun v : Fin 18 => G.degree v = 3) with hDdef
  have hmemD : ∀ v : Fin 18, v ∈ D ↔ G.degree v = 3 := by intro v; rw [hDdef]; simp
  set Hub : Finset (Fin 18) := Finset.univ.filter (fun v : Fin 18 => 4 ≤ G.degree v) with hHubdef
...
/-- **Refined deg-`≤ 5` shared-hub count for `e(M) = 5`, `|D| ≥ 10` (`n = 18`).**  In the
`M`-isolated-twin-dense regime (`s ≤ 10`, `|D| ≥ 10`) there is a degree-`≤ 5` hub adjacent to two
distinct `M`-isolated degree-`3` twins.  Each degree-`≥ 6` hub's isolated-twin incidences are bounded
by `|Iso|` rather than its degree, and `|H₆| ≤ 2`; a case-split on `|H₆|` closes the strict
pigeonhole for every `|D| ∈ {10, …, 13}`. -/
theorem shared_hub_le5_eM5_mid_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    ...
    (hDge10 : 10 ≤ (Finset.univ.filter (fun w : Fin 18 => G.degree w = 3)).card)
...
/-- **`e(M) = 5` (`s = 10`) two-twin cut for the mid corner `|D| ∈ {10, …, 13}` (`n = 18`).**  A
degree-`5` hub on the `M`-isolated cherry rules out the all-degree-`4` `SingleVertexConfig` route, so
we extract a degree-`≤ 5` hub shared by two `M`-isolated twins (`shared_hub_le5_eM5_mid_eighteen`) and
route the dominating-edge double-star through `dom_fat_centre_two_twin_le5_eighteen` and the
induced-`C₅` through `c5_shared_two_twin_le5_eighteen`. -/
theorem two_twin_eM5_mid_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
  ...
  · obtain ⟨c₁, c₂, hc1D, hc2D, hc12, hcov⟩ := hdom
    obtain ⟨k, tw1, tw2, hkle5, htw12, htw1deg, htw2deg, hAtw1k, hAtw2k, htw1iso, htw2iso⟩ :=
      shared_hub_le5_eM5_mid_eighteen G hm h3 h2k2 (by rw [← hDdef]; exact hDge10)
...
/-- **Deg-`≤ 5` shared hub for `e(M) = 4`, `|D| ≥ 9` (`n = 18`).**  At `s = 8` the `M`-isolated set
...
theorem shared_hub_le5_eM4_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    (h2k2 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
      G.degree a = 3 ∧ G.degree b = 3 ∧ G.degree c = 3 ∧ G.degree d = 3 ∧
      G.Adj a b ∧ G.Adj c d ∧ ¬G.Adj a c ∧ ¬G.Adj a d ∧ ¬G.Adj b c ∧ ¬G.Adj b d)
...
/-- **Routing a given degree-`≤ 5` shared hub to `TwoTwinConfig` (`e(M) = 5`, `n = 18`).**  Given a
degree-`≤ 5` hub `k` adjacent to two distinct `M`-isolated degree-`3` twins, the dominating-edge
double-star routes through `dom_fat_centre_two_twin_le5_eighteen` and the induced-`C₅` through
`c5_shared_two_twin_le5_eighteen`.  This is the post-pigeonhole part of `two_twin_eM5_mid_eighteen`,
factored out for reuse by the `|D| = 9` re-selection. -/
theorem two_twin_eM5_from_shared_le5_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
    ...
    (hsum10 : ∑ v ∈ Finset.univ.filter (fun w => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w => G.degree w = 3)).card = 10)
    (k tw1 tw2 : Fin 18) (hkge4 : 4 ≤ G.degree k) (hkle5 : G.degree k ≤ 5) (htw12 : tw1 ≠ tw2)
    (htw1deg : G.degree tw1 = 3) (htw2deg : G.degree tw2 = 3)
    (hAtw1k : G.Adj tw1 k) (hAtw2k : G.Adj tw2 k)
    (htw1iso : ∀ w : Fin 18, G.Adj tw1 w → G.degree w ≠ 3)
    (htw2iso : ∀ w : Fin 18, G.Adj tw2 w → G.degree w ≠ 3) :
    TwoTwinConfig G := by
  classical
  set D : Finset (Fin 18) := Finset.univ.filter (fun w => G.degree w = 3) with hDdef
  have hmemD : ∀ v : Fin 18, v ∈ D ↔ G.degree v = 3 := by intro v; rw [hDdef]; simp
  ...
  · obtain ⟨v₁, v₂, v₃, v₄, v₅, hv1D, hv2D, hv3D, hv4D, hv5D, hcard5, e12, e23, e34, e45, e51,
    ...
    exact c5_shared_two_twin_le5_eighteen G hT hC4 k tw1 tw2 hkge4 hkle5 htw12 htw1deg htw2deg
      hAtw1k hAtw2k htw1iso htw2iso
      (fun a b c hda hdb hdc hka hkb hkc hab hbc hab_ne hbc_ne hac_ne =>
        exists_nonk_two_twin_hub_eighteen G hm h3 (by rw [← hDdef]; exact hDge9)
          v₁ v₂ v₃ v₄ v₅ ((hmemD v₁).mp hv1D) ((hmemD v₂).mp hv2D) ((hmemD v₃).mp hv3D)
          ((hmemD v₄).mp hv4D) ((hmemD v₅).mp hv5D) hcard5 e12 e23 e34 e45 e51
          (by rw [← hDdef]; exact hsum10) k a b c
          hkge4 hkle5 hda hdb hdc hka hkb hkc hab hbc hab_ne hbc_ne hac_ne)
...
/-- **Routing an all-degree-`4` `M`-isolated cherry to `SingleVertexConfig` (`e(M) = 5`, `n = 18`).**
...
`halign8_eM5_eighteen`, factored out for reuse by the `|D| = 9` re-selection. -/
theorem sv_eM5_from_alldeg4_iso_eighteen (G : SimpleGraph (Fin 18))
    (hT : ¬∃ x y z : Fin 18, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
      G.degree a = 3 ∧ G.degree b = 3 ∧ G.degree c = 3 ∧ G.degree d = 3 ∧
    ...
    (hsum10 : ∑ v ∈ Finset.univ.filter (fun w => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w => G.degree w = 3)).card = 10)
    (t' : Fin 18) (ht3 : G.degree t' = 3)
    (htiso : ∀ w : Fin 18, G.Adj t' w → G.degree w ≠ 3)
    (htall : ∀ w : Fin 18, G.Adj t' w → G.degree w = 4) :
...
/-- **`|D| = 9` re-selection dichotomy for `e(M) = 5` (`n = 18`).**  At `|D| = 9` the hub-degree sum
...
only ties. -/
theorem eM5_D9_shared_or_alldeg4_eighteen (G : SimpleGraph (Fin 18))
    (hm : G.edgeFinset.card = 32) (h3 : ∀ v : Fin 18, 3 ≤ G.degree v)
    (h2k2 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
      G.degree a = 3 ∧ G.degree b = 3 ∧ G.degree c = 3 ∧ G.degree d = 3 ∧
      G.Adj a b ∧ G.Adj c d ∧ ¬G.Adj a c ∧ ¬G.Adj a d ∧ ¬G.Adj b c ∧ ¬G.Adj b d)
    (hD9 : (Finset.univ.filter (fun w : Fin 18 => G.degree w = 3)).card = 9)
    (hsumle : ∑ v ∈ Finset.univ.filter (fun w => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w => G.degree w = 3)).card ≤ 10)
    (t : Fin 18) (_ht3 : G.degree t = 3) (htiso : ∀ w : Fin 18, G.Adj t w → G.degree w ≠ 3)
    (hnall : ¬∀ w : Fin 18, G.Adj t w → G.degree w = 4) :
    (∃ k tw1 tw2 : Fin 18, G.degree k ≤ 5 ∧ tw1 ≠ tw2 ∧ G.degree tw1 = 3 ∧ G.degree tw2 = 3 ∧
        G.Adj tw1 k ∧ G.Adj tw2 k ∧ (∀ w : Fin 18, G.Adj tw1 w → G.degree w ≠ 3) ∧
        (∀ w : Fin 18, G.Adj tw2 w → G.degree w ≠ 3))
      ∨ (∃ t' : Fin 18, G.degree t' = 3 ∧ (∀ w : Fin 18, G.Adj t' w → G.degree w ≠ 3) ∧
        (∀ w : Fin 18, G.Adj t' w → G.degree w = 4)) := by
  classical
  set D : Finset (Fin 18) := Finset.univ.filter (fun w => G.degree w = 3) with hDdef
  have hmemD : ∀ v : Fin 18, v ∈ D ↔ G.degree v = 3 := by intro v; rw [hDdef]; simp
  set Hub : Finset (Fin 18) := Finset.univ.filter (fun v => 4 ≤ G.degree v) with hHubdef
  have hmemHub : ∀ v : Fin 18, v ∈ Hub ↔ 4 ≤ G.degree v := by intro v; rw [hHubdef]; simp
  set Hub4 : Finset (Fin 18) := Finset.univ.filter (fun v => G.degree v = 4) with hHub4def
  have hmemHub4 : ∀ v : Fin 18, v ∈ Hub4 ↔ G.degree v = 4 := by intro v; rw [hHub4def]; simp
  set H5 : Finset (Fin 18) := Finset.univ.filter (fun v => 5 ≤ G.degree v) with hH5def
  have hmemH5 : ∀ v : Fin 18, v ∈ H5 ↔ 5 ≤ G.degree v := by intro v; rw [hH5def]; simp
  have hHubeqDc : Hub = Dᶜ := by
    ext w; rw [hmemHub, Finset.mem_compl, hmemD]; have := h3 w; omega
  have hsum64 : ∑ v : Fin 18, G.degree v = 64 := by
    rw [SimpleGraph.sum_degrees_eq_twice_card_edges, hm]
  ...
  have hsplit : ∑ v ∈ D, G.degree v + ∑ v ∈ Dᶜ, G.degree v = 64 := by
    rw [Finset.sum_add_sum_compl]; exact hsum64
  have hHubsum : ∑ v ∈ Hub, G.degree v = 37 := by
    rw [hHubeqDc]; rw [hsumD, hD9] at hsplit; omega
  have hHubcard : Hub.card = 9 := by
  ...
  obtain ⟨w0, hw0adj, hw0ne4⟩ : ∃ w : Fin 18, G.Adj t w ∧ G.degree w ≠ 4 := by
    by_contra hc
    push Not at hc
  ...
  have hKle5 : G.degree K ≤ 5 := by
    have hHub4card : Hub4.card = 8 := by omega
  ...
  have hIso3 : 3 ≤ Iso.card := by
    ...
    rw [← hIsodef] at hpart
    omega
  by_cases hsh : ∃ a b : Fin 18, a ∈ Iso ∧ b ∈ Iso ∧ a ≠ b ∧ G.Adj a K ∧ G.Adj b K
