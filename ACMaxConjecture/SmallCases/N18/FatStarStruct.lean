...
import ACMaxConjecture.SmallCases.N18.HubTriangleFatStar

/-!
# Asymmetric dominating-edge double-star structure for `n = 18`, `e(M) = 4`, `|D| = 8`

This file supplies `fatstar_structure_eM4_D8_eighteen`, the structural dispatcher used to close the
`halign8_eM4_eighteen` dominating-edge `|D| = 8` branch.  The branch already establishes `|D| = 8`
upstream, so the dispatcher receives it directly.  Given the `e(M) = 4` dominating edge `c₁–c₂`
(covering hypothesis `hcov`), the handshake (`∑ deg = 64`, `∑_D deg = 24`) forces all ten hubs to
degree `4`, and the in-`M`-degrees split asymmetrically `(3, 2)` (one fat centre with two leaves,
one thin centre with one leaf), via `thin_eM_formula_eighteen`.  The fat double-star
`L₁, L₁' – c₁ – c₂ – L₂` is extracted exactly and fed to `exists_hub_triangle_config_fatstar_eighteen`.
-/
...
`¬SingleVertex/¬TwoTwin/¬TwoHub` residual to `exists_hub_triangle_config_fatstar_eighteen`. -/
theorem fatstar_from_fat_thin_eighteen (G : SimpleGraph (Fin 18)) (D : Finset (Fin 18))
    ...
    (hcov : ∀ p q : Fin 18, p ∈ D → q ∈ D → G.Adj p q → p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
  ...
  have hisochar : ∀ w : Fin 18, w ∈ D → w ≠ c₁ → w ≠ c₂ → w ≠ L₁ → w ≠ L₁' → w ≠ L₂ → w ∈ Iso := by
    ...
    · have hmem : w ∈ G.neighborFinset c₂ ∩ D :=
      ...
      · exact hwc1 h'
      · exact hwL2 h'
  exact exists_hub_triangle_config_fatstar_eighteen G D Iso c₁ c₂ L₁ L₁' L₂ hT hC4 hmemD hIsodef
...
/-- **Fat double-star structural dispatcher (`n = 18`, `e(M) = 4`, dominating edge, `|D| = 8`).**
Routes the `halign8_eM4_eighteen` dominating-edge `|D| = 8` branch: the handshake forces all ten
hubs to degree `4`, and the in-`M`-degree split is `(3, 2)` (`thin_eM_formula_eighteen`), giving a
fat double-star fed to `exists_hub_triangle_config_fatstar_eighteen`. -/
theorem fatstar_structure_eM4_D8_eighteen (G : SimpleGraph (Fin 18))
    ...
    (hC4 : ¬∃ a b c d : Fin 18, ({a, b, c, d} : Finset (Fin 18)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (D : Finset (Fin 18)) (hmemD : ∀ v : Fin 18, v ∈ D ↔ G.degree v = 3)
    (hD8 : D.card = 8)
    (c₁ c₂ : Fin 18) (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hc12 : G.Adj c₁ c₂)
    (hcov : ∀ p q : Fin 18, p ∈ D → q ∈ D → G.Adj p q → p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
    ...
    HubTriangleConfig G := by
  classical
  -- Handshake: `∑ deg = 64`, `∑_D deg = 24`, so `∑_{Dᶜ} deg = 40` over the ten hubs.
  ...
  have hsplit64 : ∑ v ∈ D, G.degree v + ∑ v ∈ Dᶜ, G.degree v = 64 := by
  ...
  have hDccard : Dᶜ.card = 10 := by rw [Finset.card_compl, Fintype.card_fin, hD8]
  have hdeg4 : ∀ w : Fin 18, w ∈ Dᶜ → G.degree w = 4 := by
    intro w hw
    ...
    have hsum40 : ∑ v ∈ Dᶜ, G.degree v = 40 := by omega
  ...
  · have hi2 : (G.neighborFinset c₂ ∩ D).card = 2 := by omega
    exact fatstar_from_fat_thin_eighteen G D hmemD hT hC4 c₁ c₂ hc1D hc2D hc12 hcov hfat hi2
      hdeg4 hD8 hsv htt hth
  · have hi1 : (G.neighborFinset c₁ ∩ D).card = 2 := by omega
    have hi2 : (G.neighborFinset c₂ ∩ D).card = 3 := by omega
    have hcov' : ∀ p q : Fin 18, p ∈ D → q ∈ D → G.Adj p q →
        p = c₂ ∨ p = c₁ ∨ q = c₂ ∨ q = c₁ := by
      intro p q hp hq hpq
    ...
    exact fatstar_from_fat_thin_eighteen G D hmemD hT hC4 c₂ c₁ hc2D hc1D hc12.symm hcov' hi2 hi1
      hdeg4 hD8 hsv htt hth
