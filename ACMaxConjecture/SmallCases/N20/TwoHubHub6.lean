...
/-!
# `n = 20`, `|D| = 13`, `|Hub| = 7` corner selector

The `|D| = 13`, `|Hub| = 7` regime that the dense alignment dispatcher (`halign8_twenty`) needs.
By the handshake the residual degree-sum over the seven hubs is `72 − 39 = 33`, i.e. excess
`33 − 28 = 5`; so the hubs are at most five degree-`5` (or one degree-`6` plus three degree-`5`,
etc.).

As at `n = 17` and `n = 19`, the deg-`4` shared-hub pigeonhole only ties for `n = 20` (degree sum
`72` is divisible by `3`), so the assembly routes through the deg-`≤ 5` shared hub of
`TwinCert20TwoHubHub6Deg5`:

* **dominating-edge branch (fully proved):** `shared_hub_le5_eM5_twenty` (strict because
  `|Hub| = 7`) supplies a deg-`≤ 5` hub shared by two `M`-isolated twins, and
  `dom_fat_centre_two_twin_le5_twenty` closes the `TwoTwin` cut for any `s ∈ {8, 10}`;
* **induced-`C₅` branch:** an induced `C₅` in `M` forces `e(M) = 5` (`s = 10`).  If the cherry's
  ...
  `SingleVertex` cut; otherwise `two_twin_eM5_twenty` gives the `TwoTwin` cut.

This selector is proved unconditionally: `two_twin_eM5_twenty` is sorry-free at `n = 20` (its
deg-`≤ 5` induced-`C₅` sub-case is closed via the second-hub extraction
`exists_nonk_two_twin_hub_twenty`).
-/
...
/-- **`|D| = 13`, `|Hub| = 7` four-way alignment selector for `n = 20` (`e(M) ≥ 4`).**
In the dense (`s := ∑_{v∈D}|N v ∩ D| ≥ 8`) `|Hub| = 7` residual, the alignment dichotomy
...
yields a `SingleVertex` cut (all-deg-`4` cherry) or a `TwoTwin` cut (`two_twin_eM5_twenty`). -/
theorem two_hub_or_single_vertex_hub6_twenty (G : SimpleGraph (Fin 20))
    ...
    (hD13 : (Finset.univ.filter (fun w : Fin 20 => G.degree w = 3)).card = 13)
  ...
  · -- **Dominating-edge branch (TwoTwin, fully proved).**  The deg-`≤ 5` shared hub from
    ...
    exact Or.inr (Or.inr (Or.inl (dom_fat_centre_two_twin_le5_twenty G D hmemD hT hC4 c₁ c₂
  ...
  · obtain ⟨v₁, v₂, v₃, v₄, v₅, hv1D, hv2D, hv3D, hv4D, hv5D, hcard5,
    ...
    · -- **Degree-`≥ 5` cherry-hub with an induced `C₅` (TwoTwin).**  The `C₅` forces `e(M) = 5`
      -- (`s = 10`); routed through `two_twin_eM5_twenty` (sorry-free at `n = 20`, its deg-`≤ 5`
      -- `C₅` sub-case closed via the second-hub extraction).
      have hsum10 : ∑ v ∈ D, (G.neighborFinset v ∩ D).card = 10 := by
        ...
        have two_nbrs : ∀ a b c : Fin 20, G.Adj a b → G.Adj a c → b ∈ D → c ∈ D → b ≠ c →
            2 ≤ (G.neighborFinset a ∩ D).card := by
          intro a b c hab hac hbD hcD hbc
          have hsub : ({b, c} : Finset (Fin 20)) ⊆ G.neighborFinset a ∩ D := by
          ...
          have hbc2 : ({b, c} : Finset (Fin 20)).card = 2 := by
            rw [Finset.card_insert_of_notMem (by simp [hbc]), Finset.card_singleton]
          calc 2 = ({b, c} : Finset (Fin 20)).card := hbc2.symm
      ...
      exact Or.inr (Or.inr (Or.inl
        (two_twin_eM5_twenty G hm h3 hT h2k2 hC4 hHub7 (by rw [← hDdef]; exact hsum10))))
