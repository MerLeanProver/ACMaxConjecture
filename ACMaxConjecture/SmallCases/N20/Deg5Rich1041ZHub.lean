...
import ACMaxConjecture.SmallCases.N20.Deg5SatSix

/-! # The good z-hub existence for the rich (11,7,45) Z-leaf extraction (`n = 20`) -/
namespace ACMax
...
theorem sat_bound_rich_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
  ...
  classical
  set S : Finset (Fin 20) := Hub.filter (fun h => G.degree h = 4 ∧ h ≠ g ∧ ¬G.Adj g h ∧
    2 ≤ (G.neighborFinset h ∩ Iso).card) with hSdef
  set T : Finset (Fin 20) := G.neighborFinset g ∩ Iso with hTdef
  ...
  have hswap : ∑ h ∈ S, (G.neighborFinset h ∩ T).card
      = ∑ t ∈ T, (G.neighborFinset t ∩ S).card := cross_count_twenty G S T
...
/-- **Six exactly-`2` members of a capped family.**  If `rest` has `c` members, each of
iso-degree at most `2`, and the total iso-degree is at least `c + 6`, then at least six
members sit at iso-degree exactly `2`; extract six of them. -/
theorem six_exact_two_of_capped_twenty (G : SimpleGraph (Fin 20)) (Iso rest : Finset (Fin 20))
    (c : ℕ) (hcard : rest.card = c)
    (hcap : ∀ x ∈ rest, (G.neighborFinset x ∩ Iso).card ≤ 2)
    (hsum : 6 + c ≤ ∑ x ∈ rest, (G.neighborFinset x ∩ Iso).card) :
    ∃ S : Finset (Fin 20), S ⊆ rest ∧ S.card = 6 ∧
      ∀ s ∈ S, (G.neighborFinset s ∩ Iso).card = 2 := by
  classical
  have hAsub : rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2) ⊆ rest :=
    Finset.filter_subset _ _
  have hsplit := Finset.sum_sdiff (f := fun x => (G.neighborFinset x ∩ Iso).card) hAsub
  have hAcard : (rest \ rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2)).card
      = rest.card - (rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2)).card :=
    Finset.card_sdiff_of_subset hAsub
  have hAle : (rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2)).card ≤ rest.card :=
    Finset.card_le_card hAsub
  have h1 : ∑ x ∈ rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2),
      (G.neighborFinset x ∩ Iso).card
      ≤ (rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2)).card * 2 := by
    have hle := Finset.sum_le_card_nsmul
      (rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2))
      (fun x => (G.neighborFinset x ∩ Iso).card) 2 (fun x hx => hcap x (hAsub hx))
    simpa using hle
  have h2 : ∑ x ∈ rest \ rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2),
      (G.neighborFinset x ∩ Iso).card
      ≤ (rest \ rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2)).card * 1 := by
    have hle := Finset.sum_le_card_nsmul
      (rest \ rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2))
      (fun x => (G.neighborFinset x ∩ Iso).card) 1 (fun x hx => by
        obtain ⟨hxR, hxf⟩ := Finset.mem_sdiff.mp hx
        have hxle := hcap x hxR
        rcases Nat.lt_or_ge (G.neighborFinset x ∩ Iso).card 2 with hlt | hge
        · omega
        · exact absurd (Finset.mem_filter.mpr ⟨hxR, by omega⟩) hxf)
    simpa using hle
  have h6 : 6 ≤ (rest.filter (fun x => (G.neighborFinset x ∩ Iso).card = 2)).card := by omega
  obtain ⟨S, hSsub, hScard⟩ := Finset.exists_subset_card_eq h6
  refine ⟨S, hSsub.trans hAsub, hScard, fun s hs => ?_⟩
  exact (Finset.mem_filter.mp (hSsub hs)).2

set_option maxHeartbeats 1000000 in
/-- With the rich hub `g` (isoDeg ≥ 3), the `(11,7,45)` ledger + the iso-cap
produce a degree-`4` `Z`-slot hub `h₂` of isoDeg `≥ 1`, on a `z` avoided by `g`
and non-adjacent to `g`.  The three renumbered tie corners are closed by the
six-saturated anchored kill `six_saturated_anchor_kill_twenty`. -/
theorem good_zhub_1041_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
  ...
  -- Degrees are `4` or `5`; the profile splits `11` hubs into `10` of degree `4` and one of `5`.
  ...
  have hcardsum : D4.card + D5.card = 11 := by
  ...
  have hdegsum : ∑ w ∈ Hub, G.degree w = 4 * D4.card + 5 * D5.card := by
    ...
    have e5 : ∑ w ∈ D5, G.degree w = 5 * D5.card := by
      ...
      rw [h1, Finset.sum_const, smul_eq_mul, mul_comm]
    rw [e4, e5]
  have h45 : 4 * D4.card + 5 * D5.card = 45 := by rw [← hdegsum]; exact hdsum
  ...
  have keyB : ∀ za zb : Fin 20, za ∈ Z → zb ∈ Z → G.Adj g za → ¬G.Adj g zb → False := by
    ...
    have satKill : ∀ p0 : Fin 20, p0 ∈ Hub → G.degree p0 = 4 → g ≠ p0 → ¬G.Adj g p0 →
      ...
      have hd5le4 : (G.neighborFinset d5 ∩ Iso).card ≤ 4 := by
        have hsp := nbr_split_three_twenty G Hub Iso hdisj d5
        rw [← hZdef] at hsp
      ...
      have hsum_split : (G.neighborFinset g ∩ Iso).card + ((G.neighborFinset p0 ∩ Iso).card
        ...
        omega
      have hT7card : ((D4.erase g).erase p0).card = 8 := by
      ...
      -- The saturation bound allows at most `2·3 = 6` rest-hubs with isoDeg `≥ 2`, so the
      -- eight rest-hubs sum to at most `6·2 + 2·1 = 14`; with `hT7ge` the ledger is tight,
      -- `isoDeg d5 = 4`, and six rest-hubs sit at exactly `2`: the anchored kill fires.
      have hsatsub : ((D4.erase g).erase p0).filter
        ...
        obtain ⟨hhR, hh2⟩ := hh
        have hhHub : h ∈ Hub := ((hmemD4 h).mp (hT7sub hhR)).1
        have hh4 : G.degree h = 4 := ((hmemD4 h).mp (hT7sub hhR)).2
        exact Finset.mem_filter.mpr ⟨hhHub, hh4, hT7memg h hhR, hgnohub h hhHub, hh2⟩
      have hsat6 : (((D4.erase g).erase p0).filter
          (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card)).card ≤ 6 := by
        have hle := (Finset.card_le_card hsatsub).trans hsat
        rw [hgiso3] at hle
        omega
      have hsplitf := Finset.sum_sdiff
        (f := fun x => (G.neighborFinset x ∩ Iso).card)
        (Finset.filter_subset (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card)
          ((D4.erase g).erase p0))
      have hfle : ∑ x ∈ ((D4.erase g).erase p0).filter
          (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card), (G.neighborFinset x ∩ Iso).card
          ≤ (((D4.erase g).erase p0).filter
            (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card)).card * 2 := by
        have hle := Finset.sum_le_card_nsmul (((D4.erase g).erase p0).filter
            (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card))
          (fun x => (G.neighborFinset x ∩ Iso).card) 2
          (fun x hx => hT7cap x (Finset.filter_subset _ _ hx))
        simpa using hle
      have hnle : ∑ x ∈ (D4.erase g).erase p0 \ ((D4.erase g).erase p0).filter
          (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card), (G.neighborFinset x ∩ Iso).card
          ≤ ((D4.erase g).erase p0 \ ((D4.erase g).erase p0).filter
            (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card)).card * 1 := by
        have hle := Finset.sum_le_card_nsmul ((D4.erase g).erase p0
          ...
          (fun x => (G.neighborFinset x ∩ Iso).card) 1 (fun x hx => by
            ...
            by_contra hc
            push Not at hc
            exact hxf (Finset.mem_filter.mpr ⟨hxR, by omega⟩))
        simpa using hle
      have hcards : ((D4.erase g).erase p0 \ ((D4.erase g).erase p0).filter
          (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card)).card
          = ((D4.erase g).erase p0).card - (((D4.erase g).erase p0).filter
            (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card)).card :=
        Finset.card_sdiff_of_subset (Finset.filter_subset _ _)
      have hd5ge4 : 4 ≤ (G.neighborFinset d5 ∩ Iso).card := by omega
      obtain ⟨S, hSsub, hScard, hSiso⟩ := six_exact_two_of_capped_twenty G Iso
        ((D4.erase g).erase p0) 8 hT7card hT7cap (by omega)
      exact six_saturated_anchor_kill_twenty G Hub Iso hiso3 hdisj hIso hno2hub g d5 hg hgd
        hgiso3 hd5Hub hd5ge4 S
        (fun s hs => ((hmemD4 s).mp (hT7sub (hSsub hs))).1) hScard
        (fun s hs => ((hmemD4 s).mp (hT7sub (hSsub hs))).2)
        (fun s hs => ⟨hgnohub s ((hmemD4 s).mp (hT7sub (hSsub hs))).1, hT7memg s (hSsub hs)⟩)
        hSiso
    ...
    · rcases hpd q hqHub with hq4 | hq5
      · -- Both degree `4`: both forced to iso-degree `0`; the direct count gives `∑ D4 ≤ 15 < 16`.
        ...
        have hsum6 : (G.neighborFinset g ∩ Iso).card + ((G.neighborFinset p ∩ Iso).card
          ...
          omega
        have hR6card : (((D4.erase g).erase p).erase q).card = 7 := by
        ...
        -- Seven rest-hubs, each capped at `2`, summing to `18 − isoDeg d5 ≥ 13`: this forces
        -- `isoDeg d5 ≥ 4` and six rest-hubs at exactly `2`, so the anchored kill fires.
        have hp0 : (G.neighborFinset p ∩ Iso).card = 0 := hfp hp4
        have hq0 : (G.neighborFinset q ∩ Iso).card = 0 := hfq hq4
        have hR6cap : ∀ x ∈ ((D4.erase g).erase p).erase q,
            (G.neighborFinset x ∩ Iso).card ≤ 2 := by
          intro x hx
        ...
        have hR6le : ∑ h ∈ ((D4.erase g).erase p).erase q, (G.neighborFinset h ∩ Iso).card
            ≤ (((D4.erase g).erase p).erase q).card * 2 := by
          have hle := Finset.sum_le_card_nsmul (((D4.erase g).erase p).erase q)
            (fun x => (G.neighborFinset x ∩ Iso).card) 2 hR6cap
          simpa using hle
        have hR6ge : 13 ≤ ∑ h ∈ ((D4.erase g).erase p).erase q,
            (G.neighborFinset h ∩ Iso).card := by omega
        have hd5ge4 : 4 ≤ (G.neighborFinset d5 ∩ Iso).card := by omega
        obtain ⟨S, hSsub, hScard, hSiso⟩ := six_exact_two_of_capped_twenty G Iso
          (((D4.erase g).erase p).erase q) 7 hR6card hR6cap (by omega)
        have hSD4 : ∀ s ∈ S, s ∈ Hub ∧ G.degree s = 4 := fun s hs =>
          (hmemD4 s).mp (hR6sub (hSsub hs))
        have hSg : ∀ s ∈ S, ¬G.Adj g s ∧ s ≠ g := by
          intro s hs
          refine ⟨hgnohub s (hSD4 s hs).1, ?_⟩
          exact (Finset.mem_erase.mp (Finset.mem_of_mem_erase
            (Finset.mem_of_mem_erase (hSsub hs)))).1
        exact six_saturated_anchor_kill_twenty G Hub Iso hiso3 hdisj hIso hno2hub g d5 hg hgd
          hgiso3 hd5Hub hd5ge4 S (fun s hs => (hSD4 s hs).1) hScard
          (fun s hs => (hSD4 s hs).2) hSg hSiso
  ...
  · by_cases hgz₂ : G.Adj g z₂
    ...
    · -- `g` meets no `Z`-vertex (CASE A).  Both `z₁, z₂` are avoided; count over `D4`.
      have hzg0 : (G.neighborFinset g ∩ Z).card = 0 := by
        ...
        have hxpair : x ∈ ({z₁, z₂} : Finset (Fin 20)) := by rw [← hZeqpair, ← hZdef]; exact hx.2
      ...
      have hDFcard : (D4 \ F).card = 10 - F.card := by
      ...
      -- The rest-ledger: `isoDeg g + ∑_R = 21 − isoDeg d5` over `R = (D4 \ F).erase g`.
      have hRsum : (G.neighborFinset g ∩ Iso).card
          + ∑ x ∈ (D4 \ F).erase g, (G.neighborFinset x ∩ Iso).card
          = 21 - (G.neighborFinset d5 ∩ Iso).card := by omega
      rcases Nat.eq_zero_or_pos (G.neighborFinset g ∩ Hub).card with hhub0 | hhubpos
      · -- `hubDeg g = 0`: every rest-hub caps at `2` and both `hzi` branches are absurd.
        have hgnohub0 : ∀ h ∈ Hub, ¬G.Adj g h := by
          intro h hh hadj
          have hmem : h ∈ G.neighborFinset g ∩ Hub :=
            Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hadj, hh⟩
          rw [Finset.card_eq_zero.mp hhub0] at hmem
          exact absurd hmem (Finset.notMem_empty h)
        have hRcap : ∀ x ∈ (D4 \ F).erase g, (G.neighborFinset x ∩ Iso).card ≤ 2 := by
          intro x hx
          have hxD4 : x ∈ D4 := Finset.sdiff_subset (Finset.mem_of_mem_erase hx)
          exact hcap x ((hmemD4 x).mp hxD4).1 ((hmemD4 x).mp hxD4).2
            ((Finset.mem_erase.mp hx).1).symm (hgnohub0 x ((hmemD4 x).mp hxD4).1)
        have hRle : ∑ x ∈ (D4 \ F).erase g, (G.neighborFinset x ∩ Iso).card
            ≤ ((D4 \ F).erase g).card * 2 := by
          have hle := Finset.sum_le_card_nsmul ((D4 \ F).erase g)
            (fun x => (G.neighborFinset x ∩ Iso).card) 2 hRcap
          simpa using hle
        omega
      · -- `hubDeg g = 1`: `isoDeg g = 3`; name the unique `g`-neighbour `h'`.
        have hhub1 : (G.neighborFinset g ∩ Hub).card = 1 := by omega
        have hgiso3A : (G.neighborFinset g ∩ Iso).card = 3 := by omega
        obtain ⟨h', hh'eq⟩ := Finset.card_eq_one.mp hhub1
        have hh'mem : h' ∈ G.neighborFinset g ∩ Hub := by
          rw [hh'eq]; exact Finset.mem_singleton_self h'
        have hh'adj : G.Adj g h' := (G.mem_neighborFinset _ _).mp (Finset.mem_inter.mp hh'mem).1
        have hh'Hub : h' ∈ Hub := (Finset.mem_inter.mp hh'mem).2
        have hgonly : ∀ x ∈ Hub, x ≠ h' → ¬G.Adj g x := by
          intro x hxH hxne hadj
          have hx : x ∈ G.neighborFinset g ∩ Hub :=
            Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hadj, hxH⟩
          rw [hh'eq, Finset.mem_singleton] at hx
          exact hxne hx
        have hRcapNe : ∀ x ∈ (D4 \ F).erase g, x ≠ h' →
            (G.neighborFinset x ∩ Iso).card ≤ 2 := by
          intro x hx hxne
          have hxD4 : x ∈ D4 := Finset.sdiff_subset (Finset.mem_of_mem_erase hx)
          exact hcap x ((hmemD4 x).mp hxD4).1 ((hmemD4 x).mp hxD4).2
            ((Finset.mem_erase.mp hx).1).symm (hgonly x ((hmemD4 x).mp hxD4).1 hxne)
        by_cases hh'd5 : h' = d5
        · -- The `g`-neighbour is the degree-5 hub: `ZH \ F ⊆ {d5}`, all rest-hubs cap at `2`.
          have hRcap : ∀ x ∈ (D4 \ F).erase g, (G.neighborFinset x ∩ Iso).card ≤ 2 := by
            intro x hx
            refine hRcapNe x hx (fun he => ?_)
            have hxD4 : x ∈ D4 := Finset.sdiff_subset (Finset.mem_of_mem_erase hx)
            have hx4 : G.degree x = 4 := ((hmemD4 x).mp hxD4).2
            rw [he, hh'd5] at hx4
            omega
          have hsub1 : ZH \ F ⊆ {d5} := by
            intro x hx
            rcases Finset.mem_union.mp (hZHFsub hx) with h1 | h1
            · exact (Finset.mem_inter.mp h1).2
            · rw [hh'eq, Finset.mem_singleton] at h1
              exact Finset.mem_singleton.mpr (h1.trans hh'd5)
          have hZF1 : (ZH \ F).card ≤ 1 :=
            (Finset.card_le_card hsub1).trans_eq (Finset.card_singleton d5)
          have hRle : ∑ x ∈ (D4 \ F).erase g, (G.neighborFinset x ∩ Iso).card
              ≤ ((D4 \ F).erase g).card * 2 := by
            have hle := Finset.sum_le_card_nsmul ((D4 \ F).erase g)
              (fun x => (G.neighborFinset x ∩ Iso).card) 2 hRcap
            simpa using hle
          omega
        · -- `h' ≠ d5`, so `h'` is a degree-4 hub.
          have hh'4 : G.degree h' = 4 := by
            rcases hpd h' hh'Hub with h4 | h5
            · exact h4
            · exact absurd (by
                have hmem5 : h' ∈ D5 := (hmemD5 h').mpr ⟨hh'Hub, h5⟩
                rw [hd5eq, Finset.mem_singleton] at hmem5
                exact hmem5) hh'd5
          by_cases hh'ZH : h' ∈ ZH
          · -- `h'` is a `g`-adjacent `Z`-slot hub: the refined split caps `isoDeg h' ≤ 2`.
            have hh'z : 1 ≤ (G.neighborFinset h' ∩ Z).card := by
              rw [hZHdef, Finset.mem_union] at hh'ZH
              rcases hh'ZH with h1 | h1
              · rw [Finset.mem_inter, G.mem_neighborFinset] at h1
              ...
              · rw [Finset.mem_inter, G.mem_neighborFinset] at h1
                exact Finset.card_pos.mpr ⟨z₂, Finset.mem_inter.mpr
                  ⟨(G.mem_neighborFinset _ _).mpr h1.1.symm, hz₂Z⟩⟩
            have hh'hub1 : 1 ≤ (G.neighborFinset h' ∩ Hub).card :=
              Finset.card_pos.mpr ⟨g, Finset.mem_inter.mpr
                ⟨(G.mem_neighborFinset _ _).mpr hh'adj.symm, hg⟩⟩
            have hh'cap : (G.neighborFinset h' ∩ Iso).card ≤ 2 := by
              have hsp := nbr_split_three_twenty G Hub Iso hdisj h'
              rw [← hZdef, hh'4] at hsp
              omega
            by_cases hd5ZH : d5 ∈ ZH
            · -- The tight corner: `isoDeg d5 = 4`, `|F| = 2`, six rest-hubs at exactly `2`.
              have hd5z : 1 ≤ (G.neighborFinset d5 ∩ Z).card := by
                rw [hZHdef, Finset.mem_union] at hd5ZH
                rcases hd5ZH with h1 | h1
                · rw [Finset.mem_inter, G.mem_neighborFinset] at h1
                  exact Finset.card_pos.mpr ⟨z₁, Finset.mem_inter.mpr
                ...
                · rw [Finset.mem_inter, G.mem_neighborFinset] at h1
                  exact Finset.card_pos.mpr ⟨z₂, Finset.mem_inter.mpr
                    ⟨(G.mem_neighborFinset _ _).mpr h1.1.symm, hz₂Z⟩⟩
              have hd5cap : (G.neighborFinset d5 ∩ Iso).card ≤ 4 := by
                have hsp := nbr_split_three_twenty G Hub Iso hdisj d5
                rw [← hZdef, hd5deg] at hsp
                omega
              have hh'D4 : h' ∈ D4 := (hmemD4 h').mpr ⟨hh'Hub, hh'4⟩
              have hh'notF : h' ∉ F := by
                intro hf
                rw [hFdef, Finset.mem_filter] at hf
                exact hf.2.2 hh'adj
              have hh'R : h' ∈ (D4 \ F).erase g :=
                Finset.mem_erase.mpr ⟨hh'adj.ne', Finset.mem_sdiff.mpr ⟨hh'D4, hh'notF⟩⟩
              have hRsplit := Finset.add_sum_erase ((D4 \ F).erase g)
                (fun x => (G.neighborFinset x ∩ Iso).card) hh'R
              have hScap : ∀ x ∈ ((D4 \ F).erase g).erase h',
                  (G.neighborFinset x ∩ Iso).card ≤ 2 := fun x hx =>
                hRcapNe x (Finset.mem_of_mem_erase hx) (Finset.mem_erase.mp hx).1
              have hSle : ∑ x ∈ ((D4 \ F).erase g).erase h', (G.neighborFinset x ∩ Iso).card
                  ≤ (((D4 \ F).erase g).erase h').card * 2 := by
                have hle := Finset.sum_le_card_nsmul (((D4 \ F).erase g).erase h')
                  (fun x => (G.neighborFinset x ∩ Iso).card) 2 hScap
                simpa using hle
              have hScard_eq : (((D4 \ F).erase g).erase h').card
                  = ((D4 \ F).erase g).card - 1 := Finset.card_erase_of_mem hh'R
              have hScard6 : (((D4 \ F).erase g).erase h').card = 6 := by omega
              have hd5ge4 : 4 ≤ (G.neighborFinset d5 ∩ Iso).card := by omega
              obtain ⟨S, hSsub, hScard, hSiso⟩ := six_exact_two_of_capped_twenty G Iso
                (((D4 \ F).erase g).erase h') 6 hScard6 hScap (by omega)
              have hSD4 : ∀ s ∈ S, s ∈ Hub ∧ G.degree s = 4 := fun s hs =>
                (hmemD4 s).mp (Finset.sdiff_subset (Finset.mem_of_mem_erase
                  (Finset.mem_of_mem_erase (hSsub hs))))
              have hSg : ∀ s ∈ S, ¬G.Adj g s ∧ s ≠ g := by
                intro s hs
                have hsR : s ∈ ((D4 \ F).erase g).erase h' := hSsub hs
                refine ⟨hgonly s (hSD4 s hs).1 (Finset.mem_erase.mp hsR).1, ?_⟩
                exact (Finset.mem_erase.mp (Finset.mem_of_mem_erase hsR)).1
              exact six_saturated_anchor_kill_twenty G Hub Iso hiso3 hdisj hIso hno2hub g d5
                hg hgd hgiso3A hd5Hub hd5ge4 S (fun s hs => (hSD4 s hs).1) hScard
                (fun s hs => (hSD4 s hs).2) hSg hSiso
            · -- `d5 ∉ ZH`: `|F| ≥ 3` and all rest-hubs (incl. `h'`) cap at `2`: count-kill.
              have hzd0 : (ZH ∩ {d5}).card = 0 := by
                rw [Finset.card_eq_zero, Finset.eq_empty_iff_forall_notMem]
                intro x hx
                rw [Finset.mem_inter, Finset.mem_singleton] at hx
                exact hd5ZH (hx.2 ▸ hx.1)
              ...
              have hRle : ∑ x ∈ (D4 \ F).erase g, (G.neighborFinset x ∩ Iso).card
                ...
                simpa using hle
              omega
          · -- `h' ∉ ZH`: `ZH \ F ⊆ ZH ∩ {d5}` forces `|F| ≥ 3`; `h'` caps at `3`, the rest
            -- at `2`, and both `hzi` branches are numerically absurd.
            have hsub1 : ZH \ F ⊆ ZH ∩ {d5} := by
              intro x hx
              rcases Finset.mem_union.mp (hZHFsub hx) with h1 | h1
              · exact h1
              · exfalso
                rw [hh'eq, Finset.mem_singleton] at h1
                have hxZH : x ∈ ZH := (Finset.mem_sdiff.mp hx).1
                rw [h1] at hxZH
                exact hh'ZH hxZH
            have hZF1 : (ZH \ F).card ≤ (ZH ∩ {d5}).card := Finset.card_le_card hsub1
            have hh'cap3 : (G.neighborFinset h' ∩ Iso).card ≤ 3 := by
              have hsp := nbr_split_three_twenty G Hub Iso hdisj h'
              have hh'hub1 : 1 ≤ (G.neighborFinset h' ∩ Hub).card :=
                Finset.card_pos.mpr ⟨g, Finset.mem_inter.mpr
                  ⟨(G.mem_neighborFinset _ _).mpr hh'adj.symm, hg⟩⟩
              rw [hh'4] at hsp
              omega
            have hRle3 : ∑ x ∈ (D4 \ F).erase g, (G.neighborFinset x ∩ Iso).card
                ≤ (((D4 \ F).erase g).card - 1) * 2 + 3 := by
              by_cases hh'R : h' ∈ (D4 \ F).erase g
              · have hRsplit := Finset.add_sum_erase ((D4 \ F).erase g)
                  (fun x => (G.neighborFinset x ∩ Iso).card) hh'R
                have hScard_eq : (((D4 \ F).erase g).erase h').card
                    = ((D4 \ F).erase g).card - 1 := Finset.card_erase_of_mem hh'R
                have hSle : ∑ x ∈ ((D4 \ F).erase g).erase h',
                    (G.neighborFinset x ∩ Iso).card
                    ≤ (((D4 \ F).erase g).erase h').card * 2 := by
                  have hle := Finset.sum_le_card_nsmul (((D4 \ F).erase g).erase h')
                    (fun x => (G.neighborFinset x ∩ Iso).card) 2 (fun x hx =>
                      hRcapNe x (Finset.mem_of_mem_erase hx) (Finset.mem_erase.mp hx).1)
                  simpa using hle
                omega
              · have hRle : ∑ x ∈ (D4 \ F).erase g, (G.neighborFinset x ∩ Iso).card
                  ...
                  have hle := Finset.sum_le_card_nsmul ((D4 \ F).erase g)
                    (fun x => (G.neighborFinset x ∩ Iso).card) 2 (fun x hx =>
                      hRcapNe x hx (fun he => hh'R (he ▸ hx)))
                  simpa using hle
                omega
            omega
