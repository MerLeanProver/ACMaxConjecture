...
import ACMaxConjecture.SmallCases.N20.Deg5Rich1041OctaPoorCert

/-!
# The share-2 octahedron kill for the (11,7,45) corner (`n = 20`)
...
twins.  In the `|R| ≥ 5` world (`hR5ge`, from the left disjunct of the
`rich_count_1041_twenty` dichotomy) the `n = 20` ledger is disjunctive
(`Σ_D4 = 16` over ten degree-`4` hubs: profile `{3,2,2,2,2}` with five poor at
isoDeg `1`, or `{3,3,2,2,2}`/`{4,2,2,2,2}` with four poor at `1` and one at
`0`), `hblock` pins the rigid `2+1+2` blocker partition of the rich hubs
around `{t₀, z, h₂}`, and the `M`-set covering-design (`f`'s two missed twins)
plus the poor/`Z` layer force a good triangle (`Σ ≤ 11`, `hT`) ∨ `K₂,₃`
(`Σ ≤ 19`, `hK23`) ∨ `C₄` (`Σ ≤ 14`, `hC4`).  The deg-`5` analog of the proven
`(11,6,44)` octahedron poor-cert (`TwinCert20OctahedronPoorForce2` /
`TwinCert20R5Octahedron`) and the `n = 18` `octahedron_poor_cert_eighteen`.
-/
...
/-- **The share-2 octahedron kill.**  The rigid `(11,7,45)` tie with `f` of
isoDeg `5` and `x` sharing `≤ 2` twins with `f` is contradictory. -/
theorem octahedron_share2_kill_1041_twenty (G : SimpleGraph (Fin 20))
    ...
    False :=
  octahedron_poor_cert_share2_1041_twenty G Hub Iso hiso3 hdisj hsum18 hdeg3 hisodeg3
    ...
    (octahedron_struct_share2_1041_twenty G Hub Iso hiso3 hdisj hsum18 hdeg3 hisodeg3
      hleak hdeg hdeg5 hHub hIso hdsum hT hC4 hK23 hshare hno2hub g hg hgd hgiso h₂ z hh₂
      hd₂ hzZ hz2 hgz hg2 hpoor hshared hblock hR5ge)
