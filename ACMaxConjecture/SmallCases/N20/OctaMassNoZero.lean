...
import ACMaxConjecture.SmallCases.N20.ZPoorDispatch
import ACMaxConjecture.SmallCases.N20.ZPoorCut

/-!
# The `r ≠ 7` rich-count crux and the no-iso-zero-hub bundle (`n = 20`, `|Hub| = 12`)

This file closes the structural residual `hnozero ∧ |R| = 6` for the clean `(12, 6, 48)` octahedron,
the last gap in `TwinCert20OctaMass`.

* `rich_count_ne_seven_twenty` (`r ≠ 7`).  **Mechanical port** of `rich_count_ne_seven_eighteen`:
  ...
  rich-side `Z`-mass are all unchanged at `|Hub| = 12`.  (The `n = 18` lower bound `S ≥ 15` is *not*
  ...
  `two_iso_four_hubs_impossible_twenty`.)  The only `|Hub| = 12` adaptation is that
  `zdeg_split_twenty` returns a disjunction with `ZPoorCutConfig G`, discharged by `hcut`.

* `rich_six_no_iso_zero_twenty` — the `hnozero ∧ |R| = 6` bundle.  `|R| ∈ {6, 7}`
  (`rich_count_ge_six_twenty` via the `r = 5` octahedron residual `r5_resid_twenty` and the
  all-poor-at-one boundary residual `all_poor_one_resid_twenty`, `rich_count_le_seven_twenty`);
  `r ≠ 7` pins `|R| = 6`.
-/
...
theorem two_iso_four_hubs_impossible_twenty (G : SimpleGraph (Fin 20)) (Iso : Finset (Fin 20))
    (hIso : Iso.card = 6) (a b : Fin 20)
...
/-- **`r ≠ 7` — the rich count is not seven (`n = 20`, `|Hub| = 12`).**  Port of
`rich_count_ne_seven_eighteen`; the `Z`-side rich bound `z_R ≥ 2` comes from `zdeg_split_twenty`,
whose `ZPoorCutConfig` disjunct is discharged by `hcut`. -/
theorem rich_count_ne_seven_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 12) (hIso : Iso.card = 6)
    (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
    (hdeg3 : ∀ v : Fin 20, 3 ≤ G.degree v)
  ...
  have hRnbr : ∀ a ∈ R, (G.neighborFinset a ∩ R).card ≤ 2 := by
    ...
    omega
  -- `z_R ≥ 2`.
  obtain ⟨hzRge, _⟩ := (zdeg_split_twenty G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3 hisodeg3
  ...
  have hcross : ∑ t ∈ Iso, (G.neighborFinset t ∩ R).card = S := by
    rw [hSdef]; exact cross_count_twenty G Iso R
  ...
  have hsplit3 : ∑ r ∈ R, (G.neighborFinset r ∩ Hub).card + S
    ...
    have hcong : ∑ r ∈ R, ((G.neighborFinset r ∩ Hub).card + (G.neighborFinset r ∩ Iso).card
      ...
      Finset.sum_congr rfl (fun r _ => nbr_split_three_twenty G Hub Iso hdisj r)
  ...
  exact two_iso_four_hubs_impossible_twenty G Iso hIso w1 w2 hw14 hw24 hsh

/-- **The `hnozero ∧ |R| = 6` structural bundle (`n = 20`, `|Hub| = 12`, `(12, 6, 48)`).**  In the
clean no-two-hub octahedron with `¬ ZPoorCutConfig G`: every hub has iso-degree `≥ 1`, and the rich
count is exactly `6`.

* `|R| = 6`: `|R| ∈ {6, 7}` (`rich_count_ge_six_twenty` via the `r = 5` residual
  `r5_resid_twenty` and the all-poor-at-one boundary `all_poor_one_resid_twenty`,
  `rich_count_le_seven_twenty`); `rich_count_ne_seven_twenty` kills `7`.
* `hnozero`: a hub `h₀` of iso-degree `0` has `|N(h₀) ∩ Hub| + |N(h₀) ∩ Z| = 4`.  If it meets both
  ...
  `|Hub| = 12` sixth-poor-hub residual. -/
theorem rich_six_no_iso_zero_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 12) (hIso : Iso.card = 6)
    (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
  ...
  have hge6 : 6 ≤ R.card := (rich_count_ge_six_twenty G Hub Iso hdeg4 hiso3 hdisj hHub hIso
    hshare hno2hub hoct5 hpoor1).resolve_right hcut
  have hne7 : R.card ≠ 7 := rich_count_ne_seven_twenty G Hub Iso hdeg4 hiso3 hdisj hHub hIso
  ...
  have hzfacts := z_two_hub_nbrs_twenty G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak
  ...
  · -- `h₀` meets both endpoints: good triangle `{h₀, z₁, z₂}`, degree sum `10 ≤ 11`.
    ...
    have hz1split := nbr_split_three_twenty G Hub Iso hdisj z1
  ...
  · -- `|N(h₀) ∩ Z| ≤ 1`, so `|N(h₀) ∩ Hub| ≥ 3`: the near-internal fifth-poor-hub residual.
    -- **Strategy.**  An iso-degree-`0` poor hub forces `S = ∑_R isoDeg ≥ 13` (the six poor hubs
    -- absorb `≤ 5` of the `18` incidences), so some rich hub `h₁` has iso-degree `≥ 3` — whence its
    -- whole neighbourhood is `≥ 3` twins plus `≤ 1` other vertex.  Each `M`-edge endpoint meets a
    -- rich hub (`each_z_meets_rich_twenty`, discharged by `hcut`); a degree count shows `h₁` cannot
    ...
    -- private twin and the `Z`-leaf `z` assembles a `TwoHubConfig` (`two_hub_zleaf_twenty`),
    -- contradicting `hcut`.
    have hHubge3 : 3 ≤ (G.neighborFinset h₀ ∩ Hub).card := by omega
    set P : Finset (Fin 20) := Hub.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)
      with hPdef
    ...
    -- The six poor hubs carry `≤ 5` incidences (`h₀` carries `0`, the others `≤ 1`).
    have hPsum_le : ∑ g ∈ P, (G.neighborFinset g ∩ Iso).card ≤ 5 := by
      ...
      have herase : ∑ g ∈ P.erase h₀, (G.neighborFinset g ∩ Iso).card ≤ 5 := by
        calc ∑ g ∈ P.erase h₀, (G.neighborFinset g ∩ Iso).card
          ...
          _ = 5 := by rw [Finset.card_erase_of_mem hh₀P, hPcard]
      rw [hpe, hiso0]; omega
    -- Hence `S ≥ 13`, so some rich hub has iso-degree `≥ 3`.
    ...
    have h₁split := nbr_split_three_twenty G Hub Iso hdisj h₁
    ...
    have hz1Z : z1 ∈ Z := by rw [hZeq2]; exact Finset.mem_insert_self _ _
    have hz2Z : z2 ∈ Z := by rw [hZeq2]; exact Finset.mem_insert_of_mem (Finset.mem_singleton_self _)
    have hmeetsrich := each_z_meets_rich_twenty G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3
    ...
    have mkcut : ∀ z h₂ : Fin 20, z ∈ Z → h₂ ∈ R → G.Adj z h₂ →
      ...
      obtain ⟨a, b, haIso, hbIso, hab, ha1, hb1, hna2, hnb2⟩ :=
        exists_two_private_twins_twenty G Hub Iso hshare h₁ h₂ h₁Hub h₂Hub
          (hdeg4 h₁ h₁Hub) (hdeg4 h₂ h₂Hub) hne12 hn12 h₁iso3
      obtain ⟨c, hcIso, hc2, hn1c⟩ :=
        exists_one_private_twin_twenty G Hub Iso hshare h₂ h₁ h₂Hub h₁Hub
          (hdeg4 h₂ h₂Hub) (hdeg4 h₁ h₁Hub) hne12 hn12 h₂iso2
      have htwo := two_hub_zleaf_twenty G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3
    ...
    · -- `h₁ ~ z1` and `h₁ ~ z2`: two distinct `Z`-neighbours.
      have h2z : 2 ≤ (G.neighborFinset h₁ ∩ Z).card := by
        have hsub : ({z1, z2} : Finset (Fin 20)) ⊆ G.neighborFinset h₁ ∩ Z := by
    ...
    · -- `h₁ ~ p1` and `h₁ ~ p2`: two hub-neighbours.
      ...
      · -- `p1 = p2 = ν`: a rich hub adjacent to `h₁` and meeting both `z1, z2` — degree `≥ 5`.
        subst hpp
        have hνsplit := nbr_split_three_twenty G Hub Iso hdisj p1
        ...
        have hνz : 2 ≤ (G.neighborFinset p1 ∩ Z).card := by
          have hsub : ({z1, z2} : Finset (Fin 20)) ⊆ G.neighborFinset p1 ∩ Z := by
            intro x hx; rw [Finset.mem_insert, Finset.mem_singleton] at hx
            rcases hx with rfl | rfl
      ...
      · -- `p1 ≠ p2`: two distinct hub-neighbours.
        have h2h : 2 ≤ (G.neighborFinset h₁ ∩ Hub).card := by
          have hsub : ({p1, p2} : Finset (Fin 20)) ⊆ G.neighborFinset h₁ ∩ Hub := by
            intro x hx; rw [Finset.mem_insert, Finset.mem_singleton] at hx
            rcases hx with rfl | rfl
