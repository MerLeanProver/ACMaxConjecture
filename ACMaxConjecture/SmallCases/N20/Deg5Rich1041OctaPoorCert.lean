...
import ACMaxConjecture.SmallCases.N20.Deg5Rich1041SecondMend
import ACMaxConjecture.SmallCases.N20.Deg5Rich1041OctaPoorCounts
import ACMaxConjecture.SmallCases.N20.Deg5Rich1041ZPoorOneRich
import ACMaxConjecture.SmallCases.N20.Deg5Rich1041ZPoorBothPoor

/-! # The poor/Z certificate for the share-2 (11,7,45) octahedron (`n = 20`) -/
namespace ACMax
...
(`hK23`), or `C₄` (`hC4`).  `n = 20` port of the proven `n = 19` `(10,7,41)`
certificate: `SecondMend` dispatches into the one-rich/both-poor regimes. -/
theorem octahedron_poor_cert_share2_1041_twenty (G : SimpleGraph (Fin 20))
  ...
  obtain ⟨z', p, q, hz'Z, hz'ne, hzz', hz'deg3, hz'iso0, hNz'eq, hp, hq, hdp, hdq, hpq,
    ...
    octahedron_second_mend_share2_1041_twenty G Hub Iso hiso3 hdisj hsum18 hdeg3 hisodeg3
  ...
  · -- **CASE A**: `p` or `q` is `a`/`b`.  Good `C₄`  `m–h₂–z–z'`.
    obtain ⟨m, hmHub, hmh₂adj, hz'm, hmd, hmrz, hmh₂ne⟩ :
        ∃ m : Fin 20, m ∈ Hub ∧ G.Adj m h₂ ∧ G.Adj z' m ∧ G.degree m = 4 ∧
          m ≠ r_z ∧ m ≠ h₂ := by
  ...
  · by_cases htwin : 1 ≤ (G.neighborFinset p ∩ G.neighborFinset q ∩ Iso).card
    ...
    · -- **CASE C** (escalated): `p, q ∉ {a, b}` and `p, q` share no `Iso` twin —
      ...
      have hR5ge : 5 ≤ (Hub.filter
        ...
        obtain ⟨_, _, _, _, _, hRfilter, hRcard, _⟩ := hstruct
        rw [hRfilter]; omega
      ...
      · -- `p` poor.
        ...
        · -- `q` rich → one-rich with `p, q` swapped (rich hub must be first).
          have hNz'eqS : G.neighborFinset z' ∩ Hub = ({q, p} : Finset (Fin 20)) := by
          ...
          exact octahedron_one_poor_force_share2_1041_twenty G Hub Iso hiso3 hdisj hsum18
            hdeg3 hisodeg3 hleak hdeg hdeg5 hHub hIso hdsum hT hC4 hK23 hshare hno2hub g hg hgd
            hgiso h₂ z hh₂ hd₂ hzZ hz2 hgz hg2 hpoor hshared hblock hR5ge f x hfHub hfd hfiso5
            hxHub hxd hxiso3 hxf hnadj hsh hstruct z' q p hz'Z hzz' hz'deg3 hz'iso0 hNz'eqS hq
            hp hdq hdp hpq.symm hz'q hz'p (fun h => hnpq h.symm) hshare0S hqr hpp hqh₂ hqr_z
            hph₂ hpr_z
        · -- both poor.
          have hqpoor : (G.neighborFinset q ∩ Iso).card ≤ 1 := by omega
          exact octahedron_both_poor_force_share2_1041_twenty G Hub Iso hiso3 hdisj hsum18
            hdeg3 hisodeg3 hleak hdeg hdeg5 hHub hIso hdsum hT hC4 hK23 hshare hno2hub g hg hgd
      ...
      · -- `q` poor.
        ...
        · -- `p` rich → one-rich directly.
          exact octahedron_one_poor_force_share2_1041_twenty G Hub Iso hiso3 hdisj hsum18
            hdeg3 hisodeg3 hleak hdeg hdeg5 hHub hIso hdsum hT hC4 hK23 hshare hno2hub g hg hgd
            hgiso h₂ z hh₂ hd₂ hzZ hz2 hgz hg2 hpoor hshared hblock hR5ge f x hfHub hfd hfiso5
            hxHub hxd hxiso3 hxf hnadj hsh hstruct z' p q hz'Z hzz' hz'deg3 hz'iso0 hNz'eq hp
            hq hdp hdq hpq hz'p hz'q hnpq hshare0 hpr hqp hph₂ hpr_z hqh₂ hqr_z
        · -- both poor.
