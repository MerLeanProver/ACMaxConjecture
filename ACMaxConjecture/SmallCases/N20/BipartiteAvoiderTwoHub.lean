...
theorem cherry_double_count (G : SimpleGraph (Fin 20)) (S Iso : Finset (Fin 20)) :
  ...
  have hcard : ∀ h h' : Fin 20,
    ...
    · rw [if_neg, if_neg hh]
      ...
      exact hh
  have hSdeg : ∀ t : Fin 20, (G.neighborFinset t ∩ S).card
...
theorem two_hub_cherry_pair_twenty (G : SimpleGraph (Fin 20)) (h₁ h₂ a b c d : Fin 20)
...
theorem isolated_deg4_share_le_one (G : SimpleGraph (Fin 20)) (h₁ h₂ : Fin 20)
  ...
  have hcard4 : ({h₁, x, h₂, y} : Finset (Fin 20)).card = 4 := by
...
/-- **The `Hub.card = 11` variant of `iso1_intdeg_dichotomy_twenty`.**  The same near-`K₅` ISO1
kernel counting dichotomy, but for `|Hub| = 11` (ten degree-`4` hubs plus one degree-`5` hub `h₅`)
with the same total `Iso`-incidence `15`.  The bare `|Hub| = 11` statement is FALSE — the per-hub
bound ties `∑ B = 15 = ∑ isoinc` — so the minimal fix is the extra hypothesis `hdeg5int`: the
degree-`5` hub `h₅` is *internally non-isolated* (`1 ≤ intdeg h₅`).  With it, an `h₅` carrying
`≥ 4` `Iso`-neighbours would have `cinc h₅ = 0` (from `cinc + iso + int = 5` with `int ≥ 1`),
firing the first disjunct through `h₅`; hence in the both-fail world `B h₅ = 3` and
`∑ B = 2 + 3 + 9 = 14 < 15`, closing the `14 < 15` deficit as before. -/
theorem iso1_intdeg_dichotomy_eleven (G : SimpleGraph (Fin 20))
    (Hub Iso Cherry : Finset (Fin 20)) (h₁ h₅ : Fin 20)
    (hHubcard : Hub.card = 11)
    ...
    (hdeg5int : 1 ≤ (G.neighborFinset h₅ ∩ Hub).card) :
  ...
  -- Per-hub `Iso`-incidence bound `isoinc h ≤ (if h = h₁ then 2 else if h = h₅ then 3 else 1)`.
  set B : Fin 20 → ℕ := fun h => if h = h₁ then 2 else if h = h₅ then 3 else 1 with hB
  have hbound : ∀ h ∈ Hub, (G.neighborFinset h ∩ Iso).card ≤ B h := by
    ...
    · by_cases hh5 : h = h₅
      · simp only [hB, if_neg hh1, if_pos hh5]
        ...
        rw [hdh] at hdec
        have hint : 1 ≤ (G.neighborFinset h ∩ Hub).card := by rw [hh5]; exact hdeg5int
        by_contra hgt
        push Not at hgt
        -- isoinc h ≥ 4 and intdeg h ≥ 1 ⟹ cinc = 0, contradicting `hno1`.
  ...
  have hsumB : ∑ h ∈ Hub, B h = 14 := by
    have hpt : ∀ h, B h = 1 + ((if h = h₁ then 1 else 0) + (if h = h₅ then 2 else 0)) := by
      intro h
      by_cases hh1 : h = h₁
    ...
    simp only [hpt, Finset.sum_add_distrib, Finset.sum_const, hHubcard, smul_eq_mul,
      ...
      Finset.sum_ite_eq' Hub h₅ (fun _ => (2 : ℕ)), h1mem, h5mem, if_pos]
    omega
...
theorem twotwin_of_centre_twenty (G : SimpleGraph (Fin 20))
    (Iso : Finset (Fin 20)) (hc x y z : Fin 20)
