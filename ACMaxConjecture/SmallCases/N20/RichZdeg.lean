...
/-!
# The rigid `Z`-distribution for the `n = 20` no-two-hub partition
...
`|N ∩ Hub| + |N ∩ Iso| + |N ∩ Z| = 4`; summing over the twelve hubs and using `N3`
(`∑|N ∩ Hub| = 26`) and `∑|N ∩ Iso| = 18` gives `∑_{Hub}|N ∩ Z| = 4` — the **same** `Z`-hub mass
as `n = 19` (`48 − 26 − 18 = 4`), since `|Iso| = 6` is unchanged.

The headline `zdeg_split_twenty` pins how those four `Z`-incidences distribute across the
...
This is the **mechanical port** of the `n = 19` `TwinCert19RichZdeg` to `|Hub| = 12`.  The only
profile changes are `Fin 19 → 20`, `Hub.card 11 → 12`, `∑deg 44 → 48`, and the hub-hub mass
`22 → 26`; the thresholds (good-tri `≤ 11`, good-`C₄` `≤ 14`, good-`K₂₃` `≤ 19`) and the `Z`-profile
(`(2, 0, deg 3)`, four hub-incidences) are identical.

## Dependence on the deep `z`-meets-2-poor regime

The `n = 20` proof of `z_two_poor_hubs_false_twenty` delegates its hard residual (two poor hubs that
are adjacent or share no `M`-isolated twin) to `z_meets_two_poor_forces_two_hub_twenty`, the global
two-hub extraction in `TwinCert20ZPoorDispatch` — built on the whole `rich_count` / `rich_six` /
`rich_seven` ZPoor cluster (the `|Hub| = 12` analog has 6 poor hubs vs 5).  Here `hztwopoor` is
discharged directly by that ported extraction, so every lemma in this file is fully axiom-clean.  The
local good-`C₄` sub-case (non-adjacent poor hubs sharing a twin) is proved here directly.

The single foundation lemma `no_z_two_rich_hubs_twenty` does **not** touch the deep regime and is
...
exactly the conclusion of `z_meets_two_poor_forces_two_hub_twenty`, deferred to the not-yet-ported
`z`-meets-2-poor / ZPoor cluster. -/
abbrev ZMeetsTwoPoorResidual (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20)) : Prop :=
...
`hztwopoor` (`= z_meets_two_poor_forces_two_hub_twenty`, the not-yet-ported global extraction). -/
theorem z_two_poor_hubs_false_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
  ...
  have hztwopoor : ZMeetsTwoPoorResidual G Hub Iso :=
    z_meets_two_poor_forces_two_hub_twenty G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3 hisodeg3
      hdsum hleak hshare hno2hub hC4 hK23 hT
  obtain ⟨hziso0, _, hzdeg3⟩ := z_two_hub_nbrs_twenty G Hub Iso hiso3 hdisj hHub hIso hdsum
...
hub. -/
theorem each_z_meets_rich_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 12) (hIso : Iso.card = 6)
    (hdeg3 : ∀ v : Fin 20, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
  ...
  intro z hz
  obtain ⟨_, hhub2, _⟩ := z_two_hub_nbrs_twenty G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3
    hisodeg3 hleak z hz
  ...
  have hpoor : ∀ g ∈ G.neighborFinset z ∩ Hub, (G.neighborFinset g ∩ Iso).card ≤ 1 := by
    ...
    exact hR0 g hgR
  exact z_two_poor_hubs_false_twenty G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3 hisodeg3
...
theorem zdeg_split_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 12) (hIso : Iso.card = 6)
    (hdeg3 : ∀ v : Fin 20, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
  ...
  have hsum4 : ∑ h ∈ Hub, (G.neighborFinset h ∩ Z).card = 4 :=
    zdeg_sum_four_twenty G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak
  ...
  have hrichge : 2 ≤ ∑ r ∈ R, (G.neighborFinset r ∩ Z).card := by
    have hcross : ∑ r ∈ R, (G.neighborFinset r ∩ Z).card
        = ∑ z ∈ Z, (G.neighborFinset z ∩ R).card := cross_count_twenty G R Z
    rw [hcross]
    have hez : ∀ z ∈ Z, 1 ≤ (G.neighborFinset z ∩ R).card := by
      intro z hz
      rcases each_z_meets_rich_twenty G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3 hisodeg3 hdsum
...
`3 + 4 + 4 = 11 ≤ 11`, a good triangle excluded by `hT` (threshold `11`, the `n = 20` value).  If
...
theorem no_z_two_rich_hubs_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 12) (hIso : Iso.card = 6)
    (hdeg3 : ∀ v : Fin 20, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
  ...
  set R : Finset (Fin 20) := Hub.filter (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card) with hRdef
  intro z hz
  obtain ⟨hziso0, hzhub2, hzdeg3⟩ := z_two_hub_nbrs_twenty G Hub Iso hiso3 hdisj hHub hIso hdsum
    hdeg3 hisodeg3 hleak z hz
  by_contra hgt
  push Not at hgt
  ...
  · -- Non-adjacent rich hubs share exactly one twin `t`; `t–g₁–z–g₂` is a good `C₄`.
    have hsh : (G.neighborFinset g₁ ∩ G.neighborFinset g₂ ∩ Iso).card = 1 :=
      rich_nonadj_share_eq_one_twenty G Hub Iso hshare hno2hub g₁ g₂ hg1Hub hg2Hub
...
`R = {h : 2 ≤ |N(h) ∩ Iso|}` and `P` is its complement in `Hub`.  For `|Hub| = 12` (`6` poor hubs),
this **pins `z_R = 2`** — exactly as at `n = 19`: the `Z`-hub mass (`4`) and the per-endpoint
rich/poor split are unchanged because `|Iso| = 6` and `|Z| = 2` carry over verbatim.

The rich-side ≥ bound routes through the ported `z`-meets-2-poor extraction; the ≤ bound is fully
axiom-clean. -/
theorem zdeg_split_sharp_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 12) (hIso : Iso.card = 6)
    (hdeg3 : ∀ v : Fin 20, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
  ...
  have hzRle : ∑ r ∈ R, (G.neighborFinset r ∩ Z).card ≤ 2 := by
    ...
    have hez : ∀ z ∈ Z, (G.neighborFinset z ∩ R).card ≤ 1 :=
      no_z_two_rich_hubs_twenty G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdeg3 hisodeg3 hdsum
