...
The genuinely-new counting leaf for the `P₄`-cherry corners (`TwinCert20CherryP4.lean`).  Using the
...
theorem p4_c1c2_avoider_two_twin_twenty (G : SimpleGraph (Fin 20))
    ...
    (∃ h t₁ t₂ : Fin 20, h ∈ Dᶜ ∧ G.degree h ≤ 5 ∧ ¬G.Adj h c₁ ∧ ¬G.Adj h c₂ ∧
  ...
  -- Degree sums.
  have hsum60 : ∑ v : Fin 20, G.degree v = 72 := by
  ...
  have hpathIsodisj : Disjoint path Iso := by
    ...
    · exact hL2Iso hw'
  -- `|N L₁ ∩ D| = 1` and `|N L₂ ∩ D| = 1`.
  have hIso0 : ∀ v : Fin 20, v ∈ Iso → (G.neighborFinset v ∩ D).card = 0 := by
    intro v hv
  ...
  set Int : ℕ := 78 - 6 * D.card with hIntdef
  ...
  set HubA : Finset (Fin 20) := Dᶜ \ Bad with hHubAdef
  ...
  have hfc2 : (Dᶜ.filter (fun w => G.Adj w c₂)).card ≤ 1 := by
    have heq : Dᶜ.filter (fun w => G.Adj w c₂) = G.neighborFinset c₂ ∩ Dᶜ := by
      ...
      exact ⟨fun h => ⟨h.2.symm, h.1⟩, fun h => ⟨h.2, h.1.symm⟩⟩
    rw [heq, hc2Dc]
  set Big : Finset (Fin 20) := Dᶜ.filter (fun w => 6 ≤ G.degree w) with hBigdef
  ...
  have hnbig : 2 * Big.card ≤ D.card - 8 := by
    ...
    have h2 : 4 * (Dᶜ \ Big).card ≤ ∑ w ∈ Dᶜ \ Big, G.degree w := by
      ...
      simpa [smul_eq_mul, Nat.mul_comm] using this
    have hsplitB : ∑ w ∈ Dᶜ \ Big, G.degree w + ∑ w ∈ Big, G.degree w = 72 - 3 * D.card := by
      rw [Finset.sum_sdiff hBigsub]; exact hsumDcdeg
    have hcardB : (Dᶜ \ Big).card + Big.card = 20 - D.card := by
  ...
  have hHubA_props : ∀ h : Fin 20, h ∈ HubA →
  ...
  have hHubAdeg5 : ∀ h : Fin 20, h ∈ HubA → G.degree h ≤ 5 := fun h hh => (hHubA_props h hh).2.2.2
  have hHubAsub : HubA ⊆ Dᶜ := by rw [hHubAdef]; exact Finset.sdiff_subset
  -- Per-hub Iso count for `HubA` hubs.
  have hHubA_iso : ∀ h : Fin 20, h ∈ HubA →
      (G.neighborFinset h ∩ Iso).card + (G.neighborFinset h ∩ Dᶜ).card
        + (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card = G.degree h := by
    intro h hh
    ...
    have hpathleaf :
        G.neighborFinset h ∩ path = G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20)) := by
      ext w
  ...
  have hsumIso : ∑ h ∈ HubA, (G.neighborFinset h ∩ Iso).card
      ...
      + ∑ h ∈ HubA, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card
  ...
  have hleakle : ∑ h ∈ HubA, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card ≤ 4 := by
    have hfull : ∑ h ∈ Dᶜ, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card = 4 := by
      rw [cross_count_twenty G Dᶜ ({L₁, L₂} : Finset (Fin 20)),
        Finset.sum_insert (by simp [hL1L2]), Finset.sum_singleton, hL1Dc, hL2Dc]
    calc ∑ h ∈ HubA, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card
        ≤ ∑ h ∈ Dᶜ, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card :=
  ...
  · -- **THE TIE** (every avoider carries exactly one twin): the count forces full
    ...
    have hSleaf : ∑ h ∈ HubA, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card = 4 := by
      omega
    ...
    have hfull : ∑ h ∈ Dᶜ, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card = 4 := by
      rw [cross_count_twenty G Dᶜ ({L₁, L₂} : Finset (Fin 20)),
        Finset.sum_insert (by simp [hL1L2]), Finset.sum_singleton, hL1Dc, hL2Dc]
    have hBadsplitL : ∑ h ∈ HubA, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card
        + ∑ w ∈ Bad, (G.neighborFinset w ∩ ({L₁, L₂} : Finset (Fin 20))).card
        = ∑ h ∈ Dᶜ, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 20))).card := by
      rw [hHubAdef]
      exact Finset.sum_sdiff hBadsub
    have hBadleaf : ∀ w ∈ Bad, (G.neighborFinset w ∩ ({L₁, L₂} : Finset (Fin 20))).card = 0 :=
    ...
    · -- `|D| = 9`, `|HubA| = 8`, `|Iso| = 5`: dropping the two `L₁`-hubs leaves twin-incidence
      ...
      have hqHubA : q ∈ HubA := by
        ...
        exact h0 L₁ (Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hqL1.symm,
          Finset.mem_insert_self _ _⟩)
      have hPQsub : ({p, q} : Finset (Fin 20)) ⊆ HubA := by
        intro w hw
        ...
        · rw [Finset.mem_singleton] at hw
          subst hw
          exact hqHubA
      have hpq2 : ∑ v ∈ Iso, (G.neighborFinset v ∩ ({p, q} : Finset (Fin 20))).card = 2 := by
        rw [cross_count_twenty G Iso ({p, q} : Finset (Fin 20)),
      ...
      have hIsosum : ∑ v ∈ Iso, (G.neighborFinset v ∩ HubA).card = 8 := by
        rw [cross_count_twenty G Iso HubA, hisoeq, hH8]
      have hsplitv : ∀ v : Fin 20,
          (G.neighborFinset v ∩ (HubA \ ({p, q} : Finset (Fin 20)))).card
            + (G.neighborFinset v ∩ ({p, q} : Finset (Fin 20))).card
            = (G.neighborFinset v ∩ HubA).card := by
        intro v
        have hdisj : Disjoint (G.neighborFinset v ∩ (HubA \ ({p, q} : Finset (Fin 20))))
            (G.neighborFinset v ∩ ({p, q} : Finset (Fin 20))) :=
          Finset.disjoint_left.mpr fun w hw hw' =>
      ...
      have hrest : ∑ v ∈ Iso, (G.neighborFinset v ∩ (HubA \ ({p, q} : Finset (Fin 20)))).card
          = 6 := by
        have hs : ∑ v ∈ Iso, ((G.neighborFinset v ∩ (HubA \ ({p, q} : Finset (Fin 20)))).card
            + (G.neighborFinset v ∩ ({p, q} : Finset (Fin 20))).card)
      ...
      have hIsocard : Iso.card = 5 := by
        ...
        have h2 : path.card = 4 := by
          rw [hpathdef]
          exact card_four_twenty L₁ c₁ c₂ L₂ hL1c1 hL1nc2 hL1L2 hc1c2 hc1L2 hc2L2
        omega
      have hpig : ∃ v ∈ Iso,
          2 ≤ (G.neighborFinset v ∩ (HubA \ ({p, q} : Finset (Fin 20)))).card := by
        by_contra hcon
        push Not at hcon
        have hle : ∑ v ∈ Iso, (G.neighborFinset v ∩ (HubA \ ({p, q} : Finset (Fin 20)))).card
            ≤ 5 := by
          calc ∑ v ∈ Iso, (G.neighborFinset v ∩ (HubA \ ({p, q} : Finset (Fin 20)))).card
      ...
      obtain ⟨h₁, hh₁m, h₂, hh₂m, hne12⟩ := Finset.one_lt_card.mp
        (by omega : 1 < (G.neighborFinset v ∩ (HubA \ ({p, q} : Finset (Fin 20)))).card)
      ...
      have hnh2L1 : ¬G.Adj h₂ L₁ := fun ha => hh₂pq (by
        rw [← hPQ]
        exact Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr ha.symm, hHubAsub hh₂A⟩)
      have hslotv : (G.neighborFinset v ∩ ({L₁, c₁, c₂} : Finset (Fin 20))).card = 0 := by
        rw [Finset.card_eq_zero, Finset.eq_empty_iff_forall_notMem]
        intro w hw
      ...
      have hsloth : ∀ h : Fin 20, h ∈ HubA → ¬G.Adj h L₁ →
          (G.neighborFinset h ∩ ({L₁, c₁, c₂} : Finset (Fin 20))).card = 0 := by
        intro h hhA hnL1
      ...
      have hvh2 : v ≠ h₂ := by
        rintro rfl
        omega
      have hslotsum : ∑ w ∈ ({v, h₁, h₂} : Finset (Fin 20)),
        ...
        omega
      have hbudget : 2 * (∑ w ∈ ({v, h₁, h₂} : Finset (Fin 20)),
          (G.neighborFinset w ∩ ({L₁, c₁, c₂} : Finset (Fin 20))).card)
    ...
    · -- `|D| = 10`: `Bad = {b₁, b₂, B}` with the two centre-hubs `b₁, b₂` forced to degree
      ...
      have hf2eq : Dᶜ.filter (fun w => G.Adj w c₂) = {b₂} := by
        ...
        simp only [Finset.mem_filter, Finset.mem_inter, G.mem_neighborFinset]
        exact ⟨fun h => ⟨h.2.symm, h.1⟩, fun h => ⟨h.2, h.1.symm⟩⟩
      have hBadsub3 : Bad ⊆ ({b₁, b₂} : Finset (Fin 20)) ∪ Big := by
        intro w hw
      ...
      have hb12 : b₁ ≠ b₂ := by
        ...
        have h2 := Finset.card_union_le ({b₁, b₂} : Finset (Fin 20)) Big
        have h3 : ({b₁, b₂} : Finset (Fin 20)).card ≤ 1 := by
      ...
      have hpair2 : ({b₁, b₂} : Finset (Fin 20)).card = 2 := Finset.card_pair hb12
      have hBb1 : B ≠ b₁ := by
        intro e
        have hsub : Bad ⊆ ({b₁, b₂} : Finset (Fin 20)) := by
          intro w hw
      ...
      have hBb2 : B ≠ b₂ := by
        ...
        have h1 := Finset.card_le_card hsub
        omega
      have htripcard : ({b₁, b₂, B} : Finset (Fin 20)).card = 3 := by
      ...
      have hBadEq : Bad = ({b₁, b₂, B} : Finset (Fin 20)) := by
        symm
      ...
      have hsumtrip : G.degree b₁ + G.degree b₂ + G.degree B = 14 := by
        have h1 : ∑ w ∈ ({b₁, b₂, B} : Finset (Fin 20)), G.degree w
      ...
      have hdegb2 : G.degree b₂ = 4 := by
        ...
        have h2 := hDcdeg b₂ hb2Dc
        omega
      have hb1noDc : ∀ w : Fin 20, w ∈ Dᶜ → ¬G.Adj b₁ w := by
        intro w hw hadj
      ...
      have hb2nc1 : ¬G.Adj b₂ c₁ := by
        ...
        exact hb12 hm.symm
      have hnoleaf : ∀ w : Fin 20, w ∈ Bad → ∀ x : Fin 20,
          x ∈ ({L₁, L₂} : Finset (Fin 20)) → ¬G.Adj w x := by
        intro w hw x hx hadj
        have h0 := hBadleaf w hw
        rw [Finset.card_eq_zero, Finset.eq_empty_iff_forall_notMem] at h0
        exact h0 x (Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hadj, hx⟩)
      have hpartD : ∀ w : Fin 20, (G.neighborFinset w ∩ path).card
      ...
      have hshared : ((G.neighborFinset b₁ ∩ Iso) ∩ (G.neighborFinset b₂ ∩ Iso)).card ≤ 1 := by
        ...
        have ht'b2ne : t' ≠ b₂ := by
          ...
          omega
        exact hC4 ⟨t, b₁, t', b₂,
          card_four_twenty t b₁ t' b₂ htb1ne htt' htb2ne (Ne.symm ht'b1ne) hb12 ht'b2ne,
...
theorem safe_hub_two_twin_p4_twenty (G : SimpleGraph (Fin 20))
    (Iso : Finset (Fin 20)) (L₁ c₁ c₂ L₂ h t₁ t₂ : Fin 20)
    (hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 20, G.Adj v w → G.degree w ≠ 3))
  ...
  · exact twotwin_assemble_cherry_twenty G Iso L₁ c₁ c₂ h t₁ t₂ hIsoprop
  ...
  · exact twotwin_assemble_cherry_twenty G Iso c₁ c₂ L₂ h t₁ t₂ hIsoprop
