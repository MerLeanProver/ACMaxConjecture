...
/-!
# The `|D| = 10`, `e(M) = 5` induced-`C₅` shared-hub extraction (`n = 20`)
...
(`Σ_Hub deg = 72 − 30 = 42` over `10` hubs) allows at most one degree-`≥ 6`
hub (`h₆ ≤ 1`) with `Σ_{H6} deg ≤ 2 + 4h₆`; since each such hub meets `Iso`
in at most `|Iso| = 5` vertices, the degree-`≤ 5` hubs receive at least
`15 − 5h₆ > 10 − h₆ = |Hub₅|` incidences — the pigeonhole
`shared_hub_le5_from_count_twenty` extracts a degree-`≤ 5` hub with two
...
theorem eM5_D10_c5_shared_twenty (G : SimpleGraph (Fin 20))
  ...
  have hIsoge : D.card - 5 ≤ Iso.card := by
    ...
    rw [← hIsodef] at hpart
    omega
  -- Handshake: `Σ_Hub deg = 72 − 30 = 38` over `|Hub| = 9` hubs.
  ...
  -- `|Iso| = 5` exactly: the five `C₅` vertices are `M`-non-isolated members of `D`.
  have hIsole : Iso.card ≤ 5 := by
    have hsub : Iso ⊆ D \ {v₁, v₂, v₃, v₄, v₅} := by
      intro v hv
      have hvIso := hIsoiso v hv
      rw [Finset.mem_sdiff]
      refine ⟨((hmemIso v).mp hv).1, fun hvmem => ?_⟩
      simp only [Finset.mem_insert, Finset.mem_singleton] at hvmem
      rcases hvmem with rfl | rfl | rfl | rfl | rfl
      · exact hvIso.2 v₂ e12 hv2D
      · exact hvIso.2 v₃ e23 hv3D
      · exact hvIso.2 v₄ e36 hv4D
      · exact hvIso.2 v₅ e45 hv5D
      · exact hvIso.2 v₁ e51 hv1D
    have hC5sub : ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 20)) ⊆ D := by
      intro x hx
      simp only [Finset.mem_insert, Finset.mem_singleton] at hx
      rcases hx with rfl | rfl | rfl | rfl | rfl
      · exact (hmemD _).mpr hv1D
      · exact (hmemD _).mpr hv2D
      · exact (hmemD _).mpr hv3D
      · exact (hmemD _).mpr hv4D
      · exact (hmemD _).mpr hv5D
    have hcard := Finset.card_le_card hsub
    rw [Finset.card_sdiff_of_subset hC5sub, hcard5] at hcard
    omega
  -- Each degree-`≥ 6` hub meets `Iso` in at most `|Iso| ≤ 5` vertices.
  have hH6Isobound : ∑ h ∈ H6, (G.neighborFinset h ∩ Iso).card ≤ 5 * H6.card := by
    calc ∑ h ∈ H6, (G.neighborFinset h ∩ Iso).card
        ≤ ∑ _h ∈ H6, Iso.card :=
      ...
      _ ≤ H6.card * 5 := Nat.mul_le_mul_left _ hIsole
      _ = 5 * H6.card := Nat.mul_comm _ _
  -- `Σ_{Iso}|N ∩ Hub₅| ≥ 15 − 5h₆ > 10 − h₆ = |Hub₅|` since `h₆ ≤ 1`.
