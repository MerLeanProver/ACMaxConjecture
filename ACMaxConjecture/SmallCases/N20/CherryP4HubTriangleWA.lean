...
`iso_rich_force_twenty` (the `|FF|`-dispatch core of the `|D| = 8` corner).  Because the hubs
...
* `cherry_p4_W_facts_twenty` — `(W)`: a degree-`≤ 5` cherry-avoiding hub has `≤ 1` iso-twin
  (from `¬TwoTwinConfig`).
...
theorem avoiders_ge_gen (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20)) (a b c : Fin 20)
...
theorem avoider_internal_ge_two_pt (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (g allowed : Fin 20) (hdg : 4 ≤ G.degree g) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
...
theorem fully_free_internal_ge_three_pt (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (g : Fin 20) (hdg : 4 ≤ G.degree g) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
...
theorem hub_deg_upper_pt (S : Finset (Fin 20)) (f : Fin 20 → ℕ) (T : ℕ)
    (lb : ∀ g ∈ S, 4 ≤ f g) (hsum : ∑ g ∈ S, f g = T) (g0 : Fin 20) (hg0 : g0 ∈ S) :
...
`exists_hub_triangle_config_residual_twenty`, requiring `G.degree g ≤ 5` directly. -/
theorem cherry_p4_W_facts_twenty (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (L₁ c₁ c₂ L₂ : Fin 20)
