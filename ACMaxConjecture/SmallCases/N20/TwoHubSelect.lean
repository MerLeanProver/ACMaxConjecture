...
engine** (`select_finish_twenty`, `hub_iso_sum_twenty`, `nogood_of_not_select_twenty`,
`hub_neighbor_le_twenty`, `strong_deg4_count_le_two_deg5_twenty`, `deg4_sum_le_twenty`,
`residual_arith_twenty`), the tight-profile router `two_hub_select_eM1_tight_twenty`, and the
entry point `two_hub_corner_select_twenty`.
...
* `e(M) = 0`: `(|Hub|, |Iso|, ∑deg) ∈ {(8,10,36), (9,9,37), (10,8,40)}`;
* `e(M) = 1`: `(|Hub|, |Iso|, ∑deg) ∈ {(7,9,31), (8,8,36), (9,7,37), (10,6,40)}`.
...
master inequality against each regime, the four profiles `(8,10,36)`, `(9,9,37)`, `(10,8,40)`,
`(7,9,31)` are refuted outright (`3·|Iso| + 10·|Hub| > 3·∑deg + 2`).  The three `e(M) = 1` profiles
`(8,8,36)` (tight, `104 = 104`), `(9,7,37)` (slack `2`) and `(10,6,40)` (slack `4`) survive the bare
counting — the `mod-3` slack did **not** close them (it is wider than `n = 17`'s).  They are routed
through `two_hub_select_eM1_tight_twenty`; the all-degree-`4` `(10,6,40)` profile is the analog of
...
theorem select_finish_twenty (G : SimpleGraph (Fin 20)) (Iso : Finset (Fin 20)) (h₁ h₂ : Fin 20)
...
theorem hub_iso_sum_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3) :
    ∑ a ∈ Hub, (G.neighborFinset a ∩ Iso).card = 3 * Iso.card := by
  rw [cross_count_twenty G Hub Iso]
...
/-- **No good pair from a refuted selection.**  Packages `select_finish_twenty` into the
`min(iso-deg) ≤ share + 1` form on non-adjacent degree-`4` hub pairs, given a refutation `hcon` of
the selection goal. -/
theorem nogood_of_not_select_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hcon : ¬ ∃ h₁ h₂ : Fin 20, h₁ ∈ Hub ∧ h₂ ∈ Hub ∧
  ...
  have hfin := select_finish_twenty G Iso a b hsa hsb
...
theorem hub_neighbor_le_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hdisj : Disjoint Hub Iso) (a : Fin 20) (hda : G.degree a = 4) :
...
`Hub`. -/
theorem strong_deg4_count_le_two_deg5_twenty (G : SimpleGraph (Fin 20))
    (Hub Iso : Finset (Fin 20)) (hdisj : Disjoint Hub Iso)
  ...
  have hkey : ∀ a ∈ A, A.card - 1 + (G.neighborFinset a ∩ Iso).card ≤ 4 := by
    ...
    have hc2 := hub_neighbor_le_twenty G Hub Iso hdisj a hda
    omega
...
theorem deg4_sum_le_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
...
theorem residual_arith_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
  ...
  classical
  have hiso_le : ∀ a : Fin 20, (G.neighborFinset a ∩ Iso).card ≤ G.degree a := by
    intro a
