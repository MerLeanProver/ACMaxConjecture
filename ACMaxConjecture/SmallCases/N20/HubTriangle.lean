...
/-!
# Hub-triangle existence for the `n = 20`, `e(M) = 3`, `|D| = 8` residual corner

This file isolates the hub-triangle existence obligation in the `|D| = 8` (`|Hub| = 12`,
all-degree-`4`) corner of the `e(M) = 3` `P₄` dispatch for `n = 20`, reached after the
`SingleVertex`/`TwoTwin`/`TwoHub` routes have all been excluded.  It is the `n = 20` port of
`TwinCert19HubTriangle` (`n = 19`, `|Hub| = 11`).

## The residual configuration

* `D` = the degree-`3` vertices, `|D| = 8`; `Dᶜ` = the **twelve** hubs, all of degree exactly `4`
  (`hdeg4`).
* `e(M) = 3`: the degree-`3` subgraph is the path `L₁–c₁–c₂–L₂`, plus `|Iso| = 4` `M`-isolated
  degree-`3` twins (`Iso`).
* `e(Hub) = 15` hub-hub edges (`∑_{w ∈ Dᶜ} |N(w) ∩ Dᶜ| = 30`, the per-hub split
  `path(g) + iso(g) + int(g) = 4` over the twelve hubs: `4·12 − 6 − 12 = 30`).
* `(W)`, `(A)`, `hth : ¬TwoHubConfig` as in `n = 19`.
...
## Status (`n = 20` delta from `n = 19`)

The two **direct** branches (an avoider triangle for either cherry yields `HubTriangleConfig`
immediately) and all the structural counting (`TwinCert20HubTriangleStruct`: twelve hubs, `≥ 8`
avoiders per cherry, `∑ int = 30`) are fully proved here.  The
**no-avoider-triangle ⇒ contradiction** core `core_triangle_force_twenty` is now **fully closed**
(no `sorry`): the `|FF|`-dispatch in `iso_rich_force_twenty` handles `|FF| ∈ {2, …, 10}`, and the
genuinely-new `|FF| = 8`, `|B| = 4` excess-`12` boundary is closed by threading the top-level
good-triangle threshold `≤ 11` (`hT`).  See `iso_rich_force_twenty` for the analysis.
-/
...
/-- **No avoider triangle ⇒ contradiction (the `n = 20` combinatorial core).**  Under the residual
hypotheses for the `n = 20`, `|D| = 8`, `|Hub| = 12` corner, the assumption that neither cherry
admits a triangle of pairwise-adjacent avoiding hubs (`htri1`, `htri2`) is contradictory.

It is discharged by `iso_rich_force_twenty` (`TwinCert20HubTriangleFF`), which dispatches on
`|FF| ∈ {2, …, 10}` via the *isolated-twin incidence bound* `∑_{A1} iso ≤ 4`, `∑_{A2} iso ≤ 4`
(each twin has `≤ 1` neighbour in either avoider set, by `hA`), forcing the iso-mass
redistribution `∑_B iso ≥ 4 + ∑_{FF} iso`.  With the denser budget `∑ int = 30` and
avoiders `≥ 8`:

* `|FF| ∈ {2, 3, 4, 5}`: vacuous by the `|B|` bound (`2 ≤ |B| ≤ |FF| − 4`);
* `|FF| ∈ {9, 10}`: dead by the fully-free internal budget (`∑_{FF} int ≥ 4|FF| − 4 > 30`);
* `|FF| = 6` (`|B| = 2`): two iso-`3` non-adjacent inactive hubs (`two_iso3_nonadj_false`);
* `|FF| = 7` (`|B| = 3`, `∑_{FF} int ∈ {24, 25, 26}`): `inactive_iso8_false` or a `TwoHubConfig`
  (`ff3_extract_twohub_twenty`);
* `|FF| = 8`, `|B| = 4` (`A1 = A2 = FF`): the genuinely-new excess-`12` boundary, **closed** by
  extracting the two common `L₁,L₂`-neighbours `r, s ∈ B` and case-splitting — `r ≁ s` fires the
  good-`C₄` (`Σ = 14`), `r ~ s` the degree-`11` good triangle `{r, s, L₁}` (`Σ = 11`, via `hT`). -/
theorem core_triangle_force_twenty (G : SimpleGraph (Fin 20))
    ...
    False :=
  iso_rich_force_twenty G D Iso L₁ c₁ c₂ L₂ hT hC4 hK23 hmemD hIsodef hIsoprop hisochar hcov
    hNc1D hNc2D hc1deg hc2deg hL1deg hL2deg hac1L1 hc12 hac2L2 hnL1c2 hnc1L2 hL1nc2 hL2nc1 hdeg4
    hD8 hth hL1D hc1D hc2D hL2D hW hA hDc9 hIso4 hcard_c1 hcard_c2 hcard_L1 hcard_L2 hSum18 htri1
    htri2

/-- **Hub-triangle existence in the `n = 20`, `|D| = 8`, `e(M) = 3`, `P₄` residual corner.**
Under the residual hypotheses (twelve degree-`4` hubs, `15` hub-hub edges `∑ int = 30`, `M = P₄`
`L₁–c₁–c₂–L₂` with four `M`-isolated twins, and the falsity of `SingleVertexConfig`/
`TwoTwinConfig`/`TwoHubConfig`), the graph contains three pairwise-adjacent hubs avoiding a cherry,
packaged as `HubTriangleConfig G`.  The two direct branches are fully proved; the
no-avoider-triangle case is discharged by `core_triangle_force_twenty`, now **fully closed**
(the `|FF| = 8`, `|B| = 4` excess-`12` boundary is killed by the threaded good-triangle `≤ 11`
hypothesis `hT`; see its docstring). -/
theorem exists_hub_triangle_config_residual_twenty (G : SimpleGraph (Fin 20))
    (D Iso : Finset (Fin 20)) (L₁ c₁ c₂ L₂ : Fin 20)
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 11)
    (hC4 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
  ...
  have hA : ∀ t : Fin 20, t ∈ Iso → ∀ p q : Fin 20, p ≠ q →
    ...
    · have hsum0 : ∑ w ∈ ({t, p, q} : Finset (Fin 20)),
          (G.neighborFinset w ∩ ({L₁, c₁, c₂} : Finset (Fin 20))).card = 0 := by
  ...
  -- The twelve hubs (`Dᶜ.card = 12`) and the four `M`-isolated twins (`Iso.card = 4`).
  obtain ⟨hDc9, hIso4⟩ := hub_struct G D Iso L₁ c₁ c₂ L₂ hIsodef hisochar hL1D hc1D hc2D hL2D
  ...
  -- Hub-internal-degree total: `∑ int = 4·12 − 6 − 12 = 30` (the `n = 20` delta from `n = 19`'s
  -- `26`), derived from the per-hub split `path(g) + iso(g) + int(g) = 4`.
  ...
  have hSum18 : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 30 := by
    have htot : ∑ g ∈ Dᶜ, ((G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 20))).card
  ...
  exfalso
  exact core_triangle_force_twenty G D Iso L₁ c₁ c₂ L₂ hT hC4 hK23 hmemD hIsodef hIsoprop
    hisochar hcov hNc1D hNc2D hc1deg hc2deg hL1deg hL2deg hac1L1 hc12 hac2L2 hnL1c2 hnc1L2
    hL1nc2 hL2nc1 hdeg4 hD8 hth hL1D hc1D hc2D hL2D hW hA hDc9 hIso4 hcard_c1 hcard_c2
    hcard_L1 hcard_L2 hSum18 htri1 htri2
