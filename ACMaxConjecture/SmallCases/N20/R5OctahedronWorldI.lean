...
/-!
# The `n = 20` deg-5 corner world-(i) `{3, 2, 2, 2, 2}` kill

The `n = 20` `(11, 7, 45)` deg-5 corner, in the no-two-hub octahedron world, has a set `R` of five
*rich* degree-`4` hubs, each meeting the seven-vertex `M`-isolated twin set `Iso` in `2` or `3`
places, with `∑ isoDeg = 11` and at most one rich-rich edge (`∑ e_RR ≤ 2`).  This is impossible.

The proof is a shared-twin double count (`cherry_double_count`).  Writing
`share u v = |N u ∩ N v ∩ Iso|` and `k t = |N t ∩ R|`, we have
`∑_{u,v ∈ R} share u v = ∑_{t ∈ Iso} k t ^ 2`.  The no-two-hub hypothesis forces every non-adjacent
rich pair to share **exactly one** twin, so the double count is at least `29`; the good-`C₄`
share-≤-`1` bound and `∑ k = 11`, `k ≤ 3`, `|Iso| = 7` then force at least three twins with `k = 3`;
three `3`-subsets of the five-element `R` must share a rich pair in two twins, which is either
non-adjacent (contradicting the share bound) or the unique rich edge (pushing the double count to
`33 > 31`, impossible).
-/
...
/-- **The `n = 20` deg-5 corner world-(i) `{3, 2, 2, 2, 2}` kill.**  There is no graph on `Fin 20`
carrying the octahedron-world rich configuration `R` (five non-two-hub degree-`4` hubs meeting the
seven twins in `11` incidences with at most one internal edge). -/
theorem octahedron_world_i_two_hub_r5_twenty (G : SimpleGraph (Fin 20))
    ...
    (hRcard : R.card = 5) (_hIsoRcard : Iso.card = 7)
    (hRiso11 : (∑ r ∈ R, (G.neighborFinset r ∩ Iso).card) = 11)
    (heRR : (∑ r ∈ R, (G.neighborFinset r ∩ R).card) ≤ 2) :
    False := by
  classical
  ...
  have hksum : (∑ t ∈ Iso, (G.neighborFinset t ∩ R).card) = 11 := by
  ...
  have hshare_ge1 : ∀ u ∈ R, ∀ v ∈ R, u ≠ v → ¬G.Adj u v →
    ...
    -- `share = 0` gives both private-twin sets size `≥ 2`.
  ...
  have hrow : ∀ u ∈ R,
    ...
    omega
  -- Sum the rows: `31 + (retained adjacent shares) ≤ (double count) + (rich-rich incidences)`.
  have hmaster : 31 + (∑ u ∈ R, ∑ v ∈ R.erase u,
        (if G.Adj u v then (G.neighborFinset u ∩ G.neighborFinset v ∩ Iso).card else 0))
  ...
  -- The double count is at least `29`.
  have hSS29 : 29 ≤ (∑ u ∈ R, ∑ v ∈ R,
      (G.neighborFinset u ∩ G.neighborFinset v ∩ Iso).card) := by omega
  rw [hDC] at hSS29
  -- Linear-programming upper bound `D ≤ 22 + 3 a`, where `a` counts the `k = 3` twins.
  have hDub : (∑ t ∈ Iso, (G.neighborFinset t ∩ R).card * (G.neighborFinset t ∩ R).card)
      ≤ 22 + 3 * (Iso.filter (fun t => (G.neighborFinset t ∩ R).card = 3)).card := by
    ...
    calc (∑ t ∈ Iso, (G.neighborFinset t ∩ R).card * (G.neighborFinset t ∩ R).card)
      ...
      _ = 22 + 3 * (Iso.filter (fun t => (G.neighborFinset t ∩ R).card = 3)).card := by
  ...
  -- Upper bound `D ≤ 31` (since at most three twins have `k = 3`).
  have hDle31 : (∑ t ∈ Iso, (G.neighborFinset t ∩ R).card * (G.neighborFinset t ∩ R).card)
      ≤ 31 := by
    have ha_le : 3 * (Iso.filter (fun t => (G.neighborFinset t ∩ R).card = 3)).card ≤ 11 := by
      have h1 : (∑ t ∈ Iso.filter (fun t => (G.neighborFinset t ∩ R).card = 3),
      ...
      have h2 : (∑ t ∈ Iso.filter (fun t => (G.neighborFinset t ∩ R).card = 3),
          (G.neighborFinset t ∩ R).card)
  ...
  have hpair : ∀ s ∈ Iso, ∀ w ∈ Iso, s ≠ w →
    ...
    · have hSumQ4 : 4 ≤ ∑ u ∈ R, ∑ v ∈ R.erase u,
          (if G.Adj u v then (G.neighborFinset u ∩ G.neighborFinset v ∩ Iso).card else 0) := by
        have hQp : (G.neighborFinset p ∩ G.neighborFinset q ∩ Iso).card
            ≤ ∑ v ∈ R.erase p,
              (if G.Adj p v then (G.neighborFinset p ∩ G.neighborFinset v ∩ Iso).card else 0) := by
          have := Finset.single_le_sum
            (f := fun v => if G.Adj p v then (G.neighborFinset p ∩ G.neighborFinset v ∩ Iso).card
              else 0)
            (fun i _ => Nat.zero_le _) (Finset.mem_erase.mpr ⟨Ne.symm hpq, hqR⟩)
          rwa [if_pos hadjpq] at this
        have hQq : (G.neighborFinset q ∩ G.neighborFinset p ∩ Iso).card
            ≤ ∑ v ∈ R.erase q,
              (if G.Adj q v then (G.neighborFinset q ∩ G.neighborFinset v ∩ Iso).card else 0) := by
          have := Finset.single_le_sum
            (f := fun v => if G.Adj q v then (G.neighborFinset q ∩ G.neighborFinset v ∩ Iso).card
              else 0)
            (fun i _ => Nat.zero_le _) (Finset.mem_erase.mpr ⟨hpq, hpR⟩)
          rwa [if_pos hadjpq.symm] at this
        have hpair_sub : ({p, q} : Finset (Fin 20)) ⊆ R := by
          intro x hx
          rcases Finset.mem_insert.mp hx with rfl | hx
          · exact hpR
          · rw [Finset.mem_singleton] at hx; rw [hx]; exact hqR
        have hsub_le := Finset.sum_le_sum_of_subset (f := fun u => ∑ v ∈ R.erase u,
            (if G.Adj u v then (G.neighborFinset u ∩ G.neighborFinset v ∩ Iso).card else 0))
          hpair_sub
        rw [Finset.sum_pair hpq] at hsub_le
        have hcomm : (G.neighborFinset q ∩ G.neighborFinset p ∩ Iso).card
            = (G.neighborFinset p ∩ G.neighborFinset q ∩ Iso).card := by
          rw [Finset.inter_comm (G.neighborFinset q) (G.neighborFinset p)]
        omega
      have hSS33 : 33 ≤ (∑ u ∈ R, ∑ v ∈ R,
          (G.neighborFinset u ∩ G.neighborFinset v ∩ Iso).card) := by omega
      rw [hDC] at hSS33
      omega
