...
/-!
# Hub-triangle existence for the `n = 20`, `e(M) = 4`, `|D| = 8` fat double-star corner

This file is the `n = 20` port of `TwinCert19HubTriangleFatStar`.  The `M`-structure is a **fat
...
(`N c₂ ∩ D = {c₁, L₂}`), for `4` `M`-edges.  In the `|D| = 8` (`|Hub| = 12`, all-degree-`4`) corner
there are `|Iso| = 3` `M`-isolated twins and the hub-internal-degree total is
`∑ int = 4·12 − 7 − 9 = 32` (vs the `n = 19` value `28`).
...
via `hub_triangle_cut_certificate`.  The constants (`∑ int = 32`, `|A1| ≥ 8`, `|A2| ≥ 9`) force
`|FF| ∈ {7, 8}`: the `|FF| = 7`, `|B| = 2` corner collapses via the good-`C₄` twin-share bound,
and `|FF| = 8` dies on a `Dᶜ \ FF` overflow (`5` cherry-incidences into `12 − 8 = 4` hubs).
-/
...
`Dᶜ.card = 12` and `Iso.card = 3`. -/
theorem hub_struct_fatstar (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (c₁ c₂ L₁ L₁' L₂ : Fin 20)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hisochar : ∀ w : Fin 20, w ∈ D → w ≠ c₁ → w ≠ c₂ → w ≠ L₁ → w ≠ L₁' → w ≠ L₂ → w ∈ Iso)
    ...
    (hD8 : D.card = 8) :
    Dᶜ.card = 12 ∧ Iso.card = 3 := by
  classical
  have hDc : Dᶜ.card = 12 := by rw [Finset.card_compl, Fintype.card_fin, hD8]
  refine ⟨hDc, ?_⟩
  have hc1c2 : c₁ ≠ c₂ := G.ne_of_adj hc12
  have hc1L1 : c₁ ≠ L₁ := G.ne_of_adj hac1L1
  have hc1L1' : c₁ ≠ L₁' := G.ne_of_adj hac1L1'
  have hc2L2 : c₂ ≠ L₂ := G.ne_of_adj hac2L2
  set P : Finset (Fin 20) := {c₁, c₂, L₁, L₁', L₂} with hP
...
theorem path_iso_partition_fatstar (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (c₁ c₂ L₁ L₁' L₂ : Fin 20)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hisochar : ∀ w : Fin 20, w ∈ D → w ≠ c₁ → w ≠ c₂ → w ≠ L₁ → w ≠ L₁' → w ≠ L₂ → w ∈ Iso)
    (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hL1D : L₁ ∈ D) (hL1'D : L₁' ∈ D) (hL2D : L₂ ∈ D)
    (hac1L1 : G.Adj c₁ L₁) (hac1L1' : G.Adj c₁ L₁') (hc12 : G.Adj c₁ c₂) (hac2L2 : G.Adj c₂ L₂) :
    ({c₁, c₂, L₁, L₁', L₂} : Finset (Fin 20)) ∪ Iso = D ∧
      Disjoint ({c₁, c₂, L₁, L₁', L₂} : Finset (Fin 20)) Iso := by
  classical
  set P : Finset (Fin 20) := {c₁, c₂, L₁, L₁', L₂} with hP
  have hIsoD : Iso ⊆ D := by rw [hIsodef]; exact Finset.filter_subset _ _
  have hPD : P ⊆ D := by
...
theorem path_hub_incidence_fatstar (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20))
    (c₁ c₂ L₁ L₁' L₂ : Fin 20)
...
/-- **Residual incidence sums for the fat double-star.**  Over the twelve hubs the path- and iso-
incidences total `7` and `9` respectively (so the hub-internal total is `4·12 − 7 − 9 = 32`), and
per hub the three split the degree `4`. -/
theorem incidence_sums_fatstar (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (c₁ c₂ L₁ L₁' L₂ : Fin 20)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
...
free internal degree `≥ 3`, `|A1| ≥ 8`, `|A2| ≥ 9`, `FF = A1 ∩ A2`, and the hub-internal-degree
total `∑_{Dᶜ} f = 32`, the fully-free count satisfies `2 ≤ |FF| ≤ 10` (the sharper `≤ 8` bound is
derived in `iso_rich_force_fatstar` from the `iso + int = 4` per-hub split). -/
theorem active_hub_budget_fatstar (Dc A1 A2 FF : Finset (Fin 20)) (f : Fin 20 → ℕ)
    (hA1sub : A1 ⊆ Dc) (hA2sub : A2 ⊆ Dc) (hFF : FF = A1 ∩ A2)
    (hA1card : 8 ≤ A1.card) (hA2card : 9 ≤ A2.card)
    (hA1int : ∀ g ∈ A1, 2 ≤ f g) (hA2int : ∀ g ∈ A2, 2 ≤ f g)
    (hFFint : ∀ g ∈ FF, 3 ≤ f g) (hSum24 : ∑ w ∈ Dc, f w = 32) :
    2 ≤ FF.card ∧ FF.card ≤ 10 := by
  classical
  ...
  have hle9 : FF.card ≤ 10 := by omega
  refine ⟨?_, hle9⟩
  have hUsub : A1 ∪ A2 ⊆ Dc := Finset.union_subset hA1sub hA2sub
  have hUsum_le : ∑ g ∈ A1 ∪ A2, f g ≤ 32 := by
    rw [← hSum24]
...
theorem avoider_internal_ge_two_pair (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (g a₁ a₂ : Fin 20) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
    (hnboth : ¬(G.Adj g a₁ ∧ G.Adj g a₂))
    (hclass : ∀ x : Fin 20, G.Adj g x → x ∈ D → x = a₁ ∨ x = a₂ ∨ x ∈ Iso) :
    2 ≤ (G.neighborFinset g ∩ Dᶜ).card := by
  classical
  set A : Finset (Fin 20) := if G.Adj g a₁ then {a₁} else (if G.Adj g a₂ then {a₂} else ∅) with hA
...
theorem iso_incidence_le_three (G : SimpleGraph (Fin 20)) (Iso A : Finset (Fin 20))
    (hIso3 : Iso.card = 3)
...
/-- **Generic avoider count `≥ 9`.**  If three vertices `a, b, c` together have at most `3`
hub-neighbours and there are `12` hubs, then at least `9` hubs avoid all of `a, b, c`. -/
theorem avoiders_ge_seven_aux (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20)) (a b c : Fin 20)
    (hDc10 : Dᶜ.card = 12)
    (hbound : (G.neighborFinset a ∩ Dᶜ).card + (G.neighborFinset b ∩ Dᶜ).card
      + (G.neighborFinset c ∩ Dᶜ).card ≤ 3) :
    9 ≤ (Dᶜ.filter (fun g => ¬G.Adj g a ∧ ¬G.Adj g b ∧ ¬G.Adj g c)).card := by
  classical
...
/-- **Vacuity of the `|FF| = 8` `K_{3,3}`-of-avoiders fat-star corner.**  When eight hubs avoid the
entire fat double-star, the `2 + 2 + 1 = 5` hub-incidences of `c₁`'s three degree-`3` neighbours
`L₁, L₁', c₂` must all land in `Dᶜ \ FF`, which has only `12 − 8 = 4` hubs.  Pigeonhole forces two
...
theorem fatstar_ff6_vacuous (G : SimpleGraph (Fin 20)) (D FF : Finset (Fin 20))
    (c₁ c₂ L₁ L₁' : Fin 20)
    ...
    (hdeg4 : ∀ w : Fin 20, w ∈ Dᶜ → G.degree w = 4)
    (hFFsub : FF ⊆ Dᶜ)
    (hFFav : ∀ g : Fin 20, g ∈ FF → ¬G.Adj g L₁ ∧ ¬G.Adj g L₁' ∧ ¬G.Adj g c₂)
    (hcard_c1 : (G.neighborFinset c₁ ∩ Dᶜ).card = 0)
    (hcard_L1 : (G.neighborFinset L₁ ∩ Dᶜ).card = 2)
    (hcard_L1' : (G.neighborFinset L₁' ∩ Dᶜ).card = 2)
    (hcard_c2 : (G.neighborFinset c₂ ∩ Dᶜ).card = 1)
    (hDc10 : Dᶜ.card = 12) (hFF6 : FF.card = 8) : False := by
  classical
  have hDne : ∀ u w : Fin 20, u ∈ D → w ∈ Dᶜ → u ≠ w :=
  ...
  have hc1nohub : ∀ h : Fin 20, h ∈ Dᶜ → ¬G.Adj c₁ h := by
    intro h hh hadj
...
/-- **No avoider triangle ⇒ contradiction (the fat double-star `|FF|`-dispatch core).**  The `n = 20`
budget (`|Iso| = 3`, `∑ int = 32`, `|A1| ≥ 8`, `|A2| ≥ 9`) gives `|A1 ∪ A2| ≥ 17 − |FF|` against the
`12` hubs; the budget forces `∑_FF iso ≥ 2`, hence `|B| ≥ 2`, hence `|FF| ≥ 7`, while the
`iso + int = 4` per-fully-free split caps `|FF| ≤ 8`.  Dispatches on `|FF| ∈ {7, 8}`: `|FF| = 8` is
the vacuous `Dᶜ \ FF`-overflow corner (`5` cherry-incidences into `12 − 8 = 4` hubs), and
`|FF| = 7` forces `|B| = 2` with `∑_B iso ≥ 5`, so one inactive hub carries all `3` twins and the
other `≥ 2` — a shared-twin overflow against the good-`C₄` share bound
`nonadj_hubs_share_le_one_iso`. -/
theorem iso_rich_force_fatstar (G : SimpleGraph (Fin 20))
    (D Iso : Finset (Fin 20)) (c₁ c₂ L₁ L₁' L₂ : Fin 20)
    ...
    (hDc10 : Dᶜ.card = 12) (hIso3 : Iso.card = 3)
    (hcard_c1 : (G.neighborFinset c₁ ∩ Dᶜ).card = 0)
    (hcard_c2 : (G.neighborFinset c₂ ∩ Dᶜ).card = 1)
    (hcard_L1 : (G.neighborFinset L₁ ∩ Dᶜ).card = 2)
    (hcard_L1' : (G.neighborFinset L₁' ∩ Dᶜ).card = 2)
    (hcard_L2 : (G.neighborFinset L₂ ∩ Dᶜ).card = 2)
    (hSum24 : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 32)
    (htri1 : ¬∃ a b c : Fin 20, a ∈ Dᶜ ∧ b ∈ Dᶜ ∧ c ∈ Dᶜ ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c ∧
      (¬G.Adj a L₁ ∧ ¬G.Adj a c₁ ∧ ¬G.Adj a L₁') ∧
      (¬G.Adj b L₁ ∧ ¬G.Adj b c₁ ∧ ¬G.Adj b L₁') ∧
      (¬G.Adj c L₁ ∧ ¬G.Adj c c₁ ∧ ¬G.Adj c L₁'))
    (htri2 : ¬∃ a b c : Fin 20, a ∈ Dᶜ ∧ b ∈ Dᶜ ∧ c ∈ Dᶜ ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c ∧
  ...
  have hFFint : ∀ g ∈ FF, 3 ≤ (G.neighborFinset g ∩ Dᶜ).card := by
    ...
    · exact h
  -- `|A1| ≥ 8`, `|A2| ≥ 9`.
  have hA1card : 8 ≤ A1.card := by
    have := avoiders_ge_six_aux G D L₁ c₁ L₁' hDc10 (by omega)
    rwa [← hA1def] at this
  have hA2card : 9 ≤ A2.card := by
    have := avoiders_ge_seven_aux G D c₁ c₂ L₂ hDc10 (by omega)
    rwa [← hA2def] at this
  -- Inclusion–exclusion budget: `2 ≤ |FF| ≤ 10` (sharpened to `≤ 8` below).
  obtain ⟨hFFlb, hFFub⟩ :=
  ...
  set P : Finset (Fin 20) := {c₁, c₂, L₁, L₁', L₂} with hPdef
  ...
  have hBpart : (∑ g ∈ B, (G.neighborFinset g ∩ Dᶜ).card)
      + ∑ g ∈ A1 ∪ A2, (G.neighborFinset g ∩ Dᶜ).card = 32 := by
  ...
  -- Sharpen the budget: `iso + int = 4` per fully-free hub with `∑_FF iso ≤ 3` and `∑_FF int ≤ 32`
  -- forces `|FF| ≤ 8`.
  have hFFintle : (∑ g ∈ FF, (G.neighborFinset g ∩ Dᶜ).card) ≤ 32 := by
    rw [← hSum24]
    exact Finset.sum_le_sum_of_subset_of_nonneg hFFsub (fun _ _ _ => Nat.zero_le _)
  have hFF6 : FF.card ≤ 8 := by omega
  -- Dispatch on `|FF| ∈ {7, 8}`: the int-budget forces `∑_FF iso ≥ 2`, so `∑_B iso ≥ 5`, so
  -- `|B| ≥ 2`, so `|A1 ∪ A2| ≤ 10` and `|FF| ≥ 17 − 10 = 7`.
  have hFFcases : FF.card = 7 ∨ FF.card = 8 := by omega
  ...
  have hUcardle : (A1 ∪ A2).card ≤ 12 := by rw [← hDc10]; exact Finset.card_le_card hUsub
  rcases hFFcases with hFF | hFF
  · -- `|FF| = 7`: `|A1 ∪ A2| = 17 − 7 = 10` forces `|B| = 2` (the int-budget gives `∑_FF iso ≥ 2`
    -- hence `∑_B iso ≥ 5` hence `|B| ≥ 2`; `|B| ≤ 12 − 10 = 2`).
    by_cases hB2 : B.card = 2
    · -- `|FF| = 7`, `|B| = 2`: a forced counting collapse.  The
      -- `int`/`iso` budget pins the two inactive hubs so tightly that they jointly meet all three
      -- `M`-isolated twins: `|C| = |(A1∪A2)\FF| = 3` gives `∑_C int ≥ 6`, and `∑_FF int = 28 − ∑_FF
      ...
      have hFFsum4' : (∑ g ∈ FF, (G.neighborFinset g ∩ Iso).card)
          + ∑ g ∈ FF, (G.neighborFinset g ∩ Dᶜ).card = 28 := by rw [hFFsum4, hFF]
      have hBiso5 : 5 ≤ ∑ g ∈ B, (G.neighborFinset g ∩ Iso).card := by omega
      have hisole : ∀ h : Fin 20, (G.neighborFinset h ∩ Iso).card ≤ 3 := by
        intro h
      ...
      have key : ∀ a b : Fin 20, a ∈ B → b ∈ B → a ≠ b →
    ...
    · omega
  · -- `|FF| = 8`: `c₁`'s three degree-`3` neighbours `L₁, L₁', c₂` carry `2 + 2 + 1 = 5`
    -- hub-incidences into `Dᶜ \ FF` (only `12 − 8 = 4` hubs), so two share a hub, forcing a
...
/-- **Hub-triangle existence in the `n = 20`, `|D| = 8`, `e(M) = 4` fat double-star corner.**  Under
the residual hypotheses (twelve degree-`4` hubs, the fat dominating-edge double-star
...
theorem exists_hub_triangle_config_fatstar_twenty (G : SimpleGraph (Fin 20))
    (D Iso : Finset (Fin 20)) (c₁ c₂ L₁ L₁' L₂ : Fin 20)
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
  ...
  have hA : ∀ t : Fin 20, t ∈ Iso → ∀ p q : Fin 20, p ≠ q →
      G.Adj t p → G.Adj t q →
      ¬((¬G.Adj p L₁ ∧ ¬G.Adj p c₁ ∧ ¬G.Adj p L₁' ∧
          ¬G.Adj q L₁ ∧ ¬G.Adj q c₁ ∧ ¬G.Adj q L₁') ∨
        (¬G.Adj p c₁ ∧ ¬G.Adj p c₂ ∧ ¬G.Adj p L₂ ∧
    ...
    have htiso_prop : ∀ w : Fin 20, G.Adj t w → G.degree w ≠ 3 := (hIsoprop t htIso).2
    ...
    · have hsum0 : ∑ w ∈ ({t, p, q} : Finset (Fin 20)),
          (G.neighborFinset w ∩ ({L₁, c₁, L₁'} : Finset (Fin 20))).card = 0 := by
      ...
      exact ⟨t, p, q, L₁, c₁, L₁', htdeg, hL1deg, hc1deg, hL1'deg, htp, htq,
        ...
        hac1L1.ne', hac1L1'.ne, hL1nL1'⟩
    · have hsum0 : ∑ w ∈ ({t, p, q} : Finset (Fin 20)),
          (G.neighborFinset w ∩ ({c₁, c₂, L₂} : Finset (Fin 20))).card = 0 := by
        apply Finset.sum_eq_zero
        intro w hw
        rw [Finset.card_eq_zero, Finset.eq_empty_iff_forall_notMem]
        intro a ha
  ...
  have hubne : ∀ g : Fin 20, g ∈ Dᶜ → g ≠ c₁ ∧ g ≠ c₂ ∧ g ≠ L₁ ∧ g ≠ L₁' ∧ g ≠ L₂ := by
    intro g hg
  ...
  obtain ⟨hsumP, hsumI, hper⟩ :=
    incidence_sums_fatstar G D Iso c₁ c₂ L₁ L₁' L₂ hIsodef hIsoprop hisochar hc1D hc2D hL1D hL1'D
      ...
      hL1nL2 hL1'nL2
  have hSum24 : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 32 := by
    have htot : ∑ g ∈ Dᶜ, ((G.neighborFinset g ∩ ({c₁, c₂, L₁, L₁', L₂} : Finset (Fin 20))).card
