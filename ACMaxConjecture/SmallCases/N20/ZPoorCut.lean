...
/-!
# The reusable `z`-meets-two-poor cut engine (`n = 20`, `|Hub| = 12`)

This file packages the **no-cross pigeonhole cut** that closes the `z`-meets-two-poor deep regimes
(`r ∈ {6, 7}`) uniformly, generalising the `r = 5` no-cross corner of `TwinCert20R5Resid` to the
larger rich pools (where the pigeonhole has strictly more slack).

The engine `two_poor_zleaf_pigeonhole_twenty` takes a *poor* hub `gstar` with a single twin
...
The cross-edge case is handled per-regime by the axiom-clean `cross_z_hub_c4_false_twenty` of
`TwinCert20R5ResidHelpers`; this file supplies only the no-cross packing.
-/
...
theorem two_poor_zleaf_pigeonhole_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
  ...
  have hgstar_hubdeg : (G.neighborFinset gstar ∩ Hub).card ≤ 2 := by
    ...
    have hsp := nbr_split_three_twenty G Hub Iso hdisj gstar
    rw [← hZdef] at hsp
...
theorem z_side_poor_cut_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 12) (hIso : Iso.card = 6)
    (hdsum : ∑ w ∈ Hub, G.degree w = 48)
