...
This file closes the lone documented `sorry` of `iso_rich_force_p4_twenty` (the `|D| = 9`,
...
theorem deg4_path_bad_of_two (G : SimpleGraph (Fin 20)) (L₁ c₁ c₂ L₂ h : Fin 20)
    ...
    (hp2 : 2 ≤ (G.neighborFinset h ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 20))).card) :
  ...
  have pair_mem : ∀ (S : Finset (Fin 20)) (a b : Fin 20), a ≠ b → S ⊆ {a, b} → 2 ≤ S.card →
      a ∈ S ∧ b ∈ S := by
    intro S a b hab hsub hcard
    have hcab : ({a, b} : Finset (Fin 20)).card = 2 := by
  ...
  have hP4 : ({h, c₁, c₂, L₂} : Finset (Fin 20)).card = 4 := by
  ...
  have hnc1 : ¬G.Adj h c₁ := by
    ...
    · by_cases hhc2 : G.Adj h c₂
      ...
      · have hsub : G.neighborFinset h ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 20)) ⊆ {c₁, L₂} := by
          intro x hx
  ...
  have hsub : G.neighborFinset h ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 20)) ⊆ {L₁, L₂} := by
    intro x hx
    rw [Finset.mem_inter, G.mem_neighborFinset] at hx
    obtain ⟨hadj, hxP⟩ := hx
    simp only [Finset.mem_insert, Finset.mem_singleton] at hxP ⊢
...
theorem isolated_pair_iso_rich_twenty (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (L₁ c₁ c₂ L₂ h₁ h₂ : Fin 20)
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
  ...
  have iso_count : ∀ h : Fin 20, G.neighborFinset h ∩ Dᶜ = ∅ → G.degree h = 4 →
  ...
  have hcard4 : ({h₁, L₁, h₂, L₂} : Finset (Fin 20)).card = 4 := by
...
theorem two_isolated_hub_twohubconfig_d9_ff4 (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (L₁ c₁ c₂ L₂ h₁ h₂ : Fin 20)
    (hC4 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
