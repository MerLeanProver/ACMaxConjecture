...
/-!
# Structural counts for the `n = 20`, `e(M) = 3`, `P₄`-cherry `|D| ∈ {10, 11, 12}` hub-triangle

Constant-generalised port of `TwinCert19CherryP4HubTriangleStruct`: the residual degree-`3` graph is
the path `L₁–c₁–c₂–L₂` plus `M`-isolated twins `Iso`, and the hubs are `Dᶜ`.
With `|D| = d` the counts are `Dᶜ.card = 20 − d`, `Iso.card = d − 4`, hub path-incidence total `6`,
iso-incidence total `3·(d − 4)`, and hub-internal total `78 − 6·d` (`18` at `d = 10`, `12` at
`d = 11`).  All counts are derived from `hIsoprop` (twins have no degree-`3` neighbour) without the
...
/-- **Generalised residual structural counts (`n = 20`, `P₄` cherry).**  From the path/`Iso`
description of the degree-`3` set `D` (`|D| = d`), the cherry incidence data, and the handshake
edge count, derive the hub count `20 − d`, the twin count `d − 4`, the per-vertex path/iso/internal
hub-incidence totals (`6`, `3·(d − 4)`, `78 − 6·d`), and the per-hub degree split. -/
theorem cherry_p4_hub_struct_twenty (G : SimpleGraph (Fin 20))
    (hm : G.edgeFinset.card = 36)
    ...
    Dᶜ.card = 20 - D.card ∧ Iso.card = D.card - 4 ∧
      ...
      (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Dᶜ).card = 78 - 6 * D.card) ∧
      (∀ g : Fin 20, g ∈ Dᶜ →
  ...
  have hisoND : ∀ t : Fin 20, t ∈ Iso → (G.neighborFinset t ∩ D).card = 0 := by
    intro t ht
    rw [Finset.card_eq_zero, Finset.eq_empty_iff_forall_notMem]
    intro a ha
    rw [Finset.mem_inter, G.mem_neighborFinset] at ha
    exact (hIsoprop t ht).2 a ha.1 ((hmemD a).mp ha.2)
  have hisohub : ∀ t : Fin 20, t ∈ Iso → (G.neighborFinset t ∩ Dᶜ).card = 3 := by
    intro t ht
    have := hsplit t
  ...
  have hDccard : Dᶜ.card = 20 - D.card := by
  ...
  have hsumInternal : ∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Dᶜ).card = 78 - 6 * D.card := by
