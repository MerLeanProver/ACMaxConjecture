...
/-!
# The `|D| = 12`, `s = 6` cherry-`P₄` boundary corner (`n = 20`)

The `P₄` sibling of `TwinCert20CherryP3D12` (`e(M) = 3`, path
`L₁–c₁–c₂–L₂`).  At `n = 20`, `|D| = 12`: `|J| = 8` `M`-isolated twins with
`24` hub incidences on `8` hubs of total degree `36`; the `P₄` carries
`2+1+1+2 = 6` hub slots, so the identity gives `∑_Hub hubadj = 6`
(`e_{HH} = 3`, unlike the `n = 19` analogue with `e_{HH} = 1`).  A degree-`≤ 5`
hub with two twins avoiding one of the induced `P₃`s (`L₁–c₁–c₂` or
`c₁–c₂–L₂`) yields `TwoTwinConfig`; the complementary world is **infeasible**.
A blocked twin-rich hub touches `c₁` or `c₂` (supply `1 + 1`) or both leaves
(`2` slots), so `slots(W) ≥ 2|W| − 2`, and `W ⊆ N(L₁) ∪ N(c₁) ∪ N(c₂)` forces
`|W| ≤ 4`; the twin count then pins `|W| = 4`, `|A| = 2` with the two
degree-`≤ 5` non-`W` hubs slot-free, each with `≥ 3` hub-neighbours.  But
`∑_Hub e = 6`, and two hubs of hub-degree `≥ 3` force `∑_Hub e ≥ 8` —
impossible.
-/
...
theorem cherry_p4_config_D12_twenty (G : SimpleGraph (Fin 20))
  ...
  have hHubeqDc : Hub = Dᶜ := by
    ...
    have := h3 u
    omega
  have hHubdeg : ∑ v ∈ Hub, G.degree v = 36 := by
    rw [hHubeqDc]
    omega
  ...
  · exfalso
    push Not at hgood
    set Hub5 : Finset (Fin 20) := Hub.filter (fun k => G.degree k ≤ 5) with hHub5def
    set H6 : Finset (Fin 20) := Hub.filter (fun k => ¬G.degree k ≤ 5) with hH6def
    set W : Finset (Fin 20) :=
    ...
    have hsd1 : ∑ v ∈ Hub5, G.degree v + ∑ v ∈ H6, G.degree v = 36 := by
      rw [hHub5def, hH6def, Finset.sum_filter_add_sum_filter_not Hub _ _]
      exact hHubdeg
    ...
    set C : Finset (Fin 20) := W.filter (fun k => G.Adj k c₁ ∨ G.Adj k c₂) with hCdef
    set NC : Finset (Fin 20) := W.filter (fun k => ¬(G.Adj k c₁ ∨ G.Adj k c₂)) with hNCdef
    ...
    have hWB6 : ∑ v ∈ W, (G.neighborFinset v ∩ B).card ≤ 6 := by
      calc ∑ v ∈ W, (G.neighborFinset v ∩ B).card
        ...
        _ = 6 := hcrossB
    -- hub–hub incidences: `∑_Hub e = 36 − 24 − 6 = 6`, via the `D = J ⊔ B` split.
    ...
    have hperhub : ∀ h : Fin 20, G.degree h
      ...
      omega
    have hHubHubsum : ∑ h ∈ Hub, (G.neighborFinset h ∩ Hub).card = 6 := by
      ...
      omega
    have hSBsplit : ∑ v ∈ W, (G.neighborFinset v ∩ B).card
        + ∑ v ∈ A, (G.neighborFinset v ∩ B).card ≤ 6 := by
      have hWAsub : W ∪ A ⊆ Hub := by
        intro k hk
        rw [Finset.mem_union] at hk
        rcases hk with hk | hk
        · exact ((hmemW k).mp hk).1
        · exact ((hmemA k).mp hk).1
      have hWAdisj : Disjoint W A :=
        Finset.disjoint_left.mpr (fun k hkW hkA => by
          have h1 := ((hmemW k).mp hkW).2.2
          have h2 := ((hmemA k).mp hkA).2.2
          omega)
      have h := Finset.sum_le_sum_of_subset
        (f := fun h => (G.neighborFinset h ∩ B).card) hWAsub
      rwa [Finset.sum_union hWAdisj, hcrossB] at h
    have hWub : W.card ≤ 4 := by
      have hsub : W ⊆ (G.neighborFinset L₁ ∩ Hub) ∪ (G.neighborFinset c₁ ∩ Hub)
          ∪ (G.neighborFinset c₂ ∩ Hub) := by
        intro k hk
        obtain ⟨hkHub, hk5, hk2J⟩ := (hmemW k).mp hk
        obtain ⟨hz1, _⟩ := hgood k hkHub hk5 hk2J
        obtain ⟨u, hu⟩ := Finset.card_pos.mp (Nat.pos_of_ne_zero hz1)
        rw [Finset.mem_inter] at hu
        obtain ⟨huN, huT⟩ := hu
        simp only [Finset.mem_insert, Finset.mem_singleton] at huT
        have hkadj : G.Adj k u := (G.mem_neighborFinset _ _).mp huN
        simp only [Finset.mem_union]
        rcases huT with rfl | rfl | rfl
        · exact Or.inl (Or.inl (Finset.mem_inter.mpr
            ⟨(G.mem_neighborFinset _ _).mpr hkadj.symm, hkHub⟩))
        · exact Or.inl (Or.inr (Finset.mem_inter.mpr
            ⟨(G.mem_neighborFinset _ _).mpr hkadj.symm, hkHub⟩))
        · exact Or.inr (Finset.mem_inter.mpr
            ⟨(G.mem_neighborFinset _ _).mpr hkadj.symm, hkHub⟩)
      calc W.card ≤ ((G.neighborFinset L₁ ∩ Hub) ∪ (G.neighborFinset c₁ ∩ Hub)
              ∪ (G.neighborFinset c₂ ∩ Hub)).card := Finset.card_le_card hsub
        _ ≤ (G.neighborFinset L₁ ∩ Hub).card + (G.neighborFinset c₁ ∩ Hub).card
              + (G.neighborFinset c₂ ∩ Hub).card := by
            refine (Finset.card_union_le _ _).trans ?_
            exact Nat.add_le_add_right (Finset.card_union_le _ _) _
        _ = 4 := by rw [hL1Hub, hc1Hub, hc2Hub]
    -- realizability: two hubs with `e ≥ 3` push `∑_Hub e ≥ 8 > 6`.
    have htwohigh : ∀ p q : Fin 20, p ∈ Hub → q ∈ Hub → p ≠ q →
        3 ≤ (G.neighborFinset p ∩ Hub).card → 3 ≤ (G.neighborFinset q ∩ Hub).card → False := by
      intro p q hp hq hpq hpe hqe
      have hnbr1 : ∀ h ∈ G.neighborFinset p ∩ Hub, 1 ≤ (G.neighborFinset h ∩ Hub).card := by
        intro h hh
        obtain ⟨hhN, _⟩ := Finset.mem_inter.mp hh
        have hadj : G.Adj h p := ((G.mem_neighborFinset _ _).mp hhN).symm
        exact Finset.card_pos.mpr ⟨p, Finset.mem_inter.mpr
          ⟨(G.mem_neighborFinset _ _).mpr hadj, hp⟩⟩
      have hpnotin : p ∉ G.neighborFinset p ∩ Hub := fun hmem =>
        G.irrefl ((G.mem_neighborFinset _ _).mp (Finset.mem_inter.mp hmem).1)
      have hins_sub : insert p (G.neighborFinset p ∩ Hub) ⊆ Hub := by
        intro h hh
        rw [Finset.mem_insert] at hh
        rcases hh with rfl | hh
        · exact hp
        · exact (Finset.mem_inter.mp hh).2
      have hbig : (G.neighborFinset p ∩ Hub).card
          + ∑ h ∈ G.neighborFinset p ∩ Hub, (G.neighborFinset h ∩ Hub).card
          ≤ ∑ h ∈ Hub, (G.neighborFinset h ∩ Hub).card := by
        have h := Finset.sum_le_sum_of_subset
          (f := fun h => (G.neighborFinset h ∩ Hub).card) hins_sub
        rwa [Finset.sum_insert hpnotin] at h
      by_cases hqin : q ∈ G.neighborFinset p ∩ Hub
      · have hsplit : (G.neighborFinset q ∩ Hub).card
            + ∑ h ∈ (G.neighborFinset p ∩ Hub).erase q, (G.neighborFinset h ∩ Hub).card
            = ∑ h ∈ G.neighborFinset p ∩ Hub, (G.neighborFinset h ∩ Hub).card :=
          Finset.add_sum_erase (G.neighborFinset p ∩ Hub)
            (fun h => (G.neighborFinset h ∩ Hub).card) hqin
        have herasege : ((G.neighborFinset p ∩ Hub).erase q).card
            ≤ ∑ h ∈ (G.neighborFinset p ∩ Hub).erase q, (G.neighborFinset h ∩ Hub).card := by
          calc ((G.neighborFinset p ∩ Hub).erase q).card
              = ∑ _h ∈ (G.neighborFinset p ∩ Hub).erase q, 1 := by
                rw [Finset.sum_const, smul_eq_mul, mul_one]
            _ ≤ _ := Finset.sum_le_sum (fun h hh => hnbr1 h (Finset.mem_of_mem_erase hh))
        have hcarderase : ((G.neighborFinset p ∩ Hub).erase q).card
            = (G.neighborFinset p ∩ Hub).card - 1 := Finset.card_erase_of_mem hqin
        rw [hHubHubsum] at hbig
        omega
      · have hins2_sub : insert q (insert p (G.neighborFinset p ∩ Hub)) ⊆ Hub := by
          intro h hh
          rw [Finset.mem_insert] at hh
          rcases hh with rfl | hh
          · exact hq
          · exact hins_sub hh
        have hqnotin : q ∉ insert p (G.neighborFinset p ∩ Hub) := by
          simp only [Finset.mem_insert, not_or]
          exact ⟨Ne.symm hpq, hqin⟩
        have hnbrsum : (G.neighborFinset p ∩ Hub).card
            ≤ ∑ h ∈ G.neighborFinset p ∩ Hub, (G.neighborFinset h ∩ Hub).card := by
          calc (G.neighborFinset p ∩ Hub).card
              = ∑ _h ∈ G.neighborFinset p ∩ Hub, 1 := by
                rw [Finset.sum_const, smul_eq_mul, mul_one]
            _ ≤ _ := Finset.sum_le_sum hnbr1
        have hbig2 : (G.neighborFinset q ∩ Hub).card
            + ((G.neighborFinset p ∩ Hub).card
              + ∑ h ∈ G.neighborFinset p ∩ Hub, (G.neighborFinset h ∩ Hub).card)
            ≤ ∑ h ∈ Hub, (G.neighborFinset h ∩ Hub).card := by
          have h := Finset.sum_le_sum_of_subset
            (f := fun h => (G.neighborFinset h ∩ Hub).card) hins2_sub
          rwa [Finset.sum_insert hqnotin, Finset.sum_insert hpnotin] at h
        rw [hHubHubsum] at hbig2
        omega
    -- the LP forces `|A| = 2` and `∑_A |N ∩ B| = 0`: two slot-free poor hubs with `e ≥ 3`.
    have ha2 : A.card = 2 := by omega
    have hSBA0 : ∑ v ∈ A, (G.neighborFinset v ∩ B).card = 0 := by omega
    obtain ⟨α, β, hαβ, hAeq⟩ := Finset.card_eq_two.mp ha2
    have hαA : α ∈ A := by rw [hAeq]; simp
    have hβA : β ∈ A := by rw [hAeq]; simp
    have hAe3 : ∀ k ∈ A, 3 ≤ (G.neighborFinset k ∩ Hub).card := by
      intro k hk
      obtain ⟨hkHub, hk5, hkj⟩ := (hmemA k).mp hk
      have hk4 : 4 ≤ G.degree k := (hmemHub k).mp hkHub
      have hkB0 : (G.neighborFinset k ∩ B).card = 0 :=
        Finset.sum_eq_zero_iff.mp hSBA0 k hk
      have hph := hperhub k
      omega
    exact htwohigh α β ((hmemA α).mp hαA).1 ((hmemA β).mp hβA).1 hαβ (hAe3 α hαA) (hAe3 β hβA)
