...
/-!
# Reusable cross-edge `C₄` helpers for the `r = 5` octahedron residual (`n = 20`)

The single combinatorial engine these lemmas package is the **`M`-partner good-`C₄`**: in the rigid
`e(M) = 1`, all-degree-`4`, `(|Hub|, |Iso|) = (12, 6)` profile the two `M`-edge endpoints
`Z = univ \ (Hub ∪ Iso)` are `z, z'` with `z ∼ z'`, each of degree `3` meeting exactly two hubs and
no twin (`z_two_hub_nbrs_twenty`), and no hub meets both (`no_hub_adj_both_mends_twenty`).
...
These facts are derived purely from the regime hypotheses available to `r5_resid_twenty`; they do
...
`z, z' ∈ Z` distinct, and `z` meeting exactly two hubs and no twin (`z_two_hub_nbrs_twenty`), the
three-way split leaves `z` exactly one `Z`-neighbour, which—`|Z| = 2`—must be `z'`. -/
theorem mends_adj_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
  ...
  have hzZ1 : (G.neighborFinset z ∩ Z).card = 1 := by
    have hsp := nbr_split_three_twenty G Hub Iso hdisj z
    rw [← hZdef] at hsp
    obtain ⟨h0, h2, hd⟩ :=
      z_two_hub_nbrs_twenty G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak z hz
    omega
...
would make hub `g` meet both — each excluded by `no_hub_adj_both_mends_twenty`. -/
theorem cross_z_hub_c4_false_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
  ...
  have hzz'adj : G.Adj z z' :=
    mends_adj_twenty G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak z hz z' hz' hzz'
  -- Diagonal non-adjacencies via `no_hub_adj_both_mends_twenty`.
  have hnzg' : ¬G.Adj z g' := by
    intro hadj
    exact no_hub_adj_both_mends_twenty G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3
  ...
  have hcard4 : ({z, g, g', z'} : Finset (Fin 20)).card = 4 := by
...
theorem exists_two_private_twins_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hshare : ∀ h₁ ∈ Hub, G.degree h₁ = 4 → ∀ h₂ ∈ Hub, G.degree h₂ = 4 → h₁ ≠ h₂ →
      ¬G.Adj h₁ h₂ → (G.neighborFinset h₁ ∩ G.neighborFinset h₂ ∩ Iso).card ≤ 1)
    (h₁ X : Fin 20) (hh₁Hub : h₁ ∈ Hub) (hXHub : X ∈ Hub)
    (hdh₁ : G.degree h₁ = 4) (hdX : G.degree X = 4) (hne : h₁ ≠ X) (hnadj : ¬G.Adj h₁ X)
    (htw : 3 ≤ (G.neighborFinset h₁ ∩ Iso).card) :
    ∃ a b : Fin 20, a ∈ Iso ∧ b ∈ Iso ∧ a ≠ b ∧
...
theorem exists_one_private_twin_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hshare : ∀ h₁ ∈ Hub, G.degree h₁ = 4 → ∀ h₂ ∈ Hub, G.degree h₂ = 4 → h₁ ≠ h₂ →
      ¬G.Adj h₁ h₂ → (G.neighborFinset h₁ ∩ G.neighborFinset h₂ ∩ Iso).card ≤ 1)
    (h₂ X : Fin 20) (hh₂Hub : h₂ ∈ Hub) (hXHub : X ∈ Hub)
    (hdh₂ : G.degree h₂ = 4) (hdX : G.degree X = 4) (hne : X ≠ h₂) (hnadj : ¬G.Adj X h₂)
    (htw : 2 ≤ (G.neighborFinset h₂ ∩ Iso).card) :
    ∃ c : Fin 20, c ∈ Iso ∧ G.Adj c h₂ ∧ ¬G.Adj X c := by
  classical
...
theorem two_hub_zleaf_twenty (G : SimpleGraph (Fin 20)) (Hub Iso : Finset (Fin 20))
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 12) (hIso : Iso.card = 6)
    (hdsum : ∑ w ∈ Hub, G.degree w = 48)
    (hdeg3 : ∀ v : Fin 20, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
