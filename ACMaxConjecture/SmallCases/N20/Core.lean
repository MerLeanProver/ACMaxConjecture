...
/-!
# Structural / regime core lemmas for the `n = 20` twin certificate

Ported from the proved `n = 19` development (`TwinCert19Core`).
In the `n = 20` residual the graph has `36` edges, minimum degree `3`, and
(by `residual_hub_card_le_eleven`) the hub set `Hub = filter (4 ≤ deg)` has `Hub.card ≤ 12` while the
degree-3 set `D = filter (deg = 3)` has `8 ≤ D.card`.  The total degree-excess over `3` is
`72 − 60 = 12`, giving the handshake `e(M) = 24 − 3·|Hub| + e_H` and `|Hub| ∈ {6, 7, …, 12}`.

The good-`C₄` residual threshold is `≤ 14` for `n = 20` (same as `n = 19`), so the `hC4`
...
`residual_hub_card_le_eleven` (handshake constants) and `isolated_twin_two_deg4_hubs` (which acquires
an `11 ≤ |Hub|` hypothesis: the degree-excess `12` means the contradiction needs `|Dᶜ| ≥ 11`, so at
`|Hub| = 10` a twin may still meet two degree-`5` hubs — strictly stronger than the `n = 19` need).
-/
...
theorem card_four_twenty (a b c d : Fin 20)
    (hab : a ≠ b) (hac : a ≠ c) (had : a ≠ d) (hbc : b ≠ c) (hbd : b ≠ d) (hcd : c ≠ d) :
    ({a, b, c, d} : Finset (Fin 20)).card = 4 := by
...
theorem d3_dominating (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20))
    ...
    (v a b c : Fin 20) (hvD : v ∈ D) (haD : a ∈ D) (hbD : b ∈ D) (hcD : c ∈ D)
    (hva : G.Adj v a) (hvb : G.Adj v b) (hvc : G.Adj v c)
    (hab : a ≠ b) (hac : a ≠ c) (hbc : b ≠ c)
    (hcharv : ∀ w : Fin 20, G.Adj v w → w ∈ D → w = a ∨ w = b ∨ w = c) :
    ∃ c₁ c₂ : Fin 20, c₁ ∈ D ∧ c₂ ∈ D ∧ G.Adj c₁ c₂ ∧
      ∀ p q : Fin 20, p ∈ D → q ∈ D → G.Adj p q →
        p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂ := by
  classical
  have hdeg3 : ∀ x : Fin 20, x ∈ D → G.degree x = 3 := fun x hx => (hmemD x).mp hx
  have tri : ∀ x y z : Fin 20, x ∈ D → y ∈ D → z ∈ D → x ≠ y → y ≠ z → x ≠ z →
  ...
  have c4 : ∀ x y z w : Fin 20, x ∈ D → y ∈ D → z ∈ D → w ∈ D →
      ({x, y, z, w} : Finset (Fin 20)).card = 4 →
  ...
  have k2 : ∀ x y z w : Fin 20, x ∈ D → y ∈ D → z ∈ D → w ∈ D →
      ({x, y, z, w} : Finset (Fin 20)).card = 4 →
  ...
  have hit4 : ∀ p q : Fin 20, p ∈ D → q ∈ D → G.Adj p q →
      p ∈ ({v, a, b, c} : Finset (Fin 20)) ∨ q ∈ ({v, a, b, c} : Finset (Fin 20)) := by
    intro p q hp hq hpq
    by_contra hcon
    push Not at hcon
    ...
    have notwo : ∀ w : Fin 20, w ∈ D → w ≠ v → w ≠ a → w ≠ b → w ≠ c →
      ...
      · rintro ⟨haw, hbw⟩
        exact c4 v a w b hvD haD hwD hbD
          (card_four_twenty v a w b hvna (Ne.symm hwv) hvnb (Ne.symm hwa) hab hwb)
          hva haw hbw.symm hvb.symm hnvw hnab
      · rintro ⟨haw, hcw⟩
        exact c4 v a w c hvD haD hwD hcD
          (card_four_twenty v a w c hvna (Ne.symm hwv) hvnc (Ne.symm hwa) hac hwc)
          hva haw hcw.symm hvc.symm hnvw hnac
      · rintro ⟨hbw, hcw⟩
        exact c4 v b w c hvD hbD hwD hcD
          (card_four_twenty v b w c hvnb (Ne.symm hwv) hvnc (Ne.symm hwb) hbc hwc)
    ...
    have hgeta : G.Adj a p ∨ G.Adj a q := by
      ...
      exact k2 v a p q hvD haD hp hq
        (card_four_twenty v a p q hvna (Ne.symm hpv) (Ne.symm hqv) (Ne.symm hpa)
          (Ne.symm hqa) hpq.ne)
        hva hpq hnvp hnvq hh.1 hh.2
    have hgetb : G.Adj b p ∨ G.Adj b q := by
      by_contra hh; push Not at hh
      exact k2 v b p q hvD hbD hp hq
        (card_four_twenty v b p q hvnb (Ne.symm hpv) (Ne.symm hqv) (Ne.symm hpb)
          (Ne.symm hqb) hpq.ne)
        hvb hpq hnvp hnvq hh.1 hh.2
    have hgetc : G.Adj c p ∨ G.Adj c q := by
      by_contra hh; push Not at hh
      exact k2 v c p q hvD hcD hp hq
        (card_four_twenty v c p q hvnc (Ne.symm hpv) (Ne.symm hqv) (Ne.symm hpc)
          (Ne.symm hqc) hpq.ne)
  ...
  have key : ∀ s t r : Fin 20, s ∈ D → t ∈ D → r ∈ D → s ≠ t → s ≠ r → t ≠ r →
      G.Adj v s → G.Adj v t → G.Adj v r →
      (∀ w : Fin 20, G.Adj v w → w ∈ D → w = s ∨ w = t ∨ w = r) →
...
theorem dominating_edge_or_induced_C5 (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20))
    (hmemD : ∀ v : Fin 20, v ∈ D ↔ G.degree v = 3)
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
    ...
    (h2k2 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
      G.degree a = 3 ∧ G.degree b = 3 ∧ G.degree c = 3 ∧ G.degree d = 3 ∧
      G.Adj a b ∧ G.Adj c d ∧ ¬G.Adj a c ∧ ¬G.Adj a d ∧ ¬G.Adj b c ∧ ¬G.Adj b d)
    (hne : ∃ a b : Fin 20, a ∈ D ∧ b ∈ D ∧ G.Adj a b) :
    (∃ c₁ c₂ : Fin 20, c₁ ∈ D ∧ c₂ ∈ D ∧ G.Adj c₁ c₂ ∧
      ∀ p q : Fin 20, p ∈ D → q ∈ D → G.Adj p q →
        p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
    ∨ (∃ v₁ v₂ v₃ v₄ v₅ : Fin 20,
        v₁ ∈ D ∧ v₂ ∈ D ∧ v₃ ∈ D ∧ v₄ ∈ D ∧ v₅ ∈ D ∧
        ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 20)).card = 5 ∧
  ...
  by_cases hd3 : ∃ v : Fin 20, v ∈ D ∧ 3 ≤ (G.neighborFinset v ∩ D).card
  · obtain ⟨v, hvD, hv3⟩ := hd3
    ...
    have hmem : ∀ w : Fin 20, w ∈ ({a, b, c} : Finset (Fin 20)) →
        G.Adj v w ∧ w ∈ D := by
      intro w hw
    ...
    have hcharv : ∀ w : Fin 20, G.Adj v w → w ∈ D → w = a ∨ w = b ∨ w = c := by
      intro w hvw hwD
  ...
  · -- All in-`M`-degrees are `≤ 2`.
    push Not at hd3
    have hle2 : ∀ v : Fin 20, v ∈ D → (G.neighborFinset v ∩ D).card ≤ 2 := by
      intro v hv; have := hd3 v hv; omega
    obtain ⟨a0, b0, ha0, hb0, hab0⟩ := hne
    by_cases h2 : ∃ y : Fin 20, y ∈ D ∧ (G.neighborFinset y ∩ D).card = 2
    · obtain ⟨y, hyD, hy2⟩ := h2
      ...
      have hchary : ∀ w : Fin 20, G.Adj y w → w ∈ D → w = x ∨ w = z := by
        intro w hyw hwD
      ...
      have dom_gen : ∀ n1 n2 : Fin 20, n1 ∈ D → n2 ∈ D → n1 ≠ n2 → G.Adj y n1 → G.Adj y n2 →
          (∀ w : Fin 20, G.Adj y w → w ∈ D → w = n1 ∨ w = n2) →
          (∀ w : Fin 20, G.Adj n2 w → w ∈ D → w = y) →
          ∃ c₁ c₂ : Fin 20, c₁ ∈ D ∧ c₂ ∈ D ∧ G.Adj c₁ c₂ ∧
            ∀ p q : Fin 20, p ∈ D → q ∈ D → G.Adj p q →
              p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂ := by
        ...
        exact k2 n2 y p q hn2 hyD hp hq
          (card_four_twenty n2 y p q (G.ne_of_adj hyn2).symm (Ne.symm hpn2) (Ne.symm hqn2)
            (Ne.symm hpy) (Ne.symm hqy) hpq.ne)
          hyn2.symm hpq hn2p hn2q hyp hyq
      by_cases hze : ∃ w : Fin 20, w ∈ D ∧ w ≠ y ∧ G.Adj z w
      · by_cases hxe : ∃ w : Fin 20, w ∈ D ∧ w ≠ y ∧ G.Adj x w
        · -- Both siblings have an extra neighbour: an induced `C₅` forms.
          ...
          have hxfull : G.neighborFinset x ∩ D = ({y, x'} : Finset (Fin 20)) := by
            have hsub : ({y, x'} : Finset (Fin 20)) ⊆ G.neighborFinset x ∩ D := by
            ...
            have hc2 : ({y, x'} : Finset (Fin 20)).card = 2 := by
          ...
          have hcharx : ∀ w : Fin 20, G.Adj x w → w ∈ D → w = y ∨ w = x' := by
            intro w hxw hwD
            have : w ∈ G.neighborFinset x ∩ D :=
              Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hxw, hwD⟩
            rw [hxfull] at this; simpa using this
          have hzfull : G.neighborFinset z ∩ D = ({y, z'} : Finset (Fin 20)) := by
            have hsub : ({y, z'} : Finset (Fin 20)) ⊆ G.neighborFinset z ∩ D := by
              intro w hw; simp only [Finset.mem_insert, Finset.mem_singleton] at hw
              rcases hw with rfl | rfl
            ...
            have hc2 : ({y, z'} : Finset (Fin 20)).card = 2 := by
          ...
          have hcharz : ∀ w : Fin 20, G.Adj z w → w ∈ D → w = y ∨ w = z' := by
            intro w hzw hwD
          ...
          have hx'z' : x' ≠ z' := by
            ...
            exact c4 x x' z y hxD hx'D hzD hyD
              (card_four_twenty x x' z y (G.ne_of_adj hxx') hxz (Ne.symm hynx) hx'z hx'y
                (Ne.symm hynz))
          ...
          have hadjx'z' : G.Adj x' z' := by
            ...
            exact k2 x x' z z' hxD hx'D hzD hz'D
              (card_four_twenty x x' z z' (G.ne_of_adj hxx') hxz hxz' hx'z hx'z'
                (G.ne_of_adj hzz'))
              hxx' hzz' hnxz hnxz' hnx'z hn
          -- Saturated neighbourhoods of `x'` and `z'` (closing the `C₅`).
          have hx'full : G.neighborFinset x' ∩ D = ({x, z'} : Finset (Fin 20)) := by
            have hsub : ({x, z'} : Finset (Fin 20)) ⊆ G.neighborFinset x' ∩ D := by
              intro w hw; simp only [Finset.mem_insert, Finset.mem_singleton] at hw
              rcases hw with rfl | rfl
            ...
            have hc2 : ({x, z'} : Finset (Fin 20)).card = 2 := by
          ...
          have hcharx' : ∀ w : Fin 20, G.Adj x' w → w ∈ D → w = x ∨ w = z' := by
            intro w hx'w hwD
            have : w ∈ G.neighborFinset x' ∩ D :=
              Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hx'w, hwD⟩
            rw [hx'full] at this; simpa using this
          have hz'full : G.neighborFinset z' ∩ D = ({z, x'} : Finset (Fin 20)) := by
            have hsub : ({z, x'} : Finset (Fin 20)) ⊆ G.neighborFinset z' ∩ D := by
              intro w hw; simp only [Finset.mem_insert, Finset.mem_singleton] at hw
              rcases hw with rfl | rfl
            ...
            have hc2 : ({z, x'} : Finset (Fin 20)).card = 2 := by
          ...
          have hcharz' : ∀ w : Fin 20, G.Adj z' w → w ∈ D → w = z ∨ w = x' := by
            intro w hz'w hwD
            have : w ∈ G.neighborFinset z' ∩ D :=
          ...
          · exact card_five_twenty x y z z' x' (Ne.symm hynx) hxz hxz' (G.ne_of_adj hxx')
          ...
          · -- Every `M`-edge lies inside the five-vertex `C₅`.
            ...
            exact k2 x y p q hxD hyD hp hq
              (card_four_twenty x y p q (Ne.symm hynx) (Ne.symm hpx) (Ne.symm hqx)
                (Ne.symm hpy) (Ne.symm hqy) hpq.ne)
    ...
    · -- No in-`M`-degree-2 vertex: `M` is a single edge.
      have hle1 : ∀ v : Fin 20, v ∈ D → (G.neighborFinset v ∩ D).card ≤ 1 := by
        intro v hv
        rcases Nat.lt_or_ge ((G.neighborFinset v ∩ D).card) 2 with h | h
        · omega
        · exact absurd ⟨v, hv, le_antisymm (hle2 v hv) h⟩ h2
      have ha0full : G.neighborFinset a0 ∩ D = ({b0} : Finset (Fin 20)) := by
        have hsub : ({b0} : Finset (Fin 20)) ⊆ G.neighborFinset a0 ∩ D := by
      ...
      have hb0full : G.neighborFinset b0 ∩ D = ({a0} : Finset (Fin 20)) := by
        have hsub : ({a0} : Finset (Fin 20)) ⊆ G.neighborFinset b0 ∩ D := by
          intro w hw; rw [Finset.mem_singleton] at hw; subst hw
          exact Finset.mem_inter.mpr ⟨(G.mem_neighborFinset _ _).mpr hab0.symm, ha0⟩
        exact (Finset.eq_of_subset_of_card_le hsub
          (by rw [Finset.card_singleton]; exact hle1 b0 hb0)).symm
      have hchara0 : ∀ w : Fin 20, G.Adj a0 w → w ∈ D → w = b0 := by
        intro w haw hwD
      ...
      have hcharb0 : ∀ w : Fin 20, G.Adj b0 w → w ∈ D → w = a0 := by
        intro w hbw hwD
      ...
      exact k2 a0 b0 p q ha0 hb0 hp hq
        (card_four_twenty a0 b0 p q hab0.ne (Ne.symm hpa0) (Ne.symm hqa0) (Ne.symm hpb0)
          (Ne.symm hqb0) hpq.ne)
...
theorem cross_count_twenty (G : SimpleGraph (Fin 20)) (X Y : Finset (Fin 20)) :
    ∑ v ∈ X, (G.neighborFinset v ∩ Y).card = ∑ w ∈ Y, (G.neighborFinset w ∩ X).card := by
  have hL : ∀ v : Fin 20, (G.neighborFinset v ∩ Y).card
  ...
  have hR : ∀ w : Fin 20, (G.neighborFinset w ∩ X).card
...
theorem eM_le_five (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20))
    (hmemD : ∀ v : Fin 20, v ∈ D ↔ G.degree v = 3)
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
  ...
  have hindle : ∀ x : Fin 20, x ∈ D → (G.neighborFinset x ∩ D).card ≤ 3 := by
    ...
    calc (G.neighborFinset x ∩ D).card ≤ (G.neighborFinset x).card :=
      ...
      _ = G.degree x := G.card_neighborFinset_eq_degree x
      _ = 3 := (hmemD x).mp hx
  by_cases hne : ∃ a b : Fin 20, a ∈ D ∧ b ∈ D ∧ G.Adj a b
  · rcases dominating_edge_or_induced_C5 G D hmemD hT hC4 h2k2 hne with hA | hB
    · -- **Dominating edge.**
      ...
      have hpair2 : ∑ w ∈ ({c₁, c₂} : Finset (Fin 20)),
        ...
        rw [Finset.sum_insert (by simp [hc12.ne]), Finset.sum_singleton]
      have hb1 : (G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 20)))).card ≤ 2 := by
        have hss : G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 20)))
        ...
        calc (G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 20)))).card
      ...
      have hb2 : (G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 20)))).card ≤ 2 := by
        have hss : G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 20)))
            ⊆ (G.neighborFinset c₂ ∩ D).erase c₁ := by
          intro w hw
          obtain ⟨hwN, hwS⟩ := Finset.mem_inter.mp hw
    ...
    · -- **Induced `C₅` carrying all edges.**
      obtain ⟨v₁, v₂, v₃, v₄, v₅, hv1, hv2, hv3, hv4, hv5, hcard5,
        e12, e23, e36, e45, e51, n13, n14, n24, n25, n35, hcov⟩ := hB
      have nbhd : ∀ u na nb o1 o2 : Fin 20, ¬G.Adj u o1 → ¬G.Adj u o2 →
          (∀ w : Fin 20, w ∈ D → G.Adj u w →
            w = u ∨ w = na ∨ w = nb ∨ w = o1 ∨ w = o2) →
          (G.neighborFinset u ∩ D).card ≤ 2 := by
        intro u na nb o1 o2 hno1 hno2 hucov
        have hss : G.neighborFinset u ∩ D ⊆ ({na, nb} : Finset (Fin 20)) := by
          intro w hw
          obtain ⟨hwN, hwD⟩ := Finset.mem_inter.mp hw
        ...
        calc (G.neighborFinset u ∩ D).card ≤ ({na, nb} : Finset (Fin 20)).card :=
              Finset.card_le_card hss
          _ ≤ 2 := (Finset.card_insert_le _ _).trans (by simp)
      have hbound : ∀ v ∈ ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 20)),
      ...
      have hC5sub : ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 20)) ⊆ D := by
        intro w hw; simp only [Finset.mem_insert, Finset.mem_singleton] at hw
        rcases hw with rfl | rfl | rfl | rfl | rfl
        exacts [hv1, hv2, hv3, hv4, hv5]
      have hsplit :
          ∑ v ∈ D \ ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 20)), (G.neighborFinset v ∩ D).card
            + ∑ v ∈ ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 20)), (G.neighborFinset v ∩ D).card
          = ∑ v ∈ D, (G.neighborFinset v ∩ D).card :=
        Finset.sum_sdiff hC5sub
      have hiso : ∑ v ∈ D \ ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 20)),
      ...
      have hC5sum : ∑ v ∈ ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 20)),
  ...
  · -- **No edge.**
    push Not at hne
    have hzero : ∀ v : Fin 20, v ∈ D → (G.neighborFinset v ∩ D).card = 0 := by
      intro v hv
      rw [Finset.card_eq_zero, Finset.eq_empty_iff_forall_notMem]
      intro w hw
...
/-- **Degree-partition bound for the `n = 20` residual (fully proved).**  With `36` edges and
minimum degree `3` on `20` vertices, the hub set `Hub = filter (4 ≤ deg)` has `Hub.card ≤ 12` and
the degree-3 set `D = filter (deg = 3)` has `8 ≤ D.card` (handshake:
`4·|Hub| ≤ ∑_{Hub} deg = 12 + 3·|Hub|`). -/
theorem residual_hub_card_le_eleven (G : SimpleGraph (Fin 20)) (hm : G.edgeFinset.card = 36)
    (h3 : ∀ v : Fin 20, 3 ≤ G.degree v) :
    (Finset.univ.filter (fun v => 4 ≤ G.degree v)).card ≤ 12 ∧
  ...
  have hcard18 : D.card + Hub.card = 20 := by
...
theorem eM_even (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20)) :
    Even (∑ v ∈ D, (G.neighborFinset v ∩ D).card) := by
  classical
  set M : SimpleGraph (Fin 20) :=
    { Adj := fun a b => G.Adj a b ∧ a ∈ D ∧ b ∈ D
      symm := ⟨fun a b h => ⟨h.1.symm, h.2.2, h.2.1⟩⟩
      loopless := ⟨fun a h => G.irrefl h.1⟩ } with hMdef
  have hMadj : ∀ a b : Fin 20, M.Adj a b ↔ G.Adj a b ∧ a ∈ D ∧ b ∈ D := fun _ _ => Iff.rfl
  have hMdeg : ∀ v : Fin 20,
    ...
    · simp only [hvD, if_false]
      ...
      exact hvD hw.2.1
  have hsumM : ∑ v : Fin 20, M.degree v = ∑ v ∈ D, (G.neighborFinset v ∩ D).card := by
    simp_rw [hMdeg]
...
`(N t ∩ Hub).card = 3`. -/
theorem each_iso_three_hubs (G : SimpleGraph (Fin 20)) (D Hub : Finset (Fin 20))
    (hmemD : ∀ v : Fin 20, v ∈ D ↔ G.degree v = 3)
    (hmemHub : ∀ v : Fin 20, v ∈ Hub ↔ 4 ≤ G.degree v)
    (h3 : ∀ v : Fin 20, 3 ≤ G.degree v)
    (t : Fin 20) (htD : t ∈ D) (htiso : (G.neighborFinset t ∩ D).card = 0) :
...
theorem nonisolated_component_bound (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20))
    (hmemD : ∀ v : Fin 20, v ∈ D ↔ G.degree v = 3)
    (h2k2 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
      G.degree a = 3 ∧ G.degree b = 3 ∧ G.degree c = 3 ∧ G.degree d = 3 ∧
      G.Adj a b ∧ G.Adj c d ∧ ¬G.Adj a c ∧ ¬G.Adj a d ∧ ¬G.Adj b c ∧ ¬G.Adj b d) :
  ...
  set M : SimpleGraph (Fin 20) :=
    { Adj := fun a b => G.Adj a b ∧ a ∈ D ∧ b ∈ D
      ...
      loopless := ⟨fun a h => G.irrefl h.1⟩ } with hMdef
  have hMadj : ∀ a b : Fin 20, M.Adj a b ↔ G.Adj a b ∧ a ∈ D ∧ b ∈ D := fun _ _ => Iff.rfl
  have hSmem : ∀ v : Fin 20, v ∈ S ↔ v ∈ D ∧ ∃ w : Fin 20, G.Adj v w ∧ w ∈ D := by
    intro v
  ...
  have hsupp : M.support = (↑S : Set (Fin 20)) := by
    ext v
  ...
  · have hVne : Nonempty ↥M.support := by
    ...
    have radj : ∀ {a b : Fin 20} (ha : a ∈ M.support) (hb : b ∈ M.support),
    ...
    have hpre : (M.induce M.support).Preconnected := by
      intro u v
      set a : Fin 20 := (↑u : Fin 20) with hadef
      set b : Fin 20 := (↑v : Fin 20) with hbdef
      obtain ⟨a', haa'⟩ := u.2
      obtain ⟨b', hbb'⟩ := v.2
      ...
      · have hcard4 : ({a, a', b, b'} : Finset (Fin 20)).card = 4 :=
          card_four_twenty a a' b b' haa'.ne hab hab' ha'b ha'b' hbb'.ne
...
theorem hub_meets_path_le_one_twenty (G : SimpleGraph (Fin 20))
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    {h x y z : Fin 20} (hh : G.degree h = 4)
  ...
  · rintro ⟨hhx, hhz⟩
    ...
    · exact hT ⟨h, x, y, hne_hx, hne_xy, hne_hy, hhx, hxy, hhy, by omega⟩
    · exact hC4 ⟨h, x, y, z,
        card_four_twenty h x y z hne_hx hne_hy hne_hz hne_xy hne_xz hne_yz,
        hhx, hxy, hyz, hhz.symm, hhy, hxz, by omega⟩

/-- **Non-adjacent hubs share at most one isolated twin (`n = 20`, good-`C₄` `Σ ≤ 14`).**  Two
...
degree sum `4 + 3 + 4 + 3 = 14 ≤ 14`, contradicting the `n = 20` good-`C₄` hypothesis `hC4`.  This
sharper `≤ 1` bound (a twin-hub-twin-hub `C₄` has `Σ = 14`, included at the `n = 20` good-`C₄`
threshold `≤ 14`); at a threshold `≤ 13` it would fail. -/
theorem nonadj_hubs_share_le_one_iso (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (hC4 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (hIsoD : Iso ⊆ D)
    ...
    (h₁ h₂ : Fin 20) (h1Dc : h₁ ∈ Dᶜ) (h2Dc : h₂ ∈ Dᶜ) (hne : h₁ ≠ h₂)
    (hnadj : ¬G.Adj h₁ h₂) :
  ...
  have unpack : ∀ t : Fin 20, t ∈ (G.neighborFinset h₁ ∩ G.neighborFinset h₂) ∩ Iso →
  ...
  have hti_ne : ∀ t : Fin 20, t ∈ Iso → h₁ ≠ t ∧ h₂ ≠ t := by
    intro t ht
