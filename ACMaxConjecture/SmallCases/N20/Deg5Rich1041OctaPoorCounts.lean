...
/-!
# The poor-layer counting foundation for the (11,7,45) octahedron residual (`n = 20`)

With the rich set `R := Hub.filter (deg 4 ∧ 2 ≤ isoDeg)` pinned to cardinality `5`
(`hR5ge` from the caller's `rich_count_1041_twenty` dichotomy + the inlined blocker
cover), the poor set `P := D4 ∖ R` has `5` members.  The deg-`5` hub `f` (isoDeg `5`)
carries `5` of the `21` `Iso`-incidences, so `Σ_D4 = 16 = Σ_R + Σ_P`.  Unlike
`n = 19` (`|D4| = 9`), the tenth degree-`4` hub gives the ledger one unit of slack:
either `Σ_P = 5` (all five poor hubs at isoDeg exactly `1`, `Σ_R = 11`) or
`Σ_P = 4` (one poor hub at isoDeg `0`, the other four at `1`, `Σ_R = 12`).
-/
...
/-- **The poor set `P` and the `Iso`-incidence split (disjunctive at `n = 20`).** -/
theorem octahedron_poor_counts_share2_1041_twenty (G : SimpleGraph (Fin 20))
    ...
    ((Hub.filter (fun h => G.degree h = 4 ∧ (G.neighborFinset h ∩ Iso).card ≤ 1)).card = 5 ∧
      (∃ h₀ ∈ Hub.filter (fun h => G.degree h = 4 ∧ (G.neighborFinset h ∩ Iso).card ≤ 1),
        ...
        ∀ w ∈ Hub.filter (fun h => G.degree h = 4 ∧ (G.neighborFinset h ∩ Iso).card ≤ 1) \
          ({h₀} : Finset (Fin 20)), (G.neighborFinset w ∩ Iso).card = 1) ∧
      (∑ r ∈ Hub.filter (fun h => G.degree h = 4 ∧ 2 ≤ (G.neighborFinset h ∩ Iso).card),
        (G.neighborFinset r ∩ Iso).card) = 12) := by
  classical
  -- === Degree partition: 10 deg-4 hubs, the given deg-5 hub `f`. ===
  ...
  -- === Rich lower bound `|R| ≥ 5`: the caller's `hR5ge` (left dichotomy branch). ===
  ...
  have hRle5 : (Hub.filter (fun h => G.degree h = 4 ∧
    ...
    have hh₂Hub2 : (G.neighborFinset h₂ ∩ Hub).card ≤ 2 := by
      ...
      have hzge : 1 ≤
          (G.neighborFinset h₂ ∩ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 20))).card :=
  ...
  have hPccard : (D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).card = 5 := by
    omega
  ...
  -- === `∑_P isoDeg ≤ 5` (each poor hub carries at most one twin). ===
  have hPcsum_le : (∑ h ∈ D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card),
      (G.neighborFinset h ∩ Iso).card) ≤ 5 := by
    ...
    calc (∑ h ∈ D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card),
      ...
      _ = 5 := by rw [hPccard, smul_eq_mul, mul_one]
  ...
  -- === The ledger split: `Σ_R + Σ_P = 16`, `Σ_R ≤ 12`, `Σ_P ≤ 5` pin `Σ_P ∈ {4, 5}`. ===
  have hPsplit : (∑ h ∈ D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card),
      (G.neighborFinset h ∩ Iso).card) = 5 ∨
      (∑ h ∈ D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card),
      (G.neighborFinset h ∩ Iso).card) = 4 := by omega
  rcases hPsplit with hP5 | hP4
  -- === World (i): `Σ_P = 5` — all five poor hubs at isoDeg exactly `1`, `Σ_R = 11`. ===
  · left
    have hRsum11 : (∑ h ∈ D4.filter (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card),
        (G.neighborFinset h ∩ Iso).card) = 11 := by omega
    refine ⟨?_, ?_, ?_⟩
    ...
    · rw [hPeq]
      ...
      have herasecard :
          ((D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).erase w).card = 4 := by
        rw [Finset.card_erase_of_mem hmem, hPccard]
      have herasele :
          (∑ h ∈ (D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).erase w,
        ...
        calc (∑ h ∈ (D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).erase w,
          ...
          _ = 4 := by rw [herasecard, smul_eq_mul, mul_one]
      omega
    · rw [hRD4eq]; exact hRsum11
  -- === World (ii): `Σ_P = 4` — one poor hub at isoDeg `0`, four at `1`, `Σ_R = 12`. ===
  · right
    have hRsum12 : (∑ h ∈ D4.filter (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card),
        (G.neighborFinset h ∩ Iso).card) = 12 := by omega
    have hex : ∃ h₀ ∈ D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card),
        (G.neighborFinset h₀ ∩ Iso).card = 0 := by
      by_contra hall
      have hge1 : ∀ h ∈ D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card),
          1 ≤ (G.neighborFinset h ∩ Iso).card := by
        intro h hh
        by_contra hlt
        exact hall ⟨h, hh, by omega⟩
      have hsumge := Finset.card_nsmul_le_sum
        (D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card))
        (fun h => (G.neighborFinset h ∩ Iso).card) 1 hge1
      rw [hPccard, smul_eq_mul, mul_one] at hsumge
      omega
    obtain ⟨h₀, hh₀mem, hh₀zero⟩ := hex
    have herase0 := Finset.add_sum_erase
      (D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card))
      (fun h => (G.neighborFinset h ∩ Iso).card) hh₀mem
    refine ⟨?_, ⟨h₀, ?_, hh₀zero, ?_⟩, ?_⟩
    · rw [hPeq]; exact hPccard
    · rw [hPeq]; exact hh₀mem
    · rw [hPeq]
      intro w hw
      rw [← Finset.erase_eq] at hw
      have hwPc : w ∈ D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card) :=
        Finset.mem_of_mem_erase hw
      have hwlt : ¬ 2 ≤ (G.neighborFinset w ∩ Iso).card := (Finset.mem_filter.mp hwPc).2
      have herase2 := Finset.add_sum_erase
        ((D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).erase h₀)
        (fun h => (G.neighborFinset h ∩ Iso).card) hw
      have hcard2 :
          (((D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).erase h₀).erase
          w).card = 3 := by
        rw [Finset.card_erase_of_mem hw, Finset.card_erase_of_mem hh₀mem, hPccard]
      have hle2 : (∑ h ∈ ((D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).erase
          h₀).erase w, (G.neighborFinset h ∩ Iso).card) ≤ 3 := by
        have hle1 : ∀ h ∈ ((D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).erase
            h₀).erase w, (G.neighborFinset h ∩ Iso).card ≤ 1 := by
          intro h hh
          have hhPc := Finset.mem_of_mem_erase (Finset.mem_of_mem_erase hh)
          rw [Finset.mem_filter] at hhPc
          omega
        calc (∑ h ∈ ((D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).erase
              h₀).erase w, (G.neighborFinset h ∩ Iso).card)
            ≤ (((D4.filter (fun h => ¬ 2 ≤ (G.neighborFinset h ∩ Iso).card)).erase h₀).erase
              w).card • 1 := Finset.sum_le_card_nsmul _ _ 1 hle1
          _ = 3 := by rw [hcard2, smul_eq_mul, mul_one]
      omega
