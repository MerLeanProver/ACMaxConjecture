...
/-!
# Mid-density shared-hub extraction for `n = 20`, dense `e(M)` (`halign8` residual)

Ports the proved-axiom-clean `n = 19` `TwinCert19Align8Mid` to `Fin 20`.  Two density regimes:

* `e(M) = 5` (`s = 10`), `|D| ≥ 11` with `|D| ≠ 12`: a degree-`5` hub sits on the `M`-isolated
  cherry, so the all-degree-`4` `SingleVertexConfig` route is unavailable.  We produce a
  degree-`≤ 5` hub shared by two `M`-isolated twins and route the dominating-edge / induced-`C₅`
  structure through the proved deg-`≤ 5` two-twin assemblies.  **New at `n = 20`:** the shared-hub
  pigeonhole *ties* at `|D| = 12` (`|H₆| = 2`, `∑_{H₆} deg = 12` exactly): two degree-`6` hubs can
  absorb `12` of the `18` isolated-twin incidences, leaving exactly one per degree-`4` hub (the
  `K_{2,6}` world, realisable past every cert), so `|D| = 12` is excluded here and must be
  dispatched to a dedicated handler.  The low-`|D|` routing also shifts: the second-hub extraction
  ties at `|D| = 9`, where the consecutive-cherry residual instead yields an all-degree-`4`
  `M`-isolated cherry (`alldeg4_iso_cherry_D9_twenty`) feeding `SingleVertexConfig`.

* `e(M) = 4` (`s = 8`), `|D| ∈ {9, …, 14}`: the induced-`C₅` branch is impossible, so only the
  dominating-edge double-star survives.  Here the larger `M`-isolated set (`|Iso| ≥ |D| − 5`)
  keeps the deg-`≤ 5` shared-hub pigeonhole strict throughout — in particular past the `|D| = 12`
  tie: we route the dominating double-star through `dom_fat_centre_two_twin_le5_twenty`.
-/
...
/-- **`|D| ≥ 9` from a degree-`≥ 5` hub (`n = 20`).**  With `∑ deg = 72` and every non-`D` vertex of
degree `≥ 4`, the existence of one degree-`≥ 5` vertex pushes the hub-degree sum above `4 |Hub| + 1`,
forcing `|D| ≥ 9` by the handshake (`72 − 3|D| ≥ 4(20 − |D|) + 1`). -/
theorem card_D_ge_nine_of_hub_deg5_twenty (G : SimpleGraph (Fin 20))
    (hm : G.edgeFinset.card = 36) (h3 : ∀ v : Fin 20, 3 ≤ G.degree v)
    (w : Fin 20) (hw5 : 5 ≤ G.degree w) :
    9 ≤ (Finset.univ.filter (fun v : Fin 20 => G.degree v = 3)).card := by
  classical
  set D : Finset (Fin 20) := Finset.univ.filter (fun v : Fin 20 => G.degree v = 3) with hDdef
  have hmemD : ∀ v : Fin 20, v ∈ D ↔ G.degree v = 3 := by intro v; rw [hDdef]; simp
  set Hub : Finset (Fin 20) := Finset.univ.filter (fun v : Fin 20 => 4 ≤ G.degree v) with hHubdef
...
/-- **Refined deg-`≤ 5` shared-hub count for `e(M) = 5`, `|D| ≥ 11`, `|D| ≠ 12` (`n = 20`).**  In
the `M`-isolated-twin-dense regime (`s ≤ 10`, `|D| ≥ 11`) there is a degree-`≤ 5` hub adjacent to
two distinct `M`-isolated degree-`3` twins.  Each degree-`≥ 6` hub's isolated-twin incidences are
bounded by both `|Iso|` and its degree, and `|H₆| ≤ 3`; a case-split on `|H₆|` closes the strict
pigeonhole for every `|D| ∈ {11, 13, 14, 15}`.  **`|D| = 12` is genuinely excluded**: at
`|H₆| = 2` the handshake allows `∑_{H₆} deg = 12` exactly, and two degree-`6` hubs adjacent to all
six `M`-isolated twins (the `K_{2,6}` world) absorb `12` of the `18` isolated-twin incidences,
leaving exactly one per degree-`4` hub — the pigeonhole ties and the conclusion fails, so `|D| = 12`
is dispatched to a dedicated two-hub handler by the caller. -/
theorem shared_hub_le5_eM5_mid_twenty (G : SimpleGraph (Fin 20))
    (hm : G.edgeFinset.card = 36) (h3 : ∀ v : Fin 20, 3 ≤ G.degree v)
    ...
    (hDge11 : 11 ≤ (Finset.univ.filter (fun w : Fin 20 => G.degree w = 3)).card)
    (hD12ne : (Finset.univ.filter (fun w : Fin 20 => G.degree w = 3)).card ≠ 12)
  ...
  have hcrossHub : ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub).card
      = ∑ h ∈ Hub, (G.neighborFinset h ∩ Iso).card := cross_count_twenty G Iso Hub
  ...
  have hDle15 : D.card ≤ 15 := by omega
...
/-- **`e(M) = 5` (`s = 10`) two-twin cut for the mid corner `|D| ∈ {11, 13, 14, 15}` (`n = 20`).**
A degree-`5` hub on the `M`-isolated cherry rules out the all-degree-`4` `SingleVertexConfig` route,
so we extract a degree-`≤ 5` hub shared by two `M`-isolated twins (`shared_hub_le5_eM5_mid_twenty`)
and route the dominating-edge double-star through `dom_fat_centre_two_twin_le5_twenty` and the
induced-`C₅` through `c5_shared_two_twin_le5_twenty`.  (`|D| = 12` is excluded: there the shared-hub
pigeonhole ties against the `K_{2,6}` two-hub world, which needs its own handler.) -/
theorem two_twin_eM5_mid_twenty (G : SimpleGraph (Fin 20))
    (hm : G.edgeFinset.card = 36) (h3 : ∀ v : Fin 20, 3 ≤ G.degree v)
  ...
  · obtain ⟨c₁, c₂, hc1D, hc2D, hc12, hcov⟩ := hdom
    obtain ⟨k, tw1, tw2, hkle5, htw12, htw1deg, htw2deg, hAtw1k, hAtw2k, htw1iso, htw2iso⟩ :=
      shared_hub_le5_eM5_mid_twenty G hm h3 h2k2 (by rw [← hDdef]; exact hDge11)
        (by rw [← hDdef]; exact hD12ne) (by rw [← hDdef]; exact hsum10.le)
...
/-- **Deg-`≤ 5` shared hub for `e(M) = 4`, `|D| ≥ 9` (`n = 20`).**  At `s = 8` the `M`-isolated set
...
its degree, and `|H₆| ≤ 3`; a case-split on `|H₆|` closes the count for every `|D| ∈ {9, …, 14}`
(the extra `|Iso|` margin over the `e(M) = 5` count keeps `|D| = 12` strict here). -/
theorem shared_hub_le5_eM4_twenty (G : SimpleGraph (Fin 20))
    (hm : G.edgeFinset.card = 36) (h3 : ∀ v : Fin 20, 3 ≤ G.degree v)
    (h2k2 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
      G.degree a = 3 ∧ G.degree b = 3 ∧ G.degree c = 3 ∧ G.degree d = 3 ∧
      G.Adj a b ∧ G.Adj c d ∧ ¬G.Adj a c ∧ ¬G.Adj a d ∧ ¬G.Adj b c ∧ ¬G.Adj b d)
    (hDge9 : 9 ≤ (Finset.univ.filter (fun w : Fin 20 => G.degree w = 3)).card)
    (hsumle : ∑ v ∈ Finset.univ.filter (fun w => G.degree w = 3),
...
/-- **Routing an all-degree-`4` `M`-isolated cherry to `SingleVertexConfig` (`e(M) = 5`, `n = 20`).**
...
`halign8_eM5_twenty`, factored out for reuse by the `|D| = 9` re-selection. -/
theorem sv_eM5_from_alldeg4_iso_twenty (G : SimpleGraph (Fin 20))
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
      G.degree a = 3 ∧ G.degree b = 3 ∧ G.degree c = 3 ∧ G.degree d = 3 ∧
    ...
    (hsum10 : ∑ v ∈ Finset.univ.filter (fun w => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w => G.degree w = 3)).card = 10)
    (t' : Fin 20) (ht3 : G.degree t' = 3)
    (htiso : ∀ w : Fin 20, G.Adj t' w → G.degree w ≠ 3)
    (htall : ∀ w : Fin 20, G.Adj t' w → G.degree w = 4) :
...
/-- **Routing a given degree-`≤ 5` shared hub (`e(M) = 5`, `n = 20`).**  Given a degree-`≤ 5` hub
`k` adjacent to two distinct `M`-isolated degree-`3` twins, the dominating-edge double-star routes
through `dom_fat_centre_two_twin_le5_twenty` and the induced-`C₅` through
`c5_shared_two_twin_le5_twenty`, giving `TwoTwinConfig`.  **New at `n = 20`:** in the induced-`C₅`
branch the consecutive-cherry residual needs a *second* deg-`≤ 5` shared hub, whose pigeonhole
*ties* at `|D| = 9` (`3(|D| − 5) − 2 = 10` incidences on the `10` hubs other than `k`); there the
residual instead yields an all-degree-`4` `M`-isolated cherry (`alldeg4_iso_cherry_D9_twenty`)
routed to `SingleVertexConfig`, so the conclusion is the disjunction.  This is the post-pigeonhole
part of `two_twin_eM5_mid_twenty`, factored out for reuse by the `|D| ∈ {9, 10}` re-selections. -/
theorem two_twin_eM5_from_shared_le5_twenty (G : SimpleGraph (Fin 20))
    (hm : G.edgeFinset.card = 36) (h3 : ∀ v : Fin 20, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
    ...
    (hsum10 : ∑ v ∈ Finset.univ.filter (fun w => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w => G.degree w = 3)).card = 10)
    (k tw1 tw2 : Fin 20) (hkge4 : 4 ≤ G.degree k) (hkle5 : G.degree k ≤ 5) (htw12 : tw1 ≠ tw2)
    (htw1deg : G.degree tw1 = 3) (htw2deg : G.degree tw2 = 3)
    (hAtw1k : G.Adj tw1 k) (hAtw2k : G.Adj tw2 k)
    (htw1iso : ∀ w : Fin 20, G.Adj tw1 w → G.degree w ≠ 3)
    (htw2iso : ∀ w : Fin 20, G.Adj tw2 w → G.degree w ≠ 3) :
    SingleVertexConfig G ∨ TwoTwinConfig G := by
  classical
  set D : Finset (Fin 20) := Finset.univ.filter (fun w => G.degree w = 3) with hDdef
  have hmemD : ∀ v : Fin 20, v ∈ D ↔ G.degree v = 3 := by intro v; rw [hDdef]; simp
  have hindle : ∀ x : Fin 20, x ∈ D → (G.neighborFinset x ∩ D).card ≤ 3 := by
  ...
  rcases dominating_edge_or_induced_C5 G D hmemD hT hC4 h2k2 hne with hdom | hC5
  · obtain ⟨c₁, c₂, hc1D, hc2D, hc12, hcov⟩ := hdom
    exact Or.inr (dom_fat_centre_two_twin_le5_twenty G D hmemD hT hC4 c₁ c₂ hc1D hc2D hc12 hcov
      hge hindle k tw1 tw2 hkge4 hkle5 htw12 htw1deg htw2deg hAtw1k hAtw2k htw1iso htw2iso)
  · obtain ⟨v₁, v₂, v₃, v₄, v₅, hv1D, hv2D, hv3D, hv4D, hv5D, hcard5, e12, e23, e36, e45, e51,
      n13, n14, n24, n25, n35, _hdom⟩ := hC5
    have hd1 : G.degree v₁ = 3 := (hmemD v₁).mp hv1D
    have hd2 : G.degree v₂ = 3 := (hmemD v₂).mp hv2D
    have hd3 : G.degree v₃ = 3 := (hmemD v₃).mp hv3D
    have hd4 : G.degree v₄ = 3 := (hmemD v₄).mp hv4D
    have hd5 : G.degree v₅ = 3 := (hmemD v₅).mp hv5D
    by_cases hD9 : D.card = 9
    · -- **`|D| = 9`.**  The second-hub pigeonhole ties, so dispatch on the cycle-hit cases
      -- directly: if `k` avoids a consecutive cycle cherry it anchors the two-twin cut itself;
      -- if `k` hits three consecutive cycle vertices, its five neighbours are pinned and
      -- `alldeg4_iso_cherry_D9_twenty` yields an all-degree-`4` `M`-isolated cherry feeding
      -- `SingleVertexConfig`.
      obtain ⟨d12, d13, d14, d15, d23, d24, d25, d36, d35, d45⟩ :=
        distinct_five_twenty v₁ v₂ v₃ v₄ v₅ hcard5
      have hD9' : (Finset.univ.filter (fun w : Fin 20 => G.degree w = 3)).card = 9 := by
        rw [← hDdef]; exact hD9
      have hsum10' : ∑ v ∈ Finset.univ.filter (fun w : Fin 20 => G.degree w = 3),
          (G.neighborFinset v ∩ Finset.univ.filter (fun w : Fin 20 => G.degree w = 3)).card
            = 10 := by
        rw [← hDdef]; exact hsum10
      rcases hub_cycle_cases_le5_twenty G hC4 k v₁ v₂ v₃ v₄ v₅ hkge4 hkle5 hd1 hd2 hd3 hd4 hd5
        e12 e23 e36 e45 e51 n13 n14 n24 n25 n35 d12 d13 d14 d15 d23 d24 d25 d36 d35 d45 with
        ⟨p1, p2, p3⟩ | ⟨p2, p3, p4⟩ | ⟨p3, p4, p5⟩ | ⟨p4, p5, p1⟩ | ⟨p5, p1, p2⟩ | hHit
      · exact Or.inr (dense_two_twin_assemble_le5_twenty G tw1 tw2 k v₁ v₂ v₃ htw1deg htw2deg
          hkge4 hkle5 hd1 hd2 hd3 hAtw1k hAtw2k e12 e23 htw1iso htw2iso p1 p2 p3 htw12 d12 d23
          d13)
      · exact Or.inr (dense_two_twin_assemble_le5_twenty G tw1 tw2 k v₂ v₃ v₄ htw1deg htw2deg
          hkge4 hkle5 hd2 hd3 hd4 hAtw1k hAtw2k e23 e36 htw1iso htw2iso p2 p3 p4 htw12 d23 d36
          d24)
      · exact Or.inr (dense_two_twin_assemble_le5_twenty G tw1 tw2 k v₃ v₄ v₅ htw1deg htw2deg
          hkge4 hkle5 hd3 hd4 hd5 hAtw1k hAtw2k e36 e45 htw1iso htw2iso p3 p4 p5 htw12 d36 d45
          d35)
      · exact Or.inr (dense_two_twin_assemble_le5_twenty G tw1 tw2 k v₄ v₅ v₁ htw1deg htw2deg
          hkge4 hkle5 hd4 hd5 hd1 hAtw1k hAtw2k e45 e51 htw1iso htw2iso p4 p5 p1 htw12 d45
          d15.symm d14.symm)
      · exact Or.inr (dense_two_twin_assemble_le5_twenty G tw1 tw2 k v₅ v₁ v₂ htw1deg htw2deg
          hkge4 hkle5 hd5 hd1 hd2 hAtw1k hAtw2k e51 e12 htw1iso htw2iso p5 p1 p2 htw12
          d15.symm d12 d25.symm)
      · rcases hHit with ⟨ha1, ha2, ha3⟩ | ⟨ha2, ha3, ha4⟩ | ⟨ha3, ha4, ha5⟩ | ⟨ha4, ha5, ha1⟩ |
          ⟨ha5, ha1, ha2⟩
        · obtain ⟨u, hu3, huiso, huall⟩ := alldeg4_iso_cherry_D9_twenty G hm h3 hD9'
            v₁ v₂ v₃ v₄ v₅ hd1 hd2 hd3 hd4 hd5 hcard5 e12 e23 e36 e45 e51 hsum10'
            k v₁ v₂ v₃ tw1 tw2 hkle5 hd1 hd2 hd3 ha1 ha2 ha3 e12 e23 d12 d23 d13
            htw12 htw1deg htw2deg hAtw1k hAtw2k htw1iso htw2iso
          exact Or.inl (sv_eM5_from_alldeg4_iso_twenty G hT h2k2 hC4 hsum10' u hu3 huiso huall)
        · obtain ⟨u, hu3, huiso, huall⟩ := alldeg4_iso_cherry_D9_twenty G hm h3 hD9'
            v₁ v₂ v₃ v₄ v₅ hd1 hd2 hd3 hd4 hd5 hcard5 e12 e23 e36 e45 e51 hsum10'
            k v₂ v₃ v₄ tw1 tw2 hkle5 hd2 hd3 hd4 ha2 ha3 ha4 e23 e36 d23 d36 d24
            htw12 htw1deg htw2deg hAtw1k hAtw2k htw1iso htw2iso
          exact Or.inl (sv_eM5_from_alldeg4_iso_twenty G hT h2k2 hC4 hsum10' u hu3 huiso huall)
        · obtain ⟨u, hu3, huiso, huall⟩ := alldeg4_iso_cherry_D9_twenty G hm h3 hD9'
            v₁ v₂ v₃ v₄ v₅ hd1 hd2 hd3 hd4 hd5 hcard5 e12 e23 e36 e45 e51 hsum10'
            k v₃ v₄ v₅ tw1 tw2 hkle5 hd3 hd4 hd5 ha3 ha4 ha5 e36 e45 d36 d45 d35
            htw12 htw1deg htw2deg hAtw1k hAtw2k htw1iso htw2iso
          exact Or.inl (sv_eM5_from_alldeg4_iso_twenty G hT h2k2 hC4 hsum10' u hu3 huiso huall)
        · obtain ⟨u, hu3, huiso, huall⟩ := alldeg4_iso_cherry_D9_twenty G hm h3 hD9'
            v₁ v₂ v₃ v₄ v₅ hd1 hd2 hd3 hd4 hd5 hcard5 e12 e23 e36 e45 e51 hsum10'
            k v₄ v₅ v₁ tw1 tw2 hkle5 hd4 hd5 hd1 ha4 ha5 ha1 e45 e51 d45 d15.symm d14.symm
            htw12 htw1deg htw2deg hAtw1k hAtw2k htw1iso htw2iso
          exact Or.inl (sv_eM5_from_alldeg4_iso_twenty G hT h2k2 hC4 hsum10' u hu3 huiso huall)
        · obtain ⟨u, hu3, huiso, huall⟩ := alldeg4_iso_cherry_D9_twenty G hm h3 hD9'
            v₁ v₂ v₃ v₄ v₅ hd1 hd2 hd3 hd4 hd5 hcard5 e12 e23 e36 e45 e51 hsum10'
            k v₅ v₁ v₂ tw1 tw2 hkle5 hd5 hd1 hd2 ha5 ha1 ha2 e51 e12 d15.symm d12 d25.symm
            htw12 htw1deg htw2deg hAtw1k hAtw2k htw1iso htw2iso
          exact Or.inl (sv_eM5_from_alldeg4_iso_twenty G hT h2k2 hC4 hsum10' u hu3 huiso huall)
    · -- **`|D| ≥ 10`.**  The second-hub extraction is strict: at `|D| ≥ 11` via the ported
      -- pigeonhole, at `|D| = 10` via the low-`|D|` extraction (the shared hub's five
      -- neighbours are pinned, so `deg k = 5` and every other hub is degree-`≤ 5`).
      exact Or.inr (c5_shared_two_twin_le5_twenty G hT hC4 k tw1 tw2 hkge4 hkle5 htw12 htw1deg
        htw2deg hAtw1k hAtw2k htw1iso htw2iso
        (fun a b c hda hdb hdc hka hkb hkc hab hbc hab_ne hbc_ne hac_ne => by
          by_cases hD11 : 11 ≤ D.card
          · exact exists_nonk_two_twin_hub_twenty G hm h3
              (by rw [← hDdef]; exact hD11)
              v₁ v₂ v₃ v₄ v₅ hd1 hd2 hd3 hd4 hd5 hcard5 e12 e23 e36 e45 e51
              (by rw [← hDdef]; exact hsum10) k a b c
              hkge4 hkle5 hda hdb hdc hka hkb hkc hab hbc hab_ne hbc_ne hac_ne
          · exact exists_nonk_two_twin_hub_lowD_twenty G hm h3
              (by rw [← hDdef]; omega)
              v₁ v₂ v₃ v₄ v₅ hd1 hd2 hd3 hd4 hd5 hcard5 e12 e23 e36 e45 e51
              (by rw [← hDdef]; exact hsum10) k a b c tw1 tw2
              hkle5 hda hdb hdc hka hkb hkc hab hbc hab_ne hbc_ne hac_ne
              htw12 htw1deg htw2deg hAtw1k hAtw2k htw1iso htw2iso)
        v₁ v₂ v₃ v₄ v₅ hd1 hd2 hd3 hd4 hd5 hcard5
        e12 e23 e36 e45 e51 n13 n14 n24 n25 n35)

/-- **`|D| = 9` re-selection dichotomy for `e(M) = 5` (`n = 20`).**  At `|D| = 9` the hub-degree sum
`45` over `11` hubs forces *exactly one* degree-`5` hub `K` (the rest degree-`4`).  If two distinct
...
only ties. -/
theorem eM5_D9_shared_or_alldeg4_twenty (G : SimpleGraph (Fin 20))
    (hm : G.edgeFinset.card = 36) (h3 : ∀ v : Fin 20, 3 ≤ G.degree v)
    (h2k2 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧
      G.degree a = 3 ∧ G.degree b = 3 ∧ G.degree c = 3 ∧ G.degree d = 3 ∧
      G.Adj a b ∧ G.Adj c d ∧ ¬G.Adj a c ∧ ¬G.Adj a d ∧ ¬G.Adj b c ∧ ¬G.Adj b d)
    ...
    (hsumle : ∑ v ∈ Finset.univ.filter (fun w => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w => G.degree w = 3)).card ≤ 10)
    (t : Fin 20) (_ht3 : G.degree t = 3) (htiso : ∀ w : Fin 20, G.Adj t w → G.degree w ≠ 3)
    (hnall : ¬∀ w : Fin 20, G.Adj t w → G.degree w = 4) :
    (∃ k tw1 tw2 : Fin 20, G.degree k ≤ 5 ∧ tw1 ≠ tw2 ∧ G.degree tw1 = 3 ∧ G.degree tw2 = 3 ∧
        G.Adj tw1 k ∧ G.Adj tw2 k ∧ (∀ w : Fin 20, G.Adj tw1 w → G.degree w ≠ 3) ∧
        (∀ w : Fin 20, G.Adj tw2 w → G.degree w ≠ 3))
      ∨ (∃ t' : Fin 20, G.degree t' = 3 ∧ (∀ w : Fin 20, G.Adj t' w → G.degree w ≠ 3) ∧
        (∀ w : Fin 20, G.Adj t' w → G.degree w = 4)) := by
  classical
  set D : Finset (Fin 20) := Finset.univ.filter (fun w => G.degree w = 3) with hDdef
  have hmemD : ∀ v : Fin 20, v ∈ D ↔ G.degree v = 3 := by intro v; rw [hDdef]; simp
  set Hub : Finset (Fin 20) := Finset.univ.filter (fun v => 4 ≤ G.degree v) with hHubdef
  ...
  have hsplit : ∑ v ∈ D, G.degree v + ∑ v ∈ Dᶜ, G.degree v = 72 := by
    rw [Finset.sum_add_sum_compl]; exact hsum64
  have hHubsum : ∑ v ∈ Hub, G.degree v = 45 := by
    rw [hHubeqDc]; rw [hsumD, hD9] at hsplit; omega
  have hHubcard : Hub.card = 11 := by
  ...
  obtain ⟨w0, hw0adj, hw0ne4⟩ : ∃ w : Fin 20, G.Adj t w ∧ G.degree w ≠ 4 := by
    by_contra hc
    push Not at hc
  ...
  have hKle5 : G.degree K ≤ 5 := by
    have hHub4card : Hub4.card = 10 := by omega
  ...
  have hIso3 : 3 ≤ Iso.card := by
    ...
    rw [← hIsodef] at hpart
    omega
  by_cases hsh : ∃ a b : Fin 20, a ∈ Iso ∧ b ∈ Iso ∧ a ≠ b ∧ G.Adj a K ∧ G.Adj b K
