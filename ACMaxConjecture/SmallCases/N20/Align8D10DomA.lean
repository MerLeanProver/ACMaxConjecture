...
/-!
# The `|D| = 10`, `e(M) = 5` dominating-edge double-star structure (`n = 20`)
...
theorem eM5_domedge_leaves_twenty (G : SimpleGraph (Fin 20))
    (_h3 : ∀ v : Fin 20, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (_hD10 : (Finset.univ.filter (fun w : Fin 20 => G.degree w = 3)).card = 10)
    ...
    ∃ l₁ l₂ l₃ l₄ : Fin 20,
      ...
      ¬G.Adj l₁ c₂ ∧ ¬G.Adj l₂ c₂ ∧ ¬G.Adj l₃ c₁ ∧ ¬G.Adj l₄ c₁ ∧
      (∀ w : Fin 20, G.Adj c₁ w → w = c₂ ∨ w = l₁ ∨ w = l₂) ∧
      (∀ w : Fin 20, G.Adj c₂ w → w = c₁ ∨ w = l₃ ∨ w = l₄) := by
  classical
  set D : Finset (Fin 20) := Finset.univ.filter (fun w : Fin 20 => G.degree w = 3) with hDdef
  have hmemD : ∀ v : Fin 20, v ∈ D ↔ G.degree v = 3 := by
    intro v; simp [hDdef]
  ...
  have hper : ∀ v ∈ (D.erase c₁).erase c₂, G.neighborFinset v ∩ D
      = ({c₁, c₂} : Finset (Fin 20)).filter (fun c => G.Adj v c) := by
    intro v hv
    ...
    · rintro ⟨hw, hadj⟩
      ...
      · exact hc1D'
      · exact hc2D'
  have hcardpair : ∀ v : Fin 20,
      ((({c₁, c₂} : Finset (Fin 20))).filter (fun c => G.Adj v c)).card
  ...
  obtain ⟨l₃, l₄, hl36, hset2⟩ := Finset.card_eq_two.mp herase2
  have hmem1 : ∀ w : Fin 20, w = l₁ ∨ w = l₂ → w ≠ c₂ ∧ G.Adj c₁ w ∧ G.degree w = 3 := by
    intro w hw
  ...
  have hmem2 : ∀ w : Fin 20, w = l₃ ∨ w = l₄ → w ≠ c₁ ∧ G.Adj c₂ w ∧ G.degree w = 3 := by
    intro w hw
    have hwe : w ∈ (G.neighborFinset c₂ ∩ D).erase c₁ := by
  ...
  have hsub1 : ({c₂, l₁, l₂} : Finset (Fin 20)) ⊆ G.neighborFinset c₁ := by
    intro w hw
    ...
    · exact ha1
    · exact ha2
  have hcard1 : ({c₂, l₁, l₂} : Finset (Fin 20)).card = 3 := by
  ...
  have hsub2 : ({c₁, l₃, l₄} : Finset (Fin 20)) ⊆ G.neighborFinset c₂ := by
    intro w hw
    simp only [Finset.mem_insert, Finset.mem_singleton] at hw
    rw [SimpleGraph.mem_neighborFinset]
    rcases hw with rfl | rfl | rfl
    ...
    · exact ha3
    · exact ha4
  have hcard2 : ({c₁, l₃, l₄} : Finset (Fin 20)).card = 3 := by
    rw [Finset.card_insert_of_notMem (by
          simp only [Finset.mem_insert, Finset.mem_singleton, not_or]
          exact ⟨fun e => h3c1 e.symm, fun e => h4c1 e.symm⟩),
      Finset.card_insert_of_notMem (by simp only [Finset.mem_singleton]; exact hl36),
      Finset.card_singleton]
  ...
  have hfull1 : ∀ w : Fin 20, G.Adj c₁ w → w = c₂ ∨ w = l₁ ∨ w = l₂ := by
    intro w hw
    have hmem : w ∈ G.neighborFinset c₁ := (G.mem_neighborFinset _ _).mpr hw
    rw [hNc1] at hmem
    simpa using hmem
  have hfull2 : ∀ w : Fin 20, G.Adj c₂ w → w = c₁ ∨ w = l₃ ∨ w = l₄ := by
    intro w hw
    have hmem : w ∈ G.neighborFinset c₂ := (G.mem_neighborFinset _ _).mpr hw
    rw [hNc2] at hmem
    simpa using hmem
  exact ⟨l₁, l₂, l₃, l₄, hl12, h13, h14, h23, h24, hl36,
