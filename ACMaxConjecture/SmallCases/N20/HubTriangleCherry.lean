...
/-!
# Hub-triangle existence for the `n = 20`, `e(M) = 2`, single-`P₃`-cherry residual corner

This file isolates the hub-triangle existence obligation in the `e(M) = 2`, `|D| = 8`
(`|Hub| = 12`, all-degree-`4`) corner where the residual `M` is a **single `P₃` cherry** `x–y–z`
(`y` the middle, in-`M` degree `2`; `x, z` the endpoints, in-`M` degree `1`).  It is the
`n = 20` port of `TwinCert19HubTriangleCherry`.

## The residual configuration
* `D` = the degree-`3` vertices, `|D| = 8`; `Dᶜ` = the **twelve** hubs, all of degree `4` (`hdeg4`).
...
## `n = 20` delta (the triangle-extraction core)
...
avoider set is pinned to `|A| = 7` (`|Dᶜ| − 5 = 12 − 5`).  When the in-`A`
edge mass `∑_A(N ∩ A) ≥ 25` a Mantel triangle is forced (`mantel_seven_triangle`, `2·⌊49/4⌋ = 24`);
otherwise the avoiders form a triangle-free graph, split on whether two avoiders share an
`M`-isolated twin: a shared twin assembles a `SingleVertexConfig` (contradicting
`¬SingleVertexConfig`), while the distinct-twin case assembles a `TwoHubConfig`.  The `|A| = 7`
distinct-twin `TwoHubConfig` assembly is the genuine `n = 20` boundary, now closed axiom-clean by
`cherry_C6_distinct_twin_twoHub_twenty` (the `|S| = 5`, `|Iso| = 5`, `∑_Dᶜ iso = 15` counts are
unchanged from `|A| = 6`, so the `C₅` extraction ports verbatim with `|A| = 7`, `|Dᶜ| = 12`).
-/
...
/-- **Mantel triangle on seven cherry-avoider hubs.**  A `7`-vertex set with in-set edge mass
`∑(N ∩ A) ≥ 25 > 2·⌊49/4⌋ = 24` contains a triangle.  Picking a vertex `v` of maximal in-set
degree `Δ`, its neighbourhood is independent (else a triangle), so each neighbour has `≤ 7 − Δ`
in-set neighbours while every vertex has `≤ Δ`; hence
`∑ ≤ Δ·(7 − Δ) + (7 − Δ)·Δ = 2·Δ·(7 − Δ) ≤ 24`, contradicting the hypothesis. -/
theorem mantel_seven_triangle (G : SimpleGraph (Fin 20)) (A : Finset (Fin 20))
    (hcard : A.card = 7)
    (hedge : 25 ≤ ∑ g ∈ A, (G.neighborFinset g ∩ A).card) :
    ∃ a b c : Fin 20, a ∈ A ∧ b ∈ A ∧ c ∈ A ∧ G.Adj a b ∧ G.Adj a c ∧ G.Adj b c := by
  classical
  by_contra htri
  have hAne : A.Nonempty := by rw [← Finset.card_pos, hcard]; omega
  obtain ⟨v, hvA, hvmax⟩ :=
    A.exists_max_image (fun g => (G.neighborFinset g ∩ A).card) hAne
  have hvmax' : ∀ g ∈ A, (G.neighborFinset g ∩ A).card ≤ (G.neighborFinset v ∩ A).card := hvmax
  have hNsubA : G.neighborFinset v ∩ A ⊆ A := Finset.inter_subset_right
  have hvnotN : v ∉ G.neighborFinset v ∩ A := by
    rw [Finset.mem_inter, G.mem_neighborFinset]; exact fun h => G.irrefl h.1
  have hBcard : (A \ (G.neighborFinset v ∩ A)).card = 7 - (G.neighborFinset v ∩ A).card := by
    have h := Finset.card_sdiff_add_card_inter A (G.neighborFinset v ∩ A)
    rw [Finset.inter_eq_right.mpr hNsubA, hcard] at h
    omega
  have hΔle6 : (G.neighborFinset v ∩ A).card ≤ 6 := by
    have hsub : G.neighborFinset v ∩ A ⊆ A.erase v :=
      fun w hw => Finset.mem_erase.mpr ⟨fun e => hvnotN (e ▸ hw), hNsubA hw⟩
    have := Finset.card_le_card hsub
    rwa [Finset.card_erase_of_mem hvA, hcard] at this
  have hindep : ∀ u ∈ G.neighborFinset v ∩ A, ∀ w ∈ G.neighborFinset v ∩ A, ¬G.Adj u w := by
    intro u hu w hw hadj
    rw [Finset.mem_inter, G.mem_neighborFinset] at hu hw
    exact htri ⟨v, u, w, hvA, hu.2, hw.2, hu.1, hw.1, hadj⟩
  have hNbound : ∀ u ∈ G.neighborFinset v ∩ A,
      (G.neighborFinset u ∩ A).card ≤ 7 - (G.neighborFinset v ∩ A).card := by
    intro u hu
    have hsub : G.neighborFinset u ∩ A ⊆ A \ (G.neighborFinset v ∩ A) := by
      intro w hw
      rw [Finset.mem_inter, G.mem_neighborFinset] at hw
      rw [Finset.mem_sdiff]
      exact ⟨hw.2, fun hwN => hindep u hu w hwN hw.1⟩
    calc (G.neighborFinset u ∩ A).card ≤ (A \ (G.neighborFinset v ∩ A)).card :=
          Finset.card_le_card hsub
      _ = 7 - (G.neighborFinset v ∩ A).card := hBcard
  have hsumN : (∑ g ∈ G.neighborFinset v ∩ A, (G.neighborFinset g ∩ A).card)
      ≤ (G.neighborFinset v ∩ A).card * (7 - (G.neighborFinset v ∩ A).card) := by
    calc (∑ g ∈ G.neighborFinset v ∩ A, (G.neighborFinset g ∩ A).card)
        ≤ ∑ _g ∈ G.neighborFinset v ∩ A, (7 - (G.neighborFinset v ∩ A).card) :=
          Finset.sum_le_sum hNbound
      _ = (G.neighborFinset v ∩ A).card * (7 - (G.neighborFinset v ∩ A).card) := by
          rw [Finset.sum_const, smul_eq_mul]
  have hsumB : (∑ g ∈ A \ (G.neighborFinset v ∩ A), (G.neighborFinset g ∩ A).card)
      ≤ (7 - (G.neighborFinset v ∩ A).card) * (G.neighborFinset v ∩ A).card := by
    calc (∑ g ∈ A \ (G.neighborFinset v ∩ A), (G.neighborFinset g ∩ A).card)
        ≤ ∑ _g ∈ A \ (G.neighborFinset v ∩ A), (G.neighborFinset v ∩ A).card :=
          Finset.sum_le_sum (fun g hg => hvmax' g (Finset.sdiff_subset hg))
      _ = (7 - (G.neighborFinset v ∩ A).card) * (G.neighborFinset v ∩ A).card := by
          rw [Finset.sum_const, hBcard, smul_eq_mul]
  have hsplit : (∑ g ∈ A \ (G.neighborFinset v ∩ A), (G.neighborFinset g ∩ A).card)
      + ∑ g ∈ G.neighborFinset v ∩ A, (G.neighborFinset g ∩ A).card
      = ∑ g ∈ A, (G.neighborFinset g ∩ A).card := Finset.sum_sdiff hNsubA
  have hkey : (G.neighborFinset v ∩ A).card * (7 - (G.neighborFinset v ∩ A).card)
      + (7 - (G.neighborFinset v ∩ A).card) * (G.neighborFinset v ∩ A).card ≤ 24 := by
    have h6 := hΔle6
    set k := (G.neighborFinset v ∩ A).card with hk
...
theorem cherry_partition (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20)) (x y z : Fin 20)
    ...
    ({x, y, z} : Finset (Fin 20)) ∪ Iso = D ∧ Disjoint ({x, y, z} : Finset (Fin 20)) Iso := by
  classical
  set P : Finset (Fin 20) := {x, y, z} with hP
...
theorem cherry_leaf_card (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20)) (x y z : Fin 20)
...
/-- **Cherry residual incidence sums.**  Over the twelve hubs the cherry-, iso- and hub-internal
incidences total `5`, `15`, `28` respectively, and per hub the three split the degree `4`. -/
theorem cherry_residual_sums (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20)) (x y z : Fin 20)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
...
/-- **Distinct-twin `TwoHubConfig` assembly for the `|A| = 7` `C₅` cherry corner.** -/
theorem cherry_C6_distinct_twin_twoHub_twenty (G : SimpleGraph (Fin 20))
    (D Iso A : Finset (Fin 20)) (x y z : Fin 20)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 20, G.Adj v w → G.degree w ≠ 3))
    (hIsoD : Iso ⊆ D)
    (hdeg4 : ∀ w : Fin 20, w ∈ Dᶜ → G.degree w = 4)
    ...
    (hAsub : A ⊆ Dᶜ) (hcard7 : A.card = 7) (hIso5 : Iso.card = 5) (hDc12 : Dᶜ.card = 12)
    (_havoid : ∀ g ∈ A, ¬G.Adj g x ∧ ¬G.Adj g y ∧ ¬G.Adj g z)
    (hScherry : ∀ h : Fin 20, h ∈ Dᶜ → h ∉ A → G.Adj h x ∨ G.Adj h y ∨ G.Adj h z)
    (hsplit4 : ∀ h : Fin 20, h ∈ Dᶜ → (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 20))).card
  ...
  have hisoint : ∀ h : Fin 20, h ∈ S → (G.neighborFinset h ∩ Iso).card
  ...
  have hinternal0 : ∀ h : Fin 20, h ∈ S → (G.neighborFinset h ∩ Iso).card = 3 →
      ∀ h' : Fin 20, h' ∈ Dᶜ → ¬G.Adj h h' := by
  ...
  have hint_le1 : ∀ h : Fin 20, h ∈ S → (G.neighborFinset h ∩ Iso).card = 2 →
      (G.neighborFinset h ∩ Dᶜ).card ≤ 1 := by
    intro h hh hiso2
    have hle := hisoint h hh
    omega
  -- Each twin meets `≥ 2` of the five `S`-hubs.
  have hdt : ∀ t : Fin 20, t ∈ Iso → 2 ≤ (G.neighborFinset t ∩ S).card := by
    intro t ht
  ...
  have hshare1 : ∀ p : Fin 20, p ∈ S → ∀ q : Fin 20, q ∈ S → p ≠ q → ¬G.Adj p q →
  ...
  have hsd : ∀ p q : Fin 20, (G.neighborFinset p ∩ Iso).card
  ...
  have hpair : ∃ h₁ ∈ S, ∃ h₂ ∈ S, h₁ ≠ h₂ ∧ ¬G.Adj h₁ h₂ ∧
    ...
    set R : Finset (Fin 20) := S.filter (fun h => 3 ≤ (G.neighborFinset h ∩ Iso).card) with hRdef
    ...
    · -- **Case B.**  At most one `isoDeg = 3` hub, so `∑_S isoDeg ≤ 11`.
      ...
      have hT11 : Tval ≤ 11 := by
        ...
        omega
      -- The `isoDeg ≥ 2` hubs.
      set Q : Finset (Fin 20) := S.filter (fun h => 2 ≤ (G.neighborFinset h ∩ Iso).card) with hQdef
      ...
      · -- **Sub-case `|Q| = 5`:** every `S`-hub has `isoDeg ≥ 2`.
        ...
        have hiso2 : ∀ p : Fin 20, p ∈ S → 2 ≤ (G.neighborFinset p ∩ Iso).card := by
          intro p hp
        ...
        have hsg : ∀ p : Fin 20, p ∈ S → ∀ q : Fin 20, q ∈ S → p ≠ q → ¬G.Adj p q →
        ...
        have hcross : ∑ t ∈ Iso, (G.neighborFinset t ∩ S).card = Tval :=
          (cross_count_twenty G S Iso).symm
        ...
        have hrow : ∀ p : Fin 20, p ∈ S → 2 * (G.neighborFinset p ∩ Iso).card + 1
      ...
      · -- **Sub-case `|Q| = 4`:** the rigid `isoDeg`-multiset `(1, 2, 2, 2, 3)` (`Tval = 10`).  The
        ...
        have hdt2 : ∀ t : Fin 20, t ∈ Iso → (G.neighborFinset t ∩ S).card = 2 := by
          have hsumdt : ∑ t ∈ Iso, (G.neighborFinset t ∩ S).card = 10 := by
            rw [← cross_count_twenty G S Iso, ← hTvaldef]; exact hTval10
          intro t ht
          by_contra hne
        ...
        have htQ2 : ∀ t : Fin 20, t ∈ Iso → (G.neighborFinset t ∩ Q).card ≤ 2 := by
          intro t ht
        ...
        have hcrossQ : ∑ t ∈ Iso, (G.neighborFinset t ∩ Q).card = TQ :=
          (cross_count_twenty G Q Iso).symm
        ...
        by_contra hcon
        have hsgQ : ∀ p : Fin 20, p ∈ Q → ∀ q : Fin 20, q ∈ Q → p ≠ q → ¬G.Adj p q →
            1 ≤ (G.neighborFinset p ∩ G.neighborFinset q ∩ Iso).card := by
          intro p hp q hq hpq hnadj
          by_contra hs0
          rw [not_le, Nat.lt_one_iff] at hs0
          ...
          · have := hsd q p
            have hcomm : (G.neighborFinset q ∩ G.neighborFinset p ∩ Iso).card = 0 := by
              rw [show G.neighborFinset q ∩ G.neighborFinset p
                = G.neighborFinset p ∩ G.neighborFinset q from Finset.inter_comm _ _]; exact hs0
            omega
        have hrowQ : ∀ p : Fin 20, p ∈ Q → 2 * (G.neighborFinset p ∩ Iso).card
  ...
  have getmem : ∀ w u v : Fin 20, w ∈ (G.neighborFinset u ∩ Iso) \ G.neighborFinset v →
...
/-- **Hub-triangle existence in the `n = 20`, `|D| = 8`, `e(M) = 2`, single-`P₃`-cherry corner.**
Under the residual hypotheses (twelve degree-`4` hubs, `M = P₃` cherry `x–y–z` with five
...
hub-incidences, **exactly five** hubs are cherry-incident, forcing `|A| = 7` cherry-avoiders.  When
the in-`A` edge mass `∑_A(N ∩ A) ≥ 25` a Mantel triangle is forced (`mantel_seven_triangle`);
otherwise the avoiders form a triangle-free graph, split on whether two avoiders share an
`M`-isolated twin: a shared twin assembles a `SingleVertexConfig` (contradicting
`¬SingleVertexConfig`), while the distinct-twin case assembles a `TwoHubConfig`.  The distinct-twin
`TwoHubConfig` assembly for the genuine `n = 20` `|A| = 7` avoider set is closed axiom-clean by
`cherry_C6_distinct_twin_twoHub_twenty`: the controlling counts `|S| = 5`, `|Iso| = 5`,
`∑_Dᶜ iso = 15` are unchanged from the `|A| = 6` corner, so the `C₅` extraction ports verbatim. -/
theorem exists_hub_triangle_config_cherry_residual_twenty (G : SimpleGraph (Fin 20))
  ...
  -- Structural counts: twelve hubs, five `M`-isolated twins.
  obtain ⟨hDeq, hdisj⟩ :=
    cherry_partition G D Iso x y z hIsodef hisochar hxD hyD hzD haxy hayz
  have hDc9 : Dᶜ.card = 12 := by rw [Finset.card_compl, Fintype.card_fin, hD8]
  have hIso5 : Iso.card = 5 := by
    have hPcard : ({x, y, z} : Finset (Fin 20)).card = 3 := by
  ...
  have getA : ∀ g : Fin 20, g ∈ A → g ∈ Dᶜ ∧ ¬G.Adj g x ∧ ¬G.Adj g y ∧ ¬G.Adj g z := by
  ...
  have hle1 : ∀ h : Fin 20, h ∈ Dᶜ →
      (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 20))).card ≤ 1 := by
    intro h hhDc
    ...
    have htri : ∀ u v : Fin 20, u ≠ v → G.Adj u v → G.Adj h u → G.Adj h v →
  ...
  have hA0 : ∀ g : Fin 20, g ∈ A →
      (G.neighborFinset g ∩ ({x, y, z} : Finset (Fin 20))).card = 0 := by
    intro g hg
    ...
    · exact hgy hadj
    · exact hgz hadj
  have hScherry : ∀ h : Fin 20, h ∈ Dᶜ → h ∉ A →
      1 ≤ (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 20))).card := by
    intro h hhDc hhA
    ...
    · exact Finset.card_pos.mpr
        ⟨z, by rw [Finset.mem_inter, G.mem_neighborFinset]; exact ⟨haz, by simp⟩⟩
  -- The cherry meets exactly the five non-avoider hubs, so `|Dᶜ \ A| = 5` and `|A| = 7`.
  have hScard : (Dᶜ \ A).card = 5 := by
    have hsplit := Finset.sum_sdiff (f := fun g =>
      (G.neighborFinset g ∩ ({x, y, z} : Finset (Fin 20))).card) hAsub
    have hA0sum : ∑ g ∈ A,
        (G.neighborFinset g ∩ ({x, y, z} : Finset (Fin 20))).card = 0 :=
      Finset.sum_eq_zero hA0
    have hSeq : ∀ g ∈ Dᶜ \ A,
        (G.neighborFinset g ∩ ({x, y, z} : Finset (Fin 20))).card = 1 := by
      intro g hg
      ...
      omega
    have hScount : ∑ g ∈ Dᶜ \ A,
        (G.neighborFinset g ∩ ({x, y, z} : Finset (Fin 20))).card = (Dᶜ \ A).card := by
      rw [Finset.sum_congr rfl hSeq, Finset.sum_const, smul_eq_mul, Nat.mul_one]
    omega
  have hcard7 : A.card = 7 := by
    have h := Finset.card_sdiff_add_card_inter Dᶜ A
  ...
  have htri : ∃ a b c : Fin 20, a ∈ A ∧ b ∈ A ∧ c ∈ A ∧ a ≠ b ∧ a ≠ c ∧ b ≠ c ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c := by
    rcases Nat.lt_or_ge (∑ g ∈ A, (G.neighborFinset g ∩ A).card) 25 with _hbig | hbig
    · -- `∑_A(N ∩ A) ≤ 24`: the seven cherry-avoiders form a triangle-free graph; the goal
      ...
      by_cases hshared : ∃ t : Fin 20, t ∈ Iso ∧ ∃ p : Fin 20, p ∈ A ∧ ∃ q : Fin 20, q ∈ A ∧
      ...
      · obtain ⟨t, htIso, p, hpA, q, hqA, hpq, hpt, hqt⟩ := hshared
        ...
        have htiso : ∀ w : Fin 20, G.Adj t w → G.degree w ≠ 3 := (hIsoprop t htIso).2
        have hpdeg : G.degree p = 4 := hdeg4 p hpDc
        have hqdeg : G.degree q = 4 := hdeg4 q hqDc
        have htnotxyz : t ∉ ({x, y, z} : Finset (Fin 20)) :=
        ...
        have hsum0 : ∑ w ∈ ({t, p, q} : Finset (Fin 20)),
            (G.neighborFinset w ∩ ({x, y, z} : Finset (Fin 20))).card = 0 := by
      ...
      · -- **Distinct-twin case.**  Each `M`-isolated twin meets `≤ 1` cherry-avoider (else the two
        ...
        -- assembles a `TwoHubConfig`.  At genuine `n = 20` the avoider set is `|A| = 7`, `|Dᶜ| = 12`,
        -- with the unchanged `|S| = 5`, `|Iso| = 5`, `∑_Dᶜ iso = 15`, closed by the bespoke
        -- `cherry_C6_distinct_twin_twoHub_twenty`.
        refine absurd ?_ hth
        have hIsoD : Iso ⊆ D := by rw [hIsodef]; exact Finset.filter_subset _ _
        ...
        have hScherryDisj : ∀ h : Fin 20, h ∈ Dᶜ → h ∉ A →
            G.Adj h x ∨ G.Adj h y ∨ G.Adj h z := by
          intro h hhDc hhA
          by_contra hcon
          push Not at hcon
          exact hhA (by rw [hAdef, Finset.mem_filter]; exact ⟨hhDc, hcon.1, hcon.2.1, hcon.2.2⟩)
        exact cherry_C6_distinct_twin_twoHub_twenty G D Iso A x y z hIsodef hIsoprop hIsoD
          hdeg4 hC4 hAsub hcard7 hIso5 hDc9 (fun g hg => (getA g hg).2) hScherryDisj hsplit4
          hdistinct hisoSum
    · obtain ⟨a, b, c, haA, hbA, hcA, hab, hac, hbc⟩ :=
        mantel_seven_triangle G A hcard7 hbig
  ...
  have hubne : ∀ g : Fin 20, g ∈ Dᶜ → g ≠ x ∧ g ≠ y ∧ g ≠ z := by
    intro g hg
    refine ⟨?_, ?_, ?_⟩ <;> rintro rfl
