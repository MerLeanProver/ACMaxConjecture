...
/-!
# Structural leaves for the `n = 20`, `e(M) = 3`, `|D| = 8` hub-triangle corner

This file collects the *concrete, decidable-free* structural facts about the residual
configuration consumed by `exists_hub_triangle_config_residual_twenty` in
`TwinCert20HubTriangle`: the **twelve** degree-`4` hubs (`Dᶜ`), the four `M`-isolated degree-`3`
...
The `n = 20` deltas from `n = 19` are `|Hub| = 12` (vs `11`), so each cherry has `≥ 8` avoiders
(vs `≥ 7`) and the hub-internal-degree total is `∑ int = 4·12 − 6 − 12 = 30` (vs `26`).  The wider
budget pins the fully-free count to `|FF| ∈ {2, 3, 4, 5, 6, 7, 8, 9, 10}`.
-/
...
theorem nbr_split_DC (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20)) (v : Fin 20) :
...
`L₂ ≁ c₁`. -/
theorem leaf_nbr_D (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20)) (L₁ c₁ c₂ L₂ : Fin 20)
...
exactly `2 + 1 + 1 + 2 = 6` edges to the twelve hubs: `c₁, c₂` have one hub-neighbour each (in-`M`
degree `2`), while the leaves `L₁, L₂` have two each (in-`M` degree `1`). -/
theorem path_hub_incidence (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20)) (L₁ c₁ c₂ L₂ : Fin 20)
    (hcov : ∀ p q : Fin 20, p ∈ D → q ∈ D → G.Adj p q →
      p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
...
/-- **Hub and twin counts.**  In the `|D| = 8` corner there are twelve hubs (`Dᶜ.card = 12`) and
four
...
theorem hub_struct (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20)) (L₁ c₁ c₂ L₂ : Fin 20)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    ...
    (hD8 : D.card = 8) :
    Dᶜ.card = 12 ∧ Iso.card = 4 := by
  classical
  have hDc : Dᶜ.card = 12 := by
  ...
  set P : Finset (Fin 20) := {L₁, c₁, c₂, L₂} with hP
...
hub-neighbours and there are `12` hubs, then at least `8` hubs avoid all of `a, b, c`. -/
theorem avoiders_ge_six_aux (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20)) (a b c : Fin 20)
    (hDc9 : Dᶜ.card = 12)
...
/-- **Each cherry has at least seven avoiding hubs.**  Cherry `{L₁, c₁, c₂}` collects only
`2 + 1 + 1 = 4` hub-incidences (`L₁` two, `c₁`/`c₂` one each), so `≥ 8` of the twelve hubs avoid it;
symmetrically for `{c₁, c₂, L₂}`. -/
theorem cherry_avoiders_ge_six (G : SimpleGraph (Fin 20)) (D : Finset (Fin 20))
    (L₁ c₁ c₂ L₂ : Fin 20) (hDc9 : Dᶜ.card = 12)
    ...
    8 ≤ (Dᶜ.filter (fun g => ¬G.Adj g L₁ ∧ ¬G.Adj g c₁ ∧ ¬G.Adj g c₂)).card ∧
      8 ≤ (Dᶜ.filter (fun g => ¬G.Adj g c₁ ∧ ¬G.Adj g c₂ ∧ ¬G.Adj g L₂)).card := by
...
theorem avoider_internal_ge_two (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (g allowed : Fin 20) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
    (hclass : ∀ x : Fin 20, G.Adj g x → x ∈ D → x = allowed ∨ x ∈ Iso) :
...
theorem fully_free_internal_ge_three (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (g : Fin 20) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
    (hclass : ∀ x : Fin 20, G.Adj g x → x ∈ D → x ∈ Iso) :
...
theorem path_iso_partition (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (L₁ c₁ c₂ L₂ : Fin 20)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hisochar : ∀ w : Fin 20, w ∈ D → w ≠ L₁ → w ≠ c₁ → w ≠ c₂ → w ≠ L₂ → w ∈ Iso)
    (hL1D : L₁ ∈ D) (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hL2D : L₂ ∈ D)
    (hac1L1 : G.Adj c₁ L₁) (hc12 : G.Adj c₁ c₂) (hac2L2 : G.Adj c₂ L₂) :
    ({L₁, c₁, c₂, L₂} : Finset (Fin 20)) ∪ Iso = D ∧
      Disjoint ({L₁, c₁, c₂, L₂} : Finset (Fin 20)) Iso := by
  classical
  set P : Finset (Fin 20) := {L₁, c₁, c₂, L₂} with hP
  have hIsoD : Iso ⊆ D := by rw [hIsodef]; exact Finset.filter_subset _ _
  have hPD : P ⊆ D := by
...
/-- **Residual incidence sums.**  Over the twelve hubs the path-, iso- and hub-internal incidences
total `6`, `12`, `30` respectively, and per hub the three split the degree `4`:
`path(g) + iso(g) + int(g) = 4`. -/
theorem residual_incidence_sums (G : SimpleGraph (Fin 20)) (D Iso : Finset (Fin 20))
    (L₁ c₁ c₂ L₂ : Fin 20)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 20, G.Adj v w → G.degree w ≠ 3))
    (hisochar : ∀ w : Fin 20, w ∈ D → w ≠ L₁ → w ≠ c₁ → w ≠ c₂ → w ≠ L₂ → w ∈ Iso)
    ...
    (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 20))).card = 6) ∧
      (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Iso).card = 12) ∧
      (∀ g ∈ Dᶜ, (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 20))).card
  ...
  have hsum_path : ∑ g ∈ Dᶜ, (G.neighborFinset g ∩ P).card = 6 := by
    rw [cross_count_twenty G Dᶜ P, hP, Finset.sum_insert (by simp [hL1c1, hL1nc2, hL1L2]),
...
internal degree `≥ 3` (`hFFint`), `|A1|, |A2| ≥ 8`, `FF = A1 ∩ A2`, and the hub-internal-degree
total `∑_{Dᶜ} f = 30`, the fully-free count lies in `{2, 3, 4, 5, 6, 7, 8, 9, 10}`. -/
theorem active_hub_budget_twenty (Dc A1 A2 FF : Finset (Fin 20)) (f : Fin 20 → ℕ)
    (hA1sub : A1 ⊆ Dc) (hA2sub : A2 ⊆ Dc) (hFF : FF = A1 ∩ A2)
    (hA1card : 8 ≤ A1.card) (hA2card : 8 ≤ A2.card)
    (hA1int : ∀ g ∈ A1, 2 ≤ f g) (hA2int : ∀ g ∈ A2, 2 ≤ f g)
    (hFFint : ∀ g ∈ FF, 3 ≤ f g) (hSum18 : ∑ w ∈ Dc, f w = 30) :
  ...
  -- `|FF| ≤ 10`: `3|FF| ≤ ∑_{FF} f ≤ ∑_{Dᶜ} f = 30`.
  have hFFsum_le : ∑ g ∈ FF, f g ≤ 30 := by
    rw [← hSum18]
