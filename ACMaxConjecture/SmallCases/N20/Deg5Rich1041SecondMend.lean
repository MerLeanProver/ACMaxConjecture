...
/-!
# The second `M`-endpoint `z'` for the share-2 (11,7,45) octahedron (`n = 20`)
...
theorem octahedron_second_mend_share2_1041_twenty (G : SimpleGraph (Fin 20))
    ...
    (hHub : Hub.card = 11) (_hIso : Iso.card = 7) (hdsum : ∑ w ∈ Hub, G.degree w = 45)
    (hT : ¬∃ x y z : Fin 20, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧ G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧
      G.degree x + G.degree y + G.degree z ≤ 11)
    (hC4 : ¬∃ a b c d : Fin 20, ({a, b, c, d} : Finset (Fin 20)).card = 4 ∧ G.Adj a b ∧ G.Adj b c ∧
  ...
  have hzmem : z = z₁ ∨ z = z₂ := by
    have hm : z ∈ ({z₁, z₂} : Finset (Fin 20)) := hZeq ▸ hzZ
  ...
  obtain ⟨z', hz'Z, hz'ne, hzz'adj, hnohub'⟩ :
      ∃ z', z' ∈ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 20)) ∧ z' ≠ z ∧ G.Adj z z' ∧
  ...
  have hqf : q ≠ f := by
    ...
    exact hz'notIso (hfNsubIso this)
  -- Two distinct deg-`5` hubs would push `∑_{Hub} deg ≥ 5 + 5 + 4·9 = 46 > 45`.
