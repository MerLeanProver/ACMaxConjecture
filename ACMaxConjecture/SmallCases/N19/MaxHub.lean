...
/-!
# `n = 19`, `e(M) = 0` maximal-hub corner `(11, 8, 44)` (`|D| = 8`, `|Hub| = 11`)

This file closes the `s = 0`, `|D| = 8` residual of `two_hub_config_nineteen`
(`TwinCert19.lean:517`).  There the eleven hubs all have degree `4`, the eight `M`-isolated twins
...
(`nogood_of_not_select_nineteen`), giving for every non-adjacent hub `b`:
...
* adjacent hubs share `0` twins (`hadj0`, an `hT` light triangle `4 + 4 + 3 = 11`), and there are
  `≤ 4 − A` of them (`hub_neighbor_le_nineteen`).

Splitting the `b ≠ h*` sum along adjacency to `h*` bounds `24 − A ≤ 22 − A`, a contradiction.  Hence
...
/-- **Maximal-hub good-pair selection (`n = 19`, `(11, 8, 44)`).**  Under the `s = 0`, `|D| = 8`
structure (eleven degree-`4` hubs, eight tri-incident twins, adjacent hubs sharing no twin) a good
non-adjacent degree-`4` pair with `≥ 2` private twins on each side exists. -/
theorem two_hub_maxhub_pair_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    (hHub11 : Hub.card = 11) (hIso8 : Iso.card = 8)
  ...
  have htot : ∑ h ∈ Hub, (G.neighborFinset h ∩ Iso).card = 24 := by
    have h := hub_iso_sum_nineteen G Hub Iso hiso3; rw [h, hIso8]
  by_contra hcon
  have key := nogood_of_not_select_nineteen G Hub Iso hcon
  -- `h*` maximises iso-degree.
  have hHubne : Hub.Nonempty := by rw [← Finset.card_pos, hHub11]; norm_num
  ...
  have hAle4 : A ≤ 4 := by
    ...
    calc (G.neighborFinset hs ∩ Iso).card
      ...
      _ = 4 := hdeg4 hs hsHub
  set S : Finset (Fin 19) := Hub.erase hs with hSdef
  have hScard : S.card = 10 := by rw [hSdef, Finset.card_erase_of_mem hsHub, hHub11]
  -- `φ b := shared(h*, b)`.
  set φ : Fin 19 → ℕ := fun b => (G.neighborFinset hs ∩ G.neighborFinset b ∩ Iso).card with hφ
  -- **(F1)** `∑_{b∈S} a(b) = 24 − A`.
  ...
  set Y : Finset (Fin 19) := G.neighborFinset hs ∩ Iso with hYdef
  ...
  have hcross : ∑ b ∈ Hub, (G.neighborFinset b ∩ Y).card
      = ∑ p ∈ Y, (G.neighborFinset p ∩ Hub).card := cross_count_nineteen G Hub Y
  ...
  have hφeq : ∀ b : Fin 19, (G.neighborFinset b ∩ Y).card = φ b := by
    intro b
    simp only [hφ, hYdef]
    congr 1
    ext w
  ...
  set P : Fin 19 → Prop := fun b => G.Adj hs b with hP
  ...
  have hadjcard : (S.filter P).card + A ≤ 4 := by
    rw [hAdjEq, hAdef]
    exact hub_neighbor_le_nineteen G Hub Iso hdisj hs (hdeg4 hs hsHub)
  -- `a`-sum split.
