...
(`exists_align_four/six_config_nineteen`, `halign8_nineteen`) all call: the verified `n = 18`
...
* `thin_eM_formula_nineteen` — the dominating-edge `e(M)` formula `∑ + 2 = 2·inM(c₁) + 2·inM(c₂)`
  (structural, `n`-independent up to the ambient `cross_count_nineteen`);
* `distinct_five_nineteen` / `hub_cycle_cases_nineteen` — the induced-`C₅` geometry helpers;
* `shared_deg4_hub_from_count_nineteen` — pigeonhole producing a shared degree-`4` hub;
* `claw_shared_two_twin_nineteen` — fat claw centre + shared hub ⇒ `TwoTwinConfig`.

The geometric/cherry-avoidance arguments port verbatim; only the cardinality bookkeeping changes
(`Fin 19`, the `cross_count_nineteen` / `card_four_nineteen` / `hub_meets_path_le_one_nineteen` /
...
theorem thin_eM_formula_nineteen (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19))
  ...
  have hpair2 : ∑ w ∈ ({c₁, c₂} : Finset (Fin 19)),
    ...
    rw [Finset.sum_insert (by simp [hc12.ne]), Finset.sum_singleton]
  have he1 : (G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 19)))).card
      = (G.neighborFinset c₁ ∩ D).card - 1 := by
    have hset : G.neighborFinset c₁ ∩ (D \ ({c₁, c₂} : Finset (Fin 19)))
  ...
  have he2 : (G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 19)))).card
      = (G.neighborFinset c₂ ∩ D).card - 1 := by
    have hset : G.neighborFinset c₂ ∩ (D \ ({c₁, c₂} : Finset (Fin 19)))
        = (G.neighborFinset c₂ ∩ D).erase c₁ := by
      apply Finset.Subset.antisymm
...
of cardinality `5` over `Fin 19` has its five entries pairwise distinct.  Port of
`distinct_five_fourteen`. -/
theorem distinct_five_nineteen (a b c d e : Fin 19)
    (h : ({a, b, c, d, e} : Finset (Fin 19)).card = 5) :
    a ≠ b ∧ a ≠ c ∧ a ≠ d ∧ a ≠ e ∧ b ≠ c ∧ b ≠ d ∧ b ≠ e ∧
      c ≠ d ∧ c ≠ e ∧ d ≠ e := by
  classical
  have b3 : ∀ x y z : Fin 19, ({x, y, z} : Finset (Fin 19)).card ≤ 3 := by
    intro x y z
    have h1 := Finset.card_insert_le x ({y, z} : Finset (Fin 19))
    have h2 := Finset.card_insert_le y ({z} : Finset (Fin 19))
    simp only [Finset.card_singleton] at *
    omega
  have b4 : ∀ w x y z : Fin 19, ({w, x, y, z} : Finset (Fin 19)).card ≤ 4 := by
    intro w x y z
    have h1 := Finset.card_insert_le w ({x, y, z} : Finset (Fin 19))
    have h2 := b3 x y z
    omega
  have ha : a ∉ ({b, c, d, e} : Finset (Fin 19)) := by
    intro hmem
    have : ({a, b, c, d, e} : Finset (Fin 19)).card ≤ 4 := by
      rw [Finset.insert_eq_self.mpr hmem]; exact b4 b c d e
    omega
  have hca4 : ({b, c, d, e} : Finset (Fin 19)).card = 4 := by
    rw [Finset.card_insert_of_notMem ha] at h; omega
  have hb : b ∉ ({c, d, e} : Finset (Fin 19)) := by
    intro hmem
    have : ({b, c, d, e} : Finset (Fin 19)).card ≤ 3 := by
      rw [Finset.insert_eq_self.mpr hmem]; exact b3 c d e
    omega
  have hcb3 : ({c, d, e} : Finset (Fin 19)).card = 3 := by
    rw [Finset.card_insert_of_notMem hb] at hca4; omega
  have hc : c ∉ ({d, e} : Finset (Fin 19)) := by
    intro hmem
    have h1 := Finset.card_insert_le d ({e} : Finset (Fin 19))
    have : ({c, d, e} : Finset (Fin 19)).card ≤ 2 := by
      rw [Finset.insert_eq_self.mpr hmem]
      simp only [Finset.card_singleton] at *; omega
    omega
  have hcc2 : ({d, e} : Finset (Fin 19)).card = 2 := by
...
a cycle `P₃` give a triangle or good `C₄` (`hub_meets_path_le_one_nineteen`).  Packaged as a `6`-way
split.  Port of `hub_cycle_cases`. -/
theorem hub_cycle_cases_nineteen (G : SimpleGraph (Fin 19))
    (hT : ¬∃ x y z : Fin 19, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 19, ({a, b, c, d} : Finset (Fin 19)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (h v₁ v₂ v₃ v₄ v₅ : Fin 19) (hh : G.degree h = 4)
  ...
  have P123 := hub_meets_path_le_one_nineteen G hT hC4 hh hd1 hd2 hd3 e12 e23 n13 d12 d23 d13
  have P234 := hub_meets_path_le_one_nineteen G hT hC4 hh hd2 hd3 hd4 e23 e34 n24 d23 d34 d24
  have P345 := hub_meets_path_le_one_nineteen G hT hC4 hh hd3 hd4 hd5 e34 e45 n35 d34 d45 d35
  have P451 := hub_meets_path_le_one_nineteen G hT hC4 hh hd4 hd5 hd1 e45 e51
    (fun hh => n14 hh.symm) d45 d15.symm d14.symm
  have P512 := hub_meets_path_le_one_nineteen G hT hC4 hh hd5 hd1 hd2 e51 e12
...
theorem shared_deg4_hub_from_count_nineteen (G : SimpleGraph (Fin 19)) (Iso Hub4 : Finset (Fin 19))
  ...
  have hincid : ∃ h ∈ Hub4, 2 ≤ (Iso.filter (fun v => G.Adj v h)).card := by
    ...
    have hswap : ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub4).card
      ...
      have hL : ∀ v : Fin 19, (G.neighborFinset v ∩ Hub4).card
...
theorem claw_shared_two_twin_nineteen (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19))
