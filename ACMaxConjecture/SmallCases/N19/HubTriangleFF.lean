...
case-split on the fully-free hub count `|FF| ∈ {2, 3, 4, 5, 6}` (from `active_hub_budget_nineteen`).
...
* `|FF| ∈ {4, 5}`: the extremal fully-free corners.  `|FF| = 4` splits on `∑_FF int ∈ {12, 13, 14}`
  ...
  via `ff3_extract_twohub_nineteen`; `14`: a Mantel-`4` triangle).
  `|FF| = 5` (`|B| = 4`, `A1 = A2 = FF`) forces `∑_FF int ≥ 16`, hence `∑_FF(N ∩ FF) ≥ 14`, a
  Mantel-`5` triangle.  Both Mantel triangles lie in `A2`, contradicting
  `htri2`.
-/
...
theorem iso_incidence_le_four (G : SimpleGraph (Fin 19)) (Iso A : Finset (Fin 19))
    (hIso4 : Iso.card = 4)
...
theorem two_iso3_nonadj_false (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19))
    ...
    (hdeg4 : ∀ w : Fin 19, w ∈ Dᶜ → G.degree w = 4) (hIso4 : Iso.card = 4)
    (h₁ h₂ : Fin 19) (hh1Dc : h₁ ∈ Dᶜ) (hh2Dc : h₂ ∈ Dᶜ) (hh12 : h₁ ≠ h₂)
    (hnadj : ¬G.Adj h₁ h₂)
...
(|FF| − 1)`. -/
theorem mantel_bound (G : SimpleGraph (Fin 19)) (FF : Finset (Fin 19)) (v : Fin 19)
    (hvFF : v ∈ FF)
    (htri : ¬∃ a b c : Fin 19, a ∈ FF ∧ b ∈ FF ∧ c ∈ FF ∧
...
theorem mantel_six_triangle (G : SimpleGraph (Fin 19)) (FF : Finset (Fin 19))
    (hcard : FF.card = 6)
    (hedge : 19 ≤ ∑ g ∈ FF, (G.neighborFinset g ∩ FF).card) :
    ∃ a b c : Fin 19, a ∈ FF ∧ b ∈ FF ∧ c ∈ FF ∧ G.Adj a b ∧ G.Adj a c ∧ G.Adj b c := by
  classical
  by_contra htri
...
contradiction. -/
theorem inactive_iso8_false (G : SimpleGraph (Fin 19)) (D Iso B P : Finset (Fin 19))
    (hC4 : ¬∃ a b c d : Fin 19, ({a, b, c, d} : Finset (Fin 19)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (hIsoD : Iso ⊆ D)
    ...
    (hpath1 : ∀ h : Fin 19, h ∈ B → 1 ≤ (G.neighborFinset h ∩ P).card)
  ...
  have hint0 : ∀ u : Fin 19, u ∈ B → (G.neighborFinset u ∩ Iso).card = 3 →
      (G.neighborFinset u ∩ Dᶜ).card = 0 := by
    intro u huB hu3
    have hp := hper u (hBsub huB)
    have hpa := hpath1 u huB
    omega
  have hnadj : ∀ u w : Fin 19, w ∈ Dᶜ → (G.neighborFinset u ∩ Dᶜ).card = 0 →
      ¬G.Adj u w := by
...
theorem ff3_extract_twohub_nineteen (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19))
    (L₁ c₁ c₂ L₂ : Fin 19) (B : Finset (Fin 19))
    (hth : ¬TwoHubConfig G)
    (hC4 : ¬∃ a b c d : Fin 19, ({a, b, c, d} : Finset (Fin 19)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    ...
    (hDdeg3 : ∀ v : Fin 19, v ∈ D → G.degree v = 3)
    (hDeq : ({L₁, c₁, c₂, L₂} : Finset (Fin 19)) ∪ Iso = D)
    (hBsub : B ⊆ Dᶜ) (hBcard : B.card = 3)
    ...
    (hpath1 : ∀ h : Fin 19, h ∈ B →
      1 ≤ (G.neighborFinset h ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 19))).card) :
    False := by
  classical
...
/-- **No avoider triangle ⇒ contradiction (the `n = 19` `|FF|`-dispatch core).**  Full replacement
for `core_triangle_force_nineteen`: dispatches on `|FF| ∈ {2, 3, 4, 5, 6, 7}`.  The denser budget
...
by `two_iso3_nonadj_false` / `inactive_iso8_false` / `ff3_extract_twohub_nineteen` and Mantel-`5`/`6`
triangles inside `FF ⊆ A2` (contradicting `htri2`).  At `n = 19` the genuinely-new boundary is the
`|FF| = 7`, `|B| = 4` (`A1 = A2 = FF`) corner: the avoider mass is exactly the triangle-free
Mantel-`7` extremal `K_{3,4}` (mass `24`), so the `n = 18` packing/Mantel argument loses all slack
when `r ~ s`.  This corner is **CLOSED** (no `sorry`) by threading the top-level good-triangle
threshold `≤ 11` (`hT`) down from `exists_align_*_config_nineteen`: extracting the two common
`L₁,L₂`-neighbours `r, s ∈ B` (sub-budget independently), a single case-split closes it — `r ≁ s`
fires the good-`C₄` `L₁–r–L₂–s` (`Σ = 14`, `hC4`), and `r ~ s` fires the degree-`11` good triangle
`{r, s, L₁}` (`Σ = 4 + 4 + 3 = 11`, `hT`).  See the inline block in the `|FF| = 7`, `|B| = 4`
case. -/
theorem iso_rich_force_nineteen (G : SimpleGraph (Fin 19))
    ...
    (hDc9 : Dᶜ.card = 11) (hIso4 : Iso.card = 4)
    ...
    (hSum18 : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 26)
    (_htri1 : ¬∃ a b c : Fin 19, a ∈ Dᶜ ∧ b ∈ Dᶜ ∧ c ∈ Dᶜ ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c ∧
      (¬G.Adj a L₁ ∧ ¬G.Adj a c₁ ∧ ¬G.Adj a c₂) ∧
      (¬G.Adj b L₁ ∧ ¬G.Adj b c₁ ∧ ¬G.Adj b c₂) ∧
      (¬G.Adj c L₁ ∧ ¬G.Adj c c₁ ∧ ¬G.Adj c c₂))
    (_htri2 : ¬∃ a b c : Fin 19, a ∈ Dᶜ ∧ b ∈ Dᶜ ∧ c ∈ Dᶜ ∧
      G.Adj a b ∧ G.Adj a c ∧ G.Adj b c ∧
  ...
  obtain ⟨hFFlb, hFFub⟩ :=
    active_hub_budget_nineteen Dᶜ A1 A2 FF (fun g => (G.neighborFinset g ∩ Dᶜ).card)
  ...
  have hBpart : (∑ g ∈ B, (G.neighborFinset g ∩ Dᶜ).card)
      + ∑ g ∈ A1 ∪ A2, (G.neighborFinset g ∩ Dᶜ).card = 26 := by
  ...
  have hFFpath0 : ∀ g ∈ FF,
      (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 19))).card = 0 := by
    intro g hg
    have hgA1 := hFFsubA1 hg
    have hgA2 := hFFsubA2 hg
    rw [hA1def, Finset.mem_filter] at hgA1
  ...
  have hFFsum4 : (∑ g ∈ FF, (G.neighborFinset g ∩ Iso).card)
    ...
    rw [← Finset.sum_add_distrib, Finset.sum_congr rfl hcongr, Finset.sum_const, smul_eq_mul,
      Nat.mul_comm]
  have hintFFle : (∑ g ∈ FF, (G.neighborFinset g ∩ Dᶜ).card) ≤ 26 := by
    rw [← hSum18]
  ...
  have hpath1 : ∀ h ∈ B,
      1 ≤ (G.neighborFinset h ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 19))).card := by
    intro h hh
  ...
  -- Dispatch on `|FF|`.  With `Dᶜ.card = 11`, `∑ int = 26` and avoiders `≥ 7`, the iso bound
  -- forces `|B| ≥ 2` and the internal budget forces `|B| ≤ |FF| − 3`, so the non-vacuous
  -- fully-free counts are `|FF| ∈ {5, 6, 7}`; `|FF| ∈ {2, 3, 4, 8}` collapse by pure counting.
  have hFFcases : FF.card = 2 ∨ FF.card = 3 ∨ FF.card = 4 ∨ FF.card = 5 ∨ FF.card = 6
      ∨ FF.card = 7 ∨ FF.card = 8 := by omega
  rcases hFFcases with hFF | hFF | hFF | hFF | hFF | hFF | hFF
  · -- **`|FF| = 2`:** `|B| ≤ −1` is impossible (`|B| ≥ 2`).
    exfalso; omega
  · -- **`|FF| = 3`:** `|B| ≤ 0` but `|B| ≥ 2`.
    exfalso; omega
  · -- **`|FF| = 4`:** `|B| ≤ 1` but `|B| ≥ 2` (the denser `n = 19` budget kills this corner).
    exfalso; omega
  · -- **`|FF| = 5`:** the budget forces `|B| = 2` with `∑_FF int = 18`, so both inactive hubs
    ...
    exact two_iso3_nonadj_false G D Iso hC4 hIsoD hIsoprop hdeg4 hIso4 h₁ h₂ hh1Dc hh2Dc hh12
      hnadj hiso1 hiso2
  · -- **`|FF| = 6`:** `|B| ∈ {2, 3}`.  `|B| = 2` collapses (`∑_B iso ≥ 8 > 6 = 3·|B|`).
    -- `|B| = 3` splits on `∑_FF int ∈ {20, 21, 22}`.
    have hBcase : B.card = 2 ∨ B.card = 3 := by omega
    rcases hBcase with hBc | hBc
    · -- `|B| = 2`: `∑_FF int = 20`, `∑_B iso ≥ 8 > 6`, impossible.
      exfalso; omega
    · -- `|B| = 3`: `∑_FF int ∈ {20, 21, 22}`.
      ...
      · -- `∑_FF int = 20`: `∑_B iso ≥ 8` ⇒ two iso-`3` non-adjacent inactive hubs.
      ...
      · -- `∑_FF int = 21`: `∑_B int ≤ 1`, `∑_B iso ≥ 7` ⇒ `TwoHubConfig`.
        have hBint1 : (∑ g ∈ B, (G.neighborFinset g ∩ Dᶜ).card) ≤ 1 := by omega
        have hBiso7 : 7 ≤ ∑ g ∈ B, (G.neighborFinset g ∩ Iso).card := by omega
        exact ff3_extract_twohub_nineteen G D Iso L₁ c₁ c₂ L₂ B _hth hC4 hIsoD hIsoprop hdeg4
          (fun v hv => (hmemD v).mp hv) hDeq hBsub hBc hBint1 hBiso7 hper hpath1
      · -- `∑_FF int = 22`: `∑_B int = 0`.  The two extra avoider hubs `C = (A1 ∪ A2) \ FF` each
        ...
        have hCpath : ∀ g ∈ (A1 ∪ A2) \ FF,
            1 ≤ (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 19))).card := by
          intro g hg
        ...
        have hBsplit : (∑ g ∈ B, ((G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} :
            ...
            = (∑ g ∈ B, (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 19))).card)
        ...
        exact inactive_iso8_false G D Iso B ({L₁, c₁, c₂, L₂}) hC4 hIsoD hIsoprop hdeg4 hIso4
          hBsub hBc hper hpath1 hisoB8
  · -- **`|FF| = 7`:** `|B| ∈ {2, 3, 4}`.  `|B| = 2` collapses; `|B| = 3` forces `∑_FF int = 24`
    -- (`∑_B iso ≥ 8`, `inactive_iso8_false`); `|B| = 4` (`A1 = A2 = FF`) is the genuinely-new
    -- `K_{3,4}` boundary corner (`∑_FF(N ∩ FF)` only reaches `24`, the triangle-free Mantel-`7`
    -- extremal, while a Mantel-`7` triangle needs `≥ 25`).
    have hBcase : B.card = 2 ∨ B.card = 3 ∨ B.card = 4 := by omega
    rcases hBcase with hBc | hBc | hBc
    · -- `|B| = 2`: budget impossible.
      exfalso; omega
    · -- `|B| = 3`: `∑_FF int = 24`, `∑_B iso ≥ 8` ⇒ two iso-`3` non-adjacent inactive hubs.
      have hisoB8 : 8 ≤ ∑ g ∈ B, (G.neighborFinset g ∩ Iso).card := by omega
      exact inactive_iso8_false G D Iso B ({L₁, c₁, c₂, L₂}) hC4 hIsoD hIsoprop hdeg4 hIso4
        hBsub hBc hper hpath1 hisoB8
    · -- **`|B| = 4` (`A1 = A2 = FF`): the genuine excess-`11` boundary, CLOSED by threading the
      ...
      · -- `r ≁ s`: the good `C₄` `L₁–r–L₂–s` (`Σ = 14`) fires against `hC4`.
        ...
        rw [Finset.card_insert_of_notMem (by
            ...
            Finset.card_singleton]
  · -- **`|FF| = 8`:** `∑_FF iso ≤ 4` and `iso + int = 4` per fully-free hub force
    -- `∑_FF int ≥ 28 > 26 = ∑_Dᶜ int`, impossible.  (Pure counting.)
    exfalso; omega
