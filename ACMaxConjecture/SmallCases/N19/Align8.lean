...
import ACMaxConjecture.SmallCases.N19.Align8Mid
import ACMaxConjecture.SmallCases.N19.FatStarStruct
import ACMaxConjecture.SmallCases.N19.Align8D10C5
import ACMaxConjecture.SmallCases.N19.Align8D10Dom

/-!
# Dense `e(M) ≥ 4` alignment dichotomy for `n = 19` (`halign8` helpers)

Ports the proved-axiom-clean `n = 17` `TwinCert17Align8` to `Fin 19`, split by the value of `e(M)`.

* `halign8_eM5_nineteen` (`e(M) = 5`, `s = 10`): an all-degree-`4` `M`-isolated cherry routes the
  ...
  two-twin assembly (`two_twin_eM5_mid_nineteen`) for `|D| ≥ 10`.  The `|D| = 9` thin double-star
  (`|Iso| = 3`, where the degree-`≤ 5` shared-hub pigeonhole only *ties*) is the one documented
  residual.

* `halign8_eM4_nineteen` (`e(M) = 4`, `s = 8`): the induced-`C₅` branch is impossible (`s ≥ 10 > 8`),
...
case (a degree-`5` hub on the cherry, `|D| ≥ 10`) routes through `two_twin_eM5_mid_nineteen`.  The
`|D| = 9` thin double-star is isolated as one documented `sorry`. -/
theorem halign8_eM5_nineteen (G : SimpleGraph (Fin 19))
  ...
  · -- **Residual.**  A neighbour of the `M`-isolated twin `t` has degree `≥ 5`, so `|D| ≥ 9`.
    ...
    obtain ⟨w, hw5⟩ : ∃ w : Fin 19, 5 ≤ G.degree w := by
      by_contra hc
      push Not at hc
      exact hall ⟨by have := hc p; omega, by have := hc q; omega, by have := hc r; omega⟩
    have hDge9 : 9 ≤ D.card := by
      have := card_D_ge_nine_of_hub_deg5_nineteen G hm h3 w hw5
      rwa [← hDdef] at this
    ...
    · -- **`|D| = 9` thin double-star (`|Iso| = 3`).**  The degree-`≤ 5` shared-hub pigeonhole only
      ...
      have hnall : ¬∀ w : Fin 19, G.Adj t w → G.degree w = 4 := fun hcon =>
        hall ⟨hcon p htp, hcon q htq, hcon r htr⟩
      rcases eM5_D9_shared_or_alldeg4_nineteen G hm h3 h2k2 hD9 hsum10.le t ht3 htiso hnall with
      ...
      · have hkge4 : 4 ≤ G.degree k := by have := htw1iso k hAtw1k; have := h3 k; omega
        exact Or.inr (Or.inl (two_twin_eM5_from_shared_le5_nineteen G hm h3 hT h2k2 hC4 hDge9
          hsum10 k tw1 tw2 hkge4 hkle5 htw12 htw1deg htw2deg hAtw1k hAtw2k htw1iso htw2iso))
      · exact Or.inl (sv_eM5_from_alldeg4_iso_nineteen G hT h2k2 hC4 hsum10 t' ht'3 ht'iso ht'all)
    · by_cases hD10 : D.card = 10
      · -- **`|D| = 10` thin double-star** — CLOSED: the dominating-edge branch routes through the
        ...
        -- (`eM5_D10_domedge_sv_or_shared_nineteen`), and the induced-`C₅` branch through the
        -- `|D| = 10` shared-hub count (`eM5_D10_c5_shared_nineteen`); both shared-hub outcomes
        -- feed `two_twin_eM5_from_shared_le5_nineteen`.
        have hne : ∃ a b : Fin 19, a ∈ D ∧ b ∈ D ∧ G.Adj a b := by
          by_contra hcon
          push Not at hcon
          have hz : ∀ v ∈ D, (G.neighborFinset v ∩ D).card = 0 := by
        ...
        rcases dominating_edge_or_induced_C5 G D hmemD hT hC4 h2k2 hne with hdom | hC5
        · obtain ⟨c₁, c₂, hc1D, hc2D, hc12, hcov⟩ := hdom
          rcases eM5_D10_domedge_sv_or_shared_nineteen G hm h3 hT hD10 c₁ c₂
          ...
          · have hkge4' : 4 ≤ G.degree k := by
              ...
              have := h3 k
              omega
            exact Or.inr (Or.inl (two_twin_eM5_from_shared_le5_nineteen G hm h3 hT h2k2 hC4
              hDge9 hsum10 k tw1' tw2' hkge4' hkle5' htw12' htw1deg' htw2deg'
              hAtw1k' hAtw2k' htw1iso' htw2iso'))
        · obtain ⟨v₁, v₂, v₃, v₄, v₅, hv1D, hv2D, hv3D, hv4D, hv5D, hcard5,
            e12, e23, e34, e45, e51, _n13, _n14, _n24, _n25, _n35, _⟩ := hC5
          obtain ⟨k, tw1', tw2', hkle5', htw12', htw1deg', htw2deg', hAtw1k', hAtw2k',
            htw1iso', htw2iso'⟩ :=
            eM5_D10_c5_shared_nineteen G hm h3 hD10 v₁ v₂ v₃ v₄ v₅
              ((hmemD v₁).mp hv1D) ((hmemD v₂).mp hv2D) ((hmemD v₃).mp hv3D)
              ((hmemD v₄).mp hv4D) ((hmemD v₅).mp hv5D)
              hcard5 e12 e23 e34 e45 e51 hsum10
          have hkge4' : 4 ≤ G.degree k := by
            have := htw1iso' k hAtw1k'
            have := h3 k
            omega
      ...
      · have hDge11 : 11 ≤ D.card := by omega
        exact Or.inr (Or.inl (two_twin_eM5_mid_nineteen G hm h3 hT h2k2 hC4 hDge11 hsum10))
...
theorem halign8_eM4_nineteen (G : SimpleGraph (Fin 19))
    (hm : G.edgeFinset.card = 34) (h3 : ∀ v : Fin 19, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 19, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 19, ({a, b, c, d} : Finset (Fin 19)).card = 4 ∧
  ...
  · -- **Dominating-edge double-star (the only surviving structure at `e(M) = 4`).**
    ...
    · -- **Residual `|D| = 8` (`|Hub| = 10`, all degree-`4`, `|Iso| = 3`).**  The dominating
      ...
      -- `fatstar_structure_eM4_D8_nineteen`, supplying the `¬SingleVertex/¬TwoTwin/¬TwoHub` residual.
      ...
      · exact Or.inr (Or.inr (Or.inr (fatstar_structure_eM4_D8_nineteen G hm h3 hT hC4 D hmemD hD8
          c₁ c₂ hc1D hc2D hc12 hcov hindle hs8 hsv htt hth)))
    · -- **`|D| ∈ {9, …, 13}`.**  Extract a degree-`≤ 5` hub shared by two `M`-isolated twins and
      -- route the dominating-edge double-star through `dom_fat_centre_two_twin_le5_nineteen`.
      have hDge9 : 9 ≤ D.card := by omega
      obtain ⟨k, tw1, tw2, hkle5, htw12, htw1deg, htw2deg, hAtw1k, hAtw2k, htw1iso, htw2iso⟩ :=
        shared_hub_le5_eM4_nineteen G hm h3 h2k2 hDge9 hs8.le
      have hkge4 : 4 ≤ G.degree k := by
        have := htw1iso k hAtw1k; have := h3 k; omega
      exact Or.inr (Or.inl (dom_fat_centre_two_twin_le5_nineteen G D hmemD hT hC4 c₁ c₂ hc1D hc2D
