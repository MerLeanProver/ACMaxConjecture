...
/-! # The rigid octahedron structure for the share-2 (10,7,41) corner (`n = 19`) -/
namespace ACMax
...
/-- **Force the rigid `2+1+2` octahedron partition.**  The blocked rich set is
exactly `{g, r_t, r_z, a, b}` with `c` the unique twin of `h₂` (`N c ∩ Hub =
{h₂, g, r_t}`, `N z ∩ Hub = {h₂, r_z}`, `N h₂ ∩ Hub = {a, b}`). -/
theorem octahedron_struct_share2_1041_nineteen (G : SimpleGraph (Fin 19))
  ...
  have hRge : 5 ≤ R.card :=
    rich_count_ge_five_1041_nineteen G Hub Iso hiso3 hdisj hsum17 hdeg3 hisodeg3 hleak hdeg
      hdeg5 hHub hIso hdsum hshare hno2hub g hg hgd hgiso
  have hRle : 5 ≥ R.card :=
    rich_le_five_blocked_1041_nineteen G Hub Iso hiso3 hdisj hsum17 hdeg3 hisodeg3 hleak hdeg
  ...
  have hshared_eq : G.neighborFinset g ∩ G.neighborFinset h₂ ∩ Iso = {c} := by
    have hsub : G.neighborFinset g ∩ G.neighborFinset h₂ ∩ Iso ⊆ ({c} : Finset (Fin 19)) := by
      intro x hx
  ...
  have hh₂Hub2le : (G.neighborFinset h₂ ∩ Hub).card ≤ 2 := by
    ...
    have hzge : 1 ≤ (G.neighborFinset h₂ ∩ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 19))).card :=
  ...
  have hrz_ne_b : r_z ≠ b := by
    ...
    rw [hABdisj] at hmem; simp at hmem
  have h5card : ({g, r_t, r_z, a, b} : Finset (Fin 19)).card = 5 :=
    card_five_nineteen g r_t r_z a b hg_ne_rt hg_ne_rz hg_ne_a hg_ne_b hrt_ne_rz hrt_ne_a
      hrt_ne_b hrz_ne_a hrz_ne_b hab
  -- === The three neighbourhood set equalities. ===
  have hcard3set : ({h₂, g, r_t} : Finset (Fin 19)).card = 3 :=
    Finset.card_eq_three.mpr ⟨h₂, g, r_t, hgh₂.symm, Ne.symm hr_tne_h₂, Ne.symm hr_tne_g, rfl⟩
  have hsub3 : ({h₂, g, r_t} : Finset (Fin 19)) ⊆ G.neighborFinset c ∩ Hub := by
    intro x hx
    ...
    · exact hginNc
    · exact hr_tSc
  have hNceq : G.neighborFinset c ∩ Hub = ({h₂, g, r_t} : Finset (Fin 19)) :=
    (Finset.eq_of_subset_of_card_le hsub3 (le_of_eq (hcHub3.trans hcard3set.symm))).symm
  have hcard2z : ({h₂, r_z} : Finset (Fin 19)).card = 2 :=
    Finset.card_eq_two.mpr ⟨h₂, r_z, Ne.symm hr_zne_h₂, rfl⟩
  have hsub2z : ({h₂, r_z} : Finset (Fin 19)) ⊆ G.neighborFinset z ∩ Hub := by
    intro x hx
    simp only [Finset.mem_insert, Finset.mem_singleton] at hx
    rcases hx with rfl | rfl
    · exact hh₂inNz
    · exact hr_zSz
  have hNzeq : G.neighborFinset z ∩ Hub = ({h₂, r_z} : Finset (Fin 19)) :=
    (Finset.eq_of_subset_of_card_le hsub2z (le_of_eq (hzHub2.trans hcard2z.symm))).symm
  -- === `R = {g, r_t, r_z, a, b}` via the cover `R ⊆ U ∖ {h₂}`. ===
  have hRsub5 : R ⊆ ({g, r_t, r_z, a, b} : Finset (Fin 19)) := by
    intro x hx
    have hxU := hRsub hx
  ...
  have hR5 : R = ({g, r_t, r_z, a, b} : Finset (Fin 19)) :=
