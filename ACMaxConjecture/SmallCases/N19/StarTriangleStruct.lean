...
/-!
# Structural foundation for the `n = 19` star-triangle extraction

This file collects the genuinely-clean foundational counting facts used by the star-triangle
dichotomy for the tight `e(M) = 1`, all-degree-`4` profile `(|Hub|, |Iso|, ∑deg) = (11, 6, 44)`
(`Z.card = 2`), the `n = 19` analog of the rigid no-two-hub `n = 18` regime `(10, 6, 40)`.

At `n = 19` *all* `e(M) = 1` tight profiles route through star-triangle; the headline regime here is
the NEW `|Hub| = 11` all-degree-`4` profile.  The structural counting below is `n`-generic given the
regime hypotheses (`Hub.card = 11`, `Iso.card = 6`, `∑_{Hub} deg = 44`); only the regime constants
...
theorem rich_nonadj_share_eq_one_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
...
/-- **N3 — the hubs span exactly `22` hub-hub incidences (`11` edges).**  Each hub's degree splits
...
(the leak bound caps it at `4`).  Hence `∑|N∩Hub| = 44 − 18 − 4 = 22`. -/
theorem hub_hub_edge_count_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    ...
    ∑ h ∈ Hub, (G.neighborFinset h ∩ Hub).card = 22 := by
  classical
...
`strong_deg4_count_le_two_deg5_nineteen` bound specialised through `hno2hub`. -/
theorem rich_a3_count_le_two_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
