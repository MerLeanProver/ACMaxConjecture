...
/-!
# Shared `n = 19` cherry-corner extraction helpers
...
(`TwinCert19CherryP3.lean`).  Ported from the axiom-clean `n = 17` development
(`TwinCert17CherryCore.lean`); the `n = 19` deltas are `32 → 34` edges, `64 → 68` total degree, the
internal sum `68 → 72 − 6|D|`, and `cross_count → cross_count_nineteen`.

The cherry-avoiding pigeonhole here additionally returns a *dense-avoider triangle* in the tie
branch (the `n = 19` `|D| = 9` corner needs it: there the arithmetic does not exclude the
...
/-- **Cherry-avoiding hub assembly into `TwoTwinConfig` (`n = 19`).**  A degree-`≤ 5` hub `h`
avoiding the cherry `x–y–z` (centre `y`) and carrying two distinct `M`-isolated degree-`3` twins
`t₁, t₂` yields a `TwoTwinConfig`. -/
theorem twotwin_assemble_cherry_nineteen (G : SimpleGraph (Fin 19))
    (Iso : Finset (Fin 19)) (x y z h t₁ t₂ : Fin 19)
...
/-- **Raw cherry-avoiding two-twin assembly (`n = 19`).**  Identical conclusion to
`twotwin_assemble_cherry_nineteen`, but the two twins are supplied through their raw degree-`3`
plus no-degree-`3`-neighbour facts rather than membership in an `Iso` set. -/
theorem twotwin_assemble_cherry_raw_nineteen (G : SimpleGraph (Fin 19))
    (x y z h t₁ t₂ : Fin 19)
    (hdegx : G.degree x = 3) (hdegy : G.degree y = 3) (hdegz : G.degree z = 3)
    (hxyA : G.Adj x y) (hyzA : G.Adj y z)
    (hxy_ne : x ≠ y) (hyz_ne : y ≠ z) (hxz_ne : x ≠ z)
    ...
    (ht1no : ∀ w : Fin 19, G.Adj t₁ w → G.degree w ≠ 3)
    (ht2no : ∀ w : Fin 19, G.Adj t₂ w → G.degree w ≠ 3)
    (hAt1h : G.Adj t₁ h) (hAt2h : G.Adj t₂ h) :
    TwoTwinConfig G := by
  classical
  have ht1x : ¬G.Adj t₁ x := fun ha => ht1no x ha (by rw [hdegx])
...
/-- **Cherry-avoiding dichotomy (`n = 19`).**  Same counting hypotheses as
`cherry_avoider_pigeonhole_nineteen`, but with no arithmetic side condition: either a cherry-avoiding
...
theorem cherry_avoider_or_dense_nineteen (G : SimpleGraph (Fin 19))
    (Iso K FullHub Hub5 : Finset (Fin 19)) (C Int : ℕ) (hHub5sub : Hub5 ⊆ FullHub)
    (hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 19, G.Adj v w → G.degree w ≠ 3))
    (hHubdeg4 : ∀ h : Fin 19, h ∈ Hub5 → 4 ≤ G.degree h)
    (hHubdeg5 : ∀ h : Fin 19, h ∈ Hub5 → G.degree h ≤ 5)
    (hdecomp : ∀ h : Fin 19, h ∈ Hub5 → (G.neighborFinset h ∩ K).card
    ...
    (∃ h t₁ t₂ : Fin 19, h ∈ Hub5 ∧ (∀ k : Fin 19, k ∈ K → ¬G.Adj h k) ∧
        G.degree h ≤ 5 ∧ t₁ ≠ t₂ ∧ G.degree t₁ = 3 ∧ G.degree t₂ = 3 ∧
        G.Adj t₁ h ∧ G.Adj t₂ h ∧ (∀ w : Fin 19, G.Adj t₁ w → G.degree w ≠ 3) ∧
        (∀ w : Fin 19, G.Adj t₂ w → G.degree w ≠ 3)) ∨
      (∃ S : Finset (Fin 19), S ⊆ Hub5 ∧ (∀ h : Fin 19, h ∈ S → ∀ k : Fin 19, k ∈ K → ¬G.Adj h k) ∧
        Hub5.card ≤ S.card + C ∧ 3 * S.card ≤ Int ∧
        6 * S.card ≤ (∑ h ∈ S, (G.neighborFinset h ∩ S).card) + Int) := by
  classical
  set HubA : Finset (Fin 19) := Hub5.filter (fun h => (G.neighborFinset h ∩ K).card = 0)
    with hHubAdef
  ...
  have hAvoidK : ∀ h : Fin 19, h ∈ HubA → (G.neighborFinset h ∩ K).card = 0 := by
    intro h hh; rw [hHubAdef, Finset.mem_filter] at hh; exact hh.2
  have hAvoid : ∀ h : Fin 19, h ∈ HubA → ∀ k : Fin 19, k ∈ K → ¬G.Adj h k := by
  ...
  · obtain ⟨h, t₁, t₂, hhHubA, _hd5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1no, ht2no⟩ :=
      shared_hub_le5_from_count_nineteen G Iso HubA hIsoprop
  ...
  · rw [not_lt] at hstrict
    ...
    · -- `6·a ≤ ∑_{HubA}|N ∩ HubA| + Int`.
      set a : ℕ := HubA.card with hadef
      set Rest : Finset (Fin 19) := FullHub \ HubA with hRestdef
      ...
      have hsplitFull : ∀ h : Fin 19, h ∈ HubA → (G.neighborFinset h ∩ FullHub).card
      ...
      have hAMcross : ∑ h ∈ HubA, (G.neighborFinset h ∩ Rest).card
          = ∑ h ∈ Rest, (G.neighborFinset h ∩ HubA).card := cross_count_nineteen G HubA Rest
...
/-- **`P₃`-cherry alignment dichotomy (`n = 19`).**  Packages the hub-incidence counting for the
`e(M) = 2` (`P₃` cherry `x–y–z`) regime and feeds it to `cherry_avoider_or_dense_nineteen`: either a
cherry-avoiding degree-`≤ 5` hub carries two `M`-isolated twins (`TwoTwinConfig`), or the
cherry-avoiding hub set `S ⊆ Hub5` is dense, with `Hub5.card ≤ |S| + 5`, `3|S| ≤ 72 − 6|D|` and the
within-avoider edge bound `6|S| ≤ ∑_{h∈S}|N h ∩ S| + (72 − 6|D|)`.  The dense branch routes (for
`|D| = 9`) to a hub triangle, and is arithmetically impossible for `|D| ∈ {10, 11}`. -/
theorem p3_cherry_dichotomy_nineteen (G : SimpleGraph (Fin 19)) (D Iso Hub5 : Finset (Fin 19))
    (x y z : Fin 19) (hm : G.edgeFinset.card = 34) (h3 : ∀ v : Fin 19, 3 ≤ G.degree v)
    ...
    (hxy_ne : x ≠ y) (hyz_ne : y ≠ z) (hxz_ne : x ≠ z)
    (hHub5sub : Hub5 ⊆ Dᶜ) (hHub5deg5 : ∀ h : Fin 19, h ∈ Hub5 → G.degree h ≤ 5) :
    TwoTwinConfig G ∨
      (∃ S : Finset (Fin 19), S ⊆ Hub5 ∧
        (∀ h : Fin 19, h ∈ S → ¬G.Adj h x ∧ ¬G.Adj h y ∧ ¬G.Adj h z) ∧
        Hub5.card ≤ S.card + 5 ∧ 3 * S.card ≤ 72 - 6 * D.card ∧
        6 * S.card ≤ (∑ h ∈ S, (G.neighborFinset h ∩ S).card) + (72 - 6 * D.card)) := by
  classical
  ...
  have hNxD : G.neighborFinset x ∩ D = ({y} : Finset (Fin 19)) := by
  ...
  have hNzD : G.neighborFinset z ∩ D = ({y} : Finset (Fin 19)) := by
    apply Finset.Subset.antisymm
    · intro w hw
      rw [Finset.mem_inter, G.mem_neighborFinset] at hw
  ...
  have hKIso : ({x, y, z} : Finset (Fin 19)) ∪ Iso = D := by
    apply Finset.Subset.antisymm
    · intro w hw
  ...
  have hKIsodisj : Disjoint ({x, y, z} : Finset (Fin 19)) Iso :=
    Finset.disjoint_left.mpr (fun w hw hw' => by
      simp only [Finset.mem_insert, Finset.mem_singleton] at hw
  ...
  have hother0 : ∀ v : Fin 19, v ∈ D → v ∉ ({x, y, z} : Finset (Fin 19)) →
  ...
  have hxyz_sub : ({x, y, z} : Finset (Fin 19)) ⊆ D := by
  ...
  have hsumDcdeg : ∑ w ∈ Dᶜ, G.degree w + 3 * D.card = 68 := by
  ...
  have hDcD : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ D).card + 4 = 3 * D.card := by
    ...
    omega
  have hIntinc : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 72 - 6 * D.card := by
  ...
  have hCfull : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ ({x, y, z} : Finset (Fin 19))).card = 5 := by
    have hcc := cross_count_nineteen G Dᶜ ({x, y, z} : Finset (Fin 19))
    ...
    rfl
  have hCinc : ∑ h ∈ Hub5, (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 19))).card ≤ 5 := by
    rw [← hCfull]; exact Finset.sum_le_sum_of_subset hHub5sub
  have hdecomp : ∀ h : Fin 19, h ∈ Hub5 → (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 19))).card
      + (G.neighborFinset h ∩ Iso).card + (G.neighborFinset h ∩ Dᶜ).card = G.degree h := by
    intro h _
    have hd1 : (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 19))).card
        + (G.neighborFinset h ∩ Iso).card = (G.neighborFinset h ∩ D).card := by
      have hdisj : Disjoint (G.neighborFinset h ∩ ({x, y, z} : Finset (Fin 19)))
    ...
    have hd2 := hsplit h
    omega
  have hHubdeg4 : ∀ h : Fin 19, h ∈ Hub5 → 4 ≤ G.degree h := by
    intro h hh
    have hhDc : h ∈ Dᶜ := hHub5sub hh
    rw [Finset.mem_compl, hmemD] at hhDc; have := h3 h; omega
  rcases cherry_avoider_or_dense_nineteen G Iso ({x, y, z} : Finset (Fin 19)) Dᶜ Hub5 5
      (72 - 6 * D.card) hHub5sub hIsoprop hHubdeg4 hHub5deg5 hdecomp hCinc hIntinc with hhub | hdense
  · obtain ⟨h, t₁, t₂, hhHub5, havoid, hhdeg5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1no, ht2no⟩ :=
    ...
    exact Or.inl (twotwin_assemble_cherry_raw_nineteen G x y z h t₁ t₂ hdegx hdegy hdegz hxyA hyzA
