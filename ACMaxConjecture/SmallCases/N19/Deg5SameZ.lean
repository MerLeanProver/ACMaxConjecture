...
/-! # The same-`z` hub pair (`n = 19`, deg-5 corner): non-adjacent, share-free, one is iso-poor -/
namespace ACMax
...
`isoDeg ≤ 1` (`hno2hub`). -/
theorem same_z_pair_deg5_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    ...
    (hno2hub : ¬∃ h₁ h₂ : Fin 19, h₁ ∈ Hub ∧ h₂ ∈ Hub ∧
      ...
      2 ≤ ((G.neighborFinset h₂ ∩ Iso) \ G.neighborFinset h₁).card)
    (z p q : Fin 19) (hz : z ∈ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 19)))
  ...
  obtain ⟨hziso0, _, hzdeg3⟩ :=
    zfacts_deg5_nineteen G Hub Iso hiso3 hdisj hsum17 hdeg3 hisodeg3 hleak z hz
  ...
  have hshare : G.neighborFinset p ∩ G.neighborFinset q ∩ Iso = ∅ := by
    ...
    exact hC4 ⟨z, p, t, q,
      card_four_nineteen z p t q hzp_ne hzt_ne hzq_ne htp_ne.symm hpq htq_ne,
