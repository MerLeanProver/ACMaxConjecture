...
immediately) and all the structural counting (`TwinCert19HubTriangleStruct`: nine hubs, `≥ 5`
avoiders per cherry, `∑ int = 18`) are fully proved here.  The
**no-avoider-triangle ⇒ contradiction** core `core_triangle_force_nineteen` is now **fully closed**
(no `sorry`): the `|FF|`-dispatch in `iso_rich_force_nineteen` handles `|FF| ∈ {2, …, 8}`, and the
genuinely-new `|FF| = 7`, `|B| = 4` excess-`11` boundary is closed by threading the top-level
good-triangle threshold `≤ 11` (`hT`).  See `iso_rich_force_nineteen` for the analysis.
-/
...
/-- **No avoider triangle ⇒ contradiction (the `n = 19` combinatorial core).**  Under the residual
hypotheses for the `n = 19`, `|D| = 8`, `|Hub| = 10` corner, the assumption that neither cherry
admits a triangle of pairwise-adjacent avoiding hubs (`htri1`, `htri2`) is contradictory.

It is discharged by `iso_rich_force_nineteen` (`TwinCert19HubTriangleFF`), which dispatches on
...
* `|FF| = 5`, `|B| = 3`, `∑_{FF} int ∈ {17, 18}`: a `TwoHubConfig` (`ff3_extract_twohub_nineteen`)
  or a Mantel-`5` triangle inside `FF ⊆ A2`;
* `|FF| = 7`, `|B| = 4` (`A1 = A2 = FF`): the genuinely-new excess-`11` boundary, **closed** by
...
theorem core_triangle_force_nineteen (G : SimpleGraph (Fin 19))
    ...
    False :=
  iso_rich_force_nineteen G D Iso L₁ c₁ c₂ L₂ hT hC4 hK23 hmemD hIsodef hIsoprop hisochar hcov
...
/-- **Hub-triangle existence in the `n = 19`, `|D| = 8`, `e(M) = 3`, `P₄` residual corner.**  Under
...
no-avoider-triangle case is discharged by `core_triangle_force_nineteen`, now **fully closed**
(the `|FF| = 7`, `|B| = 4` excess-`11` boundary is killed by the threaded good-triangle `≤ 11`
hypothesis `hT`; see its docstring). -/
theorem exists_hub_triangle_config_residual_nineteen (G : SimpleGraph (Fin 19))
    (D Iso : Finset (Fin 19)) (L₁ c₁ c₂ L₂ : Fin 19)
    (hT : ¬∃ x y z : Fin 19, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 11)
    (hC4 : ¬∃ a b c d : Fin 19, ({a, b, c, d} : Finset (Fin 19)).card = 4 ∧
  ...
  have hubne : ∀ g : Fin 19, g ∈ Dᶜ → g ≠ L₁ ∧ g ≠ c₁ ∧ g ≠ c₂ ∧ g ≠ L₂ := by
    intro g hg
  ...
  -- The eleven hubs (`Dᶜ.card = 11`) and the four `M`-isolated twins (`Iso.card = 4`).
  ...
  -- Hub-internal-degree total: `∑ int = 4·11 − 6 − 12 = 26` (the `n = 19` delta from `n = 18`'s
  -- `22`), derived from the per-hub split `path(g) + iso(g) + int(g) = 4`.
  ...
  have hSum18 : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 26 := by
    have htot : ∑ g ∈ Dᶜ, ((G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 19))).card
  ...
  exfalso
  exact core_triangle_force_nineteen G D Iso L₁ c₁ c₂ L₂ hT hC4 hK23 hmemD hIsodef hIsoprop
