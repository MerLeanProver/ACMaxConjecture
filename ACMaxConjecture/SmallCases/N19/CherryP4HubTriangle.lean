...
import ACMaxConjecture.SmallCases.N19.CherryP4HubTriangleStruct
...
import ACMaxConjecture.SmallCases.N19.CherryP4HubTriangleForce
...
* `hub_triangle_force_p4_D11_nineteen` — clean counting contradiction at `|D| = 11`
  (`Dᶜ.card = 7`, `∑ int = 4`); deps: the WA per-hub lemmas, `shared_hub_le5`-style routing of the
  single degree-`≥ 6` hub.  File: `TwinCert19CherryP4HubTriangleForce.lean`.
* `hub_triangle_force_p4_D10_nineteen` — `|D| = 10` (`∑ int = 10`): clean contradiction off the
  fully-free degree-`6` corner, which collapses as in `n = 17 ten_avoider_clean_contra`.  Same file.
* `hub_triangle_force_p4_D9_nineteen` — `|D| = 9` (`∑ int = 16`): the `|FF| = 5` Mantel-`5`
  ...
  `nonadj_deg4_hubs_share_le_one_iso_pointwise_nineteen`.  File:
  `TwinCert19CherryP4HubTriangleForce.lean`.
-/
...
`TwinCert19CherryP4HubTriangleForceD9FF4` by extracting a `TwoHubConfig` from two isolated
degree-`4` hubs, so this declaration is fully proved (axiom-clean). -/
theorem exists_hub_triangle_config_p4_nineteen (G : SimpleGraph (Fin 19))
    ...
    (L₁ c₁ c₂ L₂ h t₁ t₂ : Fin 19) (D Iso : Finset (Fin 19))
  ...
  obtain ⟨hDccard, hIsocard, hc1hub, hc2hub, hL1hub, hL2hub, hsumPath, hsumIso, hsumInternal,
    hper⟩ := cherry_p4_hub_struct_nineteen G hm L₁ c₁ c₂ L₂ D Iso hT hmemD hIsoprop hisochar
  ...
  exfalso
  exact iso_rich_force_p4_nineteen G D Iso L₁ c₁ c₂ L₂ h3 hmemD hIsoprop hisochar
