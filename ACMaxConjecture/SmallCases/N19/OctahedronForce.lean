...
residual that `rich_count_ge_six_nineteen` must exclude.

Writing `m = ∑_{r∈R} |N r ∩ R|` for the ordered rich-internal edge mass, the lemma closes

  `(∑_{t∈Iso} |N t ∩ R|·(|N t ∩ R| − 1)) + m = |R.offDiag| = |R|² − |R|`

(every non-adjacent rich pair shares *exactly* one twin via `rich_nonadj_share_eq_one_nineteen`, and
...
* `octahedron_trace_saturate_nineteen` — LEAF, axiom-clean.
...
`TwinCert18OctahedronPoorForce`).  See `rich_count_ge_six_nineteen` for the documented residual.
-/
...
non-adjacent rich pair shares *exactly* one twin (`rich_nonadj_share_eq_one_nineteen`) and (by the
...
theorem octahedron_trace_saturate_nineteen (G : SimpleGraph (Fin 19))
    ...
    ∑ r ∈ R, (G.neighborFinset r ∩ Iso).card = 7 + R.card := by
  classical
  ...
  set Dadj : Finset (Fin 19 × Fin 19) := R.offDiag.filter (fun p => G.Adj p.1 p.2) with hDadjdef
  set Dnadj : Finset (Fin 19 × Fin 19) := R.offDiag.filter (fun p => ¬G.Adj p.1 p.2) with hDnadjdef
  ...
  have hq_eq : (∑ t ∈ Iso, (G.neighborFinset t ∩ R).offDiag.card) = Dnadj.card := by
    ...
    · -- SurjOn: every non-adjacent rich pair is realised by its shared twin.
      ...
      have hsh := rich_nonadj_share_eq_one_nineteen G Hub Iso hshare hno2hub p.1 p.2 (hRsub h1R)
  ...
  have hm_eq : (∑ r ∈ R, (G.neighborFinset r ∩ R).card) = Dadj.card := by
    rw [← hSecard]
    apply Finset.card_nbij (fun x => ((x.1, x.2) : Fin 19 × Fin 19))
    · intro x hx
  ...
  have hPcard : P.card = 11 - R.card := by
  ...
  have hisoP : ∑ g ∈ P, (G.neighborFinset g ∩ Iso).card = 11 - R.card := by
  ...
  have hsp : ∑ r ∈ R, (G.neighborFinset r ∩ Iso).card
    ...
    exact hiso18
  -- `|R| ≤ 11` so `7 + |R| = 18 - (11 - |R|)`.
  have hRle : R.card ≤ 11 := by rw [← hHub]; exact Finset.card_le_card hRsub
  omega
