...
theorem zleaf_pack_rich_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    (_hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (_hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
    (hshare : ∀ h₁ ∈ Hub, G.degree h₁ = 4 → ∀ h₂ ∈ Hub, G.degree h₂ = 4 → h₁ ≠ h₂ →
      ¬G.Adj h₁ h₂ → (G.neighborFinset h₁ ∩ G.neighborFinset h₂ ∩ Iso).card ≤ 1)
    (h₁ h₂ z : Fin 19) (hh₁ : h₁ ∈ Hub) (hh₂ : h₂ ∈ Hub)
    (hd₁ : G.degree h₁ = 4) (hd₂ : G.degree h₂ = 4)
    (hzZ : z ∈ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 19))) (hne : h₁ ≠ h₂)
  ...
  obtain ⟨a, b, haIso, hbIso, hab, ha1, hb1, haX, hbX⟩ :=
    exists_two_private_twins_nineteen G Hub Iso hshare h₁ h₂ hh₁ hh₂ hd₁ hd₂ hne hn12 h1iso
  obtain ⟨c, hcIso, hc2, hn1c⟩ :=
    exists_one_private_twin_nineteen G Hub Iso hshare h₂ h₁ hh₂ hh₁ hd₂ hd₁ hne hn12 h2iso
...
theorem zleaf_pack_share0_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    (_hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (_hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
    (h₁ h₂ z : Fin 19) (hh₁ : h₁ ∈ Hub) (hh₂ : h₂ ∈ Hub)
    (hd₁ : G.degree h₁ = 4) (hd₂ : G.degree h₂ = 4)
    (hzZ : z ∈ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 19))) (_hne : h₁ ≠ h₂)
