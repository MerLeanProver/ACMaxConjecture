...
import ACMaxConjecture.SmallCases.N19.Align8D10DomA
...
/-!
# The `|D| = 10`, `e(M) = 5` dominating-edge dichotomy (`n = 19`)

`SingleVertexConfig` or a shared degree-`≤ 5` hub.  The double-star
(`eM5_domedge_leaves_nineteen`) leaves `|Iso| = 4` `M`-isolated twins with
`12` hub-incidences.  If no degree-`≤ 5` hub carries two of them, the count
`12 ≤ (9 − h₆) + (2 + 4h₆)` forces `h₆ = 1`: a unique degree-`6` hub `w`
with profile `{6, 4⁸}`, every iso adjacent to `w` plus a **disjoint** pair of
degree-`4` hubs, each degree-`4` hub carrying exactly one iso.  The
**leaf-coverage pigeonhole** finishes: blocking `SingleVertexConfig` for an
iso `t` with pair `(h, h′)` requires `h, h′` jointly adjacent to all four
leaves (the four induced `P₃`s `ℓ–c–c′` must each be hit, and centres are
hub-free), so the four disjoint pairs demand `16` leaf-adjacencies against
the `8` available leaf slots — impossible.  Some iso pair misses a leaf
`ℓ*`, and `(t, h, h′, ℓ*–c–c′)` is a `SingleVertexConfig` with budget
`2·0 + 4 + 4 ≤ 8`.
-/
...
twins. -/
theorem eM5_D10_domedge_sv_or_shared_nineteen (G : SimpleGraph (Fin 19))
  ...
  classical
  obtain ⟨l₁, l₂, l₃, l₄, h12, h13, h14, h23, h24, h34,
      h1c1, h1c2, h2c1, h2c2, h3c1, h3c2, h4c1, h4c2,
      hd1, hd2, hd3, hd4, ha1, -, -, -, hn1, -, -, -, hfull1, hfull2⟩ :=
    eM5_domedge_leaves_nineteen G h3 hT hD10 c₁ c₂ hc1D hc2D hc12 hcov hsum10
  by_cases hshared : ∃ k tw₁ tw₂ : Fin 19, G.degree k ≤ 5 ∧ tw₁ ≠ tw₂ ∧
      G.degree tw₁ = 3 ∧ G.degree tw₂ = 3 ∧ G.Adj tw₁ k ∧ G.Adj tw₂ k ∧
      (∀ w : Fin 19, G.Adj tw₁ w → G.degree w ≠ 3) ∧
      (∀ w : Fin 19, G.Adj tw₂ w → G.degree w ≠ 3)
  ...
  set B : Finset (Fin 19) := {c₁, c₂, l₁, l₂, l₃, l₄} with hBdef
  have hBsub : B ⊆ D := by
    intro x hx
    rw [hBdef] at hx
  ...
  set Iso : Finset (Fin 19) := D \ B with hIsodef
  ...
  have hIsoiso : ∀ v ∈ Iso, G.degree v = 3 ∧ ∀ u : Fin 19, G.Adj v u → G.degree u ≠ 3 := by
    intro v hv
  ...
  -- the handshake: hub degree-sum `38` over `9` hubs
  ...
  have hHubeqDc : Hub = Dᶜ := by
    ...
    have := h3 u
    omega
  have hHubsum : ∑ v ∈ Hub, G.degree v = 38 := by
    rw [hHubeqDc]
    omega
  have hHubcard : Hub.card = 9 := by
    rw [hHubeqDc, Finset.card_compl, Fintype.card_fin, hD10]
  -- no shared degree-`≤ 5` hub ⟹ some hub has degree `≥ 6` (`12 > 9` incidences)
  have hex6 : ∃ w ∈ Hub, 6 ≤ G.degree w := by
    ...
    obtain ⟨k, t₁, t₂, -, hk5, ht12, ht1, ht2, hA1, hA2, hi1, hi2⟩ :=
      shared_hub_le5_from_count_nineteen G Iso Hub hIsoiso hHub5 hcount
    exact hshared ⟨k, t₁, t₂, hk5, ht12, ht1, ht2, hA1, hA2, hi1, hi2⟩
  obtain ⟨w, hwHub, hw6⟩ := hex6
  -- every hub other than `w` has degree exactly `4` (`6 + 5 + 4·7 = 39 > 38`)
  have hother4 : ∀ u ∈ Hub, u ≠ w → G.degree u = 4 := by
    ...
    have hcards : ((Hub.erase w).erase u).card = 7 := by
  ...
  have hfind : ∃ t ∈ Iso, ∀ u : Fin 19, G.Adj t u → u ≠ w → ¬G.Adj u l₁ := by
    by_contra hcon
    push Not at hcon
    have hcon' : ∀ v : Fin 19, ∃ u : Fin 19,
  ...
  have htiso : ∀ u : Fin 19, G.Adj t u → G.degree u ≠ 3 := (hIsoiso t htIso).2
