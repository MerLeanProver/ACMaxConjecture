...
engine** (`select_finish_nineteen`, `hub_iso_sum_nineteen`, `nogood_of_not_select_nineteen`,
`hub_neighbor_le_nineteen`, `strong_deg4_count_le_two_deg5_nineteen`, `deg4_sum_le_nineteen`,
`residual_arith_nineteen`), the tight-profile router `two_hub_select_eM1_tight_nineteen`, and the
entry point `two_hub_corner_select_nineteen`.
...
through `two_hub_select_eM1_tight_nineteen`; the all-degree-`4` `(10,6,40)` profile is the analog of
...
theorem select_finish_nineteen (G : SimpleGraph (Fin 19)) (Iso : Finset (Fin 19)) (h₁ h₂ : Fin 19)
...
theorem hub_iso_sum_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3) :
    ∑ a ∈ Hub, (G.neighborFinset a ∩ Iso).card = 3 * Iso.card := by
  rw [cross_count_nineteen G Hub Iso]
...
/-- **No good pair from a refuted selection.**  Packages `select_finish_nineteen` into the
`min(iso-deg) ≤ share + 1` form on non-adjacent degree-`4` hub pairs, given a refutation `hcon` of
the selection goal. -/
theorem nogood_of_not_select_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    (hcon : ¬ ∃ h₁ h₂ : Fin 19, h₁ ∈ Hub ∧ h₂ ∈ Hub ∧
  ...
  have hfin := select_finish_nineteen G Iso a b hsa hsb
...
theorem hub_neighbor_le_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    (hdisj : Disjoint Hub Iso) (a : Fin 19) (hda : G.degree a = 4) :
...
`Hub`. -/
theorem strong_deg4_count_le_two_deg5_nineteen (G : SimpleGraph (Fin 19))
    (Hub Iso : Finset (Fin 19)) (hdisj : Disjoint Hub Iso)
  ...
  have hkey : ∀ a ∈ A, A.card - 1 + (G.neighborFinset a ∩ Iso).card ≤ 4 := by
    ...
    have hc2 := hub_neighbor_le_nineteen G Hub Iso hdisj a hda
    omega
...
theorem deg4_sum_le_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
...
theorem residual_arith_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
  ...
  classical
  have hiso_le : ∀ a : Fin 19, (G.neighborFinset a ∩ Iso).card ≤ G.degree a := by
    intro a
