...
/-!
# Structural leaves for the `n = 19`, `e(M) = 3`, `|D| = 8` hub-triangle corner

This file collects the *concrete, decidable-free* structural facts about the residual
configuration consumed by `exists_hub_triangle_config_residual_nineteen` in
`TwinCert19HubTriangle`: the **eleven** degree-`4` hubs (`Dᶜ`), the four `M`-isolated degree-`3`
...
The `n = 19` deltas from `n = 18` are `|Hub| = 11` (vs `10`), so each cherry has `≥ 7` avoiders
(vs `≥ 6`) and the hub-internal-degree total is `∑ int = 4·11 − 6 − 12 = 26` (vs `22`).  The wider
budget pins the fully-free count to `|FF| ∈ {2, 3, 4, 5, 6, 7, 8}`.
-/
...
theorem nbr_split_DC (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19)) (v : Fin 19) :
...
`L₂ ≁ c₁`. -/
theorem leaf_nbr_D (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19)) (L₁ c₁ c₂ L₂ : Fin 19)
...
theorem path_hub_incidence (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19)) (L₁ c₁ c₂ L₂ : Fin 19)
    (hcov : ∀ p q : Fin 19, p ∈ D → q ∈ D → G.Adj p q →
      p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
...
/-- **Hub and twin counts.**  In the `|D| = 8` corner there are eleven hubs (`Dᶜ.card = 11`) and
four
...
theorem hub_struct (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19)) (L₁ c₁ c₂ L₂ : Fin 19)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    ...
    (hD8 : D.card = 8) :
    Dᶜ.card = 11 ∧ Iso.card = 4 := by
  classical
  have hDc : Dᶜ.card = 11 := by
...
hub-neighbours and there are `11` hubs, then at least `7` hubs avoid all of `a, b, c`. -/
theorem avoiders_ge_six_aux (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19)) (a b c : Fin 19)
    (hDc9 : Dᶜ.card = 11)
...
`2 + 1 + 1 = 4` hub-incidences (`L₁` two, `c₁`/`c₂` one each), so `≥ 7` of the eleven hubs avoid it;
symmetrically for `{c₁, c₂, L₂}`. -/
theorem cherry_avoiders_ge_six (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19))
    (L₁ c₁ c₂ L₂ : Fin 19) (hDc9 : Dᶜ.card = 11)
    ...
    7 ≤ (Dᶜ.filter (fun g => ¬G.Adj g L₁ ∧ ¬G.Adj g c₁ ∧ ¬G.Adj g c₂)).card ∧
      7 ≤ (Dᶜ.filter (fun g => ¬G.Adj g c₁ ∧ ¬G.Adj g c₂ ∧ ¬G.Adj g L₂)).card := by
...
theorem avoider_internal_ge_two (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19))
    (g allowed : Fin 19) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
    (hclass : ∀ x : Fin 19, G.Adj g x → x ∈ D → x = allowed ∨ x ∈ Iso) :
...
theorem fully_free_internal_ge_three (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19))
    (g : Fin 19) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
    (hclass : ∀ x : Fin 19, G.Adj g x → x ∈ D → x ∈ Iso) :
...
theorem path_iso_partition (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19))
    (L₁ c₁ c₂ L₂ : Fin 19)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hisochar : ∀ w : Fin 19, w ∈ D → w ≠ L₁ → w ≠ c₁ → w ≠ c₂ → w ≠ L₂ → w ∈ Iso)
    (hL1D : L₁ ∈ D) (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hL2D : L₂ ∈ D)
    (hac1L1 : G.Adj c₁ L₁) (hc12 : G.Adj c₁ c₂) (hac2L2 : G.Adj c₂ L₂) :
    ({L₁, c₁, c₂, L₂} : Finset (Fin 19)) ∪ Iso = D ∧
      Disjoint ({L₁, c₁, c₂, L₂} : Finset (Fin 19)) Iso := by
  classical
  set P : Finset (Fin 19) := {L₁, c₁, c₂, L₂} with hP
  have hIsoD : Iso ⊆ D := by rw [hIsodef]; exact Finset.filter_subset _ _
  have hPD : P ⊆ D := by
...
theorem residual_incidence_sums (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19))
    (L₁ c₁ c₂ L₂ : Fin 19)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 19, G.Adj v w → G.degree w ≠ 3))
    (hisochar : ∀ w : Fin 19, w ∈ D → w ≠ L₁ → w ≠ c₁ → w ≠ c₂ → w ≠ L₂ → w ∈ Iso)
    ...
    (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 19))).card = 6) ∧
      ...
      (∀ g ∈ Dᶜ, (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 19))).card
  ...
  have hsum_path : ∑ g ∈ Dᶜ, (G.neighborFinset g ∩ P).card = 6 := by
    rw [cross_count_nineteen G Dᶜ P, hP, Finset.sum_insert (by simp [hL1c1, hL1nc2, hL1L2]),
...
internal degree `≥ 3` (`hFFint`), `|A1|, |A2| ≥ 7`, `FF = A1 ∩ A2`, and the hub-internal-degree
total `∑_{Dᶜ} f = 26`, the fully-free count lies in `{2, 3, 4, 5, 6, 7, 8}`. -/
theorem active_hub_budget_nineteen (Dc A1 A2 FF : Finset (Fin 19)) (f : Fin 19 → ℕ)
    (hA1sub : A1 ⊆ Dc) (hA2sub : A2 ⊆ Dc) (hFF : FF = A1 ∩ A2)
    (hA1card : 7 ≤ A1.card) (hA2card : 7 ≤ A2.card)
    (hA1int : ∀ g ∈ A1, 2 ≤ f g) (hA2int : ∀ g ∈ A2, 2 ≤ f g)
    (hFFint : ∀ g ∈ FF, 3 ≤ f g) (hSum18 : ∑ w ∈ Dc, f w = 26) :
