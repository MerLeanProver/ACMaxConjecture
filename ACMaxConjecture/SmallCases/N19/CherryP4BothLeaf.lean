...
import ACMaxConjecture.SmallCases.N19.CherryP4HubTriangle

/-!
# `n = 19`, `e(M) = 3` (`s = 6`), `P₄`-cherry corner: the both-leaf hub cluster

The `{c₁,c₂}`-avoider count (`p4_c1c2_avoider_two_twin_nineteen`) produces a degree-`≤ 5` hub `h`
avoiding both centres `c₁, c₂` and carrying two `M`-isolated twins.  When `h` additionally avoids a
leaf, `safe_hub_two_twin_p4_nineteen` closes the corner with a `TwoTwinConfig`.  The genuine
...
to `exists_hub_triangle_config_p4_nineteen`.
-/
...
the residual hub-triangle existence is `exists_hub_triangle_config_p4_nineteen`. -/
theorem p4_both_leaf_hub_config_nineteen (G : SimpleGraph (Fin 19))
  ...
  exact exists_hub_triangle_config_p4_nineteen G hm h3 hT hC4 L₁ c₁ c₂ L₂ h t₁ t₂ D Iso
