...
/-!
# `n = 19`, `|D| = 12`, `|Hub| = 6`, `e(M) = 5` (`s = 10`) shared-hub helper

The genuinely degenerate `e(M) = 5` corner of the `|Hub| = 6` alignment selector
(`two_hub_or_single_vertex_hub6_nineteen`), ported from `TwinCert16TwoHubHub6Deg5.lean`.  At
`e(M) = 5` the residual is a double-star (dominating edge) or an induced `C₅`.  The deg-`4`-shared-hub
pigeonhole used at `e(M) ≤ 4` degenerates; for `n = 19` (degree sum `64`, divisible by `3`) it no
...
* **induced-`C₅` branch (fully proved):** the deg-`4` pigeonhole ties (`X ≥ 3`, `|Hub₄| = 3`), so
  ...
  `exists_nonk_two_twin_hub_nineteen`: those three hit vertices have the deg-`5` hub `k` as their
  unique hub neighbour, so a second deg-`≤ 5` hub `h ≠ k` (with two `M`-isolated twins, extracted by
  the refined `hub5_iso_count_nineteen` pigeonhole on `Hub₅ \ {k}`) avoids that cherry instead.

The helper lemmas (`shared_hub_le5_from_count_nineteen`, `dense_two_twin_assemble_le5_nineteen`,
`claw_shared_two_twin_le5_nineteen`, `dom_fat_centre_two_twin_le5_nineteen`,
`shared_hub_le5_eM5_nineteen`) are all axiom-clean ports.
-/
...
/-- **Shared deg-`≤ 5` hub by pigeonhole (`n = 19`).**  Port of `shared_deg4_hub_from_count_nineteen`
...
theorem shared_hub_le5_from_count_nineteen (G : SimpleGraph (Fin 19)) (Iso Hub : Finset (Fin 19))
    (hIsoiso : ∀ v ∈ Iso, G.degree v = 3 ∧ ∀ w : Fin 19, G.Adj v w → G.degree w ≠ 3)
    (hHubdeg : ∀ h ∈ Hub, G.degree h ≤ 5)
    (hcount : Hub.card < ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub).card) :
    ∃ h t₁ t₂ : Fin 19, h ∈ Hub ∧ G.degree h ≤ 5 ∧ t₁ ≠ t₂ ∧
  ...
  have hincid : ∃ h ∈ Hub, 2 ≤ (Iso.filter (fun v => G.Adj v h)).card := by
    ...
    have hswap : ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub).card
      ...
      have hL : ∀ v : Fin 19, (G.neighborFinset v ∩ Hub).card
...
/-- **Fat-dominating claw assembly with a deg-`≤ 5` hub (`n = 19`).**  Port of
`claw_shared_two_twin_nineteen` permitting `4 ≤ deg h ≤ 5`: the C₄ obstruction `h–i–c–j` has degree
sum `deg h + 9 ≤ 14`, so it is still forbidden by `hC4` at `deg h = 5`, and the hub meets at most
one of `c`'s three degree-`3` neighbours. -/
theorem claw_shared_two_twin_le5_nineteen (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19))
  ...
  have hmem_i : ∀ w : Fin 19, w ∈ ({n₁, n₂, n₃} : Finset (Fin 19)) → G.Adj c w ∧ w ∈ D := by
    intro w hw
  ...
  have hcnbhd : ∀ w : Fin 19, G.Adj c w → w = n₁ ∨ w = n₂ ∨ w = n₃ := by
    intro w hw
  ...
  have hmeet : ∀ i j : Fin 19, G.degree i = 3 → G.degree j = 3 →
    ...
    exact hC4 ⟨h, i, c, j,
      card_four_nineteen h i c j (by rintro rfl; omega) (by rintro rfl; omega)
  ...
  · exact dense_two_twin_assemble_le5_nineteen G t₁ t₂ h n₂ c n₃ ht1deg ht2deg hhge4 hhle5
...
/-- **Dominating-edge fat-centre dispatch with a deg-`≤ 5` hub (`n = 19`).**  Port of
`dom_fat_centre_two_twin_nineteen` with the hub permitted degree `4` or `5`. -/
theorem dom_fat_centre_two_twin_le5_nineteen (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19))
    (hmemD : ∀ v : Fin 19, v ∈ D ↔ G.degree v = 3)
    (hT : ¬∃ x y z : Fin 19, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 19, ({a, b, c, d} : Finset (Fin 19)).card = 4 ∧
  ...
  · exact claw_shared_two_twin_le5_nineteen G D hmemD hT hC4 c₁ t₁ t₂ k hc1D hc1 ht1deg ht2deg
      hkge4 hkle5 ht12 hAt1k hAt2k ht1iso ht2iso
  · have hc2 : 3 ≤ (G.neighborFinset c₂ ∩ D).card := by omega
    exact claw_shared_two_twin_le5_nineteen G D hmemD hT hC4 c₂ t₁ t₂ k hc2D hc2 ht1deg ht2deg
      hkge4 hkle5 ht12 hAt1k hAt2k ht1iso ht2iso

/-- **Shared deg-`≤ 5` hub in the `e(M) = 5`, `|D| = 12`, `|Hub| = 6` regime (`n = 19`).**  The
...
theorem shared_hub_le5_eM5_nineteen (G : SimpleGraph (Fin 19))
    ...
    (hD12 : (Finset.univ.filter (fun w : Fin 19 => G.degree w = 3)).card = 13)
  ...
  have hsum56 : ∑ v : Fin 19, G.degree v = 68 := by
...
/-- **An induced `C₅` accounts for all of `M` at `e(M) = 5` (`n = 19`).**  If `v₁–⋯–v₅` is an
...
theorem nonIso_le_five_of_C5_nineteen (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19))
    (v₁ v₂ v₃ v₄ v₅ : Fin 19)
  ...
  set C : Finset (Fin 19) := {v₁, v₂, v₃, v₄, v₅} with hCdef
  have two_nbrs : ∀ a x y : Fin 19, G.Adj a x → G.Adj a y → x ∈ D → y ∈ D → x ≠ y →
      2 ≤ (G.neighborFinset a ∩ D).card := by
    intro a x y hax hay hxD hyD hxy
    have hsub : ({x, y} : Finset (Fin 19)) ⊆ G.neighborFinset a ∩ D := by
    ...
    have hcard2 : ({x, y} : Finset (Fin 19)).card = 2 := by
      rw [Finset.card_insert_of_notMem (by simp [hxy]), Finset.card_singleton]
    calc 2 = ({x, y} : Finset (Fin 19)).card := hcard2.symm
...
/-- **Refined `e(M) = 5` handshake count (`|Hub| = 6`, `n = 19`).**  With at most one degree-`6` hub
...
`< ` of `shared_hub_le5_eM5_nineteen` to leave room for deleting one hub. -/
theorem hub5_iso_count_nineteen (G : SimpleGraph (Fin 19))
    (D Hub Hub5 H6 Iso : Finset (Fin 19))
    (hmemD : ∀ v : Fin 19, v ∈ D ↔ G.degree v = 3)
    (hmemHub : ∀ v : Fin 19, v ∈ Hub ↔ 4 ≤ G.degree v)
    (hmemHub5 : ∀ v : Fin 19, v ∈ Hub5 ↔ 4 ≤ G.degree v ∧ G.degree v ≤ 5)
    (hmemH6 : ∀ v : Fin 19, v ∈ H6 ↔ 6 ≤ G.degree v)
...
/-- **A second deg-`≤ 5` shared hub avoiding a given hub `k` (`e(M) = 5`, `|Hub| = 6`, `n = 19`).**
...
`hub5_iso_count_nineteen` (`|Hub₅| + 2 ≤ ∑_{Iso}|N ∩ Hub₅|`) and the bound that `k` absorbs at most
two `Iso`-incidences (its other three neighbours `a, b, c` are non-isolated) keep the pigeonhole of
`shared_hub_le5_from_count_nineteen` strict on `Hub₅ \ {k}`. -/
theorem exists_nonk_two_twin_hub_nineteen (G : SimpleGraph (Fin 19))
    (hm : G.edgeFinset.card = 34) (h3 : ∀ v : Fin 19, 3 ≤ G.degree v)
    (hDge9 : 11 ≤ (Finset.univ.filter (fun w : Fin 19 => G.degree w = 3)).card)
    (v₁ v₂ v₃ v₄ v₅ : Fin 19)
    ...
    (hsum10 : ∑ v ∈ Finset.univ.filter (fun w : Fin 19 => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w : Fin 19 => G.degree w = 3)).card = 10)
    (k a b c : Fin 19) (hk4 : 4 ≤ G.degree k) (hk5 : G.degree k ≤ 5)
    ...
    ∃ h tt₁ tt₂ : Fin 19, h ≠ k ∧ 4 ≤ G.degree h ∧ G.degree h ≤ 5 ∧ tt₁ ≠ tt₂ ∧
      ...
      (∀ w : Fin 19, G.Adj tt₂ w → G.degree w ≠ 3) := by
  classical
  set D : Finset (Fin 19) := Finset.univ.filter (fun w => G.degree w = 3) with hDdef
  have hmemD : ∀ v : Fin 19, v ∈ D ↔ G.degree v = 3 := by intro v; rw [hDdef]; simp
  set Hub : Finset (Fin 19) := Finset.univ.filter (fun v => 4 ≤ G.degree v) with hHubdef
  ...
  have hcount2 : Hub5.card + 2 ≤ ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub5).card :=
    hub5_iso_count_nineteen G D Hub Hub5 H6 Iso hmemD hmemHub hmemHub5 hmemH6 hIsodef hm h3
      hDge9 hIsoge
  ...
  set S : Finset (Fin 19) := Iso.filter (fun v => G.Adj v k) with hSdef
  ...
  -- Pigeonhole on `Hub₅ \ {k}` via `shared_hub_le5_from_count_nineteen`.
  obtain ⟨h, tt₁, tt₂, hhErase, hh5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1iso, ht2iso⟩ :=
    shared_hub_le5_from_count_nineteen G Iso (Hub5.erase k) hIsoiso
...
`exists_nonk_two_twin_hub_nineteen`, using the shared hub's **own two twins**.
...
`68 − 3|D|` over `19 − |D|` hubs excludes every degree-`≥ 6` hub whenever
`|D| ≤ 10` (`5 + 6 + 4(|Hub|−2) = 79 − 4|D| > 68 − 3|D|`).  All hubs are then
`Hub₅`, every `M`-isolated twin carries three `Hub₅`-incidences, and deleting
`k` (which absorbs exactly two) leaves `3(|D|−5) − 2 > 18 − |D|` incidences on
`Hub₅ \ {k}` (strict for `|D| ≥ 9`), so the pigeonhole extractor
`shared_hub_le5_from_count_nineteen` returns the second hub. -/
theorem exists_nonk_two_twin_hub_lowD_nineteen (G : SimpleGraph (Fin 19))
    (hm : G.edgeFinset.card = 34) (h3 : ∀ v : Fin 19, 3 ≤ G.degree v)
    (hDge9 : 9 ≤ (Finset.univ.filter (fun w : Fin 19 => G.degree w = 3)).card)
    (hDle10 : (Finset.univ.filter (fun w : Fin 19 => G.degree w = 3)).card ≤ 10)
    (v₁ v₂ v₃ v₄ v₅ : Fin 19)
    (hv1D : G.degree v₁ = 3) (hv2D : G.degree v₂ = 3) (hv3D : G.degree v₃ = 3)
    (hv4D : G.degree v₄ = 3) (hv5D : G.degree v₅ = 3)
    (hcard5 : ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 19)).card = 5)
    ...
    (hsum10 : ∑ v ∈ Finset.univ.filter (fun w : Fin 19 => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w : Fin 19 => G.degree w = 3)).card = 10)
    (k a b c tw₁ tw₂ : Fin 19) (hk5 : G.degree k ≤ 5)
    (hda : G.degree a = 3) (hdb : G.degree b = 3) (hdc : G.degree c = 3)
    (hka : G.Adj k a) (hkb : G.Adj k b) (hkc : G.Adj k c)
    (hab : G.Adj a b) (hbc : G.Adj b c)
    (hab_ne : a ≠ b) (hbc_ne : b ≠ c) (hac_ne : a ≠ c)
    ...
    (htw1iso : ∀ w : Fin 19, G.Adj tw₁ w → G.degree w ≠ 3)
    (htw2iso : ∀ w : Fin 19, G.Adj tw₂ w → G.degree w ≠ 3) :
    ∃ h tt₁ tt₂ : Fin 19, h ≠ k ∧ 4 ≤ G.degree h ∧ G.degree h ≤ 5 ∧ tt₁ ≠ tt₂ ∧
      G.degree tt₁ = 3 ∧ G.degree tt₂ = 3 ∧ G.Adj tt₁ h ∧ G.Adj tt₂ h ∧
      (∀ w : Fin 19, G.Adj tt₁ w → G.degree w ≠ 3) ∧
      (∀ w : Fin 19, G.Adj tt₂ w → G.degree w ≠ 3) := by
  ...
  have hsub5 : ({a, b, c, tw₁, tw₂} : Finset (Fin 19)) ⊆ G.neighborFinset k := by
    intro w hw
  ...
  have hcard5' : ({a, b, c, tw₁, tw₂} : Finset (Fin 19)).card = 5 := by
  ...
  have hNk : G.neighborFinset k = ({a, b, c, tw₁, tw₂} : Finset (Fin 19)) := by
  ...
  have hH5 : ∀ h ∈ Hub, G.degree h ≤ 5 := by
    ...
    have hcards : ((Hub.erase k).erase h6).card = Hub.card - 2 := by
      ...
      omega
    -- `68 − 3|D| ≥ 5 + 6 + 4(|Hub| − 2) = 79 − 4|D|` forces `|D| ≥ 11`
    have hDcard19 : D.card ≤ 19 := by
      rw [hDdef]
  ...
  have hIsoge : D.card - 5 ≤ Iso.card := by
    have hnon5 := nonIso_le_five_of_C5_nineteen G D v₁ v₂ v₃ v₄ v₅
      ...
      ((hmemD v₄).mpr hv4D) ((hmemD v₅).mpr hv5D) hcard5 e12 e23 e34 e45 e51 hsum10
    have hpart := Finset.card_filter_add_card_filter_not (s := D)
      (fun v => (G.neighborFinset v ∩ D).card = 0)
    rw [← hIsodef] at hpart
    omega
  ...
  set S : Finset (Fin 19) := Iso.filter (fun v => G.Adj v k) with hSdef
  have hSle2 : S.card ≤ 2 := by
    have hsub : S ⊆ ({tw₁, tw₂} : Finset (Fin 19)) := by
      intro w hw
    ...
    calc S.card ≤ ({tw₁, tw₂} : Finset (Fin 19)).card := Finset.card_le_card hsub
  ...
  have hcountE : (Hub.erase k).card
    ...
    omega
  obtain ⟨h, tt₁, tt₂, hhErase, hh5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1iso, ht2iso⟩ :=
    shared_hub_le5_from_count_nineteen G Iso (Hub.erase k) hIsoiso
      (fun h hh => hH5 h (Finset.mem_of_mem_erase hh)) hcountE
  have hhk : h ≠ k := Finset.ne_of_mem_erase hhErase
  have hh4 : 4 ≤ G.degree h := (hmemHub h).mp (Finset.mem_of_mem_erase hhErase)
  exact ⟨h, tt₁, tt₂, hhk, hh4, hh5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1iso, ht2iso⟩

/-- **`e(M) = 5` (`s = 10`) two-twin cut for the `|D| = 12`, `|Hub| = 6` corner (`n = 19`).**  Unlike
...
branch through `c5_shared_two_twin_le5_nineteen` (`TwinCert19DenseLe5`).  The former residual — a
...
extraction `exists_nonk_two_twin_hub_nineteen` threaded as `hnonk`. -/
theorem two_twin_eM5_nineteen (G : SimpleGraph (Fin 19))
    (hm : G.edgeFinset.card = 34) (h3 : ∀ v : Fin 19, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 19, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 19, ({a, b, c, d} : Finset (Fin 19)).card = 4 ∧
    ...
    (hC4 : ¬∃ a b c d : Fin 19, ({a, b, c, d} : Finset (Fin 19)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (hHub6 : (Finset.univ.filter (fun v : Fin 19 => 4 ≤ G.degree v)).card = 6)
  ...
  -- `|D| = 12` from `|Hub| = |Dᶜ| = 6`.
  have hDc : Dᶜ = Finset.univ.filter (fun v : Fin 19 => 4 ≤ G.degree v) := by
    ext w
  ...
  have hD12 : D.card = 13 := by rw [hDc] at hcc; omega
  ...
  · obtain ⟨c₁, c₂, hc1D, hc2D, hc12, hcov⟩ := hdom
    obtain ⟨k, tw1, tw2, hkle5, htw12, htw1deg, htw2deg, hAtw1k, hAtw2k, htw1iso, htw2iso⟩ :=
      shared_hub_le5_eM5_nineteen G hm h3 h2k2 (by rw [← hDdef]; exact hD12)
  ...
  · -- **Induced-`C₅` branch with a deg-`≤ 5` shared hub.**  Extract a shared deg-`≤ 5` hub and
    -- route through `c5_shared_two_twin_le5_nineteen`; the deg-`5` 3-consecutive-hit residual is
    -- closed by `exists_nonk_two_twin_hub_nineteen` supplying a second hub `h ≠ k`.
