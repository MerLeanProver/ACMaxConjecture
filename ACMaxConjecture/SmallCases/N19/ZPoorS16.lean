...
theorem poor_incidence_concentrates_S16 (G : SimpleGraph (Fin 19))
  ...
  have hcsum : ∑ t ∈ Iso, (G.neighborFinset t ∩ R).card = 16 := by
    rw [cross_count_nineteen G Iso R]; exact hS16
  have hsplit := offdiag_eq_nineteen G R
  rw [hr7] at hsplit
  -- `Dadj ≤ 12` (each rich hub spends `≥ 2` slots on twins).
  have hDadj12 : (R.offDiag.filter (fun p => G.Adj p.1 p.2)).card ≤ 12 := by
    have hsum := dadj_eq_sum_nineteen G R
    have hterm : ∀ a ∈ R,
  ...
  have hDbiU : (R.offDiag.filter (fun p => ¬G.Adj p.1 p.2))
    ...
    rich_nonadj_biUnion_nineteen G Hub Iso R hdeg4 hshare hno2hub hRHub hRrich
...
theorem rich_saturated_S16 (G : SimpleGraph (Fin 19)) (Hub Iso R : Finset (Fin 19))
    (hdeg4 : ∀ h ∈ Hub, G.degree h = 4)
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hIso : Iso.card = 6)
    (hshare : ∀ h₁ ∈ Hub, G.degree h₁ = 4 → ∀ h₂ ∈ Hub, G.degree h₂ = 4 → h₁ ≠ h₂ →
  ...
  have hsub := rich_nonadj_biUnion_nineteen G Hub Iso R hdeg4 hshare hno2hub hRHub hRrich
...
theorem rich_seven_S16_two_poor_false_nineteen (G : SimpleGraph (Fin 19))
  ...
  -- `Z`-hubs `hg1, hg2, g₃, g₄` are four distinct poor hubs `= P` (`|P| = 11 − 7 = 4`), carrying
  -- iso-incidence `18 − 16 = 2 > 0`, so at least one of them has a private twin `c*`.  That `Z`-hub
  -- `gstar` (`M`-end `zz ∈ {z, z'}`, whose two hub-neighbours are both poor) feeds the `r = 7`
  -- pigeonhole cut `two_poor_zleaf_pigeonhole_nineteen`, contradicting `hnocut`.
  classical
  apply hnocut
  refine Or.inl ?_
  ...
  obtain ⟨g3, g4, hg34, hNz'⟩ := Finset.card_eq_two.mp hz'hub2
  ...
  have hclose : ∀ gstar zz : Fin 19, gstar ∈ Hub → zz ∈ Z → G.Adj zz gstar →
  ...
  -- All four `Z`-hubs are iso-degree `0`: but `{hg1,hg2,g3,g4} = P` has iso-incidence `2 > 0`.
  exfalso
  ...
  have hcross_ne : ∀ b : Fin 19, b ∈ Hub → G.Adj z' b → hg1 ≠ b ∧ hg2 ≠ b := by
    intro b hbHub hz'b
    refine ⟨?_, ?_⟩
    · intro he; subst he
      exact no_hub_adj_both_mends_nineteen G Hub Iso hdeg4 hiso3 hdisj hHub hIso hdsum hdeg3
  ...
  have hcard4 : ({hg1, hg2, g3, g4} : Finset (Fin 19)).card = 4 := by
    rw [Finset.card_insert_of_notMem (by simp [hg1ne, hne13, hne14]),
      Finset.card_insert_of_notMem (by simp [hne23, hne24]),
      Finset.card_insert_of_notMem (by simp [hg34]), Finset.card_singleton]
  have hsubP : ({hg1, hg2, g3, g4} : Finset (Fin 19)) ⊆ P := by
    ...
    · exact hg3P
    · exact hg4P
  have hPeq : P = ({hg1, hg2, g3, g4} : Finset (Fin 19)) :=
    (Finset.eq_of_subset_of_card_le hsubP (by rw [hPcard, hcard4])).symm
  have hzerosum : ∑ g ∈ ({hg1, hg2, g3, g4} : Finset (Fin 19)),
    ...
    · exact h30
    · exact h40
  rw [hPeq, hzerosum] at hPsum
  exact absurd hPsum (by norm_num)
