...
/-! # The good-h₂ selector for the rich (9,8,38) Z-leaf extraction (`n = 19`) -/
namespace ACMax
...
/-- With the rich hub `g` (isoDeg ≥ 3), the tight `(9,8,38)` ledger + the iso-cap
(`isoDeg_le_two_of_nonadj_rich_nineteen`) produce a degree-`4` `Z`-slot hub `h₂`
of isoDeg `≥ 2`, on a `z` avoided by `g` and non-adjacent to `g`. -/
theorem exists_good_h2z_rich_938_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    ...
    (hHub : Hub.card = 9) (hIso : Iso.card = 8) (hdsum : ∑ w ∈ Hub, G.degree w = 38)
  ...
  have hnbr : ∀ v : Fin 19, (G.neighborFinset v ∩ Hub).card + (G.neighborFinset v ∩ Iso).card
      + (G.neighborFinset v ∩ Z).card = G.degree v := by
    intro v
    have h := nbr_split_three_nineteen G Hub Iso hdisj v
  ...
  -- The deg-4 / deg-5 partition: `|D4| = 7`, `|D5| = 2`.
  ...
  -- Ledger: total iso-incidence over hubs is `24`.
  ...
  -- The `(hubDeg + zDeg)` ledger sums to `14`.
  ...
  have hcapf : ∀ h ∈ Hub, h ≠ g → G.degree h = 4 → ¬G.Adj g h →
    ...
    exact isoDeg_le_two_of_nonadj_rich_nineteen G Hub Iso hshare hno2hub g h hg hh hgd h4
  ...
  -- Suppose no good `h₂`.
  by_contra hcon
  have hnogood : ∀ h z : Fin 19, h ∈ Hub → G.degree h = 4 →
  ...
  have zslots : ∀ z, z ∈ Z → ∃ p q : Fin 19, p ∈ Hub ∧ q ∈ Hub ∧ p ≠ q ∧
      G.Adj z p ∧ G.Adj z q := by
    intro z hz
    obtain ⟨_, hzhub2, _⟩ :=
      zfacts_deg5_nineteen G Hub Iso hiso3 hdisj hsum17 hdeg3 hisodeg3 hleak z
  ...
  obtain ⟨AV, hAVsub, hAVcard, hg_notin_AV, hAV_slot⟩ :
    ...
    have slotProp : ∀ (z w : Fin 19), z ∈ Z → ¬G.Adj g z → w ∈ Hub → G.Adj z w →
    ...
    · -- `zDeg g = 0`: both `M`-endpoints avoided, all four slots.
      ...
      obtain ⟨p₂, q₂, hp₂H, hq₂H, hpq₂, hzp₂, hzq₂⟩ := zslots z₂ hz₂Z
      have hcross : ∀ a b : Fin 19, a ∈ Hub → G.Adj z₁ a → G.Adj z₂ b → a ≠ b := by
      ...
      · have hc4 : ({p₁, q₁, p₂, q₂} : Finset (Fin 19)).card = 4 :=
          card_four_nineteen p₁ q₁ p₂ q₂ hpq₁ (hcross p₁ p₂ hp₁H hzp₁ hzp₂)
    ...
    · -- `zDeg g = 1`: exactly one `M`-endpoint avoided.
      ...
      · -- `z₁` met, `z₂` avoided.
        have hng2 : ¬G.Adj g z₂ := by
          intro hadj2
          have hsub : ({z₁, z₂} : Finset (Fin 19)) ⊆ G.neighborFinset g ∩ Z := by
          ...
          have hcard2 : ({z₁, z₂} : Finset (Fin 19)).card = 2 := Finset.card_pair hz12ne
  ...
  set FA : Finset (Fin 19) := (D4.erase g).filter (fun h => G.Adj g h) with hFAdef
  set FN : Finset (Fin 19) := (D4.erase g).filter (fun h => ¬G.Adj g h) with hFNdef
  ...
  have hFNFA : FN.card + FA.card = 6 := by
  ...
  set ℓ : Fin 19 → ℕ := fun h => (if h ∈ FN then 2 else 0) + (if h ∈ FA then 1 else 0)
    + (if h ∈ AV then 1 else 0)
    + (if h ∈ ({g} : Finset (Fin 19)) then (G.neighborFinset g ∩ Z).card else 0) with hℓdef
  have hpt : ∀ h ∈ Hub,
    ...
    · subst hg'
      ...
      have e4 : h ∈ ({h} : Finset (Fin 19)) := Finset.mem_singleton_self h
    ...
    · rcases hpd h hh with h4 | h5
      · by_cases hadj : G.Adj g h
        · have hFA : h ∈ FA := (hFAmem h).mpr ⟨hh, h4, hg', hadj⟩
          ...
          have e4 : h ∉ ({g} : Finset (Fin 19)) := by rw [Finset.mem_singleton]; exact hg'
  ...
  have s3 : ∑ h ∈ Hub, (if h ∈ AV then (1 : ℕ) else 0) = AV.card := by
    rw [Finset.sum_ite_mem, Finset.inter_eq_right.mpr hAVsub, Finset.sum_const, smul_eq_mul,
      mul_one]
  have s4 : ∑ h ∈ Hub, (if h ∈ ({g} : Finset (Fin 19)) then (G.neighborFinset g ∩ Z).card else 0)
  ...
  have hℓle : ∑ h ∈ Hub, ℓ h ≤ 14 := (Finset.sum_le_sum hpt).trans_eq hssum
  rw [hℓsum] at hℓle
  omega
