...
/-!
# Profile-agnostic `Z`-leaf two-hub infrastructure (`n = 19`)

This file generalizes the octahedron-specific `Z`-leaf assembler `two_hub_zleaf_nineteen`
(hard-wired to `(11, 6, 44)`) so that it also serves the degree-`5` `e(M) = 1` two-hub corners
`(9, 8, 38)` and `(10, 7, 41)`.

Two reusable, axiom-clean pieces:

* `zfacts_deg5_nineteen` — the `M`-edge-endpoint structural facts (`deg z = 3`, meets no twin, meets
  exactly two hubs) for **any** `e(M) = 1` partition with `|Hub| + |Iso| = 17` and `Z`-hub leakage
  `≤ 4`.  Unlike `z_two_hub_nbrs_nineteen` it does **not** use the rigid handshake `∑_Hub deg = 44`
  (only the leak bound and the twin structure), so it applies verbatim to the deg-`5` profiles.

* `two_hub_zleaf_gen_nineteen` — the `Z`-leaf `TwoHubConfig` assembler with the `z`-facts abstracted
...
`|Hub| + |Iso| = 17` (so `|Z| = 2`), isolated twins of degree `3` meeting exactly three hubs, all
degrees `≥ 3`, and `Z`-hub leakage `≤ 4`: every `Z`-vertex `z` meets no twin, meets exactly two
hubs, and has degree `3`. -/
theorem zfacts_deg5_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
  ...
  have hUcard : (Hub ∪ Iso).card = 17 := by
    rw [Finset.card_union_of_disjoint hdisj]; exact hsum17
  ...
  have hsum_le4 : ∑ z ∈ Z, (G.neighborFinset z ∩ Hub).card ≤ 4 := by
    rw [← cross_count_nineteen G Hub Z]; exact hleak
  intro z hz
...
/-- **The profile-agnostic `Z`-leaf two-hub assembler.**  Identical to `two_hub_zleaf_nineteen` but
with the `M`-end `z`-facts (`deg z = 3`, `N(z) ∩ Iso = ∅`) supplied as hypotheses rather than derived
from the rigid `(11, 6, 44)` handshake, so it also serves the degree-`5` `e(M) = 1` corners.  Packs a
`TwoHubConfig` whose fourth leaf `d = z` is an `M`-edge endpoint (invisible to the `Iso`-only
`hno2hub`). -/
theorem two_hub_zleaf_gen_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
