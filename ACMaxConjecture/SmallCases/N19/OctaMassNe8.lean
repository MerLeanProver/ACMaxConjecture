...
/-!
# Excluding the residual rich-internal mass `mRR = 8` (`n = 19`, `(11, 6, 44)`)

`octahedron_mass_reduce_nineteen` pins the rich-internal ordered mass `mRR ∈ {6, 8}` for the clean
no-two-hub `|Hub| = 11` octahedron.  The `mRR = 6` branch carries the four-high-twins input
`hhigh4 = (n₃ ≥ 4)` consumed by the octahedron three-way force; the `mRR = 8` branch is the residual
`n₃ = 3` profile, with twin rich-degree multiset `(n₃, n₂, n₁, n₀) = (3, 2, 0, 1)`.

The documented circularity (cf. `octahedron_struct_residual_nineteen`) is that branch (b) of
`octahedron_poor_force_b_or_c_nineteen` extracts its good `K₂,₃` only through
`octahedron_low_twin_poor_nineteen`, which itself demands `hhigh4 = n₃ ≥ 4` — exactly what `mRR = 8`
...
* `octahedron_poor_counts_nineteen` gives the poor-internal ordered mass
  `∑_{g∈P} |N g ∩ P| = mRR + 4 = 12`, i.e. **six** undirected poor edges among the five poor hubs.
* If two vertices of `Q` are adjacent, `{p₁, t₀, p₂}` is a good triangle of degree sum
  `4 + 3 + 4 = 11` (`hT`).  Otherwise `Q` is independent, and a directed double-count forces the two
  poor hubs outside `Q` to carry `∑_{w∈P\Q} |N w ∩ Q| ≥ 5`; with `|P\Q| = 2` and each `≤ 3` one of
  them, `w`, is adjacent to all of `Q`, giving the good `K₂,₃` `{t₀, w | Q}` of degree sum
  `3 + 4 + 4·3 = 19` (`hK23`).

Either way the no-good-certificate hypotheses are contradicted, so `mRR ≠ 8`.  The lemma is fully
...
/-- **The residual rich-internal mass `mRR = 8` is impossible (LEAF, axiom-clean).**  For the clean
no-two-hub `|Hub| = 11` octahedron (`R = {h : 2 ≤ |N h ∩ Iso|}`, `|R| = 6`, every hub meets a twin),
the rich-internal ordered edge mass `∑_{r∈R} |N r ∩ R|` is not `8`.  Breaks the `n₃ = 3` circularity
by extracting a good triangle or `K₂,₃` directly from the zero-rich twin `t₀`. -/
theorem octahedron_mass_ne_eight_nineteen (G : SimpleGraph (Fin 19))
  ...
  -- Trace saturation and the rich iso-incidence value `= 13`.
  ...
  rw [hcross] at hII
  rw [hIII] at hsat
  -- The `(3, 2, 0, 1)` profile: `n₀ = 1`, so there is a twin meeting no rich hub.
  obtain ⟨t₀, ht₀mem⟩ := Finset.card_pos.mp (show 0 < n0 by omega)
  rw [Finset.mem_filter] at ht₀mem
  obtain ⟨ht₀Iso, ht₀R0⟩ := ht₀mem
  -- Poor-internal ordered mass `∑_{g∈P} |N g ∩ P| = mRR + 4 = 12`.
  ...
  obtain ⟨_, _, _, hPP, _⟩ := (octahedron_poor_counts_nineteen G Hub Iso R hReq hdeg4 hiso3 hdisj
    hHub hIso hisodeg3 hdeg3 hdsum hleak hshare hno2hub hC4 hK23 hT hnozero hRcard).resolve_right hcut
  have hPP12 : ∑ g ∈ P, (G.neighborFinset g ∩ P).card = 12 := by
    rw [hPdef, hPP, hm8]
  ...
  set Q : Finset (Fin 19) := G.neighborFinset t₀ ∩ P with hQdef
  ...
  · -- **Case 2.** `Q` is independent.  Double-count the poor-internal mass to force a `K₂,₃`.
    ...
    set Pq : Finset (Fin 19) := P \ Q with hPqdef
    have hPqcard : Pq.card = 2 := by
    ...
    have hsplitP : ∀ g : Fin 19, (G.neighborFinset g ∩ P).card
    ...
    -- `A + B = 12` where `A = ∑_{P} |N · ∩ Q|`, `B = ∑_{P} |N · ∩ Pq|`.
    have hAB : (∑ g ∈ P, (G.neighborFinset g ∩ Q).card)
        + (∑ g ∈ P, (G.neighborFinset g ∩ Pq).card) = 12 := by
      rw [← Finset.sum_add_distrib, ← hPP12]
    ...
    have hBeq : ∑ g ∈ P, (G.neighborFinset g ∩ Pq).card
        = ∑ w ∈ Pq, (G.neighborFinset w ∩ P).card := cross_count_nineteen G P Pq
    -- For `w ∈ Pq`: `|N w ∩ P| ≤ |N w ∩ Q| + 1`.
    have hwbound : ∀ w ∈ Pq, (G.neighborFinset w ∩ P).card ≤ (G.neighborFinset w ∩ Q).card + 1 := by
      intro w hw
      have hle1 : (G.neighborFinset w ∩ Pq).card ≤ 1 := by
        have hsub : G.neighborFinset w ∩ Pq ⊆ Pq.erase w := by
          intro x hx
          rw [Finset.mem_inter, G.mem_neighborFinset] at hx
          rw [Finset.mem_erase]
          exact ⟨fun he => (G.ne_of_adj hx.1) he.symm, hx.2⟩
        calc (G.neighborFinset w ∩ Pq).card ≤ (Pq.erase w).card := Finset.card_le_card hsub
          _ = Pq.card - 1 := Finset.card_erase_of_mem hw
          _ = 1 := by rw [hPqcard]
      rw [hsplitP w]; omega
    -- Hence `A ≥ 5`.
    have hAle : ∑ w ∈ Pq, (G.neighborFinset w ∩ P).card
        ≤ (∑ w ∈ Pq, (G.neighborFinset w ∩ Q).card) + 2 := by
      calc ∑ w ∈ Pq, (G.neighborFinset w ∩ P).card
          ≤ ∑ w ∈ Pq, ((G.neighborFinset w ∩ Q).card + 1) := Finset.sum_le_sum hwbound
        _ = (∑ w ∈ Pq, (G.neighborFinset w ∩ Q).card) + 2 := by
            rw [Finset.sum_add_distrib, Finset.sum_const, hPqcard, smul_eq_mul, mul_one]
    have hA5 : 5 ≤ ∑ w ∈ Pq, (G.neighborFinset w ∩ Q).card := by
      rw [hAeq, hBeq] at hAB; omega
    -- Pigeonhole over `Pq = {w₁, w₂}`: some `w` is adjacent to all of `Q`.
    obtain ⟨w1, w2, hw12, hPqeq⟩ := Finset.card_eq_two.mp hPqcard
    have hw1Pq : w1 ∈ Pq := by rw [hPqeq]; exact Finset.mem_insert_self _ _
    have hw2Pq : w2 ∈ Pq := by rw [hPqeq]; exact Finset.mem_insert_of_mem (Finset.mem_singleton_self _)
    have hsum2 : (G.neighborFinset w1 ∩ Q).card + (G.neighborFinset w2 ∩ Q).card
        = ∑ w ∈ Pq, (G.neighborFinset w ∩ Q).card := by
      rw [hPqeq, Finset.sum_insert (by rw [Finset.mem_singleton]; exact hw12), Finset.sum_singleton]
    ...
    have hcard5 : ({t₀, w, q1, q2, q3} : Finset (Fin 19)).card = 5 := by
