...
`iso_rich_force_nineteen` (the `|FF|`-dispatch core of the `|D| = 8` corner).  Because the hubs
...
* `cherry_p4_W_facts_nineteen` — `(W)`: a degree-`≤ 5` cherry-avoiding hub has `≤ 1` iso-twin
  (from `¬TwoTwinConfig`).
...
theorem avoiders_ge_gen (G : SimpleGraph (Fin 19)) (D : Finset (Fin 19)) (a b c : Fin 19)
...
theorem avoider_internal_ge_two_pt (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19))
    (g allowed : Fin 19) (hdg : 4 ≤ G.degree g) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
...
theorem fully_free_internal_ge_three_pt (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19))
    (g : Fin 19) (hdg : 4 ≤ G.degree g) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
...
theorem hub_deg_upper_pt (S : Finset (Fin 19)) (f : Fin 19 → ℕ) (T : ℕ)
    (lb : ∀ g ∈ S, 4 ≤ f g) (hsum : ∑ g ∈ S, f g = T) (g0 : Fin 19) (hg0 : g0 ∈ S) :
...
`exists_hub_triangle_config_residual_nineteen`, requiring `G.degree g ≤ 5` directly. -/
theorem cherry_p4_W_facts_nineteen (G : SimpleGraph (Fin 19)) (D Iso : Finset (Fin 19))
    (L₁ c₁ c₂ L₂ : Fin 19)
