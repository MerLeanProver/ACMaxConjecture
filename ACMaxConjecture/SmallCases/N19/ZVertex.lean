...
/-!
# Basic `Z`-vertex structure lemmas for the `n = 19` no-two-hub partition (bottom layer)
...
This is the **mechanical port** of the `n = 18` `TwinCert18ZVertex` to `|Hub| = 11`.  Since
`|Iso| = 6` is **unchanged** the leak total is the same: with `∑deg = 44`, the hub-hub edge mass
is `22` (vs `18` at `n = 18`) and the iso-incidence mass is still `18`, so the `Z`-hub mass is
`44 − 22 − 18 = 4` — identical to `n = 18`.  Hence the entire `(2, 0, deg 3)` `Z`-profile carries
over verbatim.  The only changes are `Fin 18 → 19`, `Hub.card 10 → 11`, `∑deg 40 → 44`, the
hub-hub mass `18 → 22`, and the good-triangle threshold `≤ 10 → ≤ 11` (the `{u,v,h}` triangle
...
theorem nbr_split_three_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    (hdisj : Disjoint Hub Iso) (v : Fin 19) :
...
partition of the `19` vertices, every hub degree splits three ways; summing and substituting the
`N3` hub-hub count (`22`) and the iso-incidence count (`18`) against `∑deg = 44` leaves
`∑_{h∈Hub}|N(h) ∩ Z| = 4` — the same as `n = 18` (`44 − 22 − 18 = 4`). -/
theorem zdeg_sum_four_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
  ...
  have hpart : ∀ v : Fin 19, (G.neighborFinset v ∩ Hub).card + (G.neighborFinset v ∩ Iso).card
    ...
    nbr_split_three_nineteen G Hub Iso hdisj v
  have hisosum : ∑ h ∈ Hub, (G.neighborFinset h ∩ Iso).card = 18 := by
    have := hub_iso_sum_nineteen G Hub Iso hiso3; rw [hIso] at this; omega
  have hhubsum : ∑ h ∈ Hub, (G.neighborFinset h ∩ Hub).card = 22 :=
    hub_hub_edge_count_nineteen G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak
...
`Hub`, `Iso` disjoint of cardinalities `11`, `6`, so `|Z| = 19 − 17 = 2`. -/
theorem z_card_two_nineteen (Hub Iso : Finset (Fin 19))
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 11) (hIso : Iso.card = 6) :
    (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 19)).card = 2 := by
  have hHIcard : (Hub ∪ Iso).card = 17 := by
    rw [Finset.card_union_of_disjoint hdisj, hHub, hIso]
  have hZeq : (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 19)) = (Hub ∪ Iso)ᶜ := by
...
`(11, 6, 44)` profile, the two `M`-edge endpoints `Z = univ \ (Hub ∪ Iso)` carry exactly four
...
theorem z_two_hub_nbrs_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    (hiso3 : ∀ t ∈ Iso, (G.neighborFinset t ∩ Hub).card = 3)
    (hdisj : Disjoint Hub Iso) (hHub : Hub.card = 11) (hIso : Iso.card = 6)
    (hdsum : ∑ w ∈ Hub, G.degree w = 44)
    (hdeg3 : ∀ v : Fin 19, 3 ≤ G.degree v) (hisodeg3 : ∀ t ∈ Iso, G.degree t = 3)
  ...
  have hsum4 : ∑ z ∈ Z, (G.neighborFinset z ∩ Hub).card = 4 := by
    rw [← cross_count_nineteen G Hub Z]
    exact zdeg_sum_four_nineteen G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak
...
theorem no_hub_adj_both_mends_nineteen (G : SimpleGraph (Fin 19)) (Hub Iso : Finset (Fin 19))
    ...
    (h : Fin 19) (hh : h ∈ Hub)
    (u : Fin 19) (hu : u ∈ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 19)))
    (v : Fin 19) (hv : v ∈ (Finset.univ \ (Hub ∪ Iso) : Finset (Fin 19)))
    (huv : u ≠ v) :
  ...
  obtain ⟨huiso, huhub, hudeg⟩ :=
    z_two_hub_nbrs_nineteen G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak u hu
  -- `u` meets exactly one `Z`-vertex.
  have huZ : (G.neighborFinset u ∩ Z).card = 1 := by
    have hsp := nbr_split_three_nineteen G Hub Iso hdisj u
  ...
  obtain ⟨_, _, hvdeg⟩ :=
    z_two_hub_nbrs_nineteen G Hub Iso hiso3 hdisj hHub hIso hdsum hdeg3 hisodeg3 hleak v hv
