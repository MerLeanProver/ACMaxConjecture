...
/-!
# Deg-`≤ 5` induced-`C₅` two-twin assembly (`n = 17`)

This file ports the induced-`C₅` shared-hub machinery of `TwinCert17Dense` to a hub of degree `4`
*or* `5`, as required by the `e(M) = 5`, `|Hub| = 6` corner of `n = 17` (where the degree-`4`
shared-hub pigeonhole only ties).

* `dense_two_twin_assemble_le5_seventeen` — bundle a deg-`≤ 5` hub with two twins and an avoided
  cherry into a `TwoTwinConfig` (moved here from `TwinCert17TwoHubHub6Deg5`).
* `hub_meets_cycle_dist2_le5_seventeen` — a deg-`≤ 5` hub cannot meet two cycle vertices at
  cycle-distance `2` *unless* it also meets the vertex between them (else an induced `C₄` of degree
  sum `≤ 14`, excluded by `hC4`).
* `hub_cycle_cases_le5_seventeen` — from the five distance-`2` constraints, either the hub avoids a
  consecutive triple of the cycle (a cherry) or it hits a consecutive triple.
* `c5_shared_two_twin_le5_seventeen` — wraps the cases: the cherry branch assembles a
...
/-- **Two-twin bundle assembly with a deg-`≤ 5` hub (`n = 17`).**  Port of `dense_two_twin_assemble`
...
theorem dense_two_twin_assemble_le5_seventeen (G : SimpleGraph (Fin 17)) (t₁ t₂ h x y z : Fin 17)
...
/-- **A deg-`≤ 5` hub avoids distance-`2` cycle hits without the middle (`n = 17`).**  Around an
...
theorem hub_meets_cycle_dist2_le5_seventeen (G : SimpleGraph (Fin 17))
    (hC4 : ¬∃ a b c d : Fin 17, ({a, b, c, d} : Finset (Fin 17)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    {h x y z : Fin 17} (hhge4 : 4 ≤ G.degree h) (hhle5 : G.degree h ≤ 5)
...
/-- **Cycle-hit cases for a deg-`≤ 5` hub (`n = 17`).**  Around an induced `C₅` `v₁–⋯–v₅` of
...
(`hub_meets_cycle_dist2_le5_seventeen`) rule out every other pattern. -/
theorem hub_cycle_cases_le5_seventeen (G : SimpleGraph (Fin 17))
    (hC4 : ¬∃ a b c d : Fin 17, ({a, b, c, d} : Finset (Fin 17)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (h v₁ v₂ v₃ v₄ v₅ : Fin 17) (hhge4 : 4 ≤ G.degree h) (hhle5 : G.degree h ≤ 5)
  ...
  have C234 := hub_meets_cycle_dist2_le5_seventeen G hC4 hhge4 hhle5 hd2 hd3 hd4 e23 e34 n24
    d23 d34 d24
  have C345 := hub_meets_cycle_dist2_le5_seventeen G hC4 hhge4 hhle5 hd3 hd4 hd5 e34 e45 n35
    d34 d45 d35
  have C451 := hub_meets_cycle_dist2_le5_seventeen G hC4 hhge4 hhle5 hd4 hd5 hd1 e45 e51
    (fun a => n14 a.symm) d45 d15.symm d14.symm
  have C512 := hub_meets_cycle_dist2_le5_seventeen G hC4 hhge4 hhle5 hd5 hd1 hd2 e51 e12
    (fun a => n25 a.symm) d15.symm d12 d25.symm
  have C123 := hub_meets_cycle_dist2_le5_seventeen G hC4 hhge4 hhle5 hd1 hd2 hd3 e12 e23 n13
    d12 d23 d13
...
/-- **A second hub avoids a degree-`3` vertex with two degree-`3` neighbours (`n = 17`).**  If a
...
vertex `a`). -/
theorem hub_not_adj_deg3_two_deg3_nbrs (G : SimpleGraph (Fin 17)) (h k a x y : Fin 17)
  ...
  have hsub : ({x, y, k, h} : Finset (Fin 17)) ⊆ G.neighborFinset a := by
    intro w hw
  ...
  have hcard : ({x, y, k, h} : Finset (Fin 17)).card = 4 := by
...
/-- **Induced-`C₅` two-twin assembly with a deg-`≤ 5` hub (`n = 17`).**  A hub `k` of degree `4` or
...
(`hub_cycle_cases_le5_seventeen`), `k` itself anchors the cut.  When `k` *hits* three consecutive
...
theorem c5_shared_two_twin_le5_seventeen (G : SimpleGraph (Fin 17))
    ...
    (ht1iso : ∀ w : Fin 17, G.Adj t₁ w → G.degree w ≠ 3)
    (ht2iso : ∀ w : Fin 17, G.Adj t₂ w → G.degree w ≠ 3)
    (hnonk : ∀ a b c : Fin 17, G.degree a = 3 → G.degree b = 3 → G.degree c = 3 →
  ...
  have hB := hub_cycle_cases_le5_seventeen G hC4 k v₁ v₂ v₃ v₄ v₅ hkge4 hkle5 hd1 hd2 hd3 hd4 hd5
  ...
  · exact dense_two_twin_assemble_le5_seventeen G t₁ t₂ k v₁ v₂ v₃ ht1deg ht2deg hkge4 hkle5
  ...
  · -- **Deg-`5` hub `k` adjacent to three *consecutive* `C₅` vertices.**  Here `k` hits the cherry
    ...
    -- `dense_two_twin_assemble_le5_seventeen` closes.
    ...
    · obtain ⟨h, s₁, s₂, hhk, hh4, hh5, hs12, hs1d, hs2d, hAs1, hAs2, hs1iso, hs2iso⟩ :=
      ...
      exact dense_two_twin_assemble_le5_seventeen G s₁ s₂ h v₁ v₂ v₃ hs1d hs2d hh4 hh5
