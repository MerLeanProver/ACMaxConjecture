...
iso-incidence total `3·(d − 4)`, and hub-internal total `66 − 6·d` (`12` at `d = 9`, `6` at
...
/-- **Generalised residual structural counts (`n = 17`, `P₄` cherry).**  From the path/`Iso`
...
hub-incidence totals (`6`, `3·(d − 4)`, `66 − 6·d`), and the per-hub degree split. -/
theorem cherry_p4_hub_struct_seventeen (G : SimpleGraph (Fin 17))
    (hm : G.edgeFinset.card = 30)
    ...
    Dᶜ.card = 17 - D.card ∧ Iso.card = D.card - 4 ∧
      ...
      (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Dᶜ).card = 66 - 6 * D.card) ∧
      (∀ g : Fin 17, g ∈ Dᶜ →
  ...
  have hisoND : ∀ t : Fin 17, t ∈ Iso → (G.neighborFinset t ∩ D).card = 0 := by
    intro t ht
    rw [Finset.card_eq_zero, Finset.eq_empty_iff_forall_notMem]
    intro a ha
    rw [Finset.mem_inter, G.mem_neighborFinset] at ha
    exact (hIsoprop t ht).2 a ha.1 ((hmemD a).mp ha.2)
  have hisohub : ∀ t : Fin 17, t ∈ Iso → (G.neighborFinset t ∩ Dᶜ).card = 3 := by
    intro t ht
    have := hsplit t
  ...
  have hDccard : Dᶜ.card = 17 - D.card := by
  ...
  have hper : ∀ g : Fin 17, g ∈ Dᶜ →
    ...
    have := hsplit g
    omega
  -- Hub-internal total via handshake `∑ deg = 60`.
  have hsumInternal : ∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Dᶜ).card = 66 - 6 * D.card := by
