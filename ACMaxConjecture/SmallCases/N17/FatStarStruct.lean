...
import ACMaxConjecture.SmallCases.N17.HubTriangleFatStar

/-!
# Asymmetric dominating-edge double-star structure for `n = 17`, `e(M) = 4`, `|D| = 8`

This file supplies `fatstar_structure_eM4_D8_seventeen`, the structural dispatcher used to close the
`halign8_eM4_seventeen` dominating-edge `|D| = 8` branch.  Given the `e(M) = 4` dominating edge
`c₁–c₂` (covering hypothesis `hcov`), the in-`M`-degrees split asymmetrically `(3, 2)` (one fat
centre with two leaves, one thin centre with one leaf), via `thin_eM_formula_seventeen`.  The fat
double-star `L₁, L₁' – c₁ – c₂ – L₂` is extracted exactly and fed to the proved-axiom-clean
`exists_hub_triangle_config_fatstar_seventeen`.

The `|D| = 11` case (`|Hub| = 6`) never reaches this branch: it is dispatched upstream in
`halign8_seventeen` to `two_hub_or_single_vertex_hub6_seventeen`, so `fatstar_structure_eM4_D8_seventeen`
receives `hDne11 : D.card ≠ 11` as a hypothesis, which combines with the handshake bounds
`8 ≤ |D| ≤ 11` to force `|D| = 8`.
-/
...
`¬SingleVertex/¬TwoTwin/¬TwoHub` residual to `exists_hub_triangle_config_fatstar_seventeen`. -/
theorem fatstar_from_fat_thin_seventeen (G : SimpleGraph (Fin 17)) (D : Finset (Fin 17))
    ...
    (hcov : ∀ p q : Fin 17, p ∈ D → q ∈ D → G.Adj p q → p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
  ...
  have hisochar : ∀ w : Fin 17, w ∈ D → w ≠ c₁ → w ≠ c₂ → w ≠ L₁ → w ≠ L₁' → w ≠ L₂ → w ∈ Iso := by
    ...
    · have hmem : w ∈ G.neighborFinset c₂ ∩ D :=
      ...
      · exact hwc1 h'
      · exact hwL2 h'
  exact exists_hub_triangle_config_fatstar_seventeen G D Iso c₁ c₂ L₁ L₁' L₂ hT hC4 hmemD hIsodef
...
/-- **Fat double-star structural dispatcher (`n = 17`, `e(M) = 4`, dominating edge, `|D| = 8`).**
Routes the `halign8_eM4_seventeen` dominating-edge `|D| = 8` branch: the in-`M`-degree split is
`(3, 2)` (`thin_eM_formula_seventeen`), giving a fat double-star fed to
`exists_hub_triangle_config_fatstar_seventeen`. -/
theorem fatstar_structure_eM4_D8_seventeen (G : SimpleGraph (Fin 17))
    ...
    (hDnotmid : ¬(9 ≤ D.card ∧ D.card ≤ 10))
    (hsv : ¬SingleVertexConfig G) (htt : ¬TwoTwinConfig G) (hth : ¬TwoHubConfig G) :
    HubTriangleConfig G := by
  classical
  -- Handshake bounds `8 ≤ |D| ≤ 11`.
  ...
  have hsplit60 : ∑ v ∈ D, G.degree v + ∑ v ∈ Dᶜ, G.degree v = 60 := by
    rw [Finset.sum_add_sum_compl]; exact hsum60
  have hsumD_split : ∑ v ∈ D, (G.neighborFinset v ∩ D).card
      + ∑ v ∈ D, (G.neighborFinset v ∩ Dᶜ).card = ∑ v ∈ D, G.degree v := by
    rw [← Finset.sum_add_distrib]
    exact Finset.sum_congr rfl (fun v _ => nbr_split_DC G D v)
  have hsumDc_split : ∑ v ∈ Dᶜ, (G.neighborFinset v ∩ D).card
      + ∑ v ∈ Dᶜ, (G.neighborFinset v ∩ Dᶜ).card = ∑ v ∈ Dᶜ, G.degree v := by
    rw [← Finset.sum_add_distrib]
    exact Finset.sum_congr rfl (fun v _ => nbr_split_DC G D v)
  have hcc : ∑ v ∈ Dᶜ, (G.neighborFinset v ∩ D).card
      = ∑ v ∈ D, (G.neighborFinset v ∩ Dᶜ).card := cross_count G Dᶜ D
  have hint_eq : ∑ v ∈ Dᶜ, (G.neighborFinset v ∩ Dᶜ).card + 6 * D.card = 68 := by
    omega
  have h11 : D.card ≤ 11 := by omega
  obtain ⟨_, h8⟩ := residual_hub_card_le_nine G hm h3
  have h8' : 8 ≤ D.card := by
    have : D = Finset.univ.filter (fun w : Fin 17 => G.degree w = 3) := by
      ext v; rw [hmemD, Finset.mem_filter]; simp
    rw [this]; exact h8
  have hne11 : D.card ≠ 11 := hDne11
  have hD8 : D.card = 8 := by omega
  -- All hubs degree `4`.
  have hDcdeg : ∀ w ∈ Dᶜ, 4 ≤ G.degree w := by
    intro w hw; rw [Finset.mem_compl, hmemD] at hw; have := h3 w; omega
  have hDccard : Dᶜ.card = 9 := by rw [Finset.card_compl, Fintype.card_fin, hD8]
  have hdeg4 : ∀ w : Fin 17, w ∈ Dᶜ → G.degree w = 4 := by
    intro w hw
    ...
    have hsum36 : ∑ v ∈ Dᶜ, G.degree v = 36 := by omega
  ...
  · have hi2 : (G.neighborFinset c₂ ∩ D).card = 2 := by omega
    exact fatstar_from_fat_thin_seventeen G D hmemD hT hC4 c₁ c₂ hc1D hc2D hc12 hcov hfat hi2
      hdeg4 hD8 hsv htt hth
  · have hi1 : (G.neighborFinset c₁ ∩ D).card = 2 := by omega
    have hi2 : (G.neighborFinset c₂ ∩ D).card = 3 := by omega
    have hcov' : ∀ p q : Fin 17, p ∈ D → q ∈ D → G.Adj p q →
        p = c₂ ∨ p = c₁ ∨ q = c₂ ∨ q = c₁ := by
      intro p q hp hq hpq
    ...
    exact fatstar_from_fat_thin_seventeen G D hmemD hT hC4 c₂ c₁ hc2D hc1D hc12.symm hcov' hi2 hi1
      hdeg4 hD8 hsv htt hth
