...
/-!
# Structural leaves for the `n = 17`, `e(M) = 3`, `|D| = 8` hub-triangle corner

This file collects the *concrete, decidable-free* structural facts about the residual
configuration consumed by `exists_hub_triangle_config_residual_seventeen` in
`TwinCert17HubTriangle`: the **nine** degree-`4` hubs (`Dᶜ`), the four `M`-isolated degree-`3`
...
The `n = 17` deltas from `n = 16` are `|Hub| = 9` (vs `8`), so each cherry has `≥ 5` avoiders
(vs `≥ 4`) and the hub-internal-degree total is `∑ int = 4·9 − 6 − 12 = 18` (vs `14`).  The wider
budget pins the fully-free count to `|FF| ∈ {2, 3, 4, 5, 6}`.
-/
...
theorem nbr_split_DC (G : SimpleGraph (Fin 17)) (D : Finset (Fin 17)) (v : Fin 17) :
...
`L₂ ≁ c₁`. -/
theorem leaf_nbr_D (G : SimpleGraph (Fin 17)) (D : Finset (Fin 17)) (L₁ c₁ c₂ L₂ : Fin 17)
...
theorem path_hub_incidence (G : SimpleGraph (Fin 17)) (D : Finset (Fin 17)) (L₁ c₁ c₂ L₂ : Fin 17)
    (hcov : ∀ p q : Fin 17, p ∈ D → q ∈ D → G.Adj p q →
      p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
...
/-- **Hub and twin counts.**  In the `|D| = 8` corner there are nine hubs (`Dᶜ.card = 9`) and four
...
theorem hub_struct (G : SimpleGraph (Fin 17)) (D Iso : Finset (Fin 17)) (L₁ c₁ c₂ L₂ : Fin 17)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    ...
    (hD8 : D.card = 8) :
    Dᶜ.card = 9 ∧ Iso.card = 4 := by
  classical
  have hDc : Dᶜ.card = 9 := by
  ...
  set P : Finset (Fin 17) := {L₁, c₁, c₂, L₂} with hP
...
hub-neighbours and there are `9` hubs, then at least `5` hubs avoid all of `a, b, c`. -/
theorem avoiders_ge_five_aux (G : SimpleGraph (Fin 17)) (D : Finset (Fin 17)) (a b c : Fin 17)
    (hDc9 : Dᶜ.card = 9)
    (hbound : (G.neighborFinset a ∩ Dᶜ).card + (G.neighborFinset b ∩ Dᶜ).card
      + (G.neighborFinset c ∩ Dᶜ).card ≤ 4) :
    5 ≤ (Dᶜ.filter (fun g => ¬G.Adj g a ∧ ¬G.Adj g b ∧ ¬G.Adj g c)).card := by
  classical
...
/-- **Each cherry has at least five avoiding hubs.**  Cherry `{L₁, c₁, c₂}` collects only
`2 + 1 + 1 = 4` hub-incidences (`L₁` two, `c₁`/`c₂` one each), so `≥ 5` of the nine hubs avoid it;
symmetrically for `{c₁, c₂, L₂}`. -/
theorem cherry_avoiders_ge_five (G : SimpleGraph (Fin 17)) (D : Finset (Fin 17))
    (L₁ c₁ c₂ L₂ : Fin 17) (hDc9 : Dᶜ.card = 9)
    ...
    5 ≤ (Dᶜ.filter (fun g => ¬G.Adj g L₁ ∧ ¬G.Adj g c₁ ∧ ¬G.Adj g c₂)).card ∧
      5 ≤ (Dᶜ.filter (fun g => ¬G.Adj g c₁ ∧ ¬G.Adj g c₂ ∧ ¬G.Adj g L₂)).card := by
  refine ⟨avoiders_ge_five_aux G D L₁ c₁ c₂ hDc9 (by omega),
    avoiders_ge_five_aux G D c₁ c₂ L₂ hDc9 (by omega)⟩
...
theorem avoider_internal_ge_two (G : SimpleGraph (Fin 17)) (D Iso : Finset (Fin 17))
    (g allowed : Fin 17) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
...
theorem fully_free_internal_ge_three (G : SimpleGraph (Fin 17)) (D Iso : Finset (Fin 17))
    (g : Fin 17) (hdg : G.degree g = 4) (hIso1 : (G.neighborFinset g ∩ Iso).card ≤ 1)
...
theorem path_iso_partition (G : SimpleGraph (Fin 17)) (D Iso : Finset (Fin 17))
    (L₁ c₁ c₂ L₂ : Fin 17)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hisochar : ∀ w : Fin 17, w ∈ D → w ≠ L₁ → w ≠ c₁ → w ≠ c₂ → w ≠ L₂ → w ∈ Iso)
    (hL1D : L₁ ∈ D) (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hL2D : L₂ ∈ D)
    (hac1L1 : G.Adj c₁ L₁) (hc12 : G.Adj c₁ c₂) (hac2L2 : G.Adj c₂ L₂) :
    ({L₁, c₁, c₂, L₂} : Finset (Fin 17)) ∪ Iso = D ∧
      Disjoint ({L₁, c₁, c₂, L₂} : Finset (Fin 17)) Iso := by
  classical
  set P : Finset (Fin 17) := {L₁, c₁, c₂, L₂} with hP
  have hIsoD : Iso ⊆ D := by rw [hIsodef]; exact Finset.filter_subset _ _
  have hPD : P ⊆ D := by
...
/-- **Residual incidence sums.**  Over the nine hubs the path-, iso- and hub-internal incidences
total `6`, `12`, `18` respectively, and per hub the three split the degree `4`:
`path(g) + iso(g) + int(g) = 4`. -/
theorem residual_incidence_sums (G : SimpleGraph (Fin 17)) (D Iso : Finset (Fin 17))
    (L₁ c₁ c₂ L₂ : Fin 17)
    (hIsodef : Iso = D.filter (fun v => (G.neighborFinset v ∩ D).card = 0))
    (hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 17, G.Adj v w → G.degree w ≠ 3))
    (hisochar : ∀ w : Fin 17, w ∈ D → w ≠ L₁ → w ≠ c₁ → w ≠ c₂ → w ≠ L₂ → w ∈ Iso)
    ...
    (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 17))).card = 6) ∧
      (∑ g ∈ Dᶜ, (G.neighborFinset g ∩ Iso).card = 12) ∧
      (∀ g ∈ Dᶜ, (G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 17))).card
...
internal degree `≥ 3` (`hFFint`), `|A1|, |A2| ≥ 5`, `FF = A1 ∩ A2`, and the hub-internal-degree
total `∑_{Dᶜ} f = 18`, the fully-free count lies in `{2, 3, 4, 5, 6}`. -/
theorem active_hub_budget_seventeen (Dc A1 A2 FF : Finset (Fin 17)) (f : Fin 17 → ℕ)
    (hA1sub : A1 ⊆ Dc) (hA2sub : A2 ⊆ Dc) (hFF : FF = A1 ∩ A2)
    (hA1card : 5 ≤ A1.card) (hA2card : 5 ≤ A2.card)
    (hA1int : ∀ g ∈ A1, 2 ≤ f g) (hA2int : ∀ g ∈ A2, 2 ≤ f g)
    (hFFint : ∀ g ∈ FF, 3 ≤ f g) (hSum18 : ∑ w ∈ Dc, f w = 18) :
  ...
  -- `|FF| ≤ 6`: `3|FF| ≤ ∑_{FF} f ≤ ∑_{Dᶜ} f = 18`.
  have hFFsum_le : ∑ g ∈ FF, f g ≤ 18 := by
    rw [← hSum18]
  ...
  have hUsum_le : ∑ g ∈ A1 ∪ A2, f g ≤ 18 := by
    rw [← hSum18]
