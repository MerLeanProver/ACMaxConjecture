...
immediately) and all the structural counting (`TwinCert17HubTriangleStruct`: nine hubs, `≥ 5`
avoiders per cherry, `∑ int = 18`) are fully proved here.  The single remaining obligation is the
**no-avoider-triangle ⇒ contradiction** core `core_triangle_force_seventeen`, which carries one
...
branch.  See `core_triangle_force_seventeen` for the analysis.
-/
...
/-- **No avoider triangle ⇒ contradiction (the `n = 17` combinatorial core).**  Under the residual
hypotheses for the `n = 17`, `|D| = 8`, `|Hub| = 9` corner, the assumption that neither cherry
admits a triangle of pairwise-adjacent avoiding hubs (`htri1`, `htri2`) is contradictory.

This is the genuinely new obligation for `n = 17` (the `n = 15`/`n = 16` arguments do not port: the
looser budget `∑ int = 18` admits `|FF| ∈ {2, 3, 4, 5, 6}`).  It is discharged by
`iso_rich_force_seventeen` (`TwinCert17HubTriangleFF`), which dispatches on `|FF|` via the new
*isolated-twin incidence bound* `∑_{A1} iso ≤ 4`, `∑_{A2} iso ≤ 4` (each twin has `≤ 1` neighbour in
either avoider set, by `hA`), forcing the iso-mass redistribution `∑_B iso ≥ 4 + ∑_{FF} iso`:

* `|FF| = 6`: `∑_{FF} iso ≤ 4` with `iso + int = 4` per fully-free hub gives `∑_{FF} int ≥ 20 > 18`
  (pure counting — no `K_{3,3}` analysis needed);
* `|FF| = 2`: `|B| = 1` but `∑_B iso ≥ 4 > 3`;
* `|FF| = 3`: `|B| = 2` with both inactive hubs `iso = 3`, a `nonadj_hubs_share_le_one_iso` share
  contradiction;
* `|FF| ∈ {4, 5}`: the generic sub-cases close the same way (`|FF| = 4`, `|B| = 2`: counting;
  `|FF| = 5`, `|B| = 3`: two iso-`3` inactive hubs share), leaving **two documented `sorry`s** for
  the extremal fully-free `(5, 5)` corners (`|FF| = 4`, `|B| = 3` and `|FF| = 5`, `|B| = 4`), which
  need a Mantel triangle inside `FF`. -/
theorem core_triangle_force_seventeen (G : SimpleGraph (Fin 17))
    ...
    False :=
  iso_rich_force_seventeen G D Iso L₁ c₁ c₂ L₂ hT hC4 hK23 hmemD hIsodef hIsoprop hisochar hcov
...
/-- **Hub-triangle existence in the `n = 17`, `|D| = 8`, `e(M) = 3`, `P₄` residual corner.**  Under
the residual hypotheses (nine degree-`4` hubs, `9` hub-hub edges, `M = P₄` `L₁–c₁–c₂–L₂` with four
...
`core_triangle_force_seventeen` (carrying one documented `sorry`, see its docstring). -/
theorem exists_hub_triangle_config_residual_seventeen (G : SimpleGraph (Fin 17))
    (D Iso : Finset (Fin 17)) (L₁ c₁ c₂ L₂ : Fin 17)
    (hT : ¬∃ x y z : Fin 17, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 17, ({a, b, c, d} : Finset (Fin 17)).card = 4 ∧
  ...
  have hA : ∀ t : Fin 17, t ∈ Iso → ∀ p q : Fin 17, p ≠ q →
    ...
    · have hsum0 : ∑ w ∈ ({t, p, q} : Finset (Fin 17)),
          (G.neighborFinset w ∩ ({L₁, c₁, c₂} : Finset (Fin 17))).card = 0 := by
  ...
  have hSum18 : ∑ w ∈ Dᶜ, (G.neighborFinset w ∩ Dᶜ).card = 18 := by
    have htot : ∑ g ∈ Dᶜ, ((G.neighborFinset g ∩ ({L₁, c₁, c₂, L₂} : Finset (Fin 17))).card
  ...
  exfalso
  exact core_triangle_force_seventeen G D Iso L₁ c₁ c₂ L₂ hT hC4 hK23 hmemD hIsodef hIsoprop
