...
/-!
# Hub-pair selection for the `n = 17` two-hub `e(M) ≤ 1` residual

This file supplies the **shared selection engine** (`select_finish_seventeen`,
`nogood_of_not_select_seventeen`, `strong_deg4_count_le_two_deg5_seventeen`, `deg4_sum_le_seventeen`,
`residual_arith`, …) feeding the `e(M) ≤ 1` branch of the `n = 17` two-hub configuration.  The
selection entry points `two_hub_corner_select_seventeen` and the tight residual
`two_hub_select_eM1_tight_seventeen` live in `TwinCert17TwoHubTight` (which imports this engine).

At `e(M) ≤ 1` the handshake `e(M) = 21 − 3·|Hub| + e(Hub)` (total `e(G) = 30`) together with the
degree relation `∑_{Hub} deg = 3·|Hub| + 9` (equivalently `#{deg-5 hubs} = 9 − |Hub|`) yields the
regimes `|Hub| ∈ {7, 8, 9}`, `|Iso| ∈ {6, 7, 8, 9, 10}`:

* `e(M) = 0`: `(|Hub|, |Iso|, ∑deg) ∈ {(7,10,30), (8,9,33), (9,8,36)}`;
* `e(M) = 1`: `(|Hub|, |Iso|, ∑deg) ∈ {(7,8,30), (8,7,33), (9,6,36)}`.

The **key simplification** (as for `n = 16`) is the sharp share bound `nonadj_hubs_share_le_one_iso`
(non-adjacent degree-`4` hubs share at most **one** `M`-isolated twin).  With share `≤ 1`, the strong
degree-`4` hubs (iso-degree `≥ 3`) form a clique, each with `≤ 1` hub-neighbour, so they number
`≤ 2`.  Erasing the `9 − |Hub|` degree-`5` hubs, the degree-`4` iso-degree total
`∑_{deg-4} isoDeg ≥ 3·|Iso| − 5·(9 − |Hub|)` clashes with the threshold bound
`∑_{deg-4} isoDeg ≤ 2·|deg-4| + 2`, closing every regime with `3·|Iso| > 29 − |Hub|`.  The two
`e(M) = 1` profiles `(8,7,33)` and `(9,6,36)` are tight (`3·|Iso| = 29 − |Hub|`); the larger good-`C₄`
share-sum `6·|Iso|` is too weak there, so they are routed through the documented placeholder
`two_hub_select_eM1_tight_seventeen` pending a separate structural development.
-/
...
theorem select_finish_seventeen (G : SimpleGraph (Fin 17)) (Iso : Finset (Fin 17)) (h₁ h₂ : Fin 17)
...
theorem hub_iso_sum_seventeen (G : SimpleGraph (Fin 17)) (Hub Iso : Finset (Fin 17))
...
/-- **No good pair from a refuted selection.**  Packages `select_finish_seventeen` into the
`min(iso-deg) ≤ share + 1` form on non-adjacent degree-`4` hub pairs, given a refutation `hcon` of
the selection goal. -/
theorem nogood_of_not_select_seventeen (G : SimpleGraph (Fin 17)) (Hub Iso : Finset (Fin 17))
    (hcon : ¬ ∃ h₁ h₂ : Fin 17, h₁ ∈ Hub ∧ h₂ ∈ Hub ∧
  ...
  have hfin := select_finish_seventeen G Iso a b hsa hsb
...
theorem hub_neighbor_le_seventeen (G : SimpleGraph (Fin 17)) (Hub Iso : Finset (Fin 17))
    (hdisj : Disjoint Hub Iso) (a : Fin 17) (hda : G.degree a = 4) :
...
`Hub`. -/
theorem strong_deg4_count_le_two_deg5_seventeen (G : SimpleGraph (Fin 17))
    (Hub Iso : Finset (Fin 17)) (hdisj : Disjoint Hub Iso)
  ...
  have hkey : ∀ a ∈ A, A.card - 1 + (G.neighborFinset a ∩ Iso).card ≤ 4 := by
    ...
    have hc2 := hub_neighbor_le_seventeen G Hub Iso hdisj a hda
    omega
...
/-- **Degree-`4` iso-degree threshold sum bound.**  For the degree-`4` hubs `T = {h ∈ Hub : deg = 4}`
...
theorem deg4_sum_le_seventeen (G : SimpleGraph (Fin 17)) (Hub Iso : Finset (Fin 17))
...
theorem residual_arith (G : SimpleGraph (Fin 17)) (Hub Iso : Finset (Fin 17))
