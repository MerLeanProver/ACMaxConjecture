...
/-!
# `n = 17`, `e(M) = 3` (`s = 6`), `P₄`-cherry corner: the `{c₁,c₂}`-avoider count

The genuinely-new counting leaf for the `P₄`-cherry corners (`TwinCert17CherryP4.lean`).  Using the
...
strict pigeonhole margin (`3·|HubA| > Int + 4` at `|D| ∈ {9,10}`).
-/
...
/-- **`{c₁,c₂}`-avoider with two `M`-isolated twins (`n = 17`, `P₄` corner).**  For the path
`L₁–c₁–c₂–L₂` residual with `|D| ∈ {9,10}`, there is a degree-`≤ 5` hub `h` avoiding both centres
`c₁, c₂` and carrying two distinct `M`-isolated twins `t₁, t₂ ∈ Iso`. -/
theorem p4_c1c2_avoider_two_twin_seventeen (G : SimpleGraph (Fin 17))
    ...
    ∃ h t₁ t₂ : Fin 17, h ∈ Dᶜ ∧ G.degree h ≤ 5 ∧ ¬G.Adj h c₁ ∧ ¬G.Adj h c₂ ∧
  ...
  have hsumDcdeg : ∑ w ∈ Dᶜ, G.degree w = 60 - 3 * D.card := by
    ...
    omega
  have hDc_card : Dᶜ.card = 17 - D.card := by
  ...
  -- `D = {L₁,c₁,c₂,L₂} ∪ Iso`.
  set path : Finset (Fin 17) := {L₁, c₁, c₂, L₂} with hpathdef
  ...
  have hpathIsodisj : Disjoint path Iso := by
    ...
    · exact hL2Iso hw'
  -- `|N L₁ ∩ D| = 1` and `|N L₂ ∩ D| = 1`.
  have hIso0 : ∀ v : Fin 17, v ∈ Iso → (G.neighborFinset v ∩ D).card = 0 := by
    intro v hv
  ...
  have hL2Dc : (G.neighborFinset L₂ ∩ Dᶜ).card = 2 := by
    rw [hpathexpand, hin1, hin2] at hsumpath; have := hsplit L₂; omega
  -- Internal sum `∑_{Dᶜ}|N w ∩ Dᶜ| = 66 − 6|D|`.
  set Int : ℕ := 66 - 6 * D.card with hIntdef
  ...
  set HubA : Finset (Fin 17) := Dᶜ \ Bad with hHubAdef
  ...
  have hfc2 : (Dᶜ.filter (fun w => G.Adj w c₂)).card ≤ 1 := by
    have heq : Dᶜ.filter (fun w => G.Adj w c₂) = G.neighborFinset c₂ ∩ Dᶜ := by
      ...
      exact ⟨fun h => ⟨h.2.symm, h.1⟩, fun h => ⟨h.2, h.1.symm⟩⟩
    rw [heq, hc2Dc]
  set Big : Finset (Fin 17) := Dᶜ.filter (fun w => 6 ≤ G.degree w) with hBigdef
  ...
  have hnbig : 2 * Big.card ≤ D.card - 8 := by
    ...
    have h2 : 4 * (Dᶜ \ Big).card ≤ ∑ w ∈ Dᶜ \ Big, G.degree w := by
      ...
      simpa [smul_eq_mul, Nat.mul_comm] using this
    have hsplitB : ∑ w ∈ Dᶜ \ Big, G.degree w + ∑ w ∈ Big, G.degree w = 60 - 3 * D.card := by
      rw [Finset.sum_sdiff hBigsub]; exact hsumDcdeg
    have hcardB : (Dᶜ \ Big).card + Big.card = 17 - D.card := by
  ...
  have hHubA_props : ∀ h : Fin 17, h ∈ HubA →
  ...
  have hHubAdeg5 : ∀ h : Fin 17, h ∈ HubA → G.degree h ≤ 5 := fun h hh => (hHubA_props h hh).2.2.2
  have hHubAsub : HubA ⊆ Dᶜ := by rw [hHubAdef]; exact Finset.sdiff_subset
  -- Per-hub Iso count for `HubA` hubs.
  have hHubA_iso : ∀ h : Fin 17, h ∈ HubA →
      (G.neighborFinset h ∩ Iso).card + (G.neighborFinset h ∩ Dᶜ).card
        + (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 17))).card = G.degree h := by
    intro h hh
    ...
    have hpathleaf :
        G.neighborFinset h ∩ path = G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 17)) := by
      ext w
  ...
  have hsumIso : ∑ h ∈ HubA, (G.neighborFinset h ∩ Iso).card
      ...
      + ∑ h ∈ HubA, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 17))).card
  ...
  have hleakle : ∑ h ∈ HubA, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 17))).card ≤ 4 := by
    have hfull : ∑ h ∈ Dᶜ, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 17))).card = 4 := by
      rw [cross_count G Dᶜ ({L₁, L₂} : Finset (Fin 17)),
        Finset.sum_insert (by simp [hL1L2]), Finset.sum_singleton, hL1Dc, hL2Dc]
    calc ∑ h ∈ HubA, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 17))).card
        ≤ ∑ h ∈ Dᶜ, (G.neighborFinset h ∩ ({L₁, L₂} : Finset (Fin 17))).card :=
  ...
  obtain ⟨h, t₁, t₂, hhHubA, _hd5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1no, ht2no⟩ :=
    shared_hub_le5_from_count_seventeen G Iso HubA hIsoprop hHubAdeg5 hstrict
  obtain ⟨hhDc, hhnc1, hhnc2, hhdeg5⟩ := hHubA_props h hhHubA
  have htwinIso : ∀ t : Fin 17, G.degree t = 3 → (∀ w : Fin 17, G.Adj t w → G.degree w ≠ 3) →
      t ∈ Iso := by
    intro t htdeg htno
