...
/-!
# `n = 17`, `|D| = 11`, `|Hub| = 6`, `e(M) = 5` (`s = 10`) shared-hub helper

The genuinely degenerate `e(M) = 5` corner of the `|Hub| = 6` alignment selector
(`two_hub_or_single_vertex_hub6_seventeen`), ported from `TwinCert16TwoHubHub6Deg5.lean`.  At
`e(M) = 5` the residual is a double-star (dominating edge) or an induced `C₅`.  The deg-`4`-shared-hub
pigeonhole used at `e(M) ≤ 4` degenerates; for `n = 17` (degree sum `60`, divisible by `3`) it no
...
* **induced-`C₅` branch (fully proved):** the deg-`4` pigeonhole ties (`X ≥ 3`, `|Hub₄| = 3`), so
  ...
  `exists_nonk_two_twin_hub_seventeen`: those three hit vertices have the deg-`5` hub `k` as their
  unique hub neighbour, so a second deg-`≤ 5` hub `h ≠ k` (with two `M`-isolated twins, extracted by
  the refined `hub5_iso_count_seventeen` pigeonhole on `Hub₅ \ {k}`) avoids that cherry instead.

The helper lemmas (`shared_hub_le5_from_count_seventeen`, `dense_two_twin_assemble_le5_seventeen`,
`claw_shared_two_twin_le5_seventeen`, `dom_fat_centre_two_twin_le5_seventeen`,
`shared_hub_le5_eM5_seventeen`) are all axiom-clean ports.
-/
...
/-- **Shared deg-`≤ 5` hub by pigeonhole (`n = 17`).**  Port of `shared_deg4_hub_from_count_seventeen`
...
theorem shared_hub_le5_from_count_seventeen (G : SimpleGraph (Fin 17)) (Iso Hub : Finset (Fin 17))
    (hIsoiso : ∀ v ∈ Iso, G.degree v = 3 ∧ ∀ w : Fin 17, G.Adj v w → G.degree w ≠ 3)
    (hHubdeg : ∀ h ∈ Hub, G.degree h ≤ 5)
    (hcount : Hub.card < ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub).card) :
    ∃ h t₁ t₂ : Fin 17, h ∈ Hub ∧ G.degree h ≤ 5 ∧ t₁ ≠ t₂ ∧
  ...
  have hincid : ∃ h ∈ Hub, 2 ≤ (Iso.filter (fun v => G.Adj v h)).card := by
    ...
    have hswap : ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub).card
      ...
      have hL : ∀ v : Fin 17, (G.neighborFinset v ∩ Hub).card
...
/-- **Fat-dominating claw assembly with a deg-`≤ 5` hub (`n = 17`).**  Port of
`claw_shared_two_twin_seventeen` permitting `4 ≤ deg h ≤ 5`: the C₄ obstruction `h–i–c–j` has degree
sum `deg h + 9 ≤ 14`, so it is still forbidden by `hC4` at `deg h = 5`, and the hub meets at most
one of `c`'s three degree-`3` neighbours. -/
theorem claw_shared_two_twin_le5_seventeen (G : SimpleGraph (Fin 17)) (D : Finset (Fin 17))
    ...
    (ht1iso : ∀ w : Fin 17, G.Adj t₁ w → G.degree w ≠ 3)
    (ht2iso : ∀ w : Fin 17, G.Adj t₂ w → G.degree w ≠ 3) :
    TwoTwinConfig G := by
  classical
  ...
  have hmem_i : ∀ w : Fin 17, w ∈ ({n₁, n₂, n₃} : Finset (Fin 17)) → G.Adj c w ∧ w ∈ D := by
    intro w hw
  ...
  have hcnbhd : ∀ w : Fin 17, G.Adj c w → w = n₁ ∨ w = n₂ ∨ w = n₃ := by
    intro w hw
  ...
  have hmeet : ∀ i j : Fin 17, G.degree i = 3 → G.degree j = 3 →
    ...
    exact hC4 ⟨h, i, c, j,
      card_four_seventeen h i c j (by rintro rfl; omega) (by rintro rfl; omega)
  ...
  · exact dense_two_twin_assemble_le5_seventeen G t₁ t₂ h n₂ c n₃ ht1deg ht2deg hhge4 hhle5
...
/-- **Dominating-edge fat-centre dispatch with a deg-`≤ 5` hub (`n = 17`).**  Port of
`dom_fat_centre_two_twin_seventeen` with the hub permitted degree `4` or `5`. -/
theorem dom_fat_centre_two_twin_le5_seventeen (G : SimpleGraph (Fin 17)) (D : Finset (Fin 17))
    (hmemD : ∀ v : Fin 17, v ∈ D ↔ G.degree v = 3)
    (hT : ¬∃ x y z : Fin 17, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (hC4 : ¬∃ a b c d : Fin 17, ({a, b, c, d} : Finset (Fin 17)).card = 4 ∧
      G.Adj a b ∧ G.Adj b c ∧ G.Adj c d ∧ G.Adj d a ∧ ¬G.Adj a c ∧ ¬G.Adj b d ∧
      G.degree a + G.degree b + G.degree c + G.degree d ≤ 14)
    (c₁ c₂ : Fin 17) (hc1D : c₁ ∈ D) (hc2D : c₂ ∈ D) (hc12 : G.Adj c₁ c₂)
    (hcov : ∀ p q : Fin 17, p ∈ D → q ∈ D → G.Adj p q →
      p = c₁ ∨ p = c₂ ∨ q = c₁ ∨ q = c₂)
    (hge : 8 ≤ ∑ v ∈ D, (G.neighborFinset v ∩ D).card)
    (hindle : ∀ x : Fin 17, x ∈ D → (G.neighborFinset x ∩ D).card ≤ 3)
    (k t₁ t₂ : Fin 17) (hkge4 : 4 ≤ G.degree k) (hkle5 : G.degree k ≤ 5) (ht12 : t₁ ≠ t₂)
    (ht1deg : G.degree t₁ = 3) (ht2deg : G.degree t₂ = 3)
  ...
  · exact claw_shared_two_twin_le5_seventeen G D hmemD hT hC4 c₁ t₁ t₂ k hc1D hc1 ht1deg ht2deg
      hkge4 hkle5 ht12 hAt1k hAt2k ht1iso ht2iso
  · have hc2 : 3 ≤ (G.neighborFinset c₂ ∩ D).card := by omega
    exact claw_shared_two_twin_le5_seventeen G D hmemD hT hC4 c₂ t₁ t₂ k hc2D hc2 ht1deg ht2deg
      hkge4 hkle5 ht12 hAt1k hAt2k ht1iso ht2iso

/-- **Shared deg-`≤ 5` hub in the `e(M) = 5`, `|D| = 11`, `|Hub| = 6` regime (`n = 17`).**  The
...
theorem shared_hub_le5_eM5_seventeen (G : SimpleGraph (Fin 17))
...
/-- **An induced `C₅` accounts for all of `M` at `e(M) = 5` (`n = 17`).**  If `v₁–⋯–v₅` is an
...
theorem nonIso_le_five_of_C5_seventeen (G : SimpleGraph (Fin 17)) (D : Finset (Fin 17))
    (v₁ v₂ v₃ v₄ v₅ : Fin 17)
    (hv1D : v₁ ∈ D) (hv2D : v₂ ∈ D) (hv3D : v₃ ∈ D) (hv4D : v₄ ∈ D) (hv5D : v₅ ∈ D)
    (hcard5 : ({v₁, v₂, v₃, v₄, v₅} : Finset (Fin 17)).card = 5)
  ...
  have two_nbrs : ∀ a x y : Fin 17, G.Adj a x → G.Adj a y → x ∈ D → y ∈ D → x ≠ y →
      2 ≤ (G.neighborFinset a ∩ D).card := by
    intro a x y hax hay hxD hyD hxy
    have hsub : ({x, y} : Finset (Fin 17)) ⊆ G.neighborFinset a ∩ D := by
    ...
    have hcard2 : ({x, y} : Finset (Fin 17)).card = 2 := by
      rw [Finset.card_insert_of_notMem (by simp [hxy]), Finset.card_singleton]
    calc 2 = ({x, y} : Finset (Fin 17)).card := hcard2.symm
...
/-- **Refined `e(M) = 5` handshake count (`|Hub| = 6`, `n = 17`).**  With at most one degree-`6` hub
...
`< ` of `shared_hub_le5_eM5_seventeen` to leave room for deleting one hub. -/
theorem hub5_iso_count_seventeen (G : SimpleGraph (Fin 17))
    (D Hub Hub5 H6 Iso : Finset (Fin 17))
    (hmemD : ∀ v : Fin 17, v ∈ D ↔ G.degree v = 3)
    (hmemHub : ∀ v : Fin 17, v ∈ Hub ↔ 4 ≤ G.degree v)
    (hmemHub5 : ∀ v : Fin 17, v ∈ Hub5 ↔ 4 ≤ G.degree v ∧ G.degree v ≤ 5)
    (hmemH6 : ∀ v : Fin 17, v ∈ H6 ↔ 6 ≤ G.degree v)
  ...
  have hHub5sub : ∀ v ∈ Hub5, 4 ≤ G.degree v := fun v hv => ((hmemHub5 v).mp hv).1
  have hH6ge : ∀ v ∈ H6, 6 ≤ G.degree v := fun v hv => (hmemH6 v).mp hv
  have hmemIso : ∀ v : Fin 17, v ∈ Iso ↔ v ∈ D ∧ (G.neighborFinset v ∩ D).card = 0 := by
...
/-- **A second deg-`≤ 5` shared hub avoiding a given hub `k` (`e(M) = 5`, `|Hub| = 6`, `n = 17`).**
...
`hub5_iso_count_seventeen` (`|Hub₅| + 2 ≤ ∑_{Iso}|N ∩ Hub₅|`) and the bound that `k` absorbs at most
two `Iso`-incidences (its other three neighbours `a, b, c` are non-isolated) keep the pigeonhole of
`shared_hub_le5_from_count_seventeen` strict on `Hub₅ \ {k}`. -/
theorem exists_nonk_two_twin_hub_seventeen (G : SimpleGraph (Fin 17))
    (hm : G.edgeFinset.card = 30) (h3 : ∀ v : Fin 17, 3 ≤ G.degree v)
    ...
    (e12 : G.Adj v₁ v₂) (e23 : G.Adj v₂ v₃) (e34 : G.Adj v₃ v₄)
    (e45 : G.Adj v₄ v₅) (e51 : G.Adj v₅ v₁)
    (hsum10 : ∑ v ∈ Finset.univ.filter (fun w : Fin 17 => G.degree w = 3),
      (G.neighborFinset v ∩ Finset.univ.filter (fun w : Fin 17 => G.degree w = 3)).card = 10)
    (k a b c : Fin 17) (hk4 : 4 ≤ G.degree k) (hk5 : G.degree k ≤ 5)
    ...
    ∃ h tt₁ tt₂ : Fin 17, h ≠ k ∧ 4 ≤ G.degree h ∧ G.degree h ≤ 5 ∧ tt₁ ≠ tt₂ ∧
      G.degree tt₁ = 3 ∧ G.degree tt₂ = 3 ∧ G.Adj tt₁ h ∧ G.Adj tt₂ h ∧
      (∀ w : Fin 17, G.Adj tt₁ w → G.degree w ≠ 3) ∧
      (∀ w : Fin 17, G.Adj tt₂ w → G.degree w ≠ 3) := by
  classical
  set D : Finset (Fin 17) := Finset.univ.filter (fun w => G.degree w = 3) with hDdef
  have hmemD : ∀ v : Fin 17, v ∈ D ↔ G.degree v = 3 := by intro v; rw [hDdef]; simp
  set Hub : Finset (Fin 17) := Finset.univ.filter (fun v => 4 ≤ G.degree v) with hHubdef
  ...
  have hIsoge : D.card - 5 ≤ Iso.card := by
    have hnon5 := nonIso_le_five_of_C5_seventeen G D v₁ v₂ v₃ v₄ v₅
  ...
  have hcount2 : Hub5.card + 2 ≤ ∑ v ∈ Iso, (G.neighborFinset v ∩ Hub5).card :=
    hub5_iso_count_seventeen G D Hub Hub5 H6 Iso hmemD hmemHub hmemHub5 hmemH6 hIsodef hm h3
      hDge9 hIsoge
  ...
  set S : Finset (Fin 17) := Iso.filter (fun v => G.Adj v k) with hSdef
  ...
  -- Pigeonhole on `Hub₅ \ {k}` via `shared_hub_le5_from_count_seventeen`.
  obtain ⟨h, tt₁, tt₂, hhErase, hh5, ht12, ht1deg, ht2deg, hAt1h, hAt2h, ht1iso, ht2iso⟩ :=
    shared_hub_le5_from_count_seventeen G Iso (Hub5.erase k) hIsoiso
...
/-- **`e(M) = 5` (`s = 10`) two-twin cut for the `|D| = 11`, `|Hub| = 6` corner (`n = 17`).**  Unlike
`n = 16`, the degree sum `60` is divisible by `3`, so the *degree-`4`* shared-hub pigeonhole only
...
branch through `c5_shared_two_twin_le5_seventeen` (`TwinCert17DenseLe5`).  The former residual — a
...
extraction `exists_nonk_two_twin_hub_seventeen` threaded as `hnonk`. -/
theorem two_twin_eM5_seventeen (G : SimpleGraph (Fin 17))
    (hm : G.edgeFinset.card = 30) (h3 : ∀ v : Fin 17, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 17, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 17, ({a, b, c, d} : Finset (Fin 17)).card = 4 ∧
  ...
  -- `|D| = 11` from `|Hub| = |Dᶜ| = 6`.
  have hDc : Dᶜ = Finset.univ.filter (fun v : Fin 17 => 4 ≤ G.degree v) := by
    ext w
  ...
  have hD11 : D.card = 11 := by rw [hDc] at hcc; omega
  ...
  · -- **Induced-`C₅` branch with a deg-`≤ 5` shared hub.**  Extract a shared deg-`≤ 5` hub and
    -- route through `c5_shared_two_twin_le5_seventeen`; the deg-`5` 3-consecutive-hit residual is
    -- closed by `exists_nonk_two_twin_hub_seventeen` supplying a second hub `h ≠ k`.
    ...
    exact c5_shared_two_twin_le5_seventeen G hT hC4 k tw1 tw2 hkge4 hkle5 htw12 htw1deg htw2deg
      ...
      (fun a b c hda hdb hdc hka hkb hkc hab hbc hab_ne hbc_ne hac_ne =>
        exists_nonk_two_twin_hub_seventeen G hm h3 (by rw [← hDdef]; omega)
