...
import ACMaxConjecture.SmallCases.N17.TwoHubDeg5
import ACMaxConjecture.SmallCases.N17.TwoHubDeg6
...
import ACMaxConjecture.SmallCases.N17.Align8
import ACMaxConjecture.SmallCases.N17.CherryP3
import ACMaxConjecture.SmallCases.N17.CherryP4

/-!
# Existence of a twin signed-cut certificate for `n = 17` (structural assembly skeleton)

This file assembles the `n = 17` twin signed-cut certificate from the proved boundary certificates
(`TwinCert17Cert`) and the structural-alignment dichotomies, mirroring the proved `n = 16`
`exists_twin_signed_cert_sixteen` (`ACMaxConjecture/TwinCert16.lean`).  It is specialised to
`Fin 17`, `edgeFinset.card = 30`, the residual good-`C₄` threshold `≤ 14` and the residual
good-`K_{2,3}` threshold `≤ 19` (the bounds the `Fintype.card`-keyed `algConn_le_two_of_good_C4` /
`algConn_le_two_of_good_K23` deliver for `Fintype.card V = 17`).

In the sparse-hub residual (`δ ≥ 3`, no good triangle, no induced `2K₂` on degree-`3` vertices, no
good `C₄` of degree-sum `≤ 14`, no good `K_{2,3}` of degree-sum `≤ 19`, with an `M`-isolated
degree-`3` vertex) one produces an explicit signed cut `P, N` with
`4·e(P,N) + e(P,Z) + e(N,Z) ≤ 4·|P|`, fed to `algConn_le_two_of_signed`.
...
certificate (mirroring `n = 16`, every non-trivial branch is a **four-way** dichotomy
`SingleVertexConfig ∨ TwoTwinConfig ∨ TwoHubConfig ∨ HubTriangleConfig`).

Three of the four alignment dichotomies (`two_hub_config_seventeen`,
`exists_align_four_config_seventeen`, `exists_align_six_config_seventeen`) are now landed except for
one precisely-named residual corner each (the degree-`6`-hub `|D| = 10` two-hub corner, and the
`P₃`/`P₄`-cherry `|D| ∈ {9, 10, 11}` corners — see each lemma); `halign8_seventeen` (the dense
`e(M) ≥ 4` regime) remains a documented stub.  The dispatch and the assembly are sorry-free.

`n = 17` cardinality specifics: `|Hub| ∈ {6, 7, 8, 9}` (`|Hub| = 9` is the all-degree-`4` regime
new to `n = 17`) and `|D| ∈ {8, 9, 10, 11}`; residual thresholds `hC4 ≤ 14`, `hK23 ≤ 19`.
-/
...
/-- **Two-hub opposite-twin selection for `n = 17`, `e(M) ≤ 1` (LANDED modulo one corner).**
In the `e(M) ≤ 1` regime (`s ≤ 2`) there is no degree-`3` cherry through two hubs, so the two-hub
opposite-twin cut is available.  Ported from `two_hub_config_sixteen`: `|D| ∈ {8, 9, 10}` from
`6·|D| ≤ 60 + s`; `|D| = 8` (`|Hub| = 9`, all degree-`4`) is wired through the general selector
`two_hub_corner_select_seventeen` (regime `(9, 8, 36)` / `(9, 6, 36)`, the latter supplying
`h3`/`hisodeg`/`hNI ≤ 4`/`hT`), and `|D| ∈ {9, 10}` through the degree-`5`-aware
`two_hub_corner_select_deg5_seventeen`.  **Residual corner (one `sorry`):** `|D| = 10`,
`|Hub| = 7` with a single degree-`6` hub (excess `2`), where `hdeg5` fails and a degree-`6`-aware
selector is not yet available. -/
theorem two_hub_config_seventeen (G : SimpleGraph (Fin 17))
  ...
  have hHubnotD : ∀ x : Fin 17, x ∈ Hub → x ∉ D := by
    intro x hx hxD; have := (hmemHub x).mp hx; have := (hmemD x).mp hxD; omega
  set Iso : Finset (Fin 17) := D.filter (fun x => (G.neighborFinset x ∩ D).card = 0) with hIsodef
  have hIsomem : ∀ x : Fin 17, x ∈ Iso ↔ x ∈ D ∧ (G.neighborFinset x ∩ D).card = 0 := by
    intro x; rw [hIsodef, Finset.mem_filter]
  have hisoD : ∀ x : Fin 17, x ∈ Iso → x ∈ D := fun x hx => ((hIsomem x).mp hx).1
  have hisoNoD : ∀ x y : Fin 17, x ∈ Iso → y ∈ D → ¬G.Adj x y := by
  ...
  have hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 17, G.Adj v w → G.degree w ≠ 3) := by
    intro v hv
    refine ⟨(hmemD v).mp (hisoD v hv), fun w hadj hw3 => ?_⟩
    exact hisoNoD v w hv ((hmemD w).mpr hw3) hadj
  have hDH : ∀ v : Fin 17, v ∈ D ∨ v ∈ Hub := fun v => by
  ...
  have hsumpart : ∑ v ∈ D, G.degree v + ∑ v ∈ Hub, G.degree v = 60 := by
  ...
  have hIntEq : (∑ w ∈ Hub, (G.neighborFinset w ∩ Hub).card) + 6 * D.card = 60 + s := by
    have h1 := hsumpart; rw [hsumD] at h1; omega
  have hD8 : 8 ≤ D.card := by
    have h := (residual_hub_card_le_nine G hm h3).2; rwa [← hDdef] at h
  have heven : Even s := by rw [hsdef]; exact eM_even G D
  have hle2 : s ≤ 2 := by rw [hsdef, hDdef]; exact hle
  have hd : D.card = 8 ∨ D.card = 9 ∨ D.card = 10 := by
  ...
  have hshare4 : ∀ p ∈ Hub, G.degree p = 4 → ∀ q ∈ Hub, G.degree q = 4 → p ≠ q →
    ...
    have unpack : ∀ t : Fin 17, t ∈ G.neighborFinset p ∩ G.neighborFinset q ∩ Iso →
    ...
    have hpne : ∀ t : Fin 17, t ∈ Iso → p ≠ t ∧ q ≠ t := by
      intro t ht
  ...
  have hIsocard : Iso.card = D.card - s := by
    set S : Finset (Fin 17) := D.filter (fun v => ¬ (G.neighborFinset v ∩ D).card = 0) with hSdef
    ...
    · have hSle : 2 * S.card ≤ s + 2 := by
      ...
      have hSge : 2 ≤ S.card := by
        have hsub : ({v0, w0} : Finset (Fin 17)) ⊆ S := by
          intro x hx
          rw [Finset.mem_insert, Finset.mem_singleton] at hx
          rcases hx with h | h
          · rw [h]; exact hv0S
          · rw [h]; exact hw0S
        calc 2 = ({v0, w0} : Finset (Fin 17)).card := by rw [Finset.card_pair hvw0ne]
          _ ≤ S.card := Finset.card_le_card hsub
      omega
  -- **Hub-pair selection.**
  have hpair : ∃ h₁ h₂ : Fin 17, h₁ ∈ Hub ∧ h₂ ∈ Hub ∧
      G.degree h₁ = 4 ∧ G.degree h₂ = 4 ∧ h₁ ≠ h₂ ∧ ¬G.Adj h₁ h₂ ∧
      2 ≤ ((G.neighborFinset h₁ ∩ Iso) \ G.neighborFinset h₂).card ∧
      2 ≤ ((G.neighborFinset h₂ ∩ Iso) \ G.neighborFinset h₁).card := by
    -- `hdeg5` (degree `≤ 5`) holds whenever the hub-degree excess is `≤ 1`, i.e. `|D| ∈ {8, 9}`.
    have hdeg5_of : Hub.card * 4 + 1 ≥ ∑ w ∈ Hub, G.degree w →
        ∀ h ∈ Hub, G.degree h ≤ 5 := by
      intro hexc h hh
      ...
      have hpos : 1 ≤ Hub.card := Finset.card_pos.mpr ⟨h, hh⟩
      omega
    have hsumHub : ∑ w ∈ Hub, G.degree w = 60 - 3 * D.card := by omega
    rcases hd with hd8 | hd9 | hd10
    · -- **`|D| = 8 ⟹ |Hub| = 9`, all degree `4` (general selector, regimes `5`/`6`).**
      have hHub9 : Hub.card = 9 := by omega
      have hsum36 : ∑ w ∈ Hub, G.degree w = 36 := by rw [hsumHub, hd8]
      ...
      have hdeg4Dc : ∀ w : Fin 17, w ∈ Dᶜ → G.degree w = 4 := fun w hw =>
      ...
      · have hIso8 : Iso.card = 8 := by rw [hIsocard, hd8, hs0]
        exact two_hub_corner_select_seventeen G Hub Iso hdeg hdeg5 hiso3 hisoIndep hshare hdisjHI
          (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl ⟨hHub9, hIso8, hsum36⟩)))))
      · have hIso6 : Iso.card = 6 := by rw [hIsocard, hd8, hs2]
        -- Regime `(9, 6, 36)`: supply `h3`, `hisodeg`, `hNI`, `hT`.
        have hisodeg : ∀ t ∈ Iso, G.degree t = 3 := fun t ht => (hIsoprop t ht).1
        have hNI : ∑ h ∈ Hub, (G.neighborFinset h ∩ (Finset.univ \ (Hub ∪ Iso))).card ≤ 4 := by
          set X : Finset (Fin 17) := Finset.univ \ (Hub ∪ Iso) with hXdef
          ...
          have hXcard : X.card = 2 := by
            ...
            have hIso6 : Iso.card = 6 := by rw [hIsocard, hd8, hs2]
            rw [hXeq, Finset.card_sdiff, Finset.inter_eq_left.mpr hsub]; omega
        ...
        exact two_hub_corner_select_seventeen G Hub Iso hdeg hdeg5 hiso3 hisoIndep hshare hdisjHI
          (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr
            ⟨hHub9, hIso6, hsum36, h3, hisodeg, hNI, hT⟩)))))
    · -- **`|D| = 9 ⟹ |Hub| = 8` (one degree-`5` hub): the deg-`5`-aware selector.**
      have hHub8 : Hub.card = 8 := by omega
      have hsum33 : ∑ w ∈ Hub, G.degree w = 33 := by rw [hsumHub, hd9]
      have hdeg5 : ∀ h ∈ Hub, G.degree h ≤ 5 := hdeg5_of (by omega)
      rcases (by obtain ⟨k, hk⟩ := heven; omega : s = 0 ∨ s = 2) with hs0 | hs2
      · have hIso9 : Iso.card = 9 := by rw [hIsocard, hd9, hs0]
        exact two_hub_corner_select_deg5_seventeen G Hub Iso hdeg hdeg5 hiso3 hshare4 hdisjHI
          (Or.inr (Or.inr (Or.inl ⟨hHub8, hIso9, hsum33⟩)))
      · have hIso7 : Iso.card = 7 := by rw [hIsocard, hd9, hs2]
        exact two_hub_corner_select_deg5_seventeen G Hub Iso hdeg hdeg5 hiso3 hshare4 hdisjHI
          (Or.inr (Or.inr (Or.inr ⟨hHub8, hIso7, hsum33⟩)))
    · -- **`|D| = 10 ⟹ |Hub| = 7` (excess `2`).**  If all hubs are degree `≤ 5` the deg-`5`-aware
      -- selector applies; the one-degree-`6`-hub sub-corner is isolated below.
      have hHub7 : Hub.card = 7 := by omega
      have hsum30 : ∑ w ∈ Hub, G.degree w = 30 := by rw [hsumHub, hd10]
      by_cases hdeg5 : ∀ h ∈ Hub, G.degree h ≤ 5
      · rcases (by obtain ⟨k, hk⟩ := heven; omega : s = 0 ∨ s = 2) with hs0 | hs2
        · have hIso10 : Iso.card = 10 := by rw [hIsocard, hd10, hs0]
          exact two_hub_corner_select_deg5_seventeen G Hub Iso hdeg hdeg5 hiso3 hshare4 hdisjHI
            (Or.inl ⟨hHub7, hIso10, hsum30⟩)
        · have hIso8 : Iso.card = 8 := by rw [hIsocard, hd10, hs2]
          exact two_hub_corner_select_deg5_seventeen G Hub Iso hdeg hdeg5 hiso3 hshare4 hdisjHI
            (Or.inr (Or.inl ⟨hHub7, hIso8, hsum30⟩))
      · -- **Residual corner: `|D| = 10`, `|Hub| = 7`, one degree-`6` hub.**  The
        -- degree-`6`-aware selector `two_hub_deg6_select_seventeen` routes around the degree-`6`
        -- hub via the degree-`4`-restricted share, closing both `|Iso| ∈ {8, 10}` sub-corners.
        ...
        rcases (by obtain ⟨k, hk⟩ := heven; omega : s = 0 ∨ s = 2) with hs0 | hs2
        · have hIso10 : Iso.card = 10 := by rw [hIsocard, hd10, hs0]
          exact two_hub_deg6_select_seventeen G Hub Iso hdeg hdeg6 hiso3 hshare4 hdisjHI
            (Or.inl ⟨hHub7, hIso10, hsum30⟩)
        · have hIso8 : Iso.card = 8 := by rw [hIsocard, hd10, hs2]
          exact two_hub_deg6_select_seventeen G Hub Iso hdeg hdeg6 hiso3 hshare4 hdisjHI
            (Or.inr ⟨hHub7, hIso8, hsum30⟩)
  obtain ⟨h₁, h₂, hh1, hh2, hdeg1, hdeg2, hne12, hnadj12, hAcard, hBcard⟩ := hpair
  set A : Finset (Fin 17) := (G.neighborFinset h₁ ∩ Iso) \ G.neighborFinset h₂ with hAdef
  set B : Finset (Fin 17) := (G.neighborFinset h₂ ∩ Iso) \ G.neighborFinset h₁ with hBdef
  obtain ⟨a, ha, b, hb, hab⟩ := Finset.one_lt_card.mp hAcard
  obtain ⟨c, hc, d, hd2, hcd⟩ := Finset.one_lt_card.mp hBcard
  have hAprop : ∀ x : Fin 17, x ∈ A → G.Adj x h₁ ∧ x ∈ Iso ∧ ¬G.Adj x h₂ := by
    intro x hx
  ...
  have hBprop : ∀ x : Fin 17, x ∈ B → G.Adj x h₂ ∧ x ∈ Iso ∧ ¬G.Adj x h₁ := by
    intro x hx
...
/-- **Four-way alignment dichotomy for `n = 17`, `e(M) = 2` (LANDED modulo one corner).**
At `s = 4` the matching `M` is a single `P₃` cherry `x–y–z` (centre `y`); the covering combination
is `SingleVertexConfig ∨ TwoTwinConfig ∨ TwoHubConfig ∨ HubTriangleConfig`.  Ported from
`exists_align_four_config_sixteen`.  **Landed:** the induced-`C₅` branch is impossible at `e(M) = 2`
(each cycle vertex carries `≥ 2` `D`-neighbours, forcing `s ≥ 10`); the dominating-edge branch
rebuilds the cherry and, for `|D| = 8` (`|Hub| = 9`, all degree-`4`), dispatches to
`exists_hub_triangle_config_cherry_residual_seventeen` after the three signed-cut `by_cases`.
**Landed (`sorry`-free):** the `P₃`-cherry case for `|D| ∈ {9, 10, 11}` is dispatched to the proved
`TwinCert17CherryP3` corners (`cherry_p3_config_D9_seventeen` two-twin for `|D| = 9`;
`cherry_p3_two_hub_D10_seventeen` for `|D| = 10`; `cherry_p3_D11_vacuous_seventeen` for `|D| = 11`),
the cross-count bounding `|D| ≤ 11`. -/
theorem exists_align_four_config_seventeen (G : SimpleGraph (Fin 17))
    (hm : G.edgeFinset.card = 30) (h3 : ∀ v : Fin 17, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 17, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 17, ({a, b, c, d} : Finset (Fin 17)).card = 4 ∧
  ...
  obtain ⟨_hHub9, _hD8le⟩ := residual_hub_card_le_nine G hm h3
  ...
  · -- **Dominating-edge branch.**  Rebuild the single `e(M) = 2` cherry `x–y–z` (centre `y`), the
    -- `M`-isolated set `Iso` and the `y`-centred covering data, then split on `|D|`.
    ...
    have hcen : 2 ≤ (G.neighborFinset c₁ ∩ D).card ∨ 2 ≤ (G.neighborFinset c₂ ∩ D).card := by
      ...
      have hrhs : ∑ v ∈ D, (if v = c₁ ∨ v = c₂ then (1 : ℕ) else 0) ≤ 2 := by
        ...
        have hsubset : D.filter (fun v => v = c₁ ∨ v = c₂) ⊆ ({c₁, c₂} : Finset (Fin 17)) := by
          intro v hv
        ...
        calc (D.filter (fun v => v = c₁ ∨ v = c₂)).card
            ≤ ({c₁, c₂} : Finset (Fin 17)).card := Finset.card_le_card hsubset
          _ ≤ 2 := by
              have := Finset.card_insert_le c₁ ({c₂} : Finset (Fin 17))
              simp only [Finset.card_singleton] at this
              omega
      omega
    have mkcherry : ∀ c : Fin 17, c ∈ D → 2 ≤ (G.neighborFinset c ∩ D).card →
        ∃ x y z : Fin 17, x ∈ D ∧ y ∈ D ∧ z ∈ D ∧ x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
    ...
    have hDsplit : ∀ w : Fin 17, w ∈ D → w ∉ ({x, y, z} : Finset (Fin 17)) →
      ...
      rw [← hNIeq] at hmem
      exact hwxyz hmem
    set Iso : Finset (Fin 17) := D.filter (fun v => (G.neighborFinset v ∩ D).card = 0)
      with hIsodef
    have hIsoprop : ∀ v ∈ Iso, G.degree v = 3 ∧ (∀ w : Fin 17, G.Adj v w → G.degree w ≠ 3) := by
      intro v hv
    ...
    have hcov : ∀ p q : Fin 17, p ∈ D → q ∈ D → G.Adj p q → p = y ∨ q = y := by
      ...
      · rcases hqNI with rfl | rfl | rfl
        ...
        · exact Or.inr rfl
        · exact (G.irrefl hpq).elim
    have hNyD : G.neighborFinset y ∩ D = ({x, z} : Finset (Fin 17)) := by
      apply Finset.Subset.antisymm
      · intro w hw
        rw [Finset.mem_inter, G.mem_neighborFinset] at hw
        obtain ⟨hyw, hwD⟩ := hw
        have hwNI : w ∈ ({x, y, z} : Finset (Fin 17)) := by
          rw [hNIeq, Finset.mem_filter]
    ...
    · -- **`|D| = 8` (`|Hub| = 9`, all degree `4`): cherry hub-triangle residual.**
      have hdeg4all : ∀ w : Fin 17, w ∈ Dᶜ → G.degree w = 4 := by
        intro w hw
        ...
        have hsum36 : ∑ v ∈ Dᶜ, G.degree v = 36 := by rw [hsumDcdeg, hD8eq]
      ...
      · exact Or.inr (Or.inr (Or.inr
          (exists_hub_triangle_config_cherry_residual_seventeen G D Iso x y z hmemD hIsodef
            hIsoprop hisochar hcov hNyD hdegx hdegy hdegz hxyA hyzA hxzN hxz_ne hdeg4all
            hD8eq hC4 hsv htt hth)))
    · -- **`P₃`-cherry corner for `|D| ∈ {9, 10, 11}` (`|Hub| ∈ {6, 7, 8}`).**  The single `e(M) = 2`
      -- cherry `x–y–z` is centred at `y`; with `|D| ≥ 9` the residual splits three ways and
      -- dispatches to the proved `TwinCert17CherryP3` corners (`|D| = 9` two-twin, `|D| = 10`
      -- two-hub, `|D| = 11` vacuous).  The cross-count bounds `|D| ≤ 11`.
      ...
      · exact cherry_p3_config_D9_seventeen G hm h3 hT h2k2 hC4 hK23 D Iso x y z hmemD hIsoprop
          hisochar hcov hNyD hdegx hdegy hdegz hxyA hyzA hxzN hxy_ne hyz_ne hxz_ne h9
      by_cases h10 : D.card = 10
      · exact cherry_p3_two_hub_D10_seventeen G hm h3 hT h2k2 hC4 hK23 D Iso x y z hmemD hIsoprop
          hisochar hcov hNyD hdegx hdegy hdegz hxyA hyzA hxzN hxy_ne hyz_ne hxz_ne h10
      by_cases h11 : D.card = 11
      · exact cherry_p3_D11_vacuous_seventeen G hm D x y z hmemD hcov hxD hyD hzD hxyA hyzA
          hxy_ne hyz_ne hxz_ne hNyD h11
      · exfalso; have hlb : 8 ≤ D.card := _hD8le; omega
...
/-- **Four-way alignment dichotomy for `n = 17`, `e(M) = 3` (LANDED modulo one corner).**
At `s = 6` the covering combination is
`SingleVertexConfig ∨ TwoTwinConfig ∨ TwoHubConfig ∨ HubTriangleConfig`.  Ported from
`exists_align_six_config_sixteen`.  **Landed:** the shared degree-`4` hub of two `M`-isolated twins
is available (`s = 6 ≤ 6`, via `shared_deg4_hub_deg5_seventeen`/`_nodeg5_seventeen`); a fat
dominating-edge centre (in-`M`-degree `≥ 3`) dispatches to `claw_shared_two_twin_seventeen`; the
induced-`C₅` branch is impossible at `e(M) = 3`; the `P₄` `|D| = 8` (`|Hub| = 9`) all-degree-`4`
corner dispatches to `exists_hub_triangle_config_residual_seventeen`.  **Landed:** the `P₄` case for
`|D| ∈ {9, 10, 11}` is dispatched to the proved `TwinCert17CherryP4` corners
(`cherry_p4_config_D9_seventeen`; `cherry_p4_two_hub_D10_seventeen`; `cherry_p4_hub6_D11_seventeen`),
the cross-count bounding `|D| ≤ 11` (modulo the single downstream `CherryP4BothLeaf` `sorry`). -/
theorem exists_align_six_config_seventeen (G : SimpleGraph (Fin 17))
    (hm : G.edgeFinset.card = 30) (h3 : ∀ v : Fin 17, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 17, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 17, ({a, b, c, d} : Finset (Fin 17)).card = 4 ∧
  ...
  have hindle : ∀ x : Fin 17, x ∈ D → (G.neighborFinset x ∩ D).card ≤ 3 := by
    ...
    calc (G.neighborFinset x ∩ D).card ≤ (G.neighborFinset x).card :=
      ...
      _ = 3 := (hmemD x).mp hx
  obtain ⟨_hHub9, hD8le⟩ := residual_hub_card_le_nine G hm h3
  rw [← hDdef] at hD8le
  have hs6le : ∑ v ∈ D, (G.neighborFinset v ∩ D).card ≤ 6 := by omega
  -- A shared degree-`4` hub of two `M`-isolated twins (available since `s = 6 ≤ 6`).
  have hshare : ∃ h t₁ t₂ : Fin 17, G.degree h = 4 ∧ t₁ ≠ t₂ ∧
      G.degree t₁ = 3 ∧ G.degree t₂ = 3 ∧ G.Adj t₁ h ∧ G.Adj t₂ h ∧
      (∀ w : Fin 17, G.Adj t₁ w → G.degree w ≠ 3) ∧
      (∀ w : Fin 17, G.Adj t₂ w → G.degree w ≠ 3) := by
    by_cases hg5 : ∃ g : Fin 17, 5 ≤ G.degree g
    · obtain ⟨g, hg5'⟩ := hg5
      exact shared_deg4_hub_deg5_seventeen G hm h3 hT hC4 h2k2 hs6le g hg5'
    · push Not at hg5
      exact shared_deg4_hub_nodeg5_seventeen G hm h3 h2k2 (fun v => by have := hg5 v; omega) hs6le
  ...
  · -- **Dominating-edge branch.**  A fat centre (in-`M`-degree `3`) lands `TwoTwinConfig` via
    -- `claw_shared_two_twin_seventeen`; the `P₄` case splits on `|D|`.
    obtain ⟨c₁, c₂, hc1D, hc2D, hc12, hcov⟩ := hdom
    by_cases hc1three : 3 ≤ (G.neighborFinset c₁ ∩ D).card
    · exact Or.inr (Or.inl (claw_shared_two_twin_seventeen G D hmemD hT hC4 c₁ t₁ t₂ k hc1D
        hc1three ht1deg ht2deg hkdeg4 ht12 hAt1k hAt2k ht1iso ht2iso))
    · by_cases hc2three : 3 ≤ (G.neighborFinset c₂ ∩ D).card
      · exact Or.inr (Or.inl (claw_shared_two_twin_seventeen G D hmemD hT hC4 c₂ t₁ t₂ k hc2D
      ...
      · -- **`P₄` case.**  Both endpoints have in-`M`-degree `2`; the degree-`3` subgraph is the
        ...
        set Iso : Finset (Fin 17) := D.filter (fun v => (G.neighborFinset v ∩ D).card = 0)
          with hIsodef
        have hIsoprop : ∀ v ∈ Iso,
            G.degree v = 3 ∧ (∀ w : Fin 17, G.Adj v w → G.degree w ≠ 3) := by
          intro v hv
          rw [hIsodef, Finset.mem_filter] at hv
          obtain ⟨hvD, hv0⟩ := hv
          refine ⟨hdegD v hvD, fun w hadj hw3 => ?_⟩
        ...
        have hform := thin_eM_formula_seventeen G D c₁ c₂ hc1D hc2D hc12 hcov
        rw [hs6] at hform
        ...
        have hisochar : ∀ w : Fin 17, w ∈ D → w ≠ L₁ → w ≠ c₁ → w ≠ c₂ → w ≠ L₂ → w ∈ Iso := by
          ...
          · have hmem : w ∈ G.neighborFinset c₂ ∩ D :=
            ...
            · exact hwL2 h'
        by_cases hD8eq : D.card = 8
        · -- **`|D| = 8` (`|Hub| = 9`) corner.**  All nine hubs have degree `4`; the residual
          ...
          have hdeg4all : ∀ w : Fin 17, w ∈ Dᶜ → G.degree w = 4 := by
            ...
            rw [hsum36] at hspl
            omega
          have hK23' : ¬∃ a b c d e : Fin 17, ({a, b, c, d, e} : Finset (Fin 17)).card = 5 ∧
              G.Adj a c ∧ G.Adj a d ∧ G.Adj a e ∧ G.Adj b c ∧ G.Adj b d ∧ G.Adj b e ∧
              ¬G.Adj a b ∧ ¬G.Adj c d ∧ ¬G.Adj c e ∧ ¬G.Adj d e ∧
          ...
          by_cases hth : TwoHubConfig G
          · exact Or.inr (Or.inr (Or.inl hth))
          · exact Or.inr (Or.inr (Or.inr (exists_hub_triangle_config_residual_seventeen G D Iso
        ...
        · -- **`P₄` corner for `|D| ∈ {9, 10, 11}` (`|Hub| ∈ {6, 7, 8}`).**  Both `P₄` endpoints
          -- `c₁, c₂` have in-`M`-degree `2`, so neither the fat-centre claw nor the double-star
          -- single-vertex selection applies.  Dispatch to the proved `TwinCert17CherryP4` corners
          -- (`|D| = 9` config, `|D| = 10` two-hub, `|D| = 11` hub-`6`); the cross-count bounds
          -- `|D| ≤ 11`.
          have hDub : D.card ≤ 11 := by
            have hdsplit : ∀ v : Fin 17,
                (G.neighborFinset v ∩ D).card + (G.neighborFinset v ∩ Dᶜ).card = G.degree v := by
            ...
            omega
          by_cases h9 : D.card = 9
          · exact cherry_p4_config_D9_seventeen G hm h3 hT hC4 L₁ c₁ c₂ L₂ D Iso hmemD hIsoprop
              hisochar hL1deg hc1deg hc2deg hL2deg hac1L1 hc12 hac2L2 hL1nc2 hL2nc1 hNc1D hNc2D
              hin1 hin2 hs6 h9
          by_cases h10 : D.card = 10
          · exact cherry_p4_two_hub_D10_seventeen G hm h3 hT hC4 L₁ c₁ c₂ L₂ D Iso hmemD hIsoprop
              hisochar hL1deg hc1deg hc2deg hL2deg hac1L1 hc12 hac2L2 hL1nc2 hL2nc1 hNc1D hNc2D
              hin1 hin2 hs6 h10
          by_cases h11 : D.card = 11
          · exact cherry_p4_hub6_D11_seventeen G hm h3 hT hC4 L₁ c₁ c₂ L₂ D Iso hmemD hIsoprop
              hisochar hL1deg hc1deg hc2deg hL2deg hac1L1 hc12 hac2L2 hL1nc2 hL2nc1 hNc1D hNc2D
              hin1 hin2 hs6 h11
          · exfalso; omega
...
/-- **Four-way alignment dichotomy for `n = 17`, `e(M) ≥ 4`.**
At `s ∈ {8, 10}` the matching `M` is dense; the covering combination is
`SingleVertexConfig ∨ TwoTwinConfig ∨ TwoHubConfig ∨ HubTriangleConfig`.  Dispatch (ported from the
proved `n = 16` `halign8_sixteen`):
  • `|D| = 11` (`|Hub| = 6`): the `|Hub| = 6` selector `two_hub_or_single_vertex_hub6_seventeen`
    (covers both `e(M) = 4` and `e(M) = 5`), reordered into the standard disjunction;
  • `|D| ∈ {8, 9, 10}`, `e(M) = 5` (`s = 10`): `halign8_eM5_seventeen` (`TwinCert17Align8`);
  • `|D| ∈ {8, 9, 10}`, `e(M) = 4` (`s = 8`): `halign8_eM4_seventeen` (`TwinCert17Align8`).
The two `_eM5`/`_eM4` helpers each carry one documented residual `sorry` (the degree-`5`-hub
two-twin route for `|D| ∈ {9, 10}`, and the shared-degree-`4`-hub / `|Hub| = 9` hub-triangle cut at
`e(M) = 4`, respectively — for `n = 17` the shared-hub pigeonhole only *ties* at `e(M) = 4`). -/
theorem halign8_seventeen (G : SimpleGraph (Fin 17))
    (hm : G.edgeFinset.card = 30) (h3 : ∀ v : Fin 17, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 17, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 17, ({a, b, c, d} : Finset (Fin 17)).card = 4 ∧
  ...
  have heven : Even s := by rw [hsdef]; exact eM_even G D
  by_cases hD11 : D.card = 11
  · -- **`|D| = 11` (`|Hub| = 6`).**  The dedicated `|Hub| = 6` selector covers both `e(M) = 4`
    -- and `e(M) = 5`; reorder its disjunction into the standard order.
    have hHub6 : (Finset.univ.filter (fun v : Fin 17 => 4 ≤ G.degree v)).card = 6 := by
      have heq : Finset.univ.filter (fun v : Fin 17 => 4 ≤ G.degree v) = Dᶜ := by
        ext w
        simp only [Finset.mem_filter, Finset.mem_univ, true_and, Finset.mem_compl, hmemD]
        have := h3 w; omega
      rw [heq, Finset.card_compl, Fintype.card_fin, hD11]
    rcases two_hub_or_single_vertex_hub6_seventeen G hm h3 hT h2k2 hC4 hK23 hiso hD11 hHub6 hge with
      hth | hsv | htt | hht
    · exact Or.inr (Or.inr (Or.inl hth))
    · exact Or.inl hsv
    · exact Or.inr (Or.inl htt)
    · exact Or.inr (Or.inr (Or.inr hht))
  · -- **`|D| ∈ {8, 9, 10}`.**  Split on `e(M)` (`s ∈ {8, 10}` by evenness, `hge`, `hle10`).
    by_cases hsum10 : s = 10
    · exact halign8_eM5_seventeen G hm h3 hT h2k2 hC4 hK23 hiso hsum10
    · have hs8 : s = 8 := by obtain ⟨k, hk⟩ := heven; omega
      exact halign8_eM4_seventeen G hm h3 hT h2k2 hC4 hK23 hiso
        (by simpa only [hDdef] using hD11) hs8

/-- **Twin signed-cut existence for `n = 17` (structural assembly skeleton).**  In the sparse-hub
residual (`δ ≥ 3`, no good triangle, no induced `2K₂` on degree-`3` vertices, no good `C₄` of
degree-sum `≤ 14`, no good `K_{2,3}` of degree-sum `≤ 19`, with an `M`-isolated degree-`3` vertex)
there is a signed cut `P, N` with `4·e(P,N) + e(P,Z) + e(N,Z) ≤ 4·|P|`.  The assembly and dispatch
are sorry-free; only the four alignment dichotomies remain as documented `sorry` stubs. -/
theorem exists_twin_signed_cert_seventeen (G : SimpleGraph (Fin 17))
    (hm : G.edgeFinset.card = 30) (h3 : ∀ v : Fin 17, 3 ≤ G.degree v)
    (hT : ¬∃ x y z : Fin 17, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      G.Adj x y ∧ G.Adj y z ∧ G.Adj x z ∧ G.degree x + G.degree y + G.degree z ≤ 10)
    (h2k2 : ¬∃ a b c d : Fin 17, ({a, b, c, d} : Finset (Fin 17)).card = 4 ∧
    ...
    (hK23 : ¬∃ a b c d e : Fin 17, ({a, b, c, d, e} : Finset (Fin 17)).card = 5 ∧
      ...
      G.degree a + G.degree b + G.degree c + G.degree d + G.degree e ≤ 19)
    (hiso : ∃ t : Fin 17, G.degree t = 3 ∧ ∀ w : Fin 17, G.Adj t w → G.degree w ≠ 3) :
    ∃ P N : Finset (Fin 17), Disjoint P N ∧ P.card = N.card ∧ 0 < P.card ∧
  ...
  · -- `e(M) ≤ 1` (`s < 4`, sharpened to `s ≤ 2` by evenness): two-hub opposite-twin cut.
    ...
    exact twoHubConfig_to_cut G (two_hub_config_seventeen G hm h3 hT h2k2 hC4 hK23 hiso hle2)
  · -- `e(M) ≥ 2` (`s ≥ 4`): four-way alignment dichotomy, split by the value of `e(M)`.
    ...
    · -- `e(M) = 2` (`s = 4`).
      ...
      have halign : SingleVertexConfig G ∨ TwoTwinConfig G ∨ TwoHubConfig G ∨
          HubTriangleConfig G :=
        exists_align_four_config_seventeen G hm h3 hT h2k2 hC4 hK23 hiso hs4
    ...
    · by_cases hle6 : s ≤ 6
      · -- `e(M) = 3` (`s = 6`).
        ...
        have halign : SingleVertexConfig G ∨ TwoTwinConfig G ∨ TwoHubConfig G ∨
            HubTriangleConfig G :=
          exists_align_six_config_seventeen G hm h3 hT h2k2 hC4 hK23 hiso hs6
        rcases halign with hsv | htt | hth | hht
        · exact singleVertexConfig_to_cut G hsv
        · exact twoTwinConfig_to_cut G htt
        · exact twoHubConfig_to_cut G hth
      ...
      · -- `e(M) ≥ 4` (`s ≥ 8`).
        ...
        have halign : SingleVertexConfig G ∨ TwoTwinConfig G ∨ TwoHubConfig G ∨
            HubTriangleConfig G :=
          halign8_seventeen G hm h3 hT h2k2 hC4 hK23 hiso hge8
        rcases halign with hsv | htt | hth | hht
        · exact singleVertexConfig_to_cut G hsv
        · exact twoTwinConfig_to_cut G htt
        · exact twoHubConfig_to_cut G hth
