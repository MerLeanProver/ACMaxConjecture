...
theorem algConn_le_two_of_two_vertex_cut_general {V : Type*} [Fintype V]
  ...
  have hxA : ∀ v ∈ A, x v = β := by
    intro v hv; simp only [hxdef]; rw [if_pos hv]
  have hxB : ∀ v ∈ B, x v = -α := by
    intro v hv
    have hvA : v ∉ A := Finset.disjoint_right.mp hdisj hv
    simp only [hxdef]; rw [if_neg hvA, if_pos hv]
  have hxOut : ∀ v, v ∉ A ∪ B → x v = 0 := by
    intro v hv
    rw [Finset.mem_union, not_or] at hv
    simp only [hxdef]; rw [if_neg hv.1, if_neg hv.2]
  have hcIn : ∀ v, v ∈ A ∪ B → c v = 0 := by
    intro v hv; simp only [hcdef]; rw [if_pos hv]
  have hcOut : ∀ v, v ∉ A ∪ B → c v = 1 := by
    intro v hv; simp only [hcdef]; rw [if_neg hv]
  ...
  have hsum0 : ∑ i, x i = 0 := by
    have hxval : ∀ v, x v =
      ...
      · have hvB : v ∉ B := Finset.disjoint_left.mp hdisj hv
        rw [hxA v hv, if_pos hv, if_neg hvB]; ring
      · have hvA : v ∉ A := Finset.disjoint_right.mp hdisj hv
        rw [hxB v hv, if_neg hvA, if_pos hv]; ring
      · rw [Finset.mem_union, not_or] at hv
        rw [hxOut v (by rw [Finset.mem_union, not_or]; exact hv),
          if_neg hv.1, if_neg hv.2]
        ring
  ...
  have hSC : ∑ v, c v = 2 := by
    have hcval : ∀ v, c v = if v ∉ A ∪ B then (1 : ℝ) else 0 := by
      intro v
      by_cases h : v ∈ A ∪ B
      · rw [hcIn v h, if_neg (not_not_intro h)]
      · rw [hcOut v h, if_pos h]
    simp_rw [hcval]
