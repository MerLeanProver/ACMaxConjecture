...
theorem exists_induced_2K2_of_triangleFree_smalldeg {V : Type*} [Fintype V]
  ...
  have hdom : ∀ z ∈ D, z = x ∨ z ∈ G.neighborFinset x ∩ D ∨
      ∃ a ∈ G.neighborFinset x ∩ D, G.Adj z a := by
    intro z hzD
    by_contra hcon
    push Not at hcon
  ...
  have hDsub : D ⊆ insert x (G.neighborFinset x ∩ D ∪ B) := by
    intro z hzD
  ...
  have hBcard : B.card ≤ 3 := by
    ...
    have hBa : ∀ a ∈ Nx,
        (B.filter (fun b => a ∈ G.neighborFinset b ∩ Nx)).card ≤ Δ - 1 := by
      intro a haNx
    ...
    have hsumcol : ∑ b ∈ B, (G.neighborFinset b ∩ Nx).card ≤ Δ * (Δ - 1) := by
    ...
    · -- Δ = 3: the hard case requiring the structural forcings.
      ...
      have hAdj : ∀ b ∈ B, ∀ b' ∈ B, b ≠ b' →
          Disjoint (G.neighborFinset b ∩ Nx) (G.neighborFinset b' ∩ Nx) → G.Adj b b' := by
        intro b hb b' hb' hbb' hdisj
        by_contra hnadj
      ...
      have hUnion : ∀ b ∈ B, ∀ b' ∈ B, b ≠ b' → G.Adj b b' →
          (G.neighborFinset b ∩ Nx) ∪ (G.neighborFinset b' ∩ Nx) = Nx := by
        intro b hb b' hb' hbb' hadj
      ...
      have hdeg : ∀ b ∈ B, (G.neighborFinset b ∩ Nx).card
          + (B.filter (fun b' => G.Adj b b')).card ≤ 3 := by
        intro b hb
      ...
      · -- `s ≥ 2`: two singletons collapse `B` onto a single colour class of size `≤ 2`.
        ...
        have hall : ∀ b ∈ B, a₀ ∈ G.neighborFinset b ∩ Nx := by
          intro b hb
          by_contra hcon
          have hbb1 : b ≠ b1 := by
