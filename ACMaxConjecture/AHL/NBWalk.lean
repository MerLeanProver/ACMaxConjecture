...
This is the foundation file of the **AHL chapter**, a self-contained formalization of the
irregular Moore bound of Alon, Hoory and Linial ("The Moore bound for irregular graphs", *Graphs
and Combinatorics* **18** (2002), 53–57).  The chapter runs the non-backtracking-walk proof of that
bound end to end — from the walk predicate here (`AHL.NBWalk`) through the counting vocabulary
(`AHL.NBWalkCount`, `AHL.NBWeighted`), the stationary weighted marginals (`AHL.AHLStationary`,
`AHL.AHLMarginals`), the weighted AM–GM and degree convexity (`AHL.AHLAmGm`), to the summit
`ahl_irregular_moore` (`AHL.AHLMoore`).  It is independently citable and used downstream only through
its summit forms.
