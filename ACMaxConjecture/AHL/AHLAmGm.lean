import ACMaxConjecture.AHL.AHLMarginals
...
* **W6 — the entropy recursion.**  `nbEntropy G k = ∑_{walks} wt·log wt⁻¹`.  Its exact one-step
  ...
  marginal (W4) to the closed form `nbEntropy_eq : nbEntropy G ℓ = (ℓ − 1)·∑_v deg v·log(deg v − 1)`.
...
* **W8 — the degree convexity** (`sum_deg_mul_log_ge`, `lambda_ge`).  The function
  `x ↦ x·log(x − 1)` is convex on `[2, ∞)` (`convexOn_deg_mul_log`, `f'' = (x−2)/(x−1)² ≥ 0`); Jensen
...
Its second derivative is `φ''(x) = (x − 2)/(x − 1)² ≥ 0` there; we feed the explicit first and second
...
/-- **The weight of a one-edge extension.**  Every non-backtracking extension `s` of a non-nil prefix
...
/-- **The per-prefix entropy fiber.**  Summing `wt·log wt⁻¹` over the `deg (end s') − 1` extensions of
...
theorem sum_weight_log_deg (hδ2 : ∀ w, 2 ≤ G.degree w) {k : ℕ} (hk : 1 ≤ k) :
  ...
  rw [step]
  show nbEndWeight (G := G) k v * Real.log ((G.degree v : ℝ) - 1) = _
...
/-- **The AHL spectral constant** `Λ = ∏_v (deg v − 1)^(deg v / D)` (`Real.rpow`), the geometric mean
...
omit [DecidableEq V] in
/-- **W8 (sharp `Λ`-form).**  Under `δ ≥ 2`, `Λ ≥ (D − n)/n = d̄ − 1`.  Exponentiating the logarithmic
