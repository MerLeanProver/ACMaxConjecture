...
import ACMaxConjecture.AHL.NBWalk

/-!
# Counting non-backtracking walks — the `deg x · (δ - 1)^{r-1}` lower bound

This file lands the *counting* node of the Alon–Hoory–Linial irregular Moore bound core (the direct
predecessor of the weighted AM–GM assembly).  Building on `IsNonBacktracking` from
`AHL.NBWalk`, it proves that a vertex `x` starts at least `deg x · (δ - 1)^{r-1}`
non-backtracking walks of length `r`, where `δ` is a lower bound on every degree.
...
theorem nb_concat_iff {x y t : V} (p : G.Walk x y) (hp : ¬ p.Nil) (h : G.Adj y t) :
  ...
  have hgv : ∀ i, i ≤ p.length → (p.concat h).getVert i = p.getVert i := by
    ...
    · rw [if_pos hlt]
    · rw [heq, if_neg (lt_irrefl _), Nat.sub_self]; simp
  have hgt : (p.concat h).getVert (p.length + 1) = t := by
    rw [Walk.concat_eq_append, Walk.getVert_append, if_neg (by omega),
