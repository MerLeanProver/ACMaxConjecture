import ACMaxConjecture.AHL.AHLStationary
...
theorem sum_nbExtend_ite {u v : V} (huv : G.Adj u v) {x : V} {k : ℕ} (hk : 1 ≤ k)
  ...
  have hinj : Set.InjOn
      (fun t => if h : G.Adj s'.1 t then ⟨t, s'.2.concat h⟩ else (⟨x, Walk.nil⟩ : Σ w : V, G.Walk x w))
      (↑(G.neighborFinset s'.1 \ {s'.2.penultimate})) := by
    intro b hb b' hb' hbb
    rw [Finset.mem_coe, Finset.mem_sdiff, mem_neighborFinset] at hb hb'
    simp only [dif_pos hb.1, dif_pos hb'.1] at hbb
  ...
  · refine Finset.sum_congr rfl fun t ht => ?_
    ...
    simp only [dif_pos ht.1, nbWeight_concat s'.2 hnn ht.1, Sigma.eta]
  · by_cases hu : s'.1 = u
    · have hsum : (∑ t ∈ G.neighborFinset s'.1 \ {s'.2.penultimate},
        ...
        by_cases htv : t = v
        · rw [if_pos ⟨htv, hu⟩, if_pos htv]
        · rw [if_neg (fun h => htv h.1), if_neg htv]
      ...
      · have hvnD : v ∉ G.neighborFinset s'.1 \ {s'.2.penultimate} := by
          ...
          rintro ⟨-, hne⟩
          exact hne hp.symm
        rw [if_neg hvnD, if_neg (fun hc => hc.2 hp)]
      · have hvD : v ∈ G.neighborFinset s'.1 \ {s'.2.penultimate} := by
        ...
        rw [if_pos hvD, if_pos ⟨hu, hp⟩, hu]
    · rw [if_neg (fun hc => hu hc.1)]
      refine Finset.sum_eq_zero fun t _ => ?_
      rw [if_neg (fun hc => hu hc.2)]
...
theorem nbLastWeight_succ_eq {u v : V} (huv : G.Adj u v) {k : ℕ} (hk : 1 ≤ k) :
  ...
  calc nbLastWeight (G := G) (k + 1) u v
    ...
    _ = (∑ w ∈ G.neighborFinset u \ {v}, nbLastWeight (G := G) k w u) * ((G.degree u : ℝ) - 1)⁻¹ := by
...
total weight of the length-`k` non-backtracking walks whose last directed edge is `(u, v)` is exactly
...
theorem nbWeightTotal_eq (hδ2 : ∀ w, 2 ≤ G.degree w) {k : ℕ} (hk : 1 ≤ k) :
  ...
  show nbEndWeight (G := G) k v = (G.degree v : ℝ)
