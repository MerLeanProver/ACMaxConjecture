...
theorem mid_usable5_card_le (n : ℕ) (G : SimpleGraph (Fin n))
  ...
  have hInner : ∀ a ∈ A,
      (((G.neighborFinset a).erase u₀).biUnion (fun b =>
        if G.degree b ≤ 8 then (G.neighborFinset b).erase a else ∅)).card ≤ (G.degree a - 1) * 7 := by
    intro a ha
    have hterm : ∀ b ∈ (G.neighborFinset a).erase u₀,
      ...
      · have hab : a ∈ G.neighborFinset b := by
        ...
        rw [if_pos hb8, Finset.card_erase_of_mem hab,
          G.card_neighborFinset_eq_degree]
        omega
      · rw [if_neg hb8, Finset.card_empty]; omega
...
theorem mid_leaves_card_le_sharp (n : ℕ) (G : SimpleGraph (Fin n))
  ...
  have hLC : (L₈ ∩ C).card ≤ ∑ a ∈ A, (G.degree a - 1) * 6 := by
    rw [hCdef]
