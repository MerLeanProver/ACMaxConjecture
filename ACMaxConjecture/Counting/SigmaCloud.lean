...
import ACMaxConjecture.Counting.HubCross
...
theorem algConn_le_two_of_sigma_apex_pair [Nonempty V]
  ...
  · -- the quadratic-form certificate
    ...
    have hsumA : ∑ w ∈ (G.neighborFinset u).erase g, ((cB - cB / ((G.degree w : ℝ) - 2)) ^ 2)
        + ∑ w ∈ (G.neighborFinset u).erase g, ((G.neighborFinset w \ insert u ((G.neighborFinset v).erase g)).card : ℝ)
          * (cB / ((G.degree w : ℝ) - 2)) ^ 2
    ...
    have hsumB : ∑ w ∈ (G.neighborFinset v).erase g, ((cA - cA / ((G.degree w : ℝ) - 2)) ^ 2)
        + ∑ w ∈ (G.neighborFinset v).erase g, ((G.neighborFinset w \ insert v ((G.neighborFinset u).erase g)).card : ℝ)
          * (cA / ((G.degree w : ℝ) - 2)) ^ 2
...
theorem algConn_le_two_of_sigma_cross_pair [Nonempty V]
  ...
  · -- the quadratic-form certificate
    ...
    have hcrossle : ∑ w ∈ G.neighborFinset u, ∑ w' ∈ G.neighborFinset v,
        (if G.Adj w w' then (cB / ((G.degree w : ℝ) - 2)
      ...
      have hrow : ∀ w ∈ G.neighborFinset u,
        ...
        · rw [if_neg hwh]
          ...
          refine if_neg fun hadj => hwh (hcross1 w hw w' hw' hadj).1
    ...
    · -- sharp slots at `h` and `h'`
      ...
      have hsh'1 : 1 / ((G.degree h' : ℝ) - 2) ≤ 1 := by
        ...
        have : (3 : ℝ) ≤ (G.degree h' : ℝ) := by exact_mod_cast h3 h'
        linarith
      have hkey : (cB / ((G.degree h : ℝ) - 2) + cA / ((G.degree h' : ℝ) - 2)) ^ 2
          - (cB / ((G.degree h : ℝ) - 2)) ^ 2 - (cA / ((G.degree h' : ℝ) - 2)) ^ 2
          + cB ^ 2 * sigS G u + cA ^ 2 * sigS G v ≤ 2 * cB ^ 2 + 2 * cA ^ 2 := by
        ...
        have hs2le : s ^ 2 ≤ s := by nlinarith [hsh, hsh1]
        have hs'2le : s' ^ 2 ≤ s' := by nlinarith [hsh', hsh'1]
        have hcross2 : 2 * (cB * s) * (cA * s') ≤ cB ^ 2 * s + cA ^ 2 * s' := by
          nlinarith [sq_nonneg (cB * s - cA * s'),
            mul_le_mul_of_nonneg_left hs2le (sq_nonneg cB),
            mul_le_mul_of_nonneg_left hs'2le (sq_nonneg cA)]
        nlinarith [hs1, hs2, hcross2]
      linarith [hsideU, hsideV, hcrossle, hkey]
...
(`capped_overflow_le`) — their base `(5/4)(d−4)` share rides in the `X`-term.  The transfer terms cancel
...
theorem w5big_cloud_le (G : SimpleGraph (Fin n)) (h3 : ∀ v, 3 ≤ G.degree v)
  ...
  have hPsum : ∑ x ∈ P, (G.degree x - 1) ≤ 10 * T.card := by
    calc ∑ x ∈ P, (G.degree x - 1)
      ...
      _ = 10 * T.card := by ring
  have hP3le : P3.card ≤ 78 * T.card := by
...
theorem w5big_card_le (G : SimpleGraph (Fin n)) (h3 : ∀ v, 3 ≤ G.degree v)
  ...
  have hP2le : P2.card ≤ 100 * T.card := by
  ...
  have hP3le : P3.card ≤ 2200 * T.card := by
    ...
    have harith4 : ∀ nxt b : ℕ, nxt + b ≤ 4 → b ≤ 3 →
      ...
      intro nxt b h1 h2
      interval_cases b <;> omega
    have hperx : ∀ x ∈ P, (∑ y ∈ G.neighborFinset x,
        (((G.neighborFinset x ∩ T) ×ˢ (G.neighborFinset y ∩ T)) ∪
