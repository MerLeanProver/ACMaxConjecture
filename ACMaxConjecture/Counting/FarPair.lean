...
theorem algConn_le_two_of_apex_double_star [Nonempty V]
  ...
  have hterm : ∀ i j : V,
    ...
    · rw [if_pos hadj]
      ...
      · by_cases hjg : j = g
        · -- apex edge `u → g`.
          have e11 : (if i = u ∧ j = g then au ^ 2 else 0) = au ^ 2 := if_pos ⟨hiu, hjg⟩
          have hxi : x i = au := by rw [hiu]; exact hxu
      ...
      · by_cases hju : j = u
        · by_cases hig : i = g
          · -- apex edge `g → u`.
            have e12 : (if j = u ∧ i = g then au ^ 2 else 0) = au ^ 2 := if_pos ⟨hju, hig⟩
                  ...
                  · by_cases hjB : j ∈ B
                    · -- internal `B` edge.
                      ...
                      have e10 : (if G.Adj i j ∧ j ∈ B ∧ i ≠ v ∧ i ∉ A then q j ^ 2
                          else 0) = q j ^ 2 := if_pos ⟨hadj, hjB, hiv, hiA⟩
                      have hb : (-(q i) - -(q j)) ^ 2 ≤ q i ^ 2 + q j ^ 2 := by
                        nlinarith [mul_nonneg (hq i hiB) (hq j hjB)]
                      rw [hxB i hiB, hxB j hjB]
                      linarith
  ...
  have hC1 : (∑ i, ∑ j, if i = u ∧ j ∈ A then (au - p j) ^ 2 else 0)
      = ∑ w ∈ A, (au - p w) ^ 2 := by
  ...
  have hC3 : (∑ i, ∑ j, if i = v ∧ j ∈ B then (av - q j) ^ 2 else 0)
      = ∑ w ∈ B, (av - q w) ^ 2 := by
  ...
  have hC11 : (∑ i, ∑ j, if i = u ∧ j = g then au ^ 2 else 0) = au ^ 2 := by
  ...
  have hC12 : (∑ i, ∑ j, if j = u ∧ i = g then au ^ 2 else 0) = au ^ 2 := by
    rw [Finset.sum_comm]
    exact hC11
  have hC13 : (∑ i, ∑ j, if i = v ∧ j = g then av ^ 2 else 0) = av ^ 2 := by
