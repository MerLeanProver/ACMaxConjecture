...
theorem algConn_le_two_of_hub_cross_pair [Nonempty V]
  ...
  have hleakA : ∀ w ∈ A.erase g,
      ((G.neighborFinset w \ insert u B).card : ℝ) ≤ 3 := by
    intro w hw
    ...
    have hc : (G.neighborFinset w \ insert u B).card ≤ 3 := by
      calc (G.neighborFinset w \ insert u B).card
        ...
        _ ≤ 3 := by
            ...
            omega
    exact_mod_cast hc
  have hleakB : ∀ w ∈ B.erase g',
      ((G.neighborFinset w \ insert v A).card : ℝ) ≤ 3 := by
    intro w hw
  ...
  have hcross_sum : (∑ w ∈ A, ∑ w' ∈ B, (if G.Adj w w' then (p w + q w') ^ 2 else 0))
    ...
    have hpt : ∀ w ∈ A, ∀ w' ∈ B, (if G.Adj w w' then (p w + q w') ^ 2 else 0)
        ≤ (if w = g ∧ w' = g' then (if G.Adj g g' then (s + s') ^ 2 else 0) else 0) := by
      intro w hwA w' hw'B
      by_cases hadj : G.Adj w w'
      · obtain ⟨rfl, rfl⟩ := hcross w w' ((G.mem_neighborFinset u w).mp hwA)
          ((G.mem_neighborFinset v w').mp hw'B) hadj
        rw [if_pos hadj, if_pos ⟨rfl, rfl⟩, if_pos hadj, hpg, hqg']
      · rw [if_neg hadj]
        split <;> [skip; exact le_refl 0]
        split <;> positivity
    have houter : (∑ w ∈ A, ∑ w' ∈ B,
        (if w = g ∧ w' = g' then (if G.Adj g g' then (s + s') ^ 2 else 0) else 0))
        = ∑ w' ∈ B,
          (if g = g ∧ w' = g' then (if G.Adj g g' then (s + s') ^ 2 else 0) else 0) :=
      Finset.sum_eq_single_of_mem g hgA fun w _ hwg =>
        Finset.sum_eq_zero fun w' _ => if_neg fun h => hwg h.1
    have hinner : (∑ w' ∈ B,
        (if g = g ∧ w' = g' then (if G.Adj g g' then (s + s') ^ 2 else 0) else 0))
        = (if G.Adj g g' then (s + s') ^ 2 else 0) := by
      rw [Finset.sum_eq_single_of_mem g' hg'B fun w' _ hw'g' => if_neg fun h => hw'g' h.2]
      exact if_pos ⟨rfl, rfl⟩
    calc (∑ w ∈ A, ∑ w' ∈ B, (if G.Adj w w' then (p w + q w') ^ 2 else 0))
        ≤ ∑ w ∈ A, ∑ w' ∈ B,
            (if w = g ∧ w' = g' then (if G.Adj g g' then (s + s') ^ 2 else 0) else 0) :=
          Finset.sum_le_sum (fun w hw => Finset.sum_le_sum (fun w' hw' => hpt w hw w' hw'))
      _ = (if G.Adj g g' then (s + s') ^ 2 else 0) := houter.trans hinner
  -- Leak sums.
  ...
  · -- The quadratic bound, by cases on the hub–hub edge.
    ...
    · -- Edge present: hub leaks `≤ deg − 2`, cross `≤ (s+s')²`.
      ...
      have hLg : ((G.neighborFinset g \ insert u B).card : ℝ) ≤ a - 1 := by
      ...
      have hLg' : ((G.neighborFinset g' \ insert v A).card : ℝ) ≤ b - 1 := by
      ...
      have key : (1 - s) ^ 2 + 1 / 2 + ((1 - s') ^ 2 + 2 * (1 - w0) ^ 2)
        ...
        rw [h1, h2, hw0_def]
        nlinarith [hs_pos, hs'_pos]
      linarith [hSA, hSB, hCR, key]
    · -- Edge absent: hub leaks `≤ deg − 1`, no cross term at all.
      have hLg : ((G.neighborFinset g \ insert u B).card : ℝ) ≤ a := by
        ...
        rw [ha_def]
        linarith
      have hLg' : ((G.neighborFinset g' \ insert v A).card : ℝ) ≤ b := by
      ...
      have key : (1 - s) ^ 2 + 1 / 2 + ((1 - s') ^ 2 + 2 * (1 - w0) ^ 2)
        ...
        nlinarith [hs_pos, hs'_pos, mul_pos hs_pos hs'_pos]
      linarith [hSA, hSB, hCR, key]
