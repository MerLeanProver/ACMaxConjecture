...
import ACMaxConjecture.Cuts.TwoCut
...
theorem adjInd_comm (G : SimpleGraph V) (g h : V) : adjInd G g h = adjInd G h g := by
  ...
  · rw [if_pos hadj, if_pos hadj.symm]
  · rw [if_neg hadj, if_neg fun hc => hadj hc.symm]
...
(and the trivial floor `520`), a single hypothesis — the usable degree-3 count is at most `C_m` on the
...
theorem bigclean_pair_coverage (n : ℕ) (G : SimpleGraph (Fin n))
  ...
  have hP2le : P2.card ≤ (A + 2) * T.card := by
    ...
    have hbound : ∑ w : Fin n, (G.neighborFinset w ∩ T).offDiag.card
      ...
      have hbig : ∑ w ∈ Finset.univ.filter (fun w : Fin n => 9 ≤ G.degree w),
        ...
        calc ∑ w ∈ Finset.univ.filter (fun w : Fin n => 9 ≤ G.degree w),
          ...
          _ ≤ A * T.card := Nat.mul_le_mul_left A (sum_big_inc_le G h3)
  ...
  have hP3le : P3.card ≤ (12 * A + 24) * T.card := by
...
theorem cloud_card_le_sharp {n : ℕ} (G : SimpleGraph (Fin n))
  ...
  have hperx : ∀ x ∈ P, (G.neighborFinset x ∩ C).offDiag.card
      + ∑ x' ∈ (G.neighborFinset x).erase g,
          ((G.neighborFinset x ∩ C) ×ˢ (G.neighborFinset x' ∩ C)).card
      ≤ 6 * (G.neighborFinset x ∩ C).card := by
    intro x hx
    have hxg : x ≠ g := by
      have := hPdeg x hx
