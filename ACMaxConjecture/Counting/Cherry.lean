import Mathlib
import ACMaxConjecture.Counting.PoorCorner
...
private theorem p3_pair_gap (G : SimpleGraph V) {g h : V}
  ...
  · -- W1 with the single far pad
    ...
    · unfold dsValue
      ...
      rw [hdg] at i1
      rw [hdh] at i2
      have hadj0 : adjInd G g h = 0 := by unfold adjInd; rw [if_neg hnadj]
      rw [hadj0, hmx]
      omega
...
theorem p3_pair_close (n : ℕ) (hn : 18 ≤ n) (G : SimpleGraph (Fin n))
  ...
  · -- adjacent: no shared twin, |D3| = 3 both, W1 at the tie with no pads
    ...
    · unfold dsValue
      ...
      rw [hdg] at i1
      rw [hdh] at i2
      have hadj1 : adjInd G g h = 1 := by unfold adjInd; rw [if_pos hadj]
  ...
  · -- non-adjacent: share ≤ 1, split on the D3-card comparison
    ...
    · -- equal `D3`-cards: W1 with `F = ∅`
      ...
      · unfold dsValue
        ...
        rw [hdg] at i1
        rw [hdh] at i2
        have hadj0 : adjInd G g h = 0 := by unfold adjInd; rw [if_neg hadj]
        rw [hadj0, hmxgh]
        omega
    · -- `g` is the clean one
