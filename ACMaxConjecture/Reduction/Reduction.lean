...
This file proves, **for every `n ≥ 12` simultaneously**, that the open upper-bound clause of the
ACMAX conjecture (Kolokolnikov, Conjecture 1.5, arXiv:1412.6147) — every simple graph on `Fin n`
with exactly `2(n-2)` edges has algebraic connectivity at most `2` — reduces to a *single*,
sharply-stated residual predicate `ResidualCore`.  (The cases `4 ≤ n ≤ 11` are closed
unconditionally in `CaseN4`–`CaseN11`; the per-`n` files `CaseN12`–`CaseN18` close
`ResidualCore`-type residuals for those specific `n`.)

## The reduction (mirroring the per-`n` dispatch of `CaseN12`–`CaseN19`, with `n` a variable)
...
Steps 1–5 and the whole dispatch are **sorry-free and axiom-clean**.  Step 6 is consumed as an
explicit hypothesis by the conditional dispatcher `algConn_le_two_of_card_general_cond`; the
residual statement itself (`residual_algConn_le_two`) is PROVED (2026-07-14) in
`ACMaxConjecture.Band.Final` — downstream of this file — as a corollary of the full
conjecture `ACMax.acmax_conjecture_general`, so this file no longer contains any `sorry`.

## Correspondence with the per-`n` residual (verified at `n = 19`)

`ResidualCore 19 G` implies precisely the hypothesis pack of
`exists_twin_signed_cert_nineteen` (`TwinCert19.lean`), the per-`n` residual engine:

| `ResidualCore` field        | `exists_twin_signed_cert_nineteen` hypothesis                  |
|-----------------------------|----------------------------------------------------------------|
| `edge_card` (`= 2·17`)      | `hm : G.edgeFinset.card = 34`                                  |
| `min_degree`                | `h3 : ∀ v, 3 ≤ G.degree v`                                     |
| `no_good_triangle`          | `hT` (threshold `∑deg ≤ 11` ⟺ `19·(∑deg−6) ≤ 96`)              |
| `no_deg3_ind2K2`            | `h2k2` (verbatim)                                              |
| `no_good_C4`                | `hC4` (threshold `∑deg ≤ 14` ⟺ `19·(∑deg−8) ≤ 120`)            |
| `no_good_K23`               | `hK23` (threshold `∑deg ≤ 19` ⟺ `19·(∑deg−12) ≤ 140`)          |
| `iso_deg3`                  | `hiso : ∃ t, deg t = 3 ∧ ∀ w, Adj t w → deg w ≠ 3`             |
| `connected`                 | (not needed there — the disconnected case is closed generically)|

The three threshold equivalences are checked by `example`s below.  So `ResidualCore` is a
superset-free capture of what steps 1–5 leave: it is neither weaker nor incomparable to the
class the per-`n` machinery actually handles.
-/
...
good `K_{2,3}`) are proved here unconditionally, uniformly in `n`; the only `sorry` reachable
from this theorem is `residual_algConn_le_two`. -/
theorem algConn_le_two_of_card_general_cond (n : ℕ) (hn : 12 ≤ n) [Nonempty (Fin n)]
  ...
  -- this proof match the instances of the generic certificate lemmas (as in `CaseN12`–`CaseN19`).
...
-- `ACMaxConjecture.Band.Final`; see the NOTE above.)
