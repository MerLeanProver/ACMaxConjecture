...
(`Band.Sum`) at the induced graph on the `2`-core of a subset `S`, producing a short cycle inside `S`
...
The single genuinely-new arithmetic step is the **`v`-transport** `ball_side_transport`: the per-cell
...
* **`ahl_ball_girth_subset`** — the subset wrapper: a nonempty `S` with edge excess
  ...
  `t(|S| − 1)|S|^(L/2) < (|S| + t)((|S| + 2t)^(L/2) − |S|^(L/2))` (`6 ≤ L`) carries a cycle of length
...
`v' ≤ v`.  The proof is term-by-term over the geometric sum: each summand comparison, after peeling a
      ...
      _ ≤ ∑ k ∈ Finset.range ℓ,
          ...
          have h3 : (v + 2 * t) * v' ≤ (v' + 2 * t) * v := by
            calc (v + 2 * t) * v' = v * v' + 2 * t * v' := by ring
              _ ≤ v * v' + 2 * t * v := Nat.add_le_add_left (Nat.mul_le_mul (le_refl (2 * t)) hvv') _
              _ = (v' + 2 * t) * v := by ring
...
side condition `t(|S| − 1)|S|^(L/2) < (|S| + t)((|S| + 2t)^(L/2) − |S|^(L/2))` with `6 ≤ L`, there is
