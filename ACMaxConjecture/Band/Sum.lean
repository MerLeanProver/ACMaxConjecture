...
cycle of length `≤ 2ℓ`; a walk returning to `x` would be a positive-length closed path, impossible. -/
...
theorem ahl_ball_moore (hδ2 : ∀ v, 2 ≤ G.degree v) [Nonempty V] (hℓ : 1 ≤ ℓ)
  ...
  have key : (∑ v, (G.degree v : ℝ)) *
    ...
    calc (∑ v, (G.degree v : ℝ))
      ...
      _ = (∑ k ∈ Finset.range ℓ, (nbTotalWalks G (k + 1) : ℝ)) * (Fintype.card V : ℝ) ^ (ℓ - 1) := by
          rw [Finset.sum_mul]
